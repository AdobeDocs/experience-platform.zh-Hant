...
第三條消息失敗，因為報頭中使用的IMS組織ID無效。 IMS組織必須與您嘗試發佈到的{CONNECTION_ID}匹配。 要確定哪個IMS組織ID與您使用的流連接匹配，您可以執行 `GET inlet` 使用 [[!DNL Data Ingestion API]](https://www.adobe.io/experience-platform-apis/references/data-ingestion/)。 請參閱 [檢索流連接](./create-streaming-connection.md#get-data-collection-url) 例如，如何檢索以前建立的流連接。
