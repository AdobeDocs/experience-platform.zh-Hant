...
將資料串流至Adobe Experience Platform時，進行大量HTTP呼叫的成本可能很高。 例如，建立1KB負載的200個HTTP請求，而不是建立1KB負載的200個HTTP請求，建立200個每個1KB訊息的1HTTP請求，而且單一負載為200KB，效率更高。 正確使用時，在單一要求內將多個訊息分組是最佳化傳送至Experience Platform之資料的絕佳方式。

本檔案提供教學課程，可讓您使用串流擷取功能，在單一HTTP要求內傳送多則訊息至Experience Platform。
...
您必須先建立串流連線，才能開始將資料串流至Experience Platform。 閱讀「 [建立串流連線](./create-streaming-connection.md) 」指南，瞭解如何建立串流連線。
...
您可以使用平台UI或使用API中的清單作業，取得現有資料集的ID。 前往 [Experience Platform](https://platform.adobe.com) **Tab** ，按一下您要取得ID的資料集，然後從InfoAdobest字串的 **Dataset ID** 欄位複製資料集ID，即可在 **** Experience Platform上找到資料集ID。 如需如何 [使用API擷取資料集的詳細資訊](../../catalog/home.md) ，請參閱目錄服務概觀。
...
- 消息 `imsOrgId` 標題中的欄位必須與入口定義匹配。 如果請求裝載未包含欄 `imsOrgId` 位，資料收集核心服務(DCCS)會自動新增欄位。
- 訊息的標題應參考在平台UI中建立的現有XDM架構。
- 欄位 `datasetId` 需要參考Platform中現有的資料集，其架構需要比對請求內文中每個訊息內 `header` 物件中提供的架構。
...
第一個訊息已成功傳送至平台，不受其他訊息結果的影響。 因此，嘗試重新發送失敗的消息時，不需要重新包含此消息。
...
第三則訊息因標題中使用的IMS組織ID無效而失敗。 IMS組織必須符合您嘗試張貼至的{CONNECTION_ID}。 若要判斷哪個IMS組織ID符合您使用的串流連線，您可以使用「資料擷取API」 `GET inlet` 來執行 [請求](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/ingest-api.yaml)。 如需 [如何擷取先前建立之串流連線的範例](./create-streaming-connection.md#get-data-collection-url) ，請參閱擷取串流連線。

第四條消息失敗，因為它未遵循預期的XDM模式。 請 `xdmSchema` 求的標頭和正文中包含的與的XDM模式不匹配 `{DATASET_ID}`。 修正訊息標題和內文中的架構，可讓其通過DCCS驗證並成功傳送至平台。 還必須更新消息主體，以便其與的XDM模式匹配， `{DATASET_ID}` 以便在平台上通過流驗證。 如需成功串流至平台之訊息的詳細資訊，請參閱本教 [學課程的確認訊息](#confirm-messages-ingested) 。
...
通過DCCS驗證的訊息會串流至平台。 在Platform上，批次訊息會先透過串流驗證進行測試，再被收錄到資料湖中。 批次狀態（無論是否成功）會顯示在指定的資料集中 `{DATASET_ID}`。

您可以前往「資料集」標籤，按一下您要串流至的資料集，並勾選「資料集活動」標籤，以檢視使用 [Experience Platform UI](https://platform.adobe.com)******** ，成功串流至Platform的批次訊息狀態。

在Platform上通過串流驗證的批次訊息會吸收到資料湖中。 然後，這些訊息便可供分析或匯出。

## 後續步驟

現在，您知道如何在單一請求中傳送多則訊息，並驗證訊息是否已成功傳入目標資料集，您可以開始將您自己的資料串流至平台。 如需如何從平台查詢和擷取收錄資料的概觀，請參閱資 [料存取指南](../../data-access/tutorials/dataset-data.md) 。
