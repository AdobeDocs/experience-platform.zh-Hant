...
Adobe Experience Platform可讓您輕鬆將資料匯入Platform做為批次檔案。 要提取的資料示例可以包括來自CRM系統中的平面檔案（如鑲木地板檔案）的概要檔案資料或符合模式註冊表中已知的體驗資料模型(XDM)模式的資料。
...
在「建 **立資料集** 」畫面上，選取您要「從架構建立資料集」或「從CSV檔案建立資料集」。

在本教學課程中，將使用模式來建立資料集。 按一 **下「從架構建立資料集** 」以繼續。

![選擇資料來源](../images/tutorials/ingest-batch-data/create_dataset.png)

## 選擇資料集模式

在「選 **擇方案** 」螢幕上，按一下要使用的方案旁邊的單選按鈕選擇方案。 在本教學課程中，資料集將使用「忠誠度成員」架構建立。 使用搜尋列來篩選結構描述是找到您所尋找的精確結構描述的有用方式。

在選擇了要使用的方案旁邊的單選按鈕後，按一下「下 **一步**」。
...
在Datasets的右側，您會看到與新資料集相關的 **Info**_Tab，例如_ Info _ID、AdSignDesignDesignDesignDesignDesignDatases,___________ DDDSigSiDSinasig。 「資訊」標籤也包含資料集何時建立及其「上 _次修改_ 」日 _期的相關資訊_ 。

另外，「資訊」標籤中也有 _「描述檔_ 」切換功能，用於啟用您的資料集，以便與即時客戶描述檔搭配使用。 此切換的使用，以及即時客戶個人檔案的使用，將在下一節中詳細說明。

![資料集活動](../images/tutorials/ingest-batch-data/dataset_activity.png)

## 為即時客戶個人檔案啟用資料集

資料集用於將資料擷取至Experience Platform，而資料最終用於識別個人，並將來自多個來源的資訊結合在一起。 將資訊結合在一起的資訊稱為即時客戶個人檔案。 為了讓Platform知道哪些資訊應包含在即時描述檔中，可以使用Profile（描述檔）切換來標籤資料集以 **加入** 。

依預設，此切換為關閉。 如果您選擇開啟「描述檔」，所有收錄至資料集的資料都將用來協助識別個人，並將其即時描述檔接合在一起。

若要進一步瞭解即時客戶個人檔案和使用身分識別，請參閱 [Identity Service](../../identity-service/home.md) 檔案。

若要啟用即時客戶描述檔的資料集，請按一下「資訊」標 **簽中的** 「描述檔 **」切換** 。
...
按一 **下「啟用** 」，切換將會變成藍色，表示已開啟。

![啟用設定檔](../images/tutorials/ingest-batch-data/dataset_enabled.png)

## 新增資料至資料集

資料可以多種不同的方式新增至資料集。 您可以選擇使用資料擷取API或ETL合作夥伴，例如Unifi或Informatica。 在本教學課程中，資料將會使用UI中的「新增資 **料** 」標籤新增至資料集。
...
在您拖放（或瀏覽並選取）您要上傳的鑲木地板或JSON檔案後，平台會立即開始處理檔案，而「 **Uploading** 」（新增資料）對話方塊會出現在 **Add Data** （新增資料）標籤上，顯示檔案上傳的進度。

![上傳對話方塊](../images/tutorials/ingest-batch-data/uploading.png)

## 資料集度量

檔案上傳完成後，「資料集活 **動** 」標籤不會再顯示「未新增任何批次」。 現在，「資料集活動」標籤會顯示資料集度量。 由於批次尚未載入，所有量度在此階段會顯示「0」。
...
按一下「批 _次ID_ 」以檢視「批 **次概述」**，顯示有關批次的其他詳細資訊。 在批次完成載入後，有關批次的資訊將會更新，以顯示「已收錄的記 _錄數_ 」和「檔 _案大小」_。 「狀 _態_ 」也會變更為「成功」或「失敗」。 如果批處理失敗，「錯 _誤代碼_ 」部分將包含有關接收過程中任何錯誤的詳細資訊。
...
資料集準備就緒後，「資料集活動」 **標籤的頂端會顯示** 「預覽資 **料集」選項** 。

按一 **下「預覽資料集** 」以開啟對話方塊，顯示資料集內的範例資料。 如果使用模式建立資料集，則資料集模式的詳細資料會顯示在預覽的左側。 可以使用箭頭展開模式以查看模式結構。 預覽資料中的每個欄標題代表資料集中的欄位。

![資料集詳細資料](../images/tutorials/ingest-batch-data/dataset_details.png)

## 後續步驟

現在您已建立資料集並成功將資料擷取至Experience Platform，您可以重複這些步驟以建立新資料集或將更多資料擷取至現有資料集。

若要進一步瞭解批次擷取，請閱讀「批次擷取 [」概觀](../batch-ingestion/overview.md)。
