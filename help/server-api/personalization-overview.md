...
使用伺服器API時，您必須整合個人化引擎提供的回應，與用來呈現網站內容的邏輯。 不喜歡 [Web SDK](../web-sdk/home.md)，則 [!DNL Server API] 沒有機制可自動套用Adobe個人化解決方案傳回的內容。
