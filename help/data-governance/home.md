...
在套用資料使用標籤後，資料管理員就可使用DULE [!DNL Policy Service] API或使用者介面來建立 [!DNL Experience Platform] 原則。
...
本檔案對DULE框架進行了高 [!DNL Data Governance] 級介紹。 您現在可以繼續使用資 [料使用標籤使用指南](labels/user-guide.md) ，並開始將使用標籤新增至體驗資料。
...
下表概述了與DULE框架和DULE框架 [!DNL Data Governance] 相關的關鍵術語。
...
以下視訊旨在支援您對資料使用標 [!DNL Data Governance]簽與實施(DULE)架構的瞭解，並概述其主要方面。
