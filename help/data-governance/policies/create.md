...
type: Tutorial
description: 策略服務API允許您建立和管理資料使用策略，以確定可以針對包含某些資料使用標籤的資料執行哪些市場營銷操作。 本文檔提供了使用策略服務API建立策略的逐步教程。
...
的 [策略服務API](https://www.adobe.io/experience-platform-apis/references/policy-service/) 允許您建立和管理資料使用策略，以確定可以針對包含某些資料使用標籤的資料採取哪些市場營銷操作。

本文檔提供使用 [!DNL Policy Service] API。 有關API中可用的不同操作的更全面的指南，請參見 [策略服務開發人員指南](../api/getting-started.md)。
...
以下請求使用 `marketingActions/custom` 終結點，它將獲取由IMS組織定義的所有市場營銷操作的清單。
