...
此 [策略服務API](https://www.adobe.io/experience-platform-apis/references/policy-service/) 可讓您建立和管理資料使用原則，以決定可以對包含特定資料使用標籤的資料採取哪些行銷動作。

本檔案提供使用 [!DNL Policy Service] API。 如需API中不同作業的更完整指南，請參閱 [策略服務開發人員指南](../api/getting-started.md).
...
您可以向以下任一變數發出GET請求，以查詢由政策評估的現有行銷動作： `/marketingActions` 端點。
