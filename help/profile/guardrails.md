...
本檔案中有多個護欄，指的是「設定檔片段」。 即時客戶設定檔由多個設定檔片段組成。 每個片段代表以主要身分識別之資料集的身分資料。 這表示片段可能包含XDM ExperienceEvent資料集中的主要ID和事件資料（時間序列），或由XDM個別設定檔資料集中的主要ID和記錄資料（時間無關屬性）組成。
...
| 瓜德拉伊 | 限制 | 限制類型 | 說明 |
| --- | --- | --- | --- |
| 建議貢獻[!DNL Profile]聯合結構的資料集數 | 20 | 軟 | **建議最多啟用 [!DNL Profile]20個資料集。** 若要為啟用其他資 [!DNL Profile]料集，應先移除或停用現有資料集。 |
...
| 最大配置檔案片段大小 | 50MB | 硬 | **設定檔片段的最大大小為50MB。** 大於50MB的任何設定檔片段 [的](#profile-fragments) 分段、匯出和查詢可能會失敗。 |
| 最大配置檔案儲存大小 | 50MB | 軟 | **儲存的配置檔案的最大大小為50MB。** 將新配 [置](#profile-fragments) 檔案片段添加到大於50MB的配置檔案將影響系統效能。 |
