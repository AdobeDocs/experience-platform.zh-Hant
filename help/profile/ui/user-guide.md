...
與描述檔 [計數量度類似](#profile-count) ，當將描述檔擷取至描述檔存放區時，將計數增加或減少超過5%時，會觸發工作以更新命名空間量度。 對於串流資料工作流程，會每小時檢查一次，以判斷是否符合5%增加或減少臨界值。 如果已觸發，則會自動觸發作業以更新描述檔計數。 對於批次擷取，在成功將批次擷取至描述檔商店的15分鐘內，如果達到5%增加或減少臨界值，則會執行工作以更新量度。
