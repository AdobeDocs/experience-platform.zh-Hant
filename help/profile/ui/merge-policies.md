...
為了使用自訂時間戳記，必須將&#x200B;**[!UICONTROL External Source System Audit Details Mixin]**&#x200B;新增至您的描述檔架構。 新增後，可使用`lastUpdatedDate`欄位填入自訂時間戳記。 在`lastUpdatedDate`欄位填入記錄時，Experience Platform會使用該欄位來合併資料集上的記錄。 如果`lastUpdatedDate`不存在或未填入，平台將繼續使用系統時間戳記。

>[!NOTE]
>
>在同一記錄上接收更新時，必須確保填入`lastUpdatedDate`時間戳。

以下螢幕擷取顯示[!UICONTROL External Source System Audit Details Mixin]中的欄位。 有關使用平台UI使用架構的逐步說明（包括如何將混合新增至架構），請造訪[教學課程，以使用UI](../../xdm/tutorials/create-schema-ui.md)建立架構。
