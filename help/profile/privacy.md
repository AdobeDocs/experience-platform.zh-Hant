...
>[!NOTE]
>
>本指南僅涵蓋如何在Experience Platform中向設定檔資料存放區提出隱私權請求。 如果您也打算提出Experience Platform資料湖的隱私權請求，除了本教學課程外，另請參閱資料湖[中](../catalog/privacy.md)隱私權請求處理指南。
>
...
Adobe Experience Platform [!DNL Identity Service]可跨系統和裝置橋接客戶身分資料。 [!DNL Identity Service]使用&#x200B;**身分識別名稱空間**，透過將身分識別值與其來源系統建立關聯來提供身分識別值的內容。 名稱空間可代表一般概念，例如電子郵件地址（「電子郵件」），或將身分與特定應用程式建立關聯，例如Adobe Advertising ID或Adobe Target ID。
...
以下各節概述如何使用[!DNL Real-Time Customer Profile] API或UI為[!DNL Privacy Service]提出隱私權請求。 在閱讀這些章節之前，您應該檢閱或瞭解[Privacy Service API](../privacy-service/api/getting-started.md)或[Privacy Service UI](../privacy-service/ui/overview.md)檔案。 這些檔案提供如何提交隱私權工作的完整步驟，包括如何在請求裝載中正確格式化提交的使用者身分資料。
...
在API中建立工作要求時，`userIDs`內提供的任何ID都必須使用特定的`namespace`和`type`。 必須為名稱空間值提供識別服務識別的有效[識別名稱空間](#namespaces)。 將`standard`用於標準名稱空間，並將`custom`用於自訂名稱空間。
...
>[!NOTE]
>
>如需在[陣列中使用](#profile-v-identity)和`ProfileService`之影響的詳細資訊，請參閱本檔案後面有關`identity`設定檔要求與身分要求`include`的章節。

以下請求會針對[!DNL Profile]存放區中的單一客戶資料建立新的隱私權工作。 在`userIDs`陣列中為客戶提供了兩個身分值；一個使用標準`Email`身分名稱空間，另一個使用自訂`Customer_ID`名稱空間。 它也會包含[!DNL Profile]陣列中`ProfileService` (`include`)的產品值：
...
在UI中建立工作請求時，請務必在&#x200B;**[!UICONTROL AEP Data Lake]**&#x200B;下選取&#x200B;**[!UICONTROL Profile]**&#x200B;和/或&#x200B;**[!UICONTROL Products]**，以便分別處理儲存在Data Lake或[!DNL Real-Time Customer Profile]中之資料的工作。
...
為確保您的隱私權請求可處理所有相關客戶屬性，您必須為可能儲存這些屬性的所有適用資料集提供主要身分值（每位客戶最多九個ID）。 如需通常標籤為身分的欄位詳細資訊，請參閱結構描述組合[的](../xdm/schema/composition.md#identity)基本概念中的身分欄位相關章節。
...
視您是否也在設定檔(`identity`)的隱私權要求中納入身分識別服務(`aepDataLake`)和資料湖(`ProfileService`)作為產品而定，與設定檔相關的不同資料集，可能會在不同的時間從系統中移除：
...
Privacy Service只能使用不會執行身分拼接的合併原則來處理[!DNL Profile]資料。 如果您使用UI來確認您的隱私權請求是否正在處理中，請確定您使用包含&#x200B;**[!DNL None]**&#x200B;作為其[!UICONTROL ID stitching]型別的原則。 換言之，您無法使用[!UICONTROL ID stitching]設為[!UICONTROL Private graph]的合併原則。
...
如需處理[!DNL Experience Platform]未使用之[!DNL Profile]資源的隱私權要求的相關資訊，請參閱有關[在資料湖](../catalog/privacy.md)中處理隱私權要求的檔案。
