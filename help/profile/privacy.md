...
>[!NOTE]
>
>本指南僅說明如何針對Experience Platform中的設定檔資料存放區提出隱私權要求。 如果您也打算提出Platform Data Lake的隱私權要求，請參閱 [資料湖中的隱私權要求處理](../catalog/privacy.md) 除了本教學課程之外。
>
...
>[!IMPORTANT]
>
>Privacy Service只能處理 [!DNL Profile] 使用不執行身分連結的合併原則的資料。 如果您使用UI來確認是否正在處理您的隱私權要求，請確定您使用的原則包含「[!DNL None]」作為 [!UICONTROL ID匯整] 類型。 換句話說，您無法使用合併原則， [!UICONTROL ID匯整] 設為「[!UICONTROL 專用圖表]」。
>
...
此外， `include` 要求裝載的陣列必須包含要提出要求之不同資料存放區的產品值。 向 [!DNL Data Lake]，陣列必須包含「ProfileService」值。
...
當 [!DNL Experience Platform] 從接收刪除請求 [!DNL Privacy Service], [!DNL Platform] 發送確認 [!DNL Privacy Service] 已收到請求，且受影響的資料已標示為刪除。 接著，會從 [!DNL Data Lake] 或 [!DNL Profile] 在隱私權工作完成後儲存。 雖然刪除作業仍在處理中，但資料會遭軟刪除，因此無法供任何使用者存取 [!DNL Platform] 服務。 請參閱 [[!DNL Privacy Service] 檔案](../privacy-service/home.md#monitor) 以取得追蹤工作狀態的詳細資訊。

>[!IMPORTANT]
>
>成功的刪除請求會移除客戶（或一組客戶）所收集的屬性資料，但請求不會移除在身分圖中建立的關聯。
>
>例如，使用客戶 `email_id` 和 `customer_id` 移除儲存在這些ID下的所有屬性資料。 不過，之後擷取的任何資料會依照 `customer_id` 仍會與適當 `email_id`，因為關聯仍然存在。
