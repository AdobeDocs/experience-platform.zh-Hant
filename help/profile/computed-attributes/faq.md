...
計算屬性是在 **批次** 頻率： **獨立** 與您的對象、目的地和歷程評估的排程相符。 這表示無論分段型別（批次分段或串流分段）為何，計算屬性都將依其本身的排程進行評估（每小時、每日、每週或每月）。

計算屬性的第一次評估是在其後的24小時內進行 **建立**. 後續批次評估會根據定義的回顧期間，以每小時、每日、每週或每月為基礎進行。
...
>[!IMPORTANT]
>
>僅當快速重新整理是 **非** 已啟用。 若要瞭解啟用快速重新整理時回顧期間如何變更，請參閱 [快速重新整理區段](./overview.md#fast-refresh).

兩者 **每週** 和 **每月** 重新整理會在開始時 **行事曆周** （新周的星期日）或 **行事曆月份** （新月份的第一個），而不是第一次評估日期剛好是一週或一個月。

>[!NOTE]
>
>計算的屬性值是 **非** 在每次評估執行後，立即在設定檔中重新整理。 為了確保您的設定檔中有更新的值，您應考慮在評估時間與計算的屬性使用方式之間留出數小時的緩衝區。 計算的屬性重新整理排程為 **系統決定** 和 **無法** 修改。 如需詳細資訊，請聯絡Adobe客戶服務。
