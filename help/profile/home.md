...
Adobe Experience Platform 可讓您為客戶提供協調一致且相關的體驗，無論他們何時何地與您的品牌互動。透過[!DNL Real-Time Customer Profile]，您可以透過合併來自多個管道（包括線上、離線、CRM和協力廠商）的資料，檢視每個個別客戶的整體檢視。 [!DNL Profile]可讓您將您的客戶資料合併成統一的檢視畫面，針對每個客戶互動提供可採取行動且附有時間戳記的說明。 此概覽可協助您瞭解[!DNL Real-Time Customer Profile]中[!DNL Experience Platform]的角色和使用。
...
### 聯集結構 {#profile-fragments-and-union-schemas}
