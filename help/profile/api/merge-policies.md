...
要使用UI使用合併策略，請參閱合 [並策略使用手冊](../ui/merge-policies.md)。
...
* **`timestampOrdered`**:（預設）為上次更新的設定檔指定優先順序。 使用此合併類型時， `data` 不需要屬性。 `timestampOrdered` 也支援在資料集內或跨資料集合併描述檔片段時優先使用的自訂時間戳記。 如需詳細資訊，請參閱使用自訂時間戳 [記的附錄一節](#custom-timestamps)。
...
當記錄被收錄到Experience Platform中時，系統會在擷取時取得系統時間戳記並新增至記錄。 當選 `timestampOrdered` 擇為合併策略 `attributeMerge` 的類型時，將根據系統時間戳合併配置檔案。 換言之，合併是根據記錄被收錄到平台的時間戳記進行。

有時候，可能會有使用案例，例如回填資料，或在記錄未依順序擷取時，確保事件的正確順序，而需要提供自訂時間戳記，並讓合併原則遵守自訂時間戳記，而非系統時間戳記。

若要使用自訂時間戳記，必須將 [[!DNL External Source System Audit Details Mixin]](#mixin-details) 其新增至您的描述檔結構。 新增後，可使用欄位填入自訂時間 `xdm:lastUpdatedDate` 戳記。 當記錄已填入欄位時， `xdm:lastUpdatedDate` Experience Platform會使用該欄位來合併資料集內或跨資料集的記錄或描述檔片段。 如果 `xdm:lastUpdatedDate` 未顯示或未填入，平台將繼續使用系統時間戳記。

>[!NOTE]
>
>在同一記錄上 `xdm:lastUpdatedDate` 發送PATCH時，必須確保填入時間戳。

有關使用架構註冊表API處理架構的逐步說明（包括如何將混合新增至架構），請造訪使用 [API建立架構的教學課程](../../xdm/tutorials/create-schema-api.md)。
...
下列範例顯示中已正確填入的欄位 [!DNL External Source System Audit Details Mixin]。 您也可以在GitHub的公用體驗資 [料模型(XDM)回購中檢視完整的混合](https://github.com/adobe/xdm/blob/master/components/mixins/shared/external-source-system-audit-details.schema.json) JSON。
