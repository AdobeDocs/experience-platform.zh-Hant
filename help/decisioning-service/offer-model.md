...
選件決策是決策服務的使用案例，您在此中正式化並集中管理用於吸引客戶提供選件的規則和預測。 選件決策視為內容類型 _**決策**_。 在此使用案例中， _**決策選項**_ 稱為選件 _****_，其特徵在於附加在選件上的內容。 有關決策服務所使用的物件模型的簡介，請參閱決策 [服務網域模型](experience-model.md)。
...
確定內容相依性的複雜細節並非決策服務的責任。 相反地，每個客戶會識別所有通道中的位置清單，並為這些位置指定唯一的識別碼和名稱。 設計人員參照特定位置，斷言特定內容將符合該位置。
