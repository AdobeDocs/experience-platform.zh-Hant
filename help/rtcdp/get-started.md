...
本入門指南引導您完成Real-time Customer Data Platform（即時CDP）的示例實施。 在設定自己的實現時，可以將其用作示例。 雖然本指南顯示了特定的示例，但它連結到在建立安裝程式時可以使用的其他資訊。
...
利用由Adobe Experience Platform支援的即時CDP , Luma的營銷部門可以：
...
使用 [!DNL Real-time Customer Data Platform] 將新資料源引入平台。 即時CDP包括可以快速方便地添加到配置檔案的資料源目錄。
...
一旦應用了資料使用標籤，Luma就可以使用資料管理建立資料使用策略。 資料使用策略是描述允許對包含某些標籤的資料執行的操作類型的規則。 當嘗試在構成策略違規的即時CDP中執行操作時，會阻止該操作，並發出警報以顯示違反了哪個策略以及原因。
...
Real-time CDP配置檔案將Luma營銷團隊的工作流程從數週減少到數分鐘，並基於此360度的客戶視圖解鎖個性化的可能性。 此配置檔案將她登錄前瀏覽站點時的行為資料與她現有的客戶配置檔案合併在一起，建立了Sarah的全面視圖。
...
在即時CDP中，Luma可以無縫地在其受眾群中執行個性化操作。\
...
Luma營銷人員使用Adobe Experience Platform查看即時CDP Dashboard上的禮品贈送部分。 他們觀察了這項計畫的成果，並看到它正在增長。 客戶正在響應優惠，並投入更多資金。
