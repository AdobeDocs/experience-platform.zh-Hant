...
>[!AVAILABILITY]
>
>Real-Time CDP MCP伺服器是以&#x200B;**遠端HTTP傳輸伺服器**&#x200B;的形式分佈，使用者可在支援的MCP使用者端和應用程式平台（例如Claude、ChatGPT、Claude Code、Codex、Cursor或VS Code）中進行安裝與設定。 驗證是透過&#x200B;**瀏覽器式登入流程**&#x200B;來處理 — 當您的使用者端首次連線到伺服器時，它會開啟您的預設瀏覽器，讓您可以使用您的Adobe憑證登入並授權存取。
...
Real-Time CDP MCP伺服器會公開下列工具：

| 工具 | 說明 |
| --- | --- |
| **搜尋現有的對象** | 使用選擇性篩選器（名稱、實體型別、生命週期狀態、名稱空間、來源）列出對象，或依ID擷取特定對象。 |
| **預覽對象成員資格** | 預估區段查詢（設定檔對象為PQL，關聯/帳戶對象為SDD）的大小，包括信賴區間中繼資料。 |
| **列出目的地型別** | 檢視沙箱中可用的目的地聯結器型別目錄。 |
| **列出已設定的帳戶** | 瀏覽已設定的目的地帳戶（基本連線）及其驗證詳細資訊。 |
| **列出已設定的目的地** | 瀏覽目的地資料流，可依名稱、狀態、流程規格或來源/目標連線進行篩選。 |
| **列出Source連線** | 檢查保留目的地資料流之資料集對應資訊的來源連線。 |
| **列出目標連線** | 檢查保留目的地資料格式和路徑設定的目標連線。 |
| **檢查啟動執行** | 檢閱目的地資料流執行歷程記錄、可依流程ID、狀態（成功、失敗、進行中）和完成時間範圍篩選。 |
...
* MCP伺服器端點URL： `https://rtcdp-mcp.adobe.io/mcp`。
...
對於`claude.ai`和Claude Desktop，請使用&#x200B;**將Real-Time CDP MCP伺服器新增為**&#x200B;自訂聯結器`https://rtcdp-mcp.adobe.io/mcp`。 在個別的Claude計畫中，將其新增到&#x200B;**自訂>聯結器**&#x200B;下。 在Team和Enterprise計畫中，擁有者可能需要先在&#x200B;**組織設定> Connectors**&#x200B;底下新增它，之後每個使用者會使用他們自己的Claude設定連線它。 設定之後，請在交談中啟用聯結器，並在第一次使用時完成Adobe瀏覽器登入。

#### ChatGPT

在ChatGPT中，使用&#x200B;**將Real-Time CDP MCP伺服器新增為**&#x200B;自訂應用程式/聯結器`https://rtcdp-mcp.adobe.io/mcp`。 根據您的ChatGPT計畫，這可能需要開發人員模式&#x200B;**和工作區管理員核准。**&#x200B;建立或啟用應用程式/聯結器後，從&#x200B;**設定>應用程式**&#x200B;或&#x200B;**設定>應用程式和聯結器**&#x200B;連線，然後在出現提示時透過Adobe瀏覽器登入進行驗證。

#### 其他以UI為基礎的使用者端

對於使用者端（例如Cursor、VS Code或其他支援遠端MCP的案頭和Web應用程式），請將Real-Time CDP MCP伺服器新增為&#x200B;**遠端HTTP**&#x200B;伺服器，並使用`https://rtcdp-mcp.adobe.io/mcp`。 如果使用者端支援選用的標頭或持有人權杖，請將它們留空，除非Adobe另有指示；驗證會在第一次使用時透過瀏覽器型Adobe登入流程處理。
...
不可以。 MCP伺服器是專為行銷和技術人員所設計。 行銷人員可以在任何支援的MCP使用者端中使用自然語言提示與其互動，而資料工程師和開發人員則可以在支援MCP的開發人員工具中使用它。
+++
...
可以。 `Search Existing Audiences`和`Preview Audience Membership`都支援實體型別引數。 設定檔對象可以用PQL或SDD表示；帳戶對象一律使用SDD （關聯式）語法。
+++
