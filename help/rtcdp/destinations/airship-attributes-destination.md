...
1. 為您的Token提供好記的名稱，例如「Adobe屬性目的地」，並選取「完整存取權」做為角色。
...
1. 在「 **[!UICONTROL 目標]** >目錄 **[!UICONTROL 」中，捲動至「]**&#x200B;行動參與 **** 」類別。 選擇 **[!DNL Airship Attributes]**，然後選擇 **[!UICONTROL 配置]**。
   ...
   ![連接至飛艇屬性](/help/rtcdp/destinations/assets/airship-attributes-in-catalog.png)

2. 在「 **Account** 」（帳戶）步驟中 [!DNL Airship Attributes] ，如果您先前已設定連線至您的目的地，請選取「 **** Existing Account」（現有帳戶）並選取您現有的連線。 或者，您可以選 **[!UICONTROL 擇「新帳戶]** 」來設定新連線 [!DNL Airship Attributes]。 選 **[!UICONTROL 取「連線至目的地]** 」，使用您從控制面板產生的 [!DNL Airship] 不記名Token，將Adobe Experience Platform連線至您的專 [!DNL Airship] 案。

   >[!NOTE]
   >
   >Adobe Experience Platform支援驗證程式中的認證驗證，如果您在帳戶中輸入錯誤的認證，則會顯示錯誤 [!DNL Airship] 訊息。 這可確保您不會以不正確的憑證完成工作流程。

   ![連接至飛艇屬性](/help/rtcdp/destinations/assets/airship1-connect-to-airship.png)
...
4. 在「驗 **[!UICONTROL 證]** 」步驟中，輸入啟 **[!UICONTROL 動流程的「名稱]** 」 **[!UICONTROL 和「說明]** 」。 <br> 此外，在此步驟中，您可以選擇美國或歐盟的資料中心，具體取決於哪 [!DNL Airship] 個資料中心適用於此目標。 最後，選取一或多個將資料匯出至目的地的行銷使用案例。 您可以從Adobe定義的行銷使用案例中選擇，也可以自行建立。 有關行銷使用案例的詳細資訊，請參 [閱即時CDP中的資料治理頁](/help/rtcdp/privacy/data-governance-overview.md#destinations) 。 如需個別Adobe定義之行銷使用案例的詳細資訊，請參閱「資 [料使用政策」概觀](/help/data-governance/policies/overview.md#core-actions)。 <br> 在填 **[!UICONTROL 入上述欄位後]** ，選取「建立目標」。
   ...
   ![連接至飛艇屬性](/help/rtcdp/destinations/assets/airship2-select-airship-domain.png)
...
1. 在「 **[!UICONTROL 目標>瀏覽]**」中，選 [!DNL Airship Attributes] 取您要啟用區段的目標。
   ![activate-flow](/help/rtcdp/destinations/assets/airship-attributes-activate1.png)
1. 按一下目標的名稱。 這會帶您進入「啟動」流程。
請注意，如果目的地已有啟動流程，您可以看到目前傳送至目的地的區段。 選取 **[!UICONTROL 右側邊欄]** 「編輯啟動」，然後依照下列步驟修改啟動詳細資訊。 ![activate-flow](/help/rtcdp/destinations/assets/airship-attributes-activate2.png)
1. 選擇 **[!UICONTROL 激活]**;
1. 在「啟 **[!UICONTROL 動目標]** 」工作流程的「選 **[!UICONTROL 取區段」頁面上，選取要傳送的區段]**[!DNL Airship Attributes]。
   ![區段到目的地](/help/rtcdp/destinations/assets/airship3-select-segments-to-export.png)
1. 在映 **[!UICONTROL 射步驟]** ，從 [](https://docs.adobe.com/content/help/zh-Hant/experience-platform/xdm/home.html) XDM架構中選擇要映射到目標架構的屬性和標識。 選擇 **[!UICONTROL 添加新映射]** ，以瀏覽方案並將其映射到相應的目標標識。
   ...
   ![指名用戶映](/help/rtcdp/destinations/assets/airshiptags7-mappingoption2.png)射：對於應映射到渠道（即設備）的標識符，請根據源映射到相應的渠道。 下圖顯示如何建立兩個映射：
   ...
   **映射標識**選擇源欄位：
   ![連接到飛艇屬性](/help/rtcdp/destinations/assets/airship5-select-source-identity.png)選擇目標欄位：
   ![連接至飛艇屬性](/help/rtcdp/destinations/assets/airship6-select-target-identity.png)

   **地圖屬性**

   選擇源屬性：
   ![選擇源欄位](/help/rtcdp/destinations/assets/airship7-select-source-attributes.png)選擇目標屬性：
   ![選擇目標欄位](/help/rtcdp/destinations/assets/airship8-select-target-attribute.png)驗證映射：
   ![頻道對應](/help/rtcdp/destinations/assets/airship9-mapping-final.png)

1. 在「區 **[!UICONTROL 段排程]** 」頁面上，目前停用排程。 按一 **[!UICONTROL 下「下]** 一步」(Next)以繼續檢閱步驟。 ![目前已停用排程](/help/rtcdp/destinations/assets/airship10-scheduling-step-is-disabled-for-now.png)

1. 在「審 **[!UICONTROL 閱]** 」頁面上，您可以看到您所選項目的摘要。 選擇 **[!UICONTROL 取消]** ，以劃分流程，選擇 **[!UICONTROL 返回]** ，修改設定，或選擇完 **[!UICONTROL 成]** ，確認選擇並開始向目標發送資料。
...
![審查](/help/rtcdp/destinations/assets/airship11-review-step.png)
