...
技術公司Bodea想要結合來自不同獨立資料來源的個人和帳戶資料，以便透過電子郵件和針對其新產品的LinkedIn廣告行銷活動來有效鎖定客戶。 Bodea使用行銷自動化平台，且需要從包含客戶資料的多個CRM中區隔B2B特定對象。
...
在Adobe Experience Platform UI中，Bodea行銷人員在左側邊欄中選取&#x200B;**[!UICONTROL Schemas]**，然後選取&#x200B;**[!UICONTROL Browse]**&#x200B;標籤。 由於它們使用自動產生公用程式，新的空白結構描述會出現在清單中，而且全都有前置詞「B2B」。
...
接著，Bodea行銷人員使用[來源聯結器](../sources/home.md)將資料擷取至Experience Platform，以用於下游服務。 您也可以使用Real-Time CDP B2B edition的其中一個已核准來源來內嵌資料。

>[!NOTE]
> 
>若要瞭解您的組織可以使用哪些來源聯結器，您可以在Experience Platform UI中檢視來源目錄。 若要存取目錄，請在左側導覽中選取&#x200B;**來源**，然後選取&#x200B;**目錄**。

為了建立來源帳戶與Experience Platform之間的連線，您必須取得驗證認證。 如需取得每種來源型別之驗證認證的詳細指示，請參閱[來源概觀](../sources/home.md)。

取得驗證認證後，Bodea行銷人員會建立來源帳戶與其Experience Platform組織之間的連線。 如需設定來源連線的詳細資訊，請參閱[來原始檔](../sources/home.md)。

來源聯結器提供自動對應功能，可讓將所有資料欄位對應到新建立之結構描述的程式更容易。

>[!NOTE]
> 
>如果您已在XDM結構描述中建立自訂欄位群組，在流程的這個階段可能會有未連線的欄位。 請務必勾選填入自訂欄位群組的所有值。

Bodea行銷人員會檢查所有欄位群組是否已適當對應，並透過初始化資料流來繼續來源設定流程。 藉由建立資料流以匯入來源資料，下游Experience Platform服務便可使用傳入資料。 在初始內嵌流程中，資料會以批次形式匯入Experience Platform。 之後，後續擷取的資料會以近乎即時更新的方式串流至設定檔中。

## 建立受眾以評估您的資料

下一個任務是根據來源資料中相關實體的特定屬性，為Bodea的新電子郵件行銷活動建立對象。 在Experience Platform UI中，Bodea行銷人員會先在左側導覽中選取&#x200B;**[!UICONTROL Segments]**，然後選取&#x200B;**[!UICONTROL Create segment]**。
...
成功建立對象後，工作區的[!UICONTROL Details]區段中會提供摘要。 由於目前沒有針對區段定義啟用的目的地，因此Bodea行銷人員需要將受眾匯出至資料集，以便對其進行存取和操作。

在Experience Platform UI的[!UICONTROL Segments]工作區中，Bodea行銷人員選取&#x200B;**[!UICONTROL Activate to destination]**。
