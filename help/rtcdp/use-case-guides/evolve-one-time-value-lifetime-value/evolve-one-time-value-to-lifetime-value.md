...
考慮這些造訪您屬性並偶爾購買您所提供產品或服務的客戶。 您可能會想要建立個人化行銷活動來吸引這些客戶，讓您的品牌可以為他們提供較長期的價值，而非一次性價值。 了解做法：
...
1. 您建立結構描述和資料集，然後針對[!UICONTROL Profile]標籤這些專案。
...
在[!UICONTROL Schemas]的[!DNL Adobe Experience Platform]工作區中管理Experience Data Model (XDM)資源。 您可以檢視並探索[!DNL Adobe]提供的核心資源（例如，[!UICONTROL field groups]），並為您的組織建立自訂資源和結構描述。
...
客戶屬性結構描述以[!UICONTROL XDM Individual Profile]類別表示，包括下列欄位群組：
...
此結構用於建構和參考事件資料，這些資料構成了您的網站或其他相關數位平台上發生的客戶活動。 此資料通常會透過[!DNL Adobe Experience Platform]網頁SDK[擷取到](/help/collection/js/js-overview.md)，且為參考各種瀏覽和轉換事件所必需，這些事件用於觸發歷程、詳細的線上客戶分析和增強的分段功能。
...
客戶數位交易結構描述以[!UICONTROL XDM ExperienceEvent]類別表示，包括下列欄位群組：
...
客戶離線交易結構描述以[!UICONTROL XDM ExperienceEvent]類別表示，包括下列欄位群組：
...
[[!UICONTROL Personal Contact Details]](/help/xdm/field-groups/profile/personal-contact-details.md)是[!DNL XDM Individual Profile]類別的標準結構描述欄位群組，描述個別人員的聯絡資訊。
...
[!DNL Adobe] Web聯結器結構描述以[!UICONTROL XDM ExperienceEvent]類別表示，包括下列欄位群組：

+++Adobe Analytics ExperienceEvent範本（欄位組）

[[!UICONTROL Adobe Analytics ExperienceEvent Full Extension]](/help/xdm/field-groups/event/analytics-full-extension.md)是標準結構描述欄位群組，可擷取Adobe Analytics所收集的一般量度。
...
>[!NOTE]
>
>[!DNL Adobe Journey Optimizer] 並不涵蓋圖中顯示的所有內容。所有[付費媒體廣告](/help/destinations/catalog/social/overview.md)都是在[!UICONTROL destinations] [工作區](/help/destinations/ui/destinations-workspace.md)中建立。
