...
舉個例子，考慮一個認證率較低的家居修繕品牌。該品牌希望為首次訪客提供個人化體驗，無需以前的歷史記錄或事先身份驗證，也不會逐漸依賴第三方 cookie。
...
首先，您需要建立一個合作夥伴 ID 的身分識別命名空間。導覽至左側邊欄中的&#x200B;**[!UICONTROL Customer]** > **[!UICONTROL Identities]**，然後選取畫面右上角的&#x200B;**[!UICONTROL Create identity namespace]**。
...
接下來，建立[!UICONTROL Experience Event]結構描述以保留您稍後將從Web屬性收集的時間序列資料，並確定使用[!UICONTROL XDM ExperienceEvent]作為結構描述的基底類別。 閱讀有關如何[使用 Experience Platform UI 建立結構描述](/help/xdm/ui/resources/schemas.md#create)。
...
此外，您可以建立或重複使用現有欄位群組並將其新增至您的結構描述，以擷取作夥伴提供關於訪客的洞察。閱讀如何[建立欄位群組](/help/xdm/ui/resources/field-groups.md)以及如何[新增欄位](/help/xdm/ui/resources/field-groups.md)至欄位群組。例如，如果您希望針對合作夥伴所提供的洞察 (例如年齡範圍、就業狀況、每月消費能力或購買行為) 進行個人化，請將您的欄位群組加入有關的欄位。
...
與建立結構描述時的步驟類似，您需要啟用要包含在[!UICONTROL Real-Time Customer Profile]中的資料集。 如需啟用資料集以在[!UICONTROL Real-Time Customer Profile]中使用的詳細資訊，請參閱[建立結構描述教學課程。](/help/xdm/tutorials/create-schema-ui.md#profile)

### 在您的 Web 屬性上實施事件資料收集 {#implement-data-collection}

進行您的資料管理配定後，您現在需要在您的 Web 屬性上實施即時事件的[資料收集](/help/collection/home.md)。您必須使用Adobe資料收集程式庫( [!UICONTROL Web SDK])檢測屬性，以收集即時事件呼叫，並將其傳回Real-Time CDP。 該項目由跨多個資料收集元件的數項個別任務組成。

>[!IMPORTANT]
>
>若要擷取合作夥伴提供的屬性，您還必須&#x200B;*將您的 Web 屬性與合作夥伴 API 或其他方法整合，以即時方式呼叫和擷取資料合作夥伴的屬性*。請與您選擇的合作夥伴討論這個方面，因為這不是本教學課程的主題。

首先，使用畫面右上角的應用程式切換器來導覽至&#x200B;**[!UICONTROL Data Collection]**&#x200B;區段。

>[!TIP]
>
>如果您在應用程式切換器中看不到[!UICONTROL Data Collection]，請聯絡您的系統管理員以要求存取權。

![應用程式切換器可進入資料收集部分。](/help/rtcdp/assets/partner-data/onsite-personalization/app-switcher-data-collection.png)

UI的&#x200B;**[!UICONTROL Data Collection]**&#x200B;區段看起來與下圖類似。
...
當您建立資料流時，請在&#x200B;**[!UICONTROL Event schema]**&#x200B;欄位中選取您先前建立的結構描述。

![設定新資料流時醒目顯示的事件結構描述選擇器。](/help/rtcdp/assets/partner-data/onsite-personalization/event-schema-selector-datastream.png)

[從下拉式清單中選取您先前建立的事件資料集](/help/datastreams/configure.md#aep)，勾選「**[!UICONTROL Edge Segmentation]**」和「**[!UICONTROL Personalization Destinations]**」旁的方塊，然後選取「**[!UICONTROL Save]**」。
...
請注意，[!UICONTROL Core]擴充功能已安裝。 您必須安裝另外兩個擴充功能 (依接下來部份的詳述)。
...
使用 [Experience Cloud ID 服務擴充功能](/help/tags/extensions/client/id-service/overview.md)，為所有 Experience Cloud 解決方案的訪客建立以裝置為主的獨特第一方身分識別。在擴充功能目錄中搜尋&#x200B;**[!UICONTROL ID Service]**&#x200B;並進行安裝。 安裝擴充功能時保留所有預設設定。

![擴充功能目錄中的 ID 服務擴充功能視圖。](/help/rtcdp/assets/partner-data/onsite-personalization/id-service-extension.png)

#### 設定環境

接下來，從左側導覽前往的&#x200B;**[!UICONTROL Environments]**&#x200B;區段。 在此步驟中，您必須將您的網站連接到 Adobe Edge Network，以便即時擷取並提供訪客資訊。
...
首先，設定一個 `partnerData` 元素。瀏覽至&#x200B;**[!UICONTROL Data Elements]**&#x200B;區段並選取&#x200B;**[!UICONTROL Create New Data Element]**。

![建立新資料元素。](/help/rtcdp/assets/partner-data/onsite-personalization/create-data-element.gif)

為資料元素命名`partnerData`，保留[!UICONTROL extension]值為[!UICONTROL Core]，並將&#x200B;**[!UICONTROL Data Element Type]**&#x200B;設為&#x200B;**[!UICONTROL JavaScript Variable]**。 在標題為`partnerData`的欄位中輸入&#x200B;**[!UICONTROL JavaScript variable name]**&#x200B;並選取&#x200B;**[!UICONTROL Save]**。

![醒目顯示正確設定 PartnerData 資料元素的選擇。](/help/rtcdp/assets/partner-data/onsite-personalization/create-partnerdata-data-element.png)

若要設定第二個資料元素，請將新變數命名為`pageVisit`、將&#x200B;**[!UICONTROL Extension]**&#x200B;設定為&#x200B;**[!UICONTROL Adobe Experience Platform]**&#x200B;並選擇&#x200B;**[!UICONTROL XDM Object]**&#x200B;做為資料型別。

![醒目顯示正確設定 pageVisit 資料元素的選擇。](/help/rtcdp/assets/partner-data/onsite-personalization/page-visit-data-element.png)

從結構描述中，依據您期望從資料合作夥伴獲得的值，選取與其相對應的第三方屬性。然後，選取標題為&#x200B;**[!UICONTROL Provide entire object]**&#x200B;的選項按鈕。 選擇看起來像資料庫的圖示，然後選擇您之前建立的 `partnerData` 資料元素。

#### 設定規則

在&#x200B;**[!UICONTROL Rules]**&#x200B;區段中，您可以設定網站以傳送個人化要求給Adobe，並將屬性載入您剛建立的資料元素中。 閱讀更多關於[建立規則](/help/tags/ui/managing-resources/rules.md)。

選取 **[!UICONTROL Create new Rule]**。將此規則命名為&#x200B;**[!UICONTROL Personalize]**，並選取&#x200B;**[!UICONTROL Events]**&#x200B;旁的+符號。 選取&#x200B;**[!UICONTROL Page Bottom]**&#x200B;作為事件並儲存。

![規則事件類型部分的選擇。](/help/rtcdp/assets/partner-data/onsite-personalization/add-events-rule.png)

選取&#x200B;**[!UICONTROL Actions]**&#x200B;旁的+符號。 將擴充功能更新為&#x200B;**[!UICONTROL Adobe Experience Platform Web SDK]**&#x200B;並將&#x200B;**[!UICONTROL Action Type]**&#x200B;設為&#x200B;**[!UICONTROL Send event]**。

![規則動作類型部分的選擇。](/help/rtcdp/assets/partner-data/onsite-personalization/add-action-rule.png)

從右側的&#x200B;**[!UICONTROL Type]**&#x200B;下拉式選取器中，選取`web.webpagedetails.pageViews`作為事件型別。
...
向下捲動動作設定清單，並確實勾選標題為&#x200B;**[!UICONTROL Render visual personalization decisions]**&#x200B;的方塊。 若要透過 Adobe Target 或其他類似產品傳遞在網頁上呈現視覺效果的體驗，這項設定非常重要。選取&#x200B;**[!UICONTROL Keep Changes]**，然後&#x200B;**[!UICONTROL Save]**&#x200B;規則。
...
選取&#x200B;**[!UICONTROL Publishing Flow]**，然後選取&#x200B;**[!UICONTROL Add Library]**。

選取&#x200B;**[!UICONTROL Add all Changed Resources]**，為資料庫命名，將環境設定為&#x200B;**[!UICONTROL Development]**&#x200B;並選取&#x200B;**[!UICONTROL Save & Build to Development]**。
...
在Experience Platform UI中，導覽至「**[!UICONTROL Customer]** > **[!UICONTROL Audiences]**」並建立對象以擷取您的網站訪客。
