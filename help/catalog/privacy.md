...
本檔案涵蓋處理儲存於 [!DNL Data Lake].

>[!NOTE]
>
>本指南僅說明如何以Experience Platform形式向Data Lake提出隱私權要求。 如果您也打算向即時客戶個人檔案資料存放區提出隱私權要求，請參閱 [設定檔的隱私權要求處理](../profile/privacy.md) 除了本教學課程之外。
>
...
為 [!DNL Data Lake]，必須為每個個別客戶提供有效的身分值（及其關聯的命名空間），才能找到其資料並據以處理。 因此，所有受隱私權要求約束的資料集，都必須在其相關聯的XDM結構中包含身分描述元。
...
>[!NOTE]
>
>本節說明如何設定 [!DNL Data Lake]. 強烈建議您檢閱 [[!DNL Privacy Service] UI](../privacy-service/ui/overview.md) 或 [[!DNL Privacy Service] API](../privacy-service/api/getting-started.md) 說明檔案，以取得如何提交隱私權工作的完整步驟，包括如何在要求負載中正確格式化已提交的使用者身分資料。

以下章節概述如何為 [!DNL Data Lake] 使用 [!DNL Privacy Service] UI或API。
...
在UI中建立工作請求時，請務必選取 **[!UICONTROL AEP Data Lake]** 和/或 **[!UICONTROL 設定檔]** 在 **[!UICONTROL 產品]** 以處理儲存在 [!DNL Data Lake] 或 [!DNL Real-time Customer Profile]，分別為。

<img src="images/privacy/product-value.png" width="450"><br>

### 使用 API

在API中建立工作請求時，任何 `userIDs` 提供的必須使用特定 `namespace` 和 `type` 視其套用的資料存放區而定。 的ID [!DNL Data Lake] 必須使用 `unregistered` 為 `type` 值和 `namespace` 與 [隱私權標籤](#privacy-labels) 已新增至適用資料集的資料集。

此外， `include` 要求裝載的陣列必須包含要提出要求之不同資料存放區的產品值。 向 [!DNL Data Lake]，陣列必須包含值 `aepDataLake`.

下列請求會為 [!DNL Data Lake]，使用未註冊的 `email_label` 命名空間。 也包含 [!DNL Data Lake] 在 `include` 陣列：
...
當 [!DNL Experience Platform] 從接收刪除請求 [!DNL Privacy Service], [!DNL Platform] 發送確認 [!DNL Privacy Service] 已收到請求，且受影響的資料已標示為刪除。 接著，會從 [!DNL Data Lake] 七天內。 在這七天期間，資料會遭軟性刪除，因此無法供任何使用者存取 [!DNL Platform] 服務。

在未來版本中， [!DNL Platform] 將向發送確認 [!DNL Privacy Service] 資料被實際刪除後。

## 後續步驟

閱讀本檔案後，您便了解處理 [!DNL Data Lake]. 建議您繼續閱讀本指南中提供的檔案，以深入了解如何管理身分資料和建立隱私權工作。

請參閱 [即時客戶設定檔的隱私權要求處理](../profile/privacy.md) 的隱私權要求處理步驟 [!DNL Profile] 儲存。

## 附錄

以下章節包含處理隱私權要求的其他資訊，位於 [!DNL Data Lake].
