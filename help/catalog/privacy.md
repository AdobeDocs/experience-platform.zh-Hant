...
Adobe Experience Platform跨 [!DNL Identity Service] 系統和裝置橋接客戶身分資料。 [!DNL Identity Service] 使用身份名稱空間將身份值與其來源系統關聯，以提供上下文至身份值。 命名空間可以代表一般概念，例如電子郵件地址（「電子郵件」），或將身分識別與特定應用程式(例如Adobe Advertising Cloud ID(「AdCloud」)或Adobe Target ID(「TNTID」))建立關聯。
...
為每個客戶建立隱私權要求時 [!DNL Data Lake]，必須為每個客戶提供有效的識別值（及其相關的名稱空間），以找出其資料並據以處理資料。 因此，所有受隱私請求約束的資料集都必須在其關聯的XDM模式中包含身份描述符。
