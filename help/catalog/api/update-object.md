---
keywords: Experience Platform;home;popular topics;catalog;api；更新對象
...
通過將[!DNL Catalog]對象的ID包含在PATCH請求的路徑中，可以更新該對象的部分。 本文檔介紹對目錄對象執行PATCH操作的兩種方法：
...
成功的回應會傳回包含已更新資料集ID的陣列。 此ID應與PATCH請求中發送的ID匹配。 現在，對此資料集執行GET請求時，只顯示`name`和`description`已更新，而所有其他值保持不變。
...
成功的響應返回包含已更新對象的ID的陣列。 此ID應與PATCH請求中發送的ID匹配。 現在，對此對象執行GET請求時，只顯示`name`和`description`已更新，而所有其他值保持不變。
