...
>additional-url="https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/campaigns/orchestrated-campaigns/data-configuration/schemas-datasets/manual-schema#enable" text="針對協調的行銷活動啟用資料集"

>[!CONTEXTUALHELP]
>id="platform_datasets_enableforlookup_toggle"
>title="啟用供查詢使用"
>abstract="啟用此資料集進行查詢，將其在 Journey Optimizer 中的資料用於個人化、決策和歷程協調流程。"
>additional-url="https://experienceleague.adobe.com/zh-hant/docs/journey-optimizer/using/data-management/lookup-aep-data" text="在 Journey Optimizer 中使用 Adobe Experience Platform 資料"
...
您可以從[!UICONTROL Delete]詳細資料檢視[!UICONTROL Enable a dataset for Profile]或[!UICONTROL Dataset]。 若要檢視可用的動作，請選取UI右上角的&#x200B;**[!UICONTROL ... More]**。 下拉式選單出現。
...
您也可以從[!UICONTROL Browse]標籤上找到的內嵌動作中刪除資料集或新增要與即時客戶個人檔案一起使用的資料集。 如需詳細資訊，請參閱[內嵌動作區段](#inline-actions)。
...
您可以從[!UICONTROL Browse]索引標籤的內嵌選項或[!UICONTROL Dataset activity]檢視，預覽任何資料集的最多100列範例資料。

從[!UICONTROL Browse]索引標籤中，選取資料集名稱旁的省略符號(...)，然後選擇[!UICONTROL Preview dataset]。 如果資料集為空，則會停用預覽選項。 或者，從&#x200B;**[!UICONTROL Dataset activity]**&#x200B;畫面選取靠近熒幕右上角的&#x200B;**[!UICONTROL Preview dataset]**。
...
或者，從&#x200B;**[!UICONTROL Dataset activity]**&#x200B;畫面選取&#x200B;**[!UICONTROL Preview dataset]**&#x200B;以開啟預覽視窗並檢閱資料集結構和值的範例。
...
如果您的組織有Data Distiller授權，您可以直接從資料集預覽視窗存取[!UICONTROL Advanced Query Editor]。 使用此捷徑可順暢地從預覽範例資料移至查詢服務中的執行和修訂查詢。

>[!AVAILABILITY]
>
>[!UICONTROL Advanced Query Editor]的存取權僅限於具有Data Distiller SKU授權的組織。 如果您的組織沒有所需的授權，此選項不會出現在資料集預覽視窗中。

在預覽視窗右上角選取[!UICONTROL Advanced Query Editor]以開啟查詢服務，並預先載入並執行您目前的SQL查詢。 您可以繼續分析或修改SQL，而不需要重新輸入查詢。
...
您可以選取[!UICONTROL Browse]標籤的內嵌選項，以管理資料集的資料控管標籤。 選取您要管理的資料集名稱旁的省略符號(...)，接著從下拉式選單中選取&#x200B;**[!UICONTROL Manage data and access labels]**。
...
您可以從[!UICONTROL Browse]標籤的內嵌選項以及[!UICONTROL Dataset activity]檢視中啟用設定檔的資料集。 從[!UICONTROL Browse]工作區的[!UICONTROL Datasets]索引標籤中，選取您要為設定檔啟用的資料集省略符號。 選單清單中的選項隨即顯示。 接著，從可用選項清單中選取&#x200B;**[!UICONTROL Enable unified profile]**。
...
新增自訂建立的標籤來組織資料集並改善搜尋、篩選和排序功能。 從[!UICONTROL Browse]工作區的[!UICONTROL Datasets]索引標籤中，選取您要管理的資料集省略符號，然後從下拉式選單中選取&#x200B;**[!UICONTROL Manage tags]**。

![資料集工作區的「瀏覽」索引標籤中，針對所選資料集反白顯示省略符號和「管理標籤」選項。](../images/datasets/user-guide/manage-tags.png)

[!UICONTROL Manage tags]對話方塊隨即顯示。 輸入簡短說明以建立自訂標籤，或從現有的標籤中選擇，以標籤您的資料集。 選取&#x200B;**[!UICONTROL Save]**&#x200B;以確認您的設定。

![反白顯示自訂標籤的「管理標籤」對話方塊。](../images/datasets/user-guide/manage-tags-dialog.png)

[!UICONTROL Manage tags]對話方塊也可以從資料集中移除現有標籤。 只要選取您要移除之標籤旁的&#39;x&#39;，然後選取&#x200B;**[!UICONTROL Save]**&#x200B;即可。
...
使用[!UICONTROL Browse]工作區之[!UICONTROL Datasets]索引標籤的內嵌動作功能表，管理資料集到期日和保留設定。 您可以使用此功能來設定資料在資料湖和個人資料存放區中保留的時間長度。 到期日是根據資料擷取至Experience Platform的時間和您設定的保留期間而定。

>[!IMPORTANT]
>
>若要套用或更新ExperienceEvent資料集的保留規則，您的使用者角色必須包含&#x200B;**[!UICONTROL Manage datasets]**&#x200B;許可權。 此角色型存取控制可確保只有授權的使用者才能修改資料集保留設定。
>
...
若要設定保留期間，請從下拉式選單中選取資料集旁的省略符號，然後選取&#x200B;**[!UICONTROL Set data retention policy]**。

![資料集工作區的[瀏覽]索引標籤中，省略符號和[設定資料保留原則]選項已反白顯示。](../images/datasets/user-guide/set-data-retention-policy-dropdown.png)

[!UICONTROL Set dataset retention]對話方塊隨即顯示。 此對話方塊會顯示沙箱層級授權使用量度、資料集層級詳細資料和目前的資料保留設定。 這些量度會顯示與您的權益相較之下的使用量，並協助您評估資料集特定的儲存和保留設定。 量度包括資料集名稱、型別、設定檔啟用狀態，以及資料湖和設定檔存放區使用情況。
...
若要開啟不同保留原則對儲存體影響的視覺預測，請選取&#x200B;**[!UICONTROL View Experience Event Data distribution]**。
...
當您對設定感到滿意時，請選取&#x200B;**[!UICONTROL Save]**&#x200B;以確認您的設定。
...
四個新欄可讓您更清楚地瞭解您的資料管理： **[!UICONTROL Data Lake Storage]**、**[!UICONTROL Data Lake Retention]**、**[!UICONTROL Profile Storage]**&#x200B;和&#x200B;**[!UICONTROL Profile Retention]**。 這些量度會顯示您的資料在資料湖和設定檔服務中使用的儲存空間及其保留期。
...
| 欄標題 | 說明 |
|---|---|
| [!UICONTROL Data Lake Retention] | 資料湖中每個資料集的目前保留期。 此值可設定，並決定在刪除之前保留資料的時間。 |
| [!UICONTROL Data Lake Storage] | Data Lake中每個資料集的目前儲存使用量。 使用此量度來管理儲存空間限制並最佳化使用量。 |
| [!UICONTROL Profile Storage] | 設定檔服務中每個資料集的目前儲存使用量。 協助監控儲存耗用量，並支援資料管理決策。 |
| [!UICONTROL Profile Retention] | 設定檔資料集的目前保留期。 您可以更新此值以控制設定檔資料保留的時間長度。 |
...
您可以將資料集放在資料夾中，以改善資料集管理。 若要將資料集移至資料夾，請選取您要管理的資料集名稱旁的省略符號(...)，然後從下拉式選單中選取&#x200B;**[!UICONTROL Move to folder]**。

![以橢圓和[!UICONTROL Datasets]反白顯示的[!UICONTROL Move to folder]儀表板。](../images/datasets/user-guide/move-to-folder.png)

[!UICONTROL Move]資料集至資料夾對話方塊就會顯示。 選取您要移動對象的資料夾，然後選取&#x200B;**[!UICONTROL Move]**。 快顯通知會通知您資料集移動成功。

![反白顯示[!UICONTROL Move]的[!UICONTROL Move]資料集對話方塊。](../images/datasets/user-guide/move-dialog.png)

>[!TIP]
>
>您也可以直接從「行動資料集」對話方塊建立資料夾。 若要建立資料夾，請選取建立資料夾圖示(![建立資料夾圖示。](/help/images/icons/folder-add.png))。
>
>![反白顯示建立資料夾圖示的[!UICONTROL Move]資料集對話方塊。](/help/catalog/images/datasets/user-guide/create-folder.png)

資料集放入資料夾後，您可以選擇僅顯示屬於特定資料夾的資料集。 若要開啟您的資料夾結構，請選取顯示資料夾圖示（![顯示資料夾圖示](/help/images/icons/rail-left.png)）。 接著，選取您選擇的資料夾以檢視所有關聯的資料集。

![已顯示資料集資料夾結構的[!UICONTROL Datasets]儀表板、顯示資料夾圖示，以及反白顯示的選取資料夾。](../images/datasets/user-guide/folder-structure.png)

### 刪除資料集 {#delete}

您可以從[!UICONTROL Browse]索引標籤或[!UICONTROL Dataset activity]檢視右上角的資料集內嵌動作中刪除資料集。 從[!UICONTROL Browse]檢視中，選取您要刪除的資料集名稱旁的省略符號(...)。 選單清單中的選項隨即顯示。 接著，從下拉式功能表中選取&#x200B;**[!UICONTROL Delete]**。

![資料集工作區的「瀏覽」索引標籤具有省略符號，並且所選資料集的「刪除」選項反白顯示。](../images/datasets/user-guide/inline-delete-dataset.png)

確認對話方塊隨即顯示。 選取&#x200B;**[!UICONTROL Delete]**&#x200B;以確認。

或者，從&#x200B;**[!UICONTROL Delete dataset]**&#x200B;畫面選取&#x200B;**[!UICONTROL Dataset activity]**。
...
確認方塊隨即顯示。 選取&#x200B;**[!UICONTROL Delete]**&#x200B;以確認刪除資料集。
...
依預設，只會顯示您擷取資料的資料集。 如果您想要檢視系統產生的資料集，請選取「**[!UICONTROL Yes]**」區段中的「[!UICONTROL Show system datasets]」核取方塊。 系統產生的資料集僅用於處理其他元件。 例如，系統產生的設定檔匯出資料集可用來處理設定檔控制面板。

![強調顯示[!UICONTROL Show system datasets]區段的資料集工作區的篩選選項。](../images/datasets/user-guide/show-system-datasets.png)

### 篩選設定檔啟用的資料集 {#filter-profile-enabled-datasets}

為設定檔資料啟用的資料集可在擷取資料後，用來填入客戶設定檔。 請參閱[為設定檔](#enable-profile)啟用資料集的相關小節以瞭解更多資訊。

若要根據資料集是否已為設定檔啟用來篩選資料集，請從篩選選項中選取[!UICONTROL Yes]核取方塊。

![強調顯示[!UICONTROL Included in Profile]區段的資料集工作區的篩選選項。](../images/datasets/user-guide/included-in-profile.png)

### 依標籤篩選資料集 {#filter-by-tag}

在[!UICONTROL Tags]輸入中輸入您的自訂標簽名稱，然後從可用選項清單中選取您的標籤，以搜尋和篩選與該標籤對應的資料集。

![資料集工作區的篩選選項，其中反白顯示[!UICONTROL Tags]輸入和篩選圖示。](../images/datasets/user-guide/filter-tags.png)

### 依建立日期篩選資料集 {#filter-by-creation-date}

資料集可以在自訂時段內依建立日期進行篩選。 這可用來排除歷史資料，或產生依時間順序排列的資料深入分析和報表。 選取每個欄位的行事曆圖示，以選擇[!UICONTROL Start date]和[!UICONTROL End date]。 之後，只有符合該條件的資料集才會出現在瀏覽標籤中。

### 依修改日期篩選資料集 {#filter-by-modified-date}

與建立日期的篩選類似，您可以根據資料集的上次修改日期來篩選資料集。 在[!UICONTROL Modified date]區段中，選取每個欄位的行事曆圖示，以選擇[!UICONTROL Start date]和[!UICONTROL End date]。 之後，只有在該期間修改的資料集才會出現在瀏覽標籤中。
...
[!UICONTROL Browse]索引標籤中的資料集可依遞增或遞減日期排序。 選取[!UICONTROL Created]或[!UICONTROL Last updated]欄標題，在升序和降序之間切換。 選取後，欄會以向上或向下箭頭指向欄標題的側邊來指示此專案。

![資料集工作區的[瀏覽]索引標籤，其中反白顯示已建立和上次更新的資料行。](../images/datasets/user-guide/ascending-descending-columns.png)

## 建立資料集 {#create}

若要建立新資料集，請從&#x200B;**[!UICONTROL Create dataset]**&#x200B;儀表板中選取&#x200B;**[!UICONTROL Datasets]**&#x200B;開始。
...
在&#x200B;**[!UICONTROL Create dataset]**&#x200B;畫面中，選取&#x200B;**[!UICONTROL Create dataset from schema]**&#x200B;以建立新的空白資料集。

![[從結構描述建立資料集]按鈕已反白顯示。](../images/datasets/user-guide/create-dataset-schema.png)

**[!UICONTROL Select schema]**&#x200B;步驟隨即顯示。 在選取&#x200B;**[!UICONTROL Next]**&#x200B;之前，瀏覽結構描述清單並選取資料集將遵循的結構描述。

![顯示結構描述清單。 將用來建立資料集的結構描述已反白顯示。](../images/datasets/user-guide/select-schema.png)

**[!UICONTROL Configure dataset]**&#x200B;步驟隨即顯示。 提供名稱與選擇性說明的資料集，然後選取「**[!UICONTROL Finish]**」以建立資料集。
...
在[!DNL Experience Platform] UI中，選取左側導覽中的&#x200B;**[!UICONTROL Monitoring]**。 **[!UICONTROL Monitoring]**&#x200B;儀表板可讓您檢視批次或串流擷取的傳入資料狀態。 若要檢視個別批次的狀態，請選取&#x200B;**[!UICONTROL Batch end-to-end]**&#x200B;或&#x200B;**[!UICONTROL Streaming end-to-end]**。 控制面板會列出所有批次或串流擷取執行，包括成功、失敗或仍在進行的作業。 每個清單都提供批次的詳細資訊，包括批次ID、目標資料集的名稱和擷取的記錄數。 如果為[!DNL Profile]啟用目標資料集，也會顯示擷取的身分和設定檔記錄數。

![顯示監控批次端對端畫面。 監視和批次對批次都會反白顯示。](../images/datasets/user-guide/batch-listing.png)

您可以選取個別&#x200B;**[!UICONTROL Batch ID]**&#x200B;以存取&#x200B;**[!UICONTROL Batch overview]**&#x200B;儀表板，並檢視批次的詳細資料，包括批次擷取失敗時的錯誤記錄。

![顯示選取批次的詳細資料。 這包括擷取的記錄數、失敗的記錄數、批次狀態、檔案大小、擷取開始和結束時間、資料集和批次ID、組織ID、資料集名稱和存取資訊。](../images/datasets/user-guide/batch-overview.png)

如果您想要刪除批次，請選取儀表板右上角附近的&#x200B;**[!UICONTROL Delete batch]**。 刪除批次也會從批次最初擷取的資料集中移除其記錄。
