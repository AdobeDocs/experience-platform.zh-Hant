...
1. [確認按即時客戶配置檔案接收資料](#confirm-data-ingest-by-real-time-customer-profile)
...
您可以在建立時立即或在建立資料集後的任何時間為Real-time Customer Profile和Identity Service啟用資料集。 如果要啟用已建立的資料集，請按照 [配置現有資料集](#configure-an-existing-dataset) 的下一頁。
