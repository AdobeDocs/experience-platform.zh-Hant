...
| 屬性 | 類型 | 說明 |
| --- | --- | --- |
| `schemas` | `String[]` | *可選*. 在決策中使用的方案清單，用於選擇返回的聘用類型。 |
| `scopes` | `String[]` | *可選*. 決策範圍清單。 每個請求最多30個。 |
...
| `payload.scopeDetails.decisionProvider` | 字串 | 要麼 `TGT` 如果使用Adobe Target `ODE` 如果 [!DNL Offer Decisioning]。 |
