...
>[!WARNING]
>
>`/extension_packages`端點的實作會隨著功能的新增、移除和修改而不斷變化。

擴充功能套件代表由擴充功能開發人員製作的[擴充功能](./extensions.md)。 擴充功能套件定義了可供標籤使用者使用的其他功能。 這些功能大多以[規則元件](./rule-components.md)（事件、條件和動作）和[資料元素](./data-elements.md)的形式提供，但也可以包括主模組和共用模組。

擴充功能套件會顯示在資料收集UI內的擴充功能目錄中，供使用者安裝。 將擴充功能套件新增至屬性，是透過建立包含擴充功能套件連結的擴充功能來完成。

擴充功能套件屬於建立該套件的開發人員的[company](./companies.md)。
...
您可以向`/extension_packages`發出GET請求，以檢索擴展包的清單。
...
>[!NOTE]
>
>使用查詢參數，可根據下列屬性篩選列出的擴充功能套件：<ul><li>`archive`</li><li>`created_at`</li><li>`name`</li><li>`stage`</li><li>`token`</li><li>`updated_at`</li></ul>如需詳細資訊，請參閱[篩選回應](../guides/filtering.md)的指南。
...
| 參數 | 說明 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 要查找的擴展包的`id`。 |
...
下列請求會建立新的擴充功能套件。 要上載的包檔案的本地路徑被引用為表單資料(`package`)，因此此，此終結點需要`Content-Type`標頭`multipart/form-data`。
...
| 參數 | 說明 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 要更新的擴展包的`id`。 |
...
私下發行後，您可以填寫[公開發行請求表單](https://adobe.allegiancetech.com/cgi-bin/qwebcorporate.dll?idx=7DRB5U)，以開始公開發行程式。
...
| 參數 | 說明 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 您要私下發行之擴充功能套件的`id`。 |
...
您可以透過PATCH要求，將其`discontinued`屬性設為`true`，以停止擴充功能套件。
...
| 參數 | 說明 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 要終止的擴展包的`id`。 |
...
您可以將`/versions`附加至查詢請求的路徑，以列出擴充功能套件的版本。
...
| 參數 | 說明 |
| --- | --- |
| `EXTENSION_PACKAGE_ID` | 要列出其版本的擴充功能套件的`id`。 |
