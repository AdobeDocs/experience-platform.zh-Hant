---
title: 設定檔端點
description: 瞭解如何在Reactor API中呼叫/profiles端點。
...
在Reactor API中，設定檔代表Adobe Experience Platform使用者。 Reactor API不會維護自己的使用者與許可權資料庫，而是仰賴[Adobe的識別管理系統(IMS)](https://helpx.adobe.com/tw/enterprise/using/identity.html)所管理的AdobeID。

設定檔包含有關登入使用者的所有資訊，包括所屬的所有組織、每個組織中所屬的產品設定檔，以及每個產品設定檔所擁有的許可權。
...
您可以向`/profile`端點發出GET要求，以擷取目前登入之設定檔的詳細資料。
