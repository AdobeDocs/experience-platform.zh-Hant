...
>[!NOTE]
>
>Adobe Experience Platform Launch已重新命名為Experience Platform中的資料收集技術套件。 因此，所有產品文件中出現了幾項術語變更。 如需術語變更的彙整參考資料，請參閱以下[文件](../../../launch-term-updates.md)。

>[!IMPORTANT]
>
>擴充功能旨在搭配 [!DNL Adobe Experience Platform Web SDK] 擴充功能。 若要查看與AppMeasurement搭配使用之版本的相關資訊，請參閱此連結。

請使用此參考資料來了解有關設定Web SDK外掛程式擴充功能的資訊，以及使用此擴充功能擴大時可用的選項 [!DNL Adobe Experience Platform Web SDK] 擴充功能。
...
>[!IMPORTANT]
>
>常見Web SDK外掛程式擴充功能旨在增強 [!DNL Adobe Experience Platform Web SDK] 不過，您不需要安裝擴充功能，擴充功能即可正常運作。

不必額外設定擴充功能層級。

## 在 [!DNL Adobe Experience Platform Web SDK] 擴充功能新增外掛程式

與AppMeasurement對應項目不同，除了使用提供的原生資料元素外，您不需要進行設定即可初始化外掛程式或將其新增至程式庫。 如需每個資料元素的其他資訊，請參閱下方。

## 常見Web SDK外掛程式擴充功能資料元素

常見Web SDK外掛程式擴充功能提供下列資料元素：
...
可讓使用者運用Adobe Experience Platform中的原生資料收集UI，來設定和設定 [getAndPersistValue外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getandpersistvalue.html). 此 `getAndPersistValue` 資料元素可讓您將值儲存在cookie中，以便稍後造訪時擷取。 它的角色與 Adobe Experience Platform Launch 的[!UICONTROL 「儲存期間」]功能類似。
...
可讓使用者運用Adobe Experience Platform中的原生資料收集UI，來設定和設定 [getGeoCoordinates外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getgeocoordinates.html). 此 `getGeoCoordinates` 資料元素可讓您擷取訪客裝置的經緯度。
...
可讓使用者運用原生資料收集UI來設定和設定 [getNewRepeat外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getnewrepeat.html). 此 `getNewRepeat` 資料元素可讓您判斷網站訪客是新訪客還是在指定天數內回訪的重複訪客。
...
可讓使用者運用原生資料收集UI來設定和設定 [getPageName外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpagename.html). 此 `getPageName` 資料元素可建立目前URL易讀、好記的格式化版本。
...
可讓使用者運用原生資料收集UI來設定和設定 [getPreviousValue外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpreviousvalue.html). 此 `getPreviousValue` 資料元素可讓您將變數設為先前點擊上設定的值。
...
可讓使用者運用原生資料收集UI來設定和設定 [getQueryParam外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getqueryparam.html). 此 `getQueryParam` 資料元素可讓您擷取URL中包含之任何查詢字串參數的值。 如需從登陸頁面 URL 擷取內部和外部促銷活動程式碼，此外掛程式非常有用。擷取搜尋詞或其他查詢字串參數時，它也很有用。此資料元素提供強大的功能，可剖析複雜的URL，包括雜湊和包含多個查詢字串參數的URL。
...
可讓使用者運用原生資料收集UI來設定和設定 [getTimeParting外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimeparting.html). 此 `getTimeParting` 資料元素可讓您擷取網站上發生任何可測量活動的時間詳細資訊。 如果您想要依指定日期範圍內任何可重複的時間劃分來劃分量度，此資料元素就十分實用。 例如，您可以比較一週內兩天之間的轉換率，例如所有週日比較所有週四。您也可以比較一天中的時段，例如所有早上比較所有晚上。
...
可讓使用者運用原生資料收集UI來設定和設定 [getTimeSinceLastVisit外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimesincelastvisit.html). 此 `getTimeSinceLastVisit` 資料元素可讓您追蹤訪客在上次造訪後回訪您網站所花的時間。
...
可讓使用者運用原生資料收集UI來設定和設定 [getValOnce外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvalonce.html). 此 `getValOnce` 資料元素可防止變數多次設為等於相同值。
...
可讓使用者運用原生資料收集UI來設定和設定 [getVisitDuration外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvisitduration.html). 此 `getVisitDuration` 資料元素會追蹤訪客截至該時間點為止在網站上逗留的時間長度，以分鐘為單位。
...
可讓使用者運用原生資料收集UI來設定和設定 [getVisitNum外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvisitnum.html). 此 `getVisitNum` 資料元素會傳回在指定天數內造訪過網站的所有訪客造訪次數。
...
可讓使用者運用原生資料收集UI來設定和設定 [p_fo外掛程式](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/p-fo.html). 此 `p_fo` 資料元素是公用程式，可檢查特定JavaScript物件是否存在。 如果物件不存在，外掛程式將會建立該物件並傳回 `true`。如果頁面上已存在 JavaScript 物件，則會傳回 `false`。若要在頁面上執行一次程式碼，此資料元素很實用。
