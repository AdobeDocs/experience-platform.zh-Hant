...
# Adobe MediumAnalytics for Audio and Video擴充功能概觀
...
Media Analytics (MA) 擴充功能新增核心的 JavaScript Media SDK (Media 2.x SDK)。此擴充功能提供將`MediaHeartbeat`追蹤器例項新增至標籤網站或專案的功能。 MA 擴充功能需要額外的擴充功能：
...
在您的標籤專案中納入上述的三個擴充功能後，您可以透過下列兩種方式之一繼續操作：
...
* **安裝 —** 若要安裝MA擴充功能，請開啟擴充功能屬性，選取「擴充 **[!UICONTROL 功能>目錄]**」，將游標暫留在「Adobe Medium **[!UICONTROL Analytics for Audio and Videoextension」上，然]** 後選取「安 **[!UICONTROL 裝」]**。

* **配置 —** 要配置MA擴展，請開啟「Extensionstab(擴  展)」頁簽，將游標暫留在擴展上，然後選擇「 **[!UICONTROL 配置]**」：
...
MA擴充功能會啟用[!UICONTROL Configuration]頁面中的「將API匯出至視窗物件」設定，以將MediaHeartbeat API匯出至全域視窗物件中。 這會以設定的變數名稱匯出 API。例如，如果變數名稱設定為 `ADB`，則可使用 `window.ADB.MediaHeartbeat` 來存取 MediaHeartbeat。
...
MA 擴充功能會將 `get-instance` 模組和 `media-heartbeat` 共用模組公開至其他擴充功能(如需共用模組的其他資訊，請參閱[共用模組文件](https://developer.adobelaunch.com/extensions/shared_modules/))。
...
1. **建立 MediaHeartbeat 例項：**`get-instance`共用模組
   ...
   * 選用的設定物件會顯示這些屬性：
      ...
      | 屬性 | 說明 | 必填 |
      | :--- | :--- | :--- |
      | 線上視頻提供程式 | 線上視訊平台的名稱，透過該平台來分送內容。 | 否。如果存在，則會在擴充功能設定期間覆寫定義的值。 |
      | 播放器名稱 | 使用的媒體播放器名稱 (例如：「AVPlayer」、「HTML5 播放器」、「我的自訂視訊播放器」) | 否。如果存在，則會在擴充功能設定期間覆寫定義的值。 |
      | 管道 | 管道名稱屬性 | 否。如果存在，則會在擴充功能設定期間覆寫定義的值。 |
   **傳回值：**&#x200B;一種 Promise，使用 `MediaHeartbeat` 例項解析，或拒絕並提供錯誤訊息。
