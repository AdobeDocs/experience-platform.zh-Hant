...
當您使用連結延遲時，Platform實際上會防止瀏覽器導覽離開頁面。 接著，系統會在指定的逾時後，執行JavaScript重新導向，前往原始目的地。 當您的頁面標籤具有`<a>`標籤時，預期的功能實際上並未將使用者導覽離開頁面時，這會特別危險。 如果您無法以任何其他方式解決問題，您應該將選擇器的定義精確到極致，讓此事件將完全觸發您需要的位置，而不會觸發其他位置。
