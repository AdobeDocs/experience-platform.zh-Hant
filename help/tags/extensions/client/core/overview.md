...
## 核心擴充功能事件型別 {#core-extension-event-types}

本主題說明核心擴充功能中可用的事件型別。 如需可針對多種不同事件型別設定的選項相關資訊，請參閱[選項](#options)區段。
...
表單失去焦點時，模糊事件會觸發動作。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### 聚焦

焦點事件會在表單取得焦點時觸發此動作。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### 提交

提交表單時，提交事件會觸發動作。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

### 鍵盤控制事件

#### 按鍵點按

按下按鍵時，就會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

### 媒體型事件

#### 媒體已結束

媒體結束時，就會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### 媒體載入的資料

媒體載入資料時會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### 媒體暫停

媒體暫停時會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### 媒體播放

媒體播放時會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### 媒體已中止

媒體停止時會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

#### Media-Time已播放

如果媒體播放了指定的時間長度，則會觸發此事件。 您必須指定媒體必須播放的持續時間，才能觸發事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。


#### Media-Volume已變更

如果音量提高或降低，則會觸發此事件。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。
...
如果元素是連結內容的錨點標籤(`<a>`)，您也可以指定是否將導覽延遲一段時間。 如果您的規則需要額外的執行時間，而且通常無法在頁面導覽發生之前完成，此功能會很有用。

>[!WARNING]
>
>使用此選項時請格外小心，因為如果使用不正確，可能會對使用者體驗產生負面後果。

當您使用連結延遲時，Experience Platform實際上會防止瀏覽器導覽離開頁面。 接著，系統會在指定的逾時後，執行JavaScript重新導向，前往原始目的地。 當您的頁面標籤具有`<a>`標籤時，預期的功能實際上並未將使用者導覽離開頁面時，這會特別危險。 如果您無法以任何其他方式解決問題，您應該將選擇器的定義精確到極致，讓此事件將完全觸發您需要的位置，而不會觸發其他位置。
...
如需進階選項的詳細資訊，請參閱[選項](#options)區段。

#### 游標停留

如果使用者將游標停留在指定的元素上，則會觸發事件。 您也必須設定要立即觸發規則，還是在指定的毫秒數之後觸發。 如需可自訂事件設定的詳細資訊，請參閱[選項](#options)一節。

### 其他事件

#### 自訂事件

如果發生自訂事件型別，則會觸發事件。 在程式碼基底中的其他位置定義的已命名JavaScript函式，可作為自訂事件型別使用。 您必須指定自訂事件型別的名稱，並依照下方[選項](#options)一節所述設定任何其他設定。
...
呼叫`_satellite.pageBottom();`後，事件就會觸發。 非同步載入標籤程式庫時，不應使用此事件型別。 此事件類型沒有設定。
...
本節說明核心擴充功能中可用的條件型別。 這些條件類型可與正常或例外邏輯類型一起使用。
...
此動作會觸發使用特定[直接呼叫事件](#direct-call-event)的所有規則。 設定動作時，您必須提供要觸發之直接呼叫事件的識別碼字串。 或者，您也可以透過`detail`物件將資料傳遞至直接呼叫事件，該物件可包含自訂的索引鍵值配對集。
...
標籤資料元素可用於擷取標籤JavaScript變數或物件屬性。 接著，您可在擴充功能或自訂規則中，藉由參照標籤資料元素來使用這些值。 如果資料來源變更，則只需更新來源的參照即可。
...
選取多個資料元素，每個元素都會提供物件。 這些物件將深層（遞回）合併在一起，以產生新物件。 將不會修改來源物件。 如果在多個來源物件的相同位置找到屬性，則會使用後一個物件的值。 如果來源屬性值是`undefined`，它不會覆寫先前來源物件的值。 如果在多個來源物件的相同位置找到陣列，則會串連陣列。
...
與「頁面資訊」類似，此資料元素使用常見行為型別，在規則和其他Experience Platform解決方案中擴充邏輯。
...
[值比較](#value-comparison-value-comparison)條件的包裝函式。 根據比較的結果，將會傳回表單中兩個可用值之一。 因此可以處理「如果……然後……否則……」不需要額外規則的情況。

### 執行階段環境

可讓您選取下列其中一個變數：

* 環境階段 — 傳回`_satellite.environment.stage`以區分開發/階段/生產環境。
* 程式庫建置日期 — 傳回`turbine.buildInfo.buildDate`，其中包含相同的值，例如`_satellite.buildInfo.buildDate`。
* 屬性名稱 — 傳回`_satellite.property.name`以取得Launch屬性的名稱。
* 屬性ID — 傳回`_satellite.property.id`以取得Launch屬性的識別碼
* 規則名稱 — 傳回包含已執行規則名稱的`event.$rule.name`。
* 規則識別碼 — 傳回包含已執行規則識別碼的`event.$rule.id`。
* 事件型別 — 傳回包含觸發規則之事件型別的`event.$type`。
* 事件詳細資料裝載 — 傳回包含自訂事件或直接呼叫規則之裝載的`event.detail`。
* 直接呼叫識別碼 — 傳回包含直接呼叫規則識別碼的`event.identifier`。
...
它是常見JavaScript操作的包裝函式。 它會接收資料元素作為輸入。 它會傳回資料元素值的下列其中一個轉換的結果：
