...
焦點事件會在表單取得焦點時觸發此動作。 請參閱 [選項](#options) 區段以取得可自訂事件設定的詳細資訊。
...
按下按鍵時，就會觸發此事件。 請參閱 [選項](#options) 區段以取得可自訂事件設定的詳細資訊。

### 媒體型事件

#### 媒體已結束

媒體結束時，就會觸發此事件。 請參閱 [選項](#options) 區段以取得可自訂事件設定的詳細資訊。
...
如果使用者將游標停留在指定的元素上，則會觸發事件。 您也必須設定要立即觸發規則，還是在指定的毫秒數之後觸發。 請參閱 [選項](#options) 區段以取得可自訂事件設定的詳細資訊。

### 其他事件

#### 自訂事件

如果發生自訂事件型別，則會觸發事件。 在程式碼基底的其他位置定義的已命名JavaScript函式，可作為自訂事件型別使用。 您必須指定自訂事件型別的名稱，並依照所述配置任何其他設定。 [選項](#options) 一節。
...
定義直接呼叫事件時，您必須指定將當作此事件識別碼的字串。 如果 [觸發直接呼叫動作](#direct-call-action) 會引發包含相同識別碼的任何直接呼叫事件規則，監聽該識別碼。

![資料收集UI中直接呼叫事件的熒幕擷圖](../../../images/extensions/client/core/direct-call-event.png)

#### 元素存在

如果指定的元素存在，則觸發此事件。 請參閱 [選項](#options) 區段以取得可自訂事件設定的詳細資訊。
...
* 如果您選擇 **[!UICONTROL 任何元素]**&#x200B;中，不需要進階選項來縮小元素。
...
此動作會觸發使用特定規則的所有規則 [直接呼叫事件](#direct-call-event). 設定動作時，您必須提供要觸發之直接呼叫事件的識別碼字串。 您也可以選擇透過，將資料傳遞至直接呼叫事件。 `detail` 物件，可包含一組自訂的索引鍵/值組。

![資料收集UI中觸發直接呼叫動作的熒幕擷圖](../../../images/extensions/client/core/direct-call-action.png)

此動作直接對應至 [`track` 方法](../../../ui/client-side/satellite-object.md#track) 在 `satellite` 物件，可由使用者端代碼存取。
...
的包裝函式 [值比較](#value-comparison-value-comparison) 條件。 根據比較的結果，將會傳回表單中兩個可用值之一。 因此可以處理「如果……然後……否則……」不需要額外規則的情況。
