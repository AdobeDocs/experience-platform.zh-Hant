...
>[!IMPORTANT]
>
>此擴充功能旨在搭配Adobe Experience Platform Web SDK擴充功能使用。 若要檢視預期與AppMeasurement搭配使用的版本資訊，請參閱[常見Analytics外掛程式擴充功能](../plugins/overview.md)的概觀。

本文介紹如何設定Web SDK外掛程式標籤擴充功能，並用來增強[Adobe Experience Platform Web SDK擴充功能](../web-sdk/overview.md)。

## 設定常見Web SDK外掛程式擴充功能

本節提供設定網頁SDK外掛程式擴充功能時可用選項的參考資料。
...
除了使用「常用Web SDK外掛程式」擴充功能提供的下列原生資料元素之外，不需要進行設定，即可初始化或新增外掛程式至您的程式庫：
...
* `"latitude=[LATITUDE] | longtitude=[LONGITUDE]"`：其中`[LATITUDE]/[LONGITUDE]`分別代表緯度與經度
...
* `ep` （選用，字串）：只有在也設定了`et`引數時才設定此引數。 如果您希望`"m"`引數在幾分鐘內而不是幾天內到期，請將此引數設為`et`。 預設為`"d"`，以天為單位設定`et`引數。
