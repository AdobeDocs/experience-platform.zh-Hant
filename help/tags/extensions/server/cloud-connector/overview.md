---
title: 雲連接器擴展概述
description: 瞭解Adobe Experience Platform的雲連接器事件轉發擴展。
...
雲連接器事件轉發擴展允許您建立自定義HTTP請求以將資料發送到目標或從目標檢索資料。 Cloud Connector 擴充功能就像 Adobe Experience Platform Edge Network 的郵差一樣，可將資料傳送至尚未具備專屬擴充功能的端點。
...
要選擇端點所需的請求類型，請在 [!UICONTROL 請求類型] 下拉清單。
...
為您要以查詢字串參數形式傳送的每個索引鍵/值組定義索引鍵和值。要手動輸入資料元素，請使用大括弧資料元素標籤來轉發事件。 若要以索引鍵或值的形式參照資料元素「siteSection」的值，請輸入 `{{siteSection}}`。或者，您也可以從下拉式選單中選取先前建立的資料元素。

要添加更多查詢參數，請選擇 **[!UICONTROL 添加其他]**。

#### 標頭

為您要以標頭傳送的每個索引鍵/值組定義索引鍵和值。要手動輸入資料元素，請使用大括弧資料元素標籤來轉發事件。 若要以索引鍵或值的形式參照資料元素「pageName」的值，請輸入 `{{pageName}}`。或者，您也可以從下拉式選單中選取先前建立的資料元素。

要添加更多標題，請選擇 **[!UICONTROL 添加其他]**。
...
為您要在要求內文中傳送的每個索引鍵/值組定義索引鍵和值。要手動輸入資料元素，請使用大括弧資料元素標籤來轉發事件。 若要以索引鍵或值的形式參照資料元素「appSection」的值，請輸入 `{{appSection}}`。或者，您也可以從下拉式選單中選取先前建立的資料元素。

要添加其他鍵值對，請選擇 **[!UICONTROL 添加其他]**。

#### Body as Raw

為您要在要求內文中傳送的每個索引鍵/值組定義索引鍵和值。要手動輸入資料元素，請使用大括弧資料元素標籤來轉發事件。 若要以索引鍵或值的形式參照資料元素「appSection」的值，請輸入 `{{appSection}}`。或者，您也可以從下拉式選單中選取先前建立的資料元素。您可以新增一或多個資料元素。

### 進階

規則內的事件轉發操作按順序執行。 某些情況下，您或許可以從外部來源擷取不在用戶端傳入事件上的資料，然後接受此回應，並在單一規則內的後續動作中轉換資料或將資料傳送至最終目的地。進階區段中的「儲存要求回應」會啟用此功能。

要從端點保存響應體，請檢查 **[!UICONTROL 保存請求響應]** 框中，定義文本欄位中的響應鍵。
