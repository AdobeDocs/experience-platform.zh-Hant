...
* 如果您有要最後執行的規則，您可以編輯該規則元件，並指定大於50（例如60）的數字。
* 如果有要先執行的規則，您可以編輯該規則元件，並指定小於50（例如40）的數字。
...
執行階段環境的行為取決於 **[!UICONTROL 依序執行規則元件]** 是開啟或關閉屬性。 此設定會決定是否可以同時（非同步）評估規則的元件，或必須依序評估元件。

>[!IMPORTANT]
>
>此設定只會決定如何評估每個規則中的條件和動作，不會影響在屬性上執行規則本身的順序。 請參閱 [規則排序](#rule-ordering) 以取得如何判斷多個規則之執行順序的詳細資訊。
>
>在 [事件轉送](../event-forwarding/overview.md) 屬性，規則動作一律會依序執行，且此設定無法使用。 建立規則時，請確認順序正確無誤。
