...
**動作 (Then)：**&#x200B;在規則的事件發生且滿足所有條件之後，就會觸發。標籤規則可讓您視需要觸發許多獨立動作，而且您可以控制這些動作發生的順序。 例如，電子商務「感謝您」頁面的單一規則可以從單一規則觸發您的分析工具和第三方標籤。不需要為各個擴充功能或標籤建立不同規則。
...
1. 設定 Order 參數，然後選取「**[!UICONTROL Keep Changes]**」。
...
1. 選取條件&#x200B;**[!UICONTROL Add]**&#x200B;圖示，然後選取邏輯型別、擴充功能、條件型別並設定條件的相關設定。 接著選取「**[!UICONTROL Keep Changes]**」。
...
1. 選取[動作&#x200B;**[!UICONTROL Add]**]圖示，然後選取您的擴充功能以及該擴充功能可用的其中一個動作型別，設定動作的設定，然後選取&#x200B;**[!UICONTROL Keep Changes]**。

   ![規則動作設定頁面。](../../images/action-settings.png)

   可選擇的動作型別取決於您選取的擴充功能。 動作設定會因動作類型而異。

   （進階）等候執行下一個動作：在屬性上啟用規則元件排序後，即可使用此選項。 一旦勾選，此動作完成前，標籤不會呼叫下一個動作。 取消勾選後，下一個動作會立即開始執行。 預設值為 **[!UICONTROL Checked]**。

   （進階）逾時：在屬性上啟用規則元件排序後，即可使用此選項。 這會定義動作完成的時間長度上限。 如果逾時，動作就會執行失敗，系統會從處理佇列中移除此規則的任何後續動作。 預設值為 2000 ms。


1. 檢閱您的規則，然後選取「**[!UICONTROL Save Rule]**」。
...
>[!NOTE]
>
>最終，執行動作的責任在於使用您正在使用之事件型別的擴充功能開發人員。 Adobe擴充功能開發人員可確保擴充功能正常運作。 Adobe為協力廠商擴充功能開發人員提供指引，協助他們正確執行這項工作，但無法保證會如何遵循這些指引。
...
這些事件幾乎都會執行 (除非條件評估為 false)，因此為了提高效率，這些事件會整合至主程式庫，也就是您嵌入程式碼所參考的檔案。
...
執行階段環境的行為取決於您屬性的&#x200B;**[!UICONTROL Run rule components in sequence]**&#x200B;是否開啟。 此設定決定是否可並行（非同步）評估規則的元件，或是否必須依序評估元件。

>[!IMPORTANT]
>
>此設定只會決定每個規則中條件和動作的評估方式，不會影響規則本身在屬性上執行的順序。 如需如何決定多個規則的執行順序的詳細資訊，請參閱先前有關[規則順序](#rule-ordering)的章節。
>
>在[事件轉送](../event-forwarding/overview.md)屬性中，規則動作一律依序執行，而且此設定無法使用。 建立規則時，請確認順序正確無誤。
