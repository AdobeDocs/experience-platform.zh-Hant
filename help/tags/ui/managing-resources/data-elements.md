...
如果您想要在標籤中收集JavaScript變數或物件屬性，並搭配任何擴充功能或規則使用，則可使用資料元素來擷取這些值。 這樣的話，您可以在全部規則中參照資料元素，而如果資料來源有所變更，您只需在資料收集UI的單一位置變更來源的參照（資料元素）即可。
...
使用此資料元素產生隨機數字。它通常用於取樣資料或建立 ID，例如點擊 ID。隨機數字也可用來模糊化敏感資料，或對其執行 Salt 處理。某些範例可能包括：
...
如果您先前曾使用下列任何資料元素，則必須在資料收集UI中建立自訂資料元素：
