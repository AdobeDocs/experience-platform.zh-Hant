...
如果要在標注中收集JavaScript變數或對象屬性，並將其與任何副檔名或規則一起使用，則可以使用資料元素捕獲這些值。 這樣，您就可以在整個規則中引用資料元素，如果資料的源發生變化，則只需在資料收集UI中的一個位置更改對源（資料元素）的引用。
...
如果以前使用過以下任何資料元素，則必須在資料收集UI中建立自定義資料元素：
