...
例如，如果您有參考頁面名稱或產品ID或從附屬行銷連結的查詢字串參數或從[!DNL AdWords]等來源抓取資訊的特定方式，可透過資料字典（資料元素）的來源取得資訊，然後在各種標籤規則中使用此資料，借此建立資料字典（資料元素）。
...
1. 從「屬性」頁開啟[!UICONTROL 資料元素]頁簽，然後選擇&#x200B;**[!UICONTROL 建立新資料元素]**。
1. 命名資料元素。
1. 選取擴充功能和類型。
...
使用`%..%`標籤法時，您只需指定資料元素名稱。 不需指定 `event`。
...
在[!UICONTROL 本地儲存項目名稱]欄位中提供本地儲存項目的名稱。
...
在[!UICONTROL URL參數]欄位中指定單一URL參數。
...
在[!UICONTROL 會話儲存項名]欄位中提供會話儲存項的名稱。
