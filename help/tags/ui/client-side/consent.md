...
Adobe Experience Platform不會自動引發[!DNL Adobe Advertising]。 只有在您明確指定規則動作時，才會引發 [!DNL Advertising]。使用規則條件來判斷要在何時引發以及要引發的行為。例如，若要使用 Cookie 判斷選擇加入狀態，請設定資料元素以讀取該 Cookie 並將其用作規則中的條件，以判斷何時引發「追蹤轉換」動作。
