...
>[!NOTE]
>
>您可以使用百分比(`%`)語法，用於引用資料收集UI中許多表單域的變數，從而減少調用 `_satellite.getVar()`。 例如，使用 `%product%` 將訪問product資料元素或自定義變數的值。
...
>[!NOTE]
>
>在舊 [`setCookie`](#setCookie) 設定cookie的方法，此函式調用的第三個（可選）參數是一個整數，表示cookie的生存時間(TTL)（以天為單位）。 在此新方法中，「屬性」對象被接受為第三個參數。 要使用新方法設定Cookie的TTL，必須提供 `expires` 屬性，並將其設定為所需值。 下面的示例說明了這一點。
