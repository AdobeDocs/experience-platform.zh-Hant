...
## 檢視建置詳細資料 {#build-details}

>[!IMPORTANT]
>
>程式庫會儲存您標籤資源的修訂版本，但&#x200B;**Build**&#x200B;是該程式庫的時間點快照，包含傳送至您網站的檔案。

可以從&#x200B;**資料庫**&#x200B;或&#x200B;**環境**&#x200B;存取組建和組建詳細資訊，以檢視目前的即時組建，並檢查組建包含的內容（擴充功能、資料元素和規則）。

### 從程式庫中檢視組建詳細資訊

在您的標籤屬性中，開啟&#x200B;**[!UICONTROL Publishing Flow]**&#x200B;並選取資料庫。

![在資料集合UI中強調顯示程式庫的發佈流程。](../publishing/images/builds/library.png)

在詳細資訊面板中，您可以檢閱下列內容：

* **[!UICONTROL Last Build Environment]** — 連結到收到上次組建的環境。 指出此程式庫是否為環境的目前組建（**目前**&#x200B;或&#x200B;**非目前**）。
* **[!UICONTROL Current Builds]** — 目前在其環境中上線的組建。 對於已發佈的程式庫，此區段中的閃電圖示會指出即時生產組建。
* 對於列出的每個組建，您都可以檢視：
   * **[!UICONTROL Status]** — 組建建立時。
   * **[!UICONTROL Environment]** — 部署組建的環境。
   * **[!UICONTROL User]** — 建立組建的使用者。

![資料庫詳細資料顯示在右邊的詳細資料面板中](../publishing/images/builds/library-details.png)

### 從環境檢視組建

組建與環境以及為該環境建立的程式庫相關聯。 組建實際上包含編譯的資源。

從詳細資料面板中選取&#x200B;**[!UICONTROL Environment]**。 「環境詳細資料」面板會顯示最近建置的清單、目前的即時建置及相關程式庫。

![強調目前組建的環境詳細資料面板。](../publishing/images/builds/environment.png)

接著，選取組建以開啟其詳細資料。 組建詳細資料顯示該組建中包含的&#x200B;**擴充功能**、**資料元素**&#x200B;和&#x200B;**規則**。

![強調擴充功能、資料元素和規則的組建詳細資料。](../publishing/images/builds/build.png)

>[!NOTE]
>
>組建可以包含比程式庫上列出的資源更多的資源。 組建中封裝的&#x200B;**擴充功能**、**資料元素**&#x200B;和&#x200B;**規則**&#x200B;包含程式庫的內容以及上游內容。 這是發佈至網站或應用程式的完整快照。

使用詳細資料面板導覽回&#x200B;**[!UICONTROL Environment]**&#x200B;或&#x200B;**[!UICONTROL Library]**。
