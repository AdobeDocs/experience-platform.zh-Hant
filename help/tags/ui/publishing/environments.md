...
若要存取環境的安裝指示，請瀏覽至您屬性的「**[!UICONTROL 環境]**」標籤，然後選取該環境的「**[!UICONTROL 安裝]**」圖示。
...
選取&#x200B;**[!UICONTROL 主機]**，從下拉式功能表中選擇為環境預先設定的主機。
...
內嵌程式碼是必須放置在網站頁面`<head>`區段中的`<script>`標籤，這樣才能載入並執行您建置的程式碼。 每個環境設定都會自動產生各自的內嵌程式碼，因此您只需加以複製並貼到要執行標籤的網站頁面上。
...
在您的屬性的[!UICONTROL 環境]索引標籤上，選取&#x200B;**[!UICONTROL 新增環境]**。
...
[!UICONTROL 環境]索引標籤會重新出現，並顯示新環境的安裝指示。
