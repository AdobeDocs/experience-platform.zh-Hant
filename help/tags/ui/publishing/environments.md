...
標籤環境會針對您在網站或應用程式上部署的程式庫組建，定義幾個主要層面：
...
標籤支援三種不同的環境型別，每種都對應於 [發佈工作流程](./publishing-flow.md)：
...
若要存取環境的安裝指示，請導覽至 **[!UICONTROL 環境]** 索引標籤中，然後選取 **[!UICONTROL 安裝]** 圖示來識別該環境。
...
在 [!UICONTROL 環境] 索引標籤中，選取列出的環境以顯示其設定控制項。

![](./images/environments/environment-config.png)

### Host {#host}

選取 **[!UICONTROL 主機]** ，從下拉式選單中為環境選擇預先設定的主機。
...
內嵌程式碼是 `<script>` 標籤必須放置在 `<head>` 網站頁面的區段，以載入和執行您建置的程式碼。 每個環境設定都會自動產生各自的內嵌程式碼，因此您只需要將內嵌程式碼複製並貼到您要執行標籤的網站頁面上。
...
>[!WARNING]
>
>當標籤環境的內嵌程式碼變更時，您必須手動更新HTML中的內嵌程式碼。 建議您只有在絕對必要時更新內嵌程式碼，以免維護成本過高。

## 建立環境

首次建立屬性時，系統會自動將三個環境指派給屬性：開發、預備和生產。這三種環境足以執行發佈工作流程。不過，您可視需要新增其他開發環境，因為這對同時有多位開發人員處理不同專案的大型團隊非常有用。

於 [!UICONTROL 環境] 索引標籤中，選取 **[!UICONTROL 新增環境]**.
...
下一個畫面可讓您為新環境命名、選取主機，以及選擇封存設定。完成後，選取 **[!UICONTROL 儲存]** 以建立環境。

![](./images/environments/create-config.png)

此 [!UICONTROL 環境] 標籤會重新出現，並顯示新環境的安裝指示。
