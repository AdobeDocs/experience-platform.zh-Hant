---
title: 在事件轉發中配置機密
description: 瞭解如何在資料收集UI中配置機密，以驗證在事件轉發屬性中使用的終結點。
...
| [!UICONTROL OAuth2] | 包含多個屬性以支援 [OAuth2](https://datatracker.ietf.org/doc/html/rfc6749) 驗證規範。 系統會要求您提供所需資訊，然後在指定的時間間隔內為您處理這些令牌的續訂。 當前僅 [客戶端憑據](https://datatracker.ietf.org/doc/html/rfc6749#section-1.3.4) 支援OAuth2的版本。 |

{style=&quot;table-layout:auto&quot;}

本指南提供了如何為事件轉發配置機密([!UICONTROL 邊緣])屬性。
...
本指南假定您已經熟悉如何在資料收集UI中管理標籤和事件轉發的資源，包括如何建立資料元素和事件轉發規則。 請參閱上的指南 [管理資源](../managing-resources/overview.md) 如果需要介紹。

您還應瞭解標籤和事件轉發的發佈流程，包括如何向庫添加資源並在您的網站上安裝內部版本以供測試。 查看 [發佈概述](../publishing/overview.md) 的子菜單。

## 建立密碼 {#create}

要建立機密，請登錄到資料收集UI並開啟要在其中添加機密的事件轉發屬性。 下一步，選擇 **[!UICONTROL 秘密]** 在左側導航中，然後 **[!UICONTROL 建立新密碼]**。

![建立新密碼](../../images/ui/event-forwarding/secrets/create-new-secret.png)

下一個螢幕允許您配置機密的詳細資訊。 為了使機密可通過事件轉發使用，必須將其分配給現有環境。 如果沒有為事件轉發屬性建立任何環境，請參見上的指南 [環境](../publishing/environments.md) 有關如何在繼續操作之前配置它們的指導。
...
要建立OAuth2密鑰，請選擇 **[!UICONTROL OAuth2]** 從 **[!UICONTROL 類型]** 下拉清單。 在下面顯示的欄位中，提供 [[!UICONTROL 客戶端ID] 和 [!UICONTROL 客戶端密碼]](https://www.oauth.com/oauth2-servers/client-registration/client-id-secret/)，以及 [授權URL](https://www.oauth.com/oauth2-servers/access-tokens/client-credentials/) 用於OAuth整合。 的 [!UICONTROL 授權URL] 資料收集UI中的欄位是授權伺服器主機和令牌路徑之間的級聯。

![OAuth2密碼](../../images/ui/event-forwarding/secrets/oauth-secret-1.png)
...
![保存OAuth2偏移量](../../images/ui/event-forwarding/secrets/oauth-secret-4.png)
...
| [!UICONTROL OAuth2] | 選擇 **[!UICONTROL 生成令牌]** 交換憑據並從身份驗證提供程式返回訪問令牌。 |
...
本指南介紹了如何管理資料收集UI中的機密。 有關如何使用Reactor API與機密交互的資訊，請參見 [機密端點指南](../../api/endpoints/secrets.md)。
