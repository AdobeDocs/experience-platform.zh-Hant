...
在整個API指南中，您將看到對 `TENANT_ID`。 此ID用於確保您建立的資源與IMS組織中的資源保持正確的命名空間。 如果您不知道您的ID，可以通過執行以下GET請求來訪問它：
...
別被你的獨特 `TENANT_ID`，也請參見Wiki頁。 `tenant` 容器包含由IMS組織定義的所有類、欄位組、資料類型、方案和描述符。 這是每個組織所獨有的，這意味著它們不能被其他IMS組織看到或管理。 您可以對在中建立的資源執行所有CRUD操作(GET、POST、PUT、PATCH、DELETE) `tenant` 容器。
