...
在本教學課程中，選取「描述檔 **[!UICONTROL 人員詳細資料]** 」混合，然後選 **[!UICONTROL 取「新增混合」]**。

![](../images/tutorials/create-schema/add_mixin_person_details.png)

結構畫布會重新出現。 Mixins **[!UICONTROL 區段現在會列]** 出「Profile person details[!UICONTROL 」（描述檔人員詳細資訊），而]Structure **** 區段則包含mixin貢獻的欄位。 您可以在「 **[!UICONTROL Mixins]** 」區段下選取mixin的名稱，以反白顯示它在畫布中提供的特定欄位。
...
請注意， `name` 欄位的資料類型為「[!UICONTROL Full name]」，這表示它也說明一個通用概念，並包含與名稱相關的子欄位，例如名字、姓氏、字首和字尾。

選擇畫布中的不同欄位，以顯示它們對架構結構所貢獻的其他欄位。

## 新增另一個混音 {#mixin-2}

您現在可以重複相同的步驟來新增另一個混音。 此時，當您檢視「 **[!UICONTROL Add mixin]** 」（新增混音）對話方塊時，請注意，「[!UICONTROL Profile person details]」混音已變灰，且旁邊的核取方塊無法選取。 這可防止意外複製您已包含在目前架構中的混音。

在本教學課程中，從對話方[!DNL Profile personal details]塊中選取「」混音，然後選取「 **[!UICONTROL 新增混音]** 」將其新增至架構。

![](../images/tutorials/create-schema/add_mixin_personal_details.png)

新增後，畫布會重新顯示。 「[!UICONTROL Profile personal]details **[!UICONTROL 」現在會列在「Composition]** 」（合成）區段的「 **[!UICONTROL Mixins]** 」（設定檔個人詳細資訊）下，而「 **** Structure」（結構）下則會新增家庭位址、行動電話等欄位。
...
將應用更改，並顯示新建立 `loyalty` 的對象。 選取 **[!UICONTROL 物件旁的]** 「新增欄位」，以新增其他與忠誠度相關的欄位。 「新[!UICONTROL 欄位]」隨即出現，畫布的右側 **[!UICONTROL 會顯示「欄位屬性]** 」區段。
...
每個欄位都會新增，方 **[!UICONTROL 法是選取物件上的]** 「新 `loyalty` 增」欄位並填入所需資訊。
...
在本教學課程中， [!DNL "loyalty"] 架構中的物件需要新的列舉欄位，以說明客戶的「忠誠度等級」，其中值只能是四個可能選項之一。 要將此欄位添加到方案中，請選擇對 **[!UICONTROL 像旁邊的]** 「添加欄位」, `loyalty` 並填寫「欄位名 **[!UICONTROL 」和「顯]** 示名」的必需欄位 ****。 對於 **[!UICONTROL 類型]**，選擇「[!UICONTROL 字串]」。
...
套用變更後，指令的圖示 `loyaltyId` 會顯示指紋符號，表示它現在是識別欄位。 此外，左側邊欄 [!DNL Loyalty Details] 中的混音會列出其下方的識別欄位，讓您輕鬆判斷架構中哪個混音會提供該架構的識別欄位。
