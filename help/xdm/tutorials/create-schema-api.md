...
若要檢視新建立的結構，請使用結構的URL編碼URI `meta:altId` 執行查 `$id` 閱(GET)請求。
...
可使用的標準混音不同，視所選的架構類別而定。 每個mixin都包 `intendedToExtend` 含一個欄位，該欄位定義了該mixin與其相容的類。
...
任何自訂屬性都必須巢狀內嵌在您的下 `TENANT_ID` 方，以避免與其他混音或欄位衝突。
...
您現在可以依照相同的步驟， [新增標準混音](#add-a-mixin) ，將新建的混音新增至您的架構。
...
您可以看到已成功新增mixin，因為回應現在會在陣列中顯示新新增的mixin, `meta:extends` 並在屬 `$ref` 性中包含mixin `allOf` 。
...
使用「接 `application/vnd.adobe.xed-full+json; version=1` 受」標題，您可以看到顯示所有屬性的完整架構。 這些屬性是類別和混合所貢獻的欄位，這些欄位已用來組成架構。 在此示例響應中，已最小化空間的個別屬性屬性。 您可以在本文檔末尾的附錄中查看完整方案，包括所 [有屬性](#appendix) 及其屬性。

在下 `"properties"`方，您可以看到在 `_{TENANT_ID}` 新增自訂混音時建立的命名空間。 在該名稱空間中是「忠誠度」物件，以及建立混音時定義的欄位。
...
您可以使用URL編碼的 `$id` URI來執行查閱(GET)請求，以直接檢視新的資料類型。 請務必在查閱請 `version` 求的「接受」標題中加入。
...
回應現在包含對「`$ref`loyalty」物件中資料類型的參考()，而非先前定義的欄位。
...
通常標示為「身分」的欄位包括：電子郵件地址、電 [話號碼、Experience Cloud ID(ECID)](https://marketing.adobe.com/resources/help/en_US/mcvid/)、CRM ID或其他唯一ID欄位。

請考慮您組織專屬的任何唯一識別碼，因為這些識別碼可能也是好的識別碼欄位。

身分描述符信號表明&quot;sourceSchema&quot;的&quot;sourceProperty&quot;是應被視為&quot;Identity&quot;的唯一標識符。

有關使用描述符的詳細資訊，請參 [閱Schema Registry開發人員指南](../api/getting-started.md)。
...
響應顯示操作已成功執行，而模式現在包含一個頂級屬性， `meta:immutableTags`該屬性是包含值&quot;union&quot;的陣列。
...
建立多個模式後，可以通過使用關係描述符來定義它們之間的關係。 如需詳細資訊，請 [參閱定義兩個結構之間的關係](relationship-api.md) 的教學課程。 有關如何在註冊表中執行所有操作（GET、POST、PUT、PATCH和DELETE）的詳細示例，請在使用API時參閱 [Schema Registry Developer Guide](../api/getting-started.md) 。

## 附錄 {#appendix}

以下資訊補充API教學課程。

## 完整忠誠度成員結構 {#complete-schema}
