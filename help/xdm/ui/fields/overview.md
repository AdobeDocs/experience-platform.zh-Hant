...
| 欄位屬性 | 說明 |
| --- | --- |
| [!UICONTROL 欄位名稱] | 欄位的唯一描述性名稱。 請注意，一旦保存了架構，就無法更改欄位的名稱。<br><br>最好用camelCase寫名。 它可能包含字母數字、短划線或下划線字元，但 **不能** 以下划線開頭。<ul><li>**正確**: `fieldName`</li><li>**可接受：** `field_name2`。 `Field-Name`。 `field-name_3`</li><li>**錯誤**: `_fieldName`</li></ul> |
| [!UICONTROL 顯示名稱] | 這地方的人性化名稱。 |
...
畫布將更新以顯示欄位的名稱和類型，右欄將列出欄位的路徑以及其他屬性。
...
| [!UICONTROL 圖案] | [!UICONTROL 字串] | A [規則運算式](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions) 該欄位的值必須符合，才能在接收期間接受。 |
