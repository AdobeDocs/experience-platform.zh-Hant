...
>[!TIP]
>
>使用搜尋功能，根據類別名稱篩選或尋找類別。 請參閱以下指南： [探索XDM資源](../explore.md) 以取得詳細資訊。
...
如果您選取 **[!UICONTROL 建立類別]**，則 [!UICONTROL 建立類別] 對話方塊隨即顯示。 輸入 [!UICONTROL 顯示名稱] 和 [!UICONTROL 說明] ，並使用選項按鈕選擇您類別的預期行為。 類別可以是型別、記錄序列或時間序列。 選取 **[!UICONTROL 建立]** 以確認您的選擇並返回 [!UICONTROL 類別] 標籤。

![此 [!UICONTROL 建立類別] 對話方塊 [!UICONTROL 建立] 反白顯示。](../../images/ui/resources/classes/create-class-dialog.png)

您建立的類別可供使用，並會列在 [!UICONTROL 類別] 檢視。

![此 [!UICONTROL 類別] 的標籤 [!UICONTROL 方案] 工作區中反白顯示最近建立的類別。](../../images/ui/resources/classes/new-class-listing.png)

### 建立或編輯類別 {#create-or-edit}

或者，如果您選取 **[!UICONTROL 建立結構描述]**，則 [!UICONTROL 建立結構描述] 工作流程隨即顯示。 在 [!UICONTROL 結構描述詳細資料] 區段，選取 **[!UICONTROL 其他]**. 可用的類別清單隨即顯示。 從這裡，您可以瀏覽並篩選作為新類別基礎的現有類別。
...
此 [!UICONTROL 名稱和評論] 工作流程的區段隨即顯示。 在此區段中，提供名稱和說明來識別您的結構描述。&#x200B;URL結構描述的基本結構（由類別提供）會顯示在畫布中，供您檢閱及驗證選取的類別和結構描述結構。
...
隨即顯示「結構描述編輯器」，其結構描述結構顯示在畫布中。 您現在可以開始 [將欄位新增至類別](#add-fields).

![結構描述編輯器，其結構描述顯示在畫布中。](../../images/ui/resources/classes/edit.png)
...
![結構描述編輯器反白顯示新增按鈕。](../../images/ui/resources/classes/add-field.png)
...
![結構描述編輯器畫布中的無標題欄位，其指派給類別欄位屬性已選取並反白顯示。](../../images/ui/resources/classes/assign-to-class.png)

請參閱以下指南： [在UI中定義欄位](../fields/overview.md#define) 有關如何設定欄位並將其新增到類別的特定步驟。 繼續將所需數量的欄位新增至類別。 完成後，選取 **[!UICONTROL 儲存]** 以儲存架構和類別。

![在架構編輯器的畫布上新建立的架構，包含 [!UICONTROL 儲存] 反白顯示。](../../images/ui/resources/classes/save.png)
