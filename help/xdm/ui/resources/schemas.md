...
>[!IMPORTANT]
>
>即使模式編輯器在功能上允許您將單個欄位直接添加到模式，但這不會改變以下事實： XDM模式中的所有欄位都必須由其類或與該類相容的欄位組提供。 如下各節所說明的，在將各個欄位添加到架構時，仍會將它們作為關鍵步驟與欄位組相關聯。
...
在此處，提供欄位的顯示名稱和資料類型。 下 **[!UICONTROL 分配欄位組]**，必須為要關聯的新欄位選擇一個欄位組。 開始鍵入欄位組的名稱，如果您以前 [建立自定義域組](./field-groups.md#create) 它們將顯示在下拉清單中。 或者，可以在欄位中鍵入唯一名稱，以改為建立新欄位組。
