...
提供顯示名稱和資料型別後 for 欄位，下一步是將欄位指派給上層XDM資源。 如果您的結構描述使用自訂類別，您可以選擇將欄位[新增到指派的類別](#add-to-class)或[欄位群組](#add-to-field-group)。 但是，如果您的結構描述使用標準類別，則只能將自訂欄位指派給欄位群組。
...
接下來，您必須選取要與新欄位產生關聯的欄位群組。 在提供的文字輸入中開始鍵入欄位群組的名稱。 如果您有任何符合輸入的現有自訂欄位群組，它們將會顯示在下拉式清單中。 或者，您可以輸入 a 建立新欄位群組的唯一名稱。

![選取欄位群組](../../images/ui/resources/schemas/select-field-group.png)

>[!WARNING]
>
如果您選取現有的自訂欄位群組，則採用該欄位群組的任何其他結構描述在您儲存變更後，也將繼承新新增的欄位。 因此，如果您想要此型別，請僅選取現有的欄位群組 of 傳播。 否則，您應該選擇建立新的自訂欄位群組。
...
如果您使用的結構描述具有物件型別 field 由標準欄位群組提供，您可以將自己的自訂欄位新增到該標準物件。
