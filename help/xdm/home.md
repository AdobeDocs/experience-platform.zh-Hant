...
您也可以使用Platform UI中的[!DNL Schema Registry API]或[!UICONTROL 結構描述]工作區，建立和管理貴組織專屬的新結構描述和資源。
...
* **臨機**：擷取僅供單一資料集使用的名稱空間欄位。 臨機結構用於各種資料擷取工作流程中以進行Experience Platform，包括擷取CSV檔案和建立特定型別的來源連線。
...
藉由資料科學Workspace，資料科學家可以輕鬆建立由機器學習提供支援的智慧型服務API。 這些服務可與其他Adobe解決方案搭配使用，包括Adobe Target和Adobe Analytics Cloud，協助您自動化個人化、鎖定目標的數位體驗。
