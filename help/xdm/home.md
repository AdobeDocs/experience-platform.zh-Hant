...
>title="資料行為"
>abstract="用於 Experience Platform 的資料分為三種行為類型：記錄、時間序列和臨時。記錄方案會提供有關主體屬性的資訊，而時間序列方案則會在採取動作時擷取系統的快照。臨時結構描述會擷取名稱空間僅供單一資料集使用的欄位。 如需有關 Platform 中資料行為的詳細資訊，請查看此文件。"
