...
>[!NOTE]
>
>「mixin」一詞已更新為結構「欄位群組」，以促進了解。 欄位群組是可重複使用的欄位集，可支援業務使用案例。 Schema Registry API、Adobe Experience Platform UI和所有Platform檔案現在都會反映這項變更。
...
XDM是基本架構，可讓Adobe Experience Cloud(由Experience Platform提供技術支援)在適當的時間，透過適當的管道向適當的人員傳遞適當的訊息。 建置Experience Platform的方法 — XDM System，可操作[!DNL Experience Data Model]結構，供Platform服務使用。
...
有關架構構成模型的詳細資訊，包括設計原則和最佳實踐，請參閱架構構成的[基本知識](schema/composition.md)。
...
Experience Platform提供了用戶介面和RESTful API，您可以從中查看和管理Experience Platform **[!DNL Schema Library]**&#x200B;中的所有與架構相關的資源。 [!DNL Schema Library]包含由Adobe提供給您的標準XDM元件，以及來自您使用其應用程式的Experience Platform合作夥伴和供應商的資源。

使用Platform UI中的[!DNL Schema Registry API]或[!UICONTROL 結構]工作區，您也可以建立及管理貴組織特有的新結構和資源。
...
雖然您可以在[!DNL Schema Registry]中定義自己的類，但建議您分別使用首選類&#x200B;**[!UICONTROL XDM個別設定檔]**&#x200B;和&#x200B;**[!UICONTROL XDM ExperienceEvent]**&#x200B;來記錄和時間序列資料。 以下將更詳細地介紹這些類。

### [!UICONTROL XDM個別設定檔] {#xdm-individual-profile}

[!UICONTROL XDM個別設] 定檔是以記錄為基礎的類別，可形成已識別和部分識別主題之屬性的單一表示法。高度識別的配置檔案可用於個人通信或目標性參與，並且可以包含詳細的個人資訊，如姓名、性別、出生日期、位置和聯繫資訊，包括電話號碼和電子郵件地址。
...
有關類提供的欄位的結構和使用案例的詳細資訊，請參閱[[!UICONTROL  XDM單個配置檔案]參考指南](./classes/individual-profile.md)。
...
如需類別所提供欄位的結構和使用案例的詳細資訊，請參閱[[!UICONTROL  XDM ExperienceEvent]參考指南](./classes/experienceevent.md) 。
...
有關目錄的詳細資訊，請參閱[目錄服務概述](../catalog/home.md)。 如需Adobe Experience Platform資料擷取的詳細資訊，請參閱[資料擷取概述](../ingestion/home.md)。
...
有關服務的詳細資訊，請參閱[查詢服務概述](../query-service/home.md)。

### 即時客戶個人檔案

即時客戶設定檔提供集中化的消費者設定檔，以便進行目標式和個人化的體驗管理。 每個設定檔都包含所有系統中匯總的資料，以及涉及個人事件的可操作的時間戳記帳戶，這些事件發生在您搭配Experience Platform使用的任何系統中。

即時客戶設定檔會根據[!UICONTROL XDM個別設定檔]和[!UICONTROL XDM ExperienceEvent]類別取用結構格式化資料，並根據該資料回應查詢。 設定檔不支援使用以其他類別為基礎的結構描述。
...
當[!UICONTROL XDM個別設定檔]和[!UICONTROL XDM ExperienceEvent]資料內嵌至資料湖時，即時客戶設定檔會內嵌已啟用供其使用的任何資料。 擷取的互動和詳細資訊越多，個別設定檔就越健全。

[!UICONTROL XDM個別設] 定檔資料可協助您為任何管道或Adobe產品整合的動作提供相關資訊並賦予其權能。與豐富的行為和互動資料記錄搭配使用時，這些資料可用來推動機器學習。 即時客戶設定檔API也可用來擴充協力廠商解決方案、CRM和專屬解決方案的功能。

如需詳細資訊，請參閱[即時客戶設定檔概述](../profile/home.md) 。

### Data Science Workspace

Adobe Experience Platform Data Science Workspace使用機器學習和人工智慧，從儲存在Experience Platform中的資料中獲得深入分析。 資料科學工作區可讓資料科學家根據[!UICONTROL XDM個人設定檔]和[!UICONTROL XDM ExperienceEvent]客戶及其活動的相關資料建立配方，以利於預測，例如購買傾向和建議的選件，供個人可能欣賞和使用。
...
要了解合成要與Experience Platform一起使用的架構的設計原則和最佳做法，請從閱讀架構合成的[基本知識](schema/composition.md)開始。 如需如何建立架構的逐步指示，請參閱使用API](tutorials/create-schema-api.md)或使用者介面](tutorials/create-schema-ui.md)建立架構[的教學課程。[

若要加深您對Experience Platform[!DNL XDM System]的了解，請觀看以下影片：
