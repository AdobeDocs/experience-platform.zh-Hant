...
### [!UICONTROL XDM個人配置檔案] {#xdm-individual-profile}
...
在構建模式並建立引用該模式的資料集後，資料隨後被攝取並儲存在資料湖中。 使用Query Service，您可以加入Data Lake中的任何資料集，並將查詢結果捕獲為新資料集，用於報告、機器學習或即時客戶配置檔案中的接收。

請參閱 [查詢服務概述](../query-service/home.md) 的子菜單。

### 即時客戶個人檔案

即時客戶概要資訊為有針對性的個性化體驗管理提供了集中的消費者概要資訊。 每個配置檔案都包含所有系統之間聚合的資料，以及可操作的時間戳事件帳戶，這些事件涉及您使用的任何系統中與Experience Platform一起發生的個人。
