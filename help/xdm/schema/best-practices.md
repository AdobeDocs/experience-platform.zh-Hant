...
若要設定特定欄位的限制，請從結構編輯器選取欄位以開啟 **[!UICONTROL 欄位屬性]** 側欄。 請參閱以下檔案： [型別特定欄位屬性](../ui/fields/overview.md#type-specific-properties) 以取得可用欄位的確切說明。
...
* **套用模式以獲得一致的值**：如果您的身分值遵循特定模式，則應使用 **[!UICONTROL 圖樣]** 設定以強制執行此限制。 此設定可包含僅數字、大寫或小寫或特定字元組合等規則。 使用規則運算式來比對字串中的模式。
...
* **限制會觸發自訂名稱空間欄位的警告**：設定條件約束，以便在結構欄位使用自訂名稱空間標籤時觸發警告，而不指定最小和最大長度。 警告是維護資料完整性的重要警告。 請參閱 [型別特定欄位屬性](../ui/fields/overview.md#type-specific-properties) 檔案，以瞭解如何在特定欄位上設定限制。
