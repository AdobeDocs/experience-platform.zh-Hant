...
[!DNL Experience Data Model] (XDM)是標準化客戶體驗資料的核心架構，可提供用於下游Adobe Experience Platform服務的通用結構和定義。 只要遵守XDM標準，所有客戶體驗資料都可整合至共同呈現方式，並用來從客戶動作中獲得有價值的深入分析、定義客戶對象，以及針對個人化目的表達客戶屬性。

由於XDM極為通用而且可依設計方式自訂，因此在設計結構描述時，請務必遵循資料模型化的最佳實務。 本檔案說明將客戶體驗資料對應至XDM時，必須做出的重要決定和考量事項。

## 快速入門

閱讀本指南前，請先檢閱 [XDM系統概覽](../home.md) 以取得XDM及其在Experience Platform中角色的高層級簡介。

由於本指南僅著重於綱要設計的主要考量事項，強烈建議您參閱 [結構描述組合的基本面](./composition.md) 以取得本指南中提及之個別結構描述元素的詳細解釋。
...
![一種實體關聯圖，會強調應排序為XDM類別以進行資料擷取的基本實體。](../images/best-practices/erd.png)
...
| 事件實體 | 事件實體代表與客戶可採取的動作、系統事件或您可能想要隨時間追蹤變更的任何其他概念相關的概念。 屬於此類別的實體應該由根據下列規則的結構描述表示： **[!DNL XDM ExperienceEvent]類別**. |
...
![結構描述編輯器中的Customers結構描述，並醒目提示類別和結構](../images/best-practices/profile-schema.png)
...
![強調顯示XDM體驗事件類別和訂閱結構的訂閱事件結構圖。](../images/best-practices/event-schema.png)
...
![設定檔、查詢和事件實體中包含的結構描述圖表](../images/best-practices/erd-sorted.png)
...
例如，您可以使用 [[!UICONTROL Adobe Analytics ExperienceEvent范] 欄位群組](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/analytics/experienceevent-all.schema.json) 對應 [!DNL Analytics] — 您的XDM結構描述的特定欄位。 根據您使用的Adobe應用程式，您應在結構中使用這些Adobe提供的欄位群組。

![架構圖表 [!UICONTROL Adobe Analytics ExperienceEvent范].](../images/best-practices/analytics-field-group.png)
