...
在中構建和利用模式有幾個主要優點 [!DNL Experience Platform]。 首先，模式允許更好的資料治理和資料最小化，這在隱私法規中尤其重要。 其次，使用Adobe的標準元件建立架構可讓您立即獲得深入資訊，並使用AI/ML服務，而且可將自訂設定降至最低。 最後，架構提供資料分享見解和有效協調的基礎架構。
...
架構的類決定哪些混合符合在該架構中使用的資格。 在下一節中將更詳細地討論 [此問題](#mixin)。
...
有關所有當前標準混合的清單，請參閱正式 [的XDM儲存庫](https://github.com/adobe/xdm/tree/master/components/mixins)。
...
>[!NOTE]
>
>請參 [閱附錄](#mixins-v-datatypes) ，以取得混合與資料類型之間差異的詳細資訊，以及在類似使用案例中使用其中一種與另一種的利弊。
...
>[!TIP]
>
>如需在 [物件類型欄位上使用自由格式欄位的利弊，請參閱附錄](#objects-v-freeform) 。
...
現在，您已瞭解架構構成的基本知識，現在可以開始使用來探索和構建架構 [!DNL Schema Registry]。
...
* 欄位會變得更巢狀化。
* 使用 [Adobe Experience Platform Query Service](../../query-service/home.md)，必須提供較長的參考字串來查詢物件中巢狀的欄位。
...
* 自由格式欄位直接在方案()的根對象下建立，`_tenantId`提高了可見性。
