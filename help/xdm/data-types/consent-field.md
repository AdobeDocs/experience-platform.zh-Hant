...
# [!UICONTROL 一般同意欄位] 資料類型

[!UICONTROL 一般同意欄位] 是標準XDM資料類型，可說明客戶選擇特定同意偏好設定的情形。
...
| 值 | 標題 | 說明 |
| --- | --- | --- |
| `y` | 是（選擇加入） | 客戶已選擇加入同意。 換句話說， **do** 同意使用相關同意所指出的資料。 |
| `n` | 否（選擇退出） | 客戶已選擇退出同意。 換句話說， **不** 同意使用相關同意所指出的資料。 |
| `p` | 待定驗證 | 系統尚未收到最終同意值。 這通常用於需要兩步驗證的同意。 例如，如果客戶選擇接收電子郵件，則該同意會設為 `p` 直到他們選取電子郵件中的連結，以確認他們提供的電子郵件地址正確，之後同意會更新為 `y`.<br><br>如果此同意不使用兩組驗證程式，則 `p` 可以使用選擇來指示客戶尚未響應同意提示。 例如，您可以自動將值設為 `p` 在網站的第一頁，客戶回應同意提示之前。 在不需要明確同意的司法管轄區中，您也可以使用該司法管轄區來指出客戶尚未明確選擇退出（亦即假設同意）。 |
| `u` | 未知 | 客戶的同意資訊未知。 |
