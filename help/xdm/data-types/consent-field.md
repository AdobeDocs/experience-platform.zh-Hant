...
title: 通用同意欄位資料類型
description: 此文檔概述了「通用同意欄位XDM」資料類型。
...
# [!UICONTROL 通用同意欄位] 資料類型

[!UICONTROL 通用同意欄位] 是一個標準XDM資料類型，它描述客戶對特定同意首選項的選擇。
...
| 屬性 | 資料類型 | 說明 |
| --- | --- | --- |
| `val` | 字串 | 客戶為此使用案例提供的同意選擇。 有關接受的值和定義，請參閱下表。 |
...
| 值 | 標題 | 說明 |
| --- | --- | --- |
| `y` | 是（選擇加入） | 客戶已選擇同意。 換句話說， **做** 同意使用有關同意書所示的資料。 |
| `n` | 否（選擇退出） | 客戶已選擇不同意。 換句話說， **不** 同意使用有關同意書所示的資料。 |
| `p` | 等待驗證 | 系統尚未收到最終同意值。 這通常是需要兩步驗證的同意的一部分。 例如，如果客戶選擇接收電子郵件，則該同意設定為 `p` 直到他們在電子郵件中選擇一個連結以驗證他們是否提供了正確的電子郵件地址，此時將將同意更新為 `y`。<br><br>如果此同意不使用兩組驗證過程，則 `p` 而可以使用choice指示客戶尚未響應同意提示。 例如，可以自動將值設定為 `p` 在客戶響應同意提示之前，在網站的首頁上。 在不需要明確同意的管轄區中，您也可以使用它來指示客戶沒有明確選擇不加入（換句話說，假定同意）。 |
| `u` | 未知 | 客戶的同意資訊未知。 |
