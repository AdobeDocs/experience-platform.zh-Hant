...
#  環境資料類型
 ...
 環境是標準的XDM資料類型，可描述觀察到的事件的周圍環境，具體說明網路和軟體版本等暫時性資訊。
...
| 屬性 | 資料類型 | 說明 |
| --- | --- | --- |
| `_dc` | 物件 | 包含單一欄位`language`的對象，該欄位指示表示用戶的語言、地理或文化偏好來表示資料表示。 語言在語言代碼中指定，如[IETF RFC 3066](https://www.ietf.org/rfc/rfc3066.txt)中所定義。 |
...
| `operatingSystem` | 字串 | 進行觀察時使用的作業系統名稱。 該屬性不應包含任何版本資訊，如`10.5.3`，而應包含「edition」標識，如`Ultimate`或`Professional`。 |
| `operatingSystemVendor` | 字串 | 進行觀察時使用的作業系統供應商的名稱。 |
| `operatingSystemVersion` | 字串 | 進行觀察時使用的作業系統的完整版本標識符。 版本通常以數值組成，但可能採用供應商定義的格式。 |
| `type` | 字串 | 應用程式環境的類型。 如需接受的值，請參閱[附錄](#type)。 |
