...
| `number` | 電話號碼。 請注意，電話號碼為字串，並可能包含有意義的字元，例如括弧 `()`，連字型大小 `-`，或字元來表示次撥接識別碼，例如副檔名 `x` 例如， `1-353(0)18391111` 或 `+613 9403600x1234`. |
| `primary` | 表示這是否為個人主要電話號碼的布林值。 不像地址或電子郵件地址，主要電話號碼可能會有好幾個；每個通訊通道一個。 通訊通道由型別定義（由父屬性的名稱指示）： `textMessaging`， `mobile`， `phone`， `home`， `work`， `unknown`、和 `fax`. |
