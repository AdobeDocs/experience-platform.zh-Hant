...
topic: 概述
description: 本檔案提供一般行銷偏好設定欄位XDM資料類型的概觀。
...
# [!UICONTROL Generic Marketing Preference Field] 資料類型

[!UICONTROL Generic Marketing Preference Field] 是標準的XDM資料類型，可說明客戶對特定行銷偏好的選擇。
...
>
>如果您需要特定行銷偏好設定欄位的`subscriptions`對應，則必須改用[行銷欄位及訂閱資料類型](./marketing-field-subscriptions.md)。
...
| `val` | 字串 | 此行銷使用案例的客戶提供的偏好選擇。 請參閱下表以取得接受的值和定義。 |
