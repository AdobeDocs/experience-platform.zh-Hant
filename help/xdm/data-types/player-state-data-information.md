---
title: 播放器狀態資料資料資料型別
description: 瞭解播放器狀態資料資訊體驗資料模型(XDM)資料型別。
...
# [!UICONTROL 播放器狀態資料資訊] 資料型別

[!UICONTROL 播放器狀態資料資訊] 是標準的體驗資料模型(XDM)資料型別，可描述媒體播放器中的各種狀態及其發生次數。 使用 [!UICONTROL 播放器狀態資料資訊] 資料型別來擷取不同的播放器狀態，例如，全熒幕、靜音、隱藏式字幕、子母畫面和觀看中狀態。 對於每個狀態，都會記錄狀態是否已設定、發生次數計數，以及在媒體播放期間保持作用中的總持續時間。

![播放器狀態資料資訊資料型別的圖表。](../images/data-types/player-state-data-information.png)
