...
| `_id` | [!UICONTROL 字串] | 此產品專案的明細專案識別碼。 產品本身的識別方式 `product`. |
| `currencyCode` | [!UICONTROL 字串] | 此 [ISO 4217](https://www.iso.org/iso-4217-currency-codes.html) 用於為產品定價的字母貨幣代碼。 |
| `discountAmount` | [!UICONTROL 雙倍] | 如果產品已貼現，這表示產品的一般價格與特殊價格之間的差額。 |
...
| `quantity` | [!UICONTROL 整數] | 客戶表示所需的產品單位數。 |
| `unitOfMeasureCode` | [!UICONTROL 字串] | 標準 [單位代碼](https://ucum.org/ucum) 與相關的產品 `quantity` 屬性。 |
