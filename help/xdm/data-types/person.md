...
| `birthDayAndMonth` | 字串 | 個人出生的日期和月份（MM-DD格式）。 若只知道一個人出生的日期和月份，但不知道出生年時，應使用此欄位。 此屬性的格式必須符合此規則運算式 `[0-1][0-9]-[0-9][0-9]`. |
...
| `nationality` | 字串 | 個人與其使用ISO 3166-1Alpha2代碼代表的州之間的法律關係。 此屬性的格式必須符合此規則運算式 `^[A-Z]{2}$`. |
