...
| `lists` | 物件 | 擷取最多三個[清單](https://experienceleague.adobe.com/docs/analytics/implementation/vars/page-vars/list.html?lang=zh-Hant)的物件。 此物件的屬性是以`list1`為`list3`的鍵值。 每個屬性都包含單一`list`陣列，內含[[!UICONTROL Key Value Pair]](../../data-types/key-value-pair.md)個資料型別。 |
