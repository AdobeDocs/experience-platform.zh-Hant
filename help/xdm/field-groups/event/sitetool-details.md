...
[!UICONTROL Sitetool詳細資料]是[[!DNL XDM ExperienceEvent] 類別](../../classes/experienceevent.md)的標準結構描述欄位群組。 欄位群組為結構描述提供單一`sitetool`物件，擷取Sitetool收集的資訊。

![欄位群組結構](../../images/field-groups/sitetool-details.png)

| 屬性 | 資料類型 | 說明 |
| --- | --- | --- |
| `dataGatheringEvent` | 物件 | 指出此事件是否為資料收集事件以及其他相關詳細資訊。 包含下列屬性：<ul><li>`data`： （對應）包含所收集的JSON資料，這些資料會隨著測驗、調查或輪詢提交事件而提交。</li><li>`isTrue`： （布林值）指出此事件是否為測驗、意見調查或輪詢之類的資料收集事件。</li><li>`score`： （整數）由執行者根據事件回應保護的分數。</li></ul> |
