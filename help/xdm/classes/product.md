...
在Experience Data Model (XDM)中， [!UICONTROL 產品] class會擷取定義零售產品的最低屬性集。
...
| `_id` | 字串 | 記錄的唯一、系統產生的字串識別碼。 此欄位用於追蹤個別記錄的唯一性、防止資料重複，以及在下游服務中查詢該記錄。<br><br>由於此欄位是系統產生的，因此在資料擷取期間不會提供明確的值。 不過，您仍然可以視需要選擇提供自己的唯一ID值。 |
| `productDescription` | 字串 | 產品的說明。 |
| `productID` | 字串 | 產品的唯一識別碼。 |
| `productLastModifiedDate` | 日期時間 | 一個 [RFC3339](https://datatracker.ietf.org/doc/html/rfc3339) 此產品上次修改任何更新的時間戳記。 |
...
Adobe提供數個標準欄位群組，可與搭配使用 [!UICONTROL 產品] 類別。 以下是類別的一些常用欄位群組清單：
