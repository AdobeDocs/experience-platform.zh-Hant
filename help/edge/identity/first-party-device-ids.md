...
Adobe Experience PlatformWeb SDK分配 [Adobe Experience CloudID(ECID)](https://experienceleague.adobe.com/docs/experience-platform/identity/ecid.html?lang=en) 通過網站訪問者使用cookie，以跟蹤用戶行為。 要說明Cookie生命週期中的瀏覽器限制，可以選擇設定和管理自己的設備標識符。 這些稱為第一方設備ID(FPID)。
...
當使用客戶擁有的伺服器設定第一方Cookie時，它們最有效。 使用第一方設備ID，可以使用DNS A記錄在Cookie中設定自己的設備ID。 然後，這些ID可以發送到Adobe，並用作種子以生成ECID，這些ID將繼續作為Adobe Experience Cloud應用程式中的主要標識符。
...
可以使用多種方法來設定Cookie，以防止它因瀏覽器策略而受到限制：
...
![示出在遷移到FPID後訪問期間如何更新客戶ID值的圖表](../images/identity/tracking/visits.png)
