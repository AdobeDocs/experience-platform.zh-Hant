...
Adobe Experience Platform Web SDK(AEP Web SDK)支援Interactive Advertising Bureau透明度與同意框架2.0版(IAB TCF 2.0)。 本指南說明透過整合即時客戶資料平台、Audience Manager、Experience Events、Analytics和Experience Edge的AEP Web SDK支援IAB TCF 2.0的需求。
...
Adobe Audience Manager(AAM)包含對IAB TCF 2.0的支援，可讓您評估、尊重客戶隱私權選擇，並將之轉寄給下游合作夥伴。 如需詳細資訊，請閱讀有關傳送資 [料至Audience Manager的檔案](../audience-manager/audience-manager-overview.md)。
...
如需如何將XDM體驗事件轉換為Analytics點擊的詳細資訊，請先閱讀 [Analytics總覽檔案](../analytics/analytics-overview.md) 。
...
有關該命令的詳 `setConsent` 細資訊，請閱讀「支援 [同意」文檔](../../fundamentals/supporting-consent.md)。
