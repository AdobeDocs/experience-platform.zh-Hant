...
| commerce.checkouts.value | 事件 | AppMeasurement查詢參數EVENT_LIST_FULL對應，轉換為COMMERCE_SC_CHECKOUT，使用分隔字元`,`。 |
...
| commerce.productListViews.value | 事件 | AppMeasurement查詢參數EVENT_LIST_FULL與轉換COMMERCE_SC_VIEW的對應，使用分隔字元`,`。 |
...
| productListItems[N].lineItemId | 產品 | AppMeasurement查詢參數產品名稱對應。 |
| productlistitems。[N].name | 產品 | AppMeasurement查詢參數產品類別對應。 |
