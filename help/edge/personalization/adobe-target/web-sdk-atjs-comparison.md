---
title: 將at.js與平台Web SDK進行比較
description: 瞭解at.js功能與Web SDK的比較方式
...
AEP WEB SDK自動發送帶有由WEB SDK執行的優惠的通知，這是通知請求負載的樣式示例：
...
[了解更多](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/at-js-implementation/functions-overview/adobe-target-applyoffers-atjs-2.html?lang=en)
...
為了觸發或發出單頁應用程式的「查看更改」信號，請設定 `web.webPageDetails.viewName` 屬性 `xdm` 選項 `sendEvent` 的子菜單。 AEP WEB SDK將檢查視圖快取(如果 `viewName` 指定 `sendEvent` 它將執行這些命令併發送顯示通知事件。
