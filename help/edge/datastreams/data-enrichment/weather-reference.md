...
| 欄位 | 說明 | XDM路徑 |
| --- | ---- | --- |
| [!UICONTROL 產品觸發程式] | 表示條件何時適合推動特定類別產品的銷售。  它們會對應至整數個數。 您可以從IBM取得完整清單。 | `weather.productTriggers` |
| [!UICONTROL 相對觸發程式] | 基於人類知覺的相對條件。 熱或冷之類的。 它們會對應至整數個數。 您可以從IBM取得完整清單。 | `weather.relativeTriggers` |
