...
如果使用Adobe Experience Platform Launch擴充功能，或您隨時可以傳入這些事件類型，而不需Experience Platform Launch，這些事件類型會顯示在下拉式清單中。 可在`xdm`選項中傳遞。
