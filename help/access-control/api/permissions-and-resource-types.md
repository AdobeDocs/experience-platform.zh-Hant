...
您可以向`/acl/reference`端點發出GET要求，以列出所有許可權和資源型別的名稱。 然後，這些名稱便可以用在對[檢視目前使用者的有效存取控制原則](./effective-policies.md)的API呼叫中。

許可權是透過Adobe Admin Console管理的原則，並對應到零個或多個資源型別原則。 資源型別是啟用特定型別[!DNL Experience Platform]資源（例如資料集或結構描述）的讀取、寫入和/或刪除功能的原則。
