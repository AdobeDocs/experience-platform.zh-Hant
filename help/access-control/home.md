...
| [!DNL Destinations] | [!UICONTROL 啟動目的地] | 可將資料啟用至已建立的作用中目的地。 此權限需要「檢視目的地」或「管理」 [!UICONTROL 目的地」] 授予將啟用目的地的使用者。 |
| [!DNL Destinations] | [!UICONTROL 目標編寫] | 可使用 [Adobe Experience Platform目的地SDK](../destinations/destination-sdk/overview.md). |
...
| 資料治理 | [!UICONTROL 查看審核日誌] | 對記錄的檢視的唯讀存取 [稽核記錄](../landing/governance-privacy-security/audit-logs/overview.md) 平台活動。 |
