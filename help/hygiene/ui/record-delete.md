...
>title="主要身分識別命名空間"
>abstract="主要身分識別命名空間是指將記錄和 Experience Platform 中的消費者輪廓繫結的唯一屬性。資料集的主要身分識別欄位由資料集建立基礎的結構描述定義。在此欄中，您必須提供符合資料集結構描述的主要身分識別命名空間，例如用於電子郵件地址的 `email`，或用於 Experience Cloud ID 的 `ecid`。若要了解詳細資訊，請查看「資料生命週期 UI 指南」。"
