...
>[!IMPORTANT]
> 
>為了提高效率並降低資料集操作的成本，已移至Delta格式的組織可以刪除Identity Service、即時客戶設定檔和資料湖中的資料。 此型別的使用者稱為差異移轉使用者。 已進行差異移轉的組織之使用者，可選擇從單一或所有資料集中刪除記錄。 來自未曾進行差異移轉之組織的使用者，無法從單一資料集或所有資料集中選擇性地刪除記錄，如下圖所示。 在此情況下，請繼續執行 [提供身分](#provide-identities) 部分。
...
>title="身分命名空間"
>abstract="身分名稱空間是將記錄與Experience Platform中的消費者設定檔關聯的屬性。 資料集的身分名稱空間欄位是由資料集所根據的結構描述所定義。 在此欄中，您必須提供記錄身分名稱空間的型別（或名稱空間），例如 `email` （針對電子郵件地址和） `ecid` 用於Experience CloudID。 若要深入瞭解，請參閱資料生命週期UI指南。"

>[!CONTEXTUALHELP]
>id="platform_hygiene_identityvalue"
>title="主要身分值"
>abstract="在此欄中，您必須提供記錄的身分名稱空間值，該值必須與左欄中提供的身分型別相對應。 如果身分名稱空間型別為 `email`，該值應為記錄的電子郵件地址。 若要深入瞭解，請參閱資料生命週期UI指南。"

刪除記錄時，您必須提供身分資訊，讓系統能夠決定要刪除哪些記錄。 對於Platform中的任何資料集，記錄會根據 **身分名稱空間** 由資料集結構描述定義的欄位。

和Platform中的所有身分欄位一樣，身分名稱空間是由兩個專案組成： **type** （有時稱為身分名稱空間）和 **值**. 身分型別提供欄位如何識別記錄的上下文（例如電子郵件地址）。 值代表該型別之記錄的特定身分(例如， `jdoe@example.com` 針對 `email` 身分型別)。 做為身分識別的常見欄位包括帳戶資訊、裝置ID和Cookie ID。

>[!TIP]
>
>如果您不知道特定資料集的身分名稱空間，可以在Platform UI中找到。 在 **[!UICONTROL 資料集]** 從工作區中，選取有問題的資料集。 在資料集的詳細資訊頁面上，將滑鼠移至右側邊欄中資料集的結構描述名稱上。 身分名稱空間會與結構描述名稱和說明一起顯示。
>
...
顯示的控制項可讓您一次輸入一個身分。 在 **[!UICONTROL 身分名稱空間]**，使用下拉式選單來選取身分型別。 在 **[!UICONTROL 主要身分值]**，提供記錄的身分名稱空間值。
