...
>- **管理沙箱**
>   - 此許可權可讓您&#x200B;**編輯**&#x200B;您的容量配置。
>   - 此外，您&#x200B;**必須**&#x200B;被指派存取所有沙箱，才能編輯&#x200B;**任何**&#x200B;沙箱容量。
>
...
>title="串流輸送量"
>abstract="串流輸送量值會測量每秒合併的尖峰傳入事件，以用於跨您的生產和開發沙箱將擷取串流到設定檔中。"
...
如果您的使用量超過授權容量的100%，則會被視為違反您的容量。 如果您違反容量，將套用下列限制：

>[!NOTE]
>
>如果您有Adobe Journey Optimizer的存取權，下列限制將&#x200B;**不適用**。

- 如果事件處理佇列超過12小時，則可以從串流個人化中移除事件資料&#x200B;**&#x200B;**
- 移除的事件資料&#x200B;**不會**&#x200B;內嵌到設定檔中
   - 您將會看到事件何時移除
   - 事件將根據您的權益在資料湖中提供
   - 必要時，您&#x200B;*可以*&#x200B;使用查詢服務直接重新內嵌資料
...
![顯示[容量概觀]頁面，顯示警示歷程記錄與容量詳細資料區段。](/help/landing/images/capacity/capacity-overview.png) {zoomable="yes" width="80%"}
...
### 串流容量 {#streaming-capacities}

串流容量區段概述有關您組織串流容量的資訊。 具體來說，此區段會顯示串流輸送量和串流對象的容量資訊。 您可以根據沙箱來篩選此資訊，並變更回顧期間。

![回顧期間的沙箱選擇器和日期選擇器會醒目提示。](/help/landing/images/capacity/filter-sandbox-and-date.png)

#### 串流輸送量 {#streaming-throughput}

**[!UICONTROL Streaming throughput]**&#x200B;區段會顯示有關您組織沙箱中的串流輸送量的資訊。 串流輸送量值會測量每秒將內嵌串流至設定檔的合併尖峰傳入事件。
...
#### 串流客群計數 {#streaming-audience-count}

**[!UICONTROL Streaming audience count]**&#x200B;區段顯示沙箱中的串流對象數量以及沙箱中允許的串流對象最大數量。

![顯示受眾規模區段。](/help/landing/images/capacity/audience-count.png)
...
### Edge容量 {#edge-capacities}

**[!UICONTROL Edge capacities]**&#x200B;區段概述有關您組織邊緣容量的資訊。 具體來說，此區段會顯示邊緣區段輸送量和邊緣受眾的容量資訊。 您可以變更組織邊緣容量的回顧期間。

![顯示Edge容量區段。 此概要資訊包括邊緣分段輸送量和邊緣受眾規模。](/help/landing/images/capacity/edge-capacities.png)

#### 邊緣細分輸送量 {#edge-streaming-throughput}

**[!UICONTROL Edge segmentation throughput]**&#x200B;區段會顯示有關您組織和組織沙箱中的邊緣分段輸送量的資訊。 邊緣區段輸送量值會測量每秒將邊緣擷取至設定檔的合併尖峰傳入事件。

![顯示Edge分段輸送量區段。 這會顯示貴組織及其沙箱內邊緣區段輸送量的相關資訊。](/help/landing/images/capacity/edge-segmentation-throughput.png)

| 欄名稱 | 說明 |
| ----------- | ----------- |
| 組織 | 組織的名稱。 組織的可用沙箱會列在組織的名稱下。 |
| 使用率RPS （峰值） | 在選取的回顧期間內，沙箱中資料的尖峰輸送量。 |
| 容量RPS | 組織的最大尖峰輸送量。 |
| 違規 | 如果發生違規，邊緣分割輸送量的違規型別。 |
| 建議的動作 | 說明緩解違規的建議動作的欄。 |

您可以選取組織來檢視該組織邊緣區段輸送量的更詳細檢視。

![組織已反白顯示。](/help/landing/images/capacity/select-organization.png)

顯示&#x200B;**[!UICONTROL Edge Segmentation Throughput]**&#x200B;頁面。 您可以看到一個圖表，其中顯示與容量限制相比較的請求輸送量。 在此頁面中，您可以調整所顯示圖表的回顧期間。

![顯示「Edge分段輸送量」頁面。 這會顯示一個圖表，詳述與容量限制相比較的輸送量。](/help/landing/images/capacity/edge-segmentation-throughput-details.png)

#### 邊緣客群計數 {#edge-audience-count}

**[!UICONTROL Edge audience count]**&#x200B;區段會顯示每個沙箱中的邊緣對象數量，以及沙箱中允許的最大邊緣對象數量。

![顯示受眾規模區段。 這會顯示與邊緣受眾規模相關的資訊。](/help/landing/images/capacity/edge-audience-count.png)
...
## 串流輸送量最佳實務 {#streaming-throughput-suggestions}

您可以採用下列其中一種建議來解決輸送量違規：
...
## Edge區段輸送量最佳實務 {#edge-best-practices}

您可以採用下列其中一種建議來解決邊緣分段輸送量違規：

1. 識別[監視儀表板](/help/dataflows/ui/monitor-edge.md)中的高輸送量資料串流，並視需要對這些資料串流套用節流或篩選。
2. 透過使用批次擷取來減少延遲使用案例，讓您的擷取最佳化。
3. 如果問題仍然存在，請聯絡您的Adobe客戶服務代表。
