...
除了將資料集用於一般查詢用途外，您還必須決定是否要啟用即時客戶設定檔的查詢資料集。 請遵循下列指引，以避免超過您的授許可權制。

#### 啟用查詢資料集的設定檔 {#profile-enablement-lookup-datasets}

查詢資料集是您在Experience Platform中啟用的資料集，因此應用程式可在執行階段參照它。 使用查詢資料集來儲存相對靜態的加密資訊，例如產品詳細資訊、儲存中繼資料或優惠方案設定，而非其主要目的是貢獻個人資料屬性（例如，名稱、電子郵件或忠誠度等級）或體驗事件（例如，頁面檢視或購買）的資料集。

Experience Platform應用程式（例如[!DNL Journey Optimizer]）和其他決策應用程式會使用這些資料集，根據索引鍵（例如產品ID或存放區ID）擷取其他欄位，並豐富個人化、決策和協調工作流程。 為Real-time Customer Profile啟用查詢資料集會影響您的設定檔資料量，因此請遵循下列指引以保有您的授權權利。

設定資料集以進行查詢時，請考慮資料集在Experience Platform中可以扮演的兩個角色：

* **查詢資料集**：允許應用程式擷取參考資料，用於[!DNL Journey Optimizer]中的個人化和決策等服務。
* **啟用設定檔的資料集**：將屬性和事件貢獻給即時客戶設定檔中的統一客戶設定檔。 這些資料集使其欄位可用於細分和啟動使用案例。

>[!IMPORTANT]
>
>只有在您必須在即時客戶個人檔案中使用來自該資料集的欄位時（例如，針對對象定義、啟用或多實體分段），才能啟用即時客戶個人檔案的查詢資料集。 為Real-time Customer Profile啟用查詢資料集會增加您的設定檔資料量。 如需詳細資訊，請參閱有關[多實體分段](../../segmentation/tutorials/multi-entity-segmentation.md)的教學課程。

**何時啟用即時客戶個人檔案的資料集：**

在下列情況下啟用即時客戶個人檔案的資料集：

* 資料集包含您需要整合至客戶個人檔案的客戶屬性（例如，忠誠度等級、偏好設定、帳戶資訊）。
* 此資料集包含有助於客戶行為分析和細分的體驗事件。
* 此資料集包含您必須在對象定義（包括多實體細分或下游啟用）中使用的參考或擴充屬性（例如產品、商店或帳戶屬性）。

**當不啟用即時客戶個人檔案的資料集時：**

在下列情況下，請避免為即時客戶個人檔案啟用資料集：

* 資料集包含參考資料，例如產品目錄、SKU詳細資料、存放區位置或其他非客戶資料，而您在即時客戶個人檔案中不需要這些屬性來進行細分或啟用，包括多實體細分。
* 此資料集包含僅用於執行階段查詢的擴充資料，且不是客戶身分識別或對象定義中的必要專案。
