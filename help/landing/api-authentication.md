...
一旦您被分配為開發人員，就可以開始在 [Adobe開發人員控制台](https://www.adobe.com/go/devs_console_ui)。 這些整合是從外部應用和服務到AdobeAPI的管道。
...
## 生成API密鑰、IMS組織ID和客戶機密鑰 {#api-ims-secret}
...
除了在本教程中收集的驗證值之外，許多平台API還需要有效的 `{SANDBOX_NAME}` 作為標題提供。 查看 [箱概述](../sandboxes/home.md) 的子菜單。