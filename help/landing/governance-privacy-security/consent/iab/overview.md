...
Adobe Experience Platform是已登入的[IAB TCF 2.0廠商清單](https://iabeurope.eu/vendor-list-tcf/)的一部分，ID為&#x200B;**565**。 為符合TCF 2.0要求，Experience Platform可讓您收集客戶同意資料，並將其整合至您儲存的客戶設定檔中。 之後，可根據使用者檔案的使用案例，將此同意資料納入是否將使用者檔案納入匯出的受眾區段中。

>[!IMPORTANT]
>
>Experience Platform僅能遵守2.0版（或更新版本）的TCF。 不支援舊版TCF。

本檔案概述如何設定您的資料作業和設定檔結構描述，以接受同意管理平台(CMP)產生的客戶同意資料。 此外也說明Experience Platform在匯出區段時，如何傳達使用者的同意選擇。
...
>[!IMPORTANT]
>
>如果CMP的ID無效，Experience Platform會持續依原樣處理您的資料。 若要強制執行TCF 2.0，您必須先確認您的CMP具備已向IAB TCF 2.0註冊的有效ID，才能將資料傳送至Experience Platform。

本指南也需要您實際瞭解下列Experience Platform服務：
...
Experience Platform可讓您透過下列程式收集客戶同意資料：

1. 客戶透過您網站上的對話方塊提供其資料收集的同意偏好設定。
1. 您的CMP會偵測同意偏好設定變更，並據此產生TCF同意資料。
1. 使用Experience Platform Web SDK時，產生的同意資料（由CMP傳回）會傳送至Adobe Experience Platform。
...
成功擷取TCF同意資料後，下游Experience Platform服務中將進行下列程式：

1. [!DNL Real-Time Customer Profile]會更新該客戶設定檔的已儲存同意資料。
1. 只有在為叢集中的每個ID提供Experience Platform (565)的廠商許可權時，Experience Platform才會處理客戶ID。
1. 將區段匯出至屬於TCF 2.0廠商清單成員的目的地時，只有為叢集中的每個ID提供了Experience Platform (565) *和*&#x200B;兩者的廠商許可權，Experience Platform才會包含設定檔。

本檔案的其餘章節提供如何設定Experience Platform和您的資料作業，以符合上述收集和執行要求的指引。
...
| 同意選項 | 說明 |
| --- | --- |
| **目的** | 用途定義品牌可將客戶資料用於哪些廣告技術用途。 為了方便Experience Platform處理客戶ID，必須選擇下列用途： <ul><li>**用途1**：儲存和/或存取裝置上的資訊</li><li>**用途10**：開發並改善產品</li></ul> |
...
在TCF規格中，同意字串是用來根據政策和廠商定義的特定行銷目的，編碼有關客戶同意設定的相關細節。 Experience Platform會使用這些字串來儲存每個客戶的同意設定，因此，每次這些設定變更時，都必須產生新的同意字串。
...
將CMP設定為產生同意字串後，您必須整合Experience Platform網路SDK以收集這些字串並將它們傳送至Experience Platform。 Experience Platform SDK提供兩個命令，可用來將TCF同意資料傳送至Experience Platform （以下小節中說明）。 當客戶首次提供同意資訊，且同意其後隨時變更時，應使用這些命令。

**SDK未與任何立即可用的CMP介面**。 您可以自行決定如何將SDK整合至您的網站、接聽CMP中的同意變更，以及呼叫適當的命令。

### 建立資料串流

為了讓SDK將資料傳送至Experience Platform，您必須先為Experience Platform建立資料串流。 [SDK檔案](/help/datastreams/overview.md)中提供了建立資料串流的特定步驟。
...
建立上節所述的資料流後，您就可以開始使用SDK命令，將同意資料傳送至Experience Platform。 以下各節提供如何在不同情境中使用每個SDK命令的範例。
...
您也可以使用`sendEvent`命令，針對Experience Platform中觸發的每個事件收集TCF 2.0同意資料。
...
TCF 2.0也要求資料來源在將資料傳送至目的地之前，必須先檢查目的地的廠商許可權。 因此，在包含繫結至該目的地的資料之前，Experience Platform會先檢查是否已針對叢集中的所有ID選擇加入目的地的廠商許可權。
...
本檔案說明設定Experience Platform資料作業，以履行TCF 2.0所概述之業務責任的程式。如需有關Experience Platform隱私權相關功能的詳細資訊，請參閱[治理、隱私權及安全性](../../overview.md)的概觀。
