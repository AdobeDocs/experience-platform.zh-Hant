...
您可以向助理查詢您的資料使用情況。 您必須處於助理的有效沙箱中，才能回答與組織中資料有關的資料使用問題。

![繼續回答有關資料使用的問題。](./images/ai-assistant/data-usage-question.png)

當出現資料使用問題的提示時，「助理員」也會提供如何計算答案的說明。 在以下範例中，「助理」概述為了顯示具有超過1000個設定檔的區段及其各自的啟用狀態所採取的一些步驟。

![繼續回答有關區段的問題，以說明「助理員」如何計算答案。](./images/ai-assistant/results-explained.png)

此外，「助理員」會呈現圖表以視覺化您的資料。 您也可以提供篩選器和修改查詢，以及指示「助理員」根據您包含的篩選器來呈現其結果。 例如，您可以要求「助理」按照區段建立日期的順序，顯示區段計數趨勢；移除總設定檔為零的區段；以及在顯示資料時，使用月份名稱而非整數。
