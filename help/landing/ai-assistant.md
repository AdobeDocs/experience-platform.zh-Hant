...
您也可以在下列網域中詢問AI Assistant有關您使用資料的問題：
...
對於使用資料查詢，答案可能不會反映UI的目前狀態。 支援這些問題的資料每24小時更新一次。 例如，使用者白天在Real-Time CDP中所做的變更會在夜間與資料存放區同步，然後早上就可供使用者提問。 此外，您將需要登入沙箱，以查詢與物件（例如對象、結構描述、資料集、屬性和目的地）相關的特定資料。

### 使用資料問題範例 {#example-usage-data-questions}

+++選取以檢視範例使用資料問題的清單

請閱讀下表，以取得使用資料問題的範例及其各自的使用案例：
