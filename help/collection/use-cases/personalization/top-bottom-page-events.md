...
一家運動服裝retailer想要為購物者提供個人化體驗，同時儘量減少使用者在造訪其網站時的摩擦，同時能夠準確收集訪客量度。

行銷團隊可以在Web SDK中使用頁面事件的頂端和底部，以最佳方式設定其個人化傳送：

* 網頁SDK會傳送個人化請求，並在頁面開始載入時立即載入。 這是頁面頂端事件。
...
下列程式碼範例是頁面事件設定的底端，該設定會針對在頁面上自動轉譯，但於頁面[事件的](#top-of-page)頂端隱藏顯示事件的主張，傳送顯示事件。
