...
在上述範例中，`EDGE-DOMAIN`應該取代為第一方網域。 第一方網域是針對[edgeDomain](../js/commands/configure/edgedomain.md)設定所設定。 若尚未設定第一方網域，則應將`EDGE-DOMAIN`取代為`*.adobedc.net`。 如果使用[idMigrationEnabled](../js/commands/configure/idmigrationenabled.md)開啟訪客移轉，`connect-src`指示詞也需要包含`*.demdex.net`。
