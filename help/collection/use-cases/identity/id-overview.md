...
* 若為再度訪問的訪客，系統會從[!DNL ECID] Cookie擷取`kndctr_{YOUR-ORG-ID}_AdobeOrg_identity`，並將其新增至Edge Network的要求裝載中。

設定包含[!DNL ECID]的Cookie後，網頁SDK產生的每個後續請求都會在[!DNL ECID] Cookie中包含編碼的`kndctr_{YOUR-ORG-ID}_AdobeOrg_identity`。
