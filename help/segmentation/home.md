...
平台目前支援三種評估區段的方法：串流分段、批次分段和邊緣分段。
...
每24小時評估一次批次區段。 不過，對於現有區段，增量分段可讓區段保持最新狀態達一小時。

遞增分段會針對傳入描述檔儲存區的新資料執行。 但是，下列增量區段須注意事項：

- 對於任何新的或最近修改的區段，含有新資料的描述檔會在下次增量執行中開始符合條件。 不過，未變更的設定檔會在下一個完整的批次分段工作中追蹤。
- 多實體區段將會在增量分段中重新整理。 如果有實體更新，則任何含有新資料的描述檔都會在下次增量執行中開始使用。 不過，未變更的設定檔會在下一個完整的批次分段工作中追蹤。
- 在下一個完整的批次區段工作中，會協調從區段的時間視窗拖曳的事件。
