...
當將記錄擷取至設定檔存放區後，設定檔總數增加或減少超過3%，則會觸發取樣工作以更新計數。 資料取樣觸發的方式取決於擷取方法：

* **批次擷取：**&#x200B;對於批次擷取，在成功將批次擷取到設定檔存放區後15分鐘內，如果達到3%的增加或減少臨界值，則會執行工作以更新計數。
* **串流擷取：**&#x200B;對於串流資料工作流程，會每小時進行一次檢查，以判斷是否已達到3%的增加或減少臨界值。 如果超過100次，系統就會自動觸發工作以更新計數。
...
建立預覽工作後，您可以在GET請求至`previewId`端點的路徑中使用其`/estimate`來檢視有關區段定義的統計資訊，包括預計對象大小、信賴區間和錯誤標準差。
