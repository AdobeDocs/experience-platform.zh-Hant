...
為了使用相似的見解，基礎對象 **必須** 符合下列資格條件：

- 基礎對象 **必須** 會在Platform中建立。
   - 外部產生的對象為 **非** 適用於相似的見解。
- 基礎對象 **必須** 在預設的合併原則上。
- 基礎對象 **必須** 不使用受資料控管限制的欄位。
...
>[!IMPORTANT]
>
> **您** 負責確保資料（包括敏感資料）標示正確，且資料使用原則已定義並啟用，以符合您營運的法律和法規義務。 另外也請留意，如果資料欄位或區段會籍 **非** 與通常與敏感或受保護資料型別相關聯的資料欄位直接相關，可能是潛在偏差的來源。 **您** 負責分析您的資料，以識別、標示並將適當的資料使用原則套用至您的資料，包括可能代表敏感或受保護資料型別，且應排除在模型以外的任何資料欄位。
