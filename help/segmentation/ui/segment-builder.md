...
**[!UICONTROL Attributes]**&#x200B;索引標籤可讓您瀏覽屬於[!DNL Profile]類別的[!DNL XDM Individual Profile]屬性。 每個資料夾都可以展開以顯示其他屬性，其中每個屬性都是一個可拖曳至工作區中央規則產生器畫布的圖磚。 本指南稍後會更詳細地討論[規則產生器畫布](#rule-builder-canvas)。
...
| 運算子 | 說明 | 範例 |
| -------- | ----------- | ------- |
| 等於 | 尋找與指定值完全相符的專案 | 電子郵件&#x200B;**等於** &quot;sample@example.com&quot; |
| 不等於 | 排除指定的值 | 狀態&#x200B;**不等於**「已取消」 |
| 包含 | 文字包含指定的值 | 產品名稱&#x200B;**包含** &quot;iPhone&quot; |
| 不包含 | 文字未包含指定值 | 說明&#x200B;**不包含**「已終止」 |
| 開始於 | 文字以指定的值開頭 | 客戶ID **開頭為** &quot;PREM&quot; |
| 結束於 | 文字會以指定的值結束 | 電子郵件&#x200B;**結尾是** &quot;@company.com&quot; |
| 存在 | 值已存在 | 中間名&#x200B;**存在** |
| 不存在 | 值不存在 | 熟客狀態&#x200B;**不存在** |
...
| 運算子 | 說明 | 範例 |
| -------- | ----------- | ------- |
| 等於 | 尋找與指定值完全相符的專案 | 年齡&#x200B;**等於** 25 |
| 不等於 | 排除指定的值 | 訂單計數&#x200B;**不等於** 0 |
| 大於 | 數目比指定的值大&#x200B;**個**。 此值是指定號碼的&#x200B;**exclusive**。 | 年收入&#x200B;**大於** 50000 |
| 大於或等於 | 數目比指定的值大&#x200B;**個**。 此值是指定數字的&#x200B;**inclusive**。 | 年齡&#x200B;**大於或等於** 21 |
| 小於 | 數目比指定的值小&#x200B;**個**。 此值是指定號碼的&#x200B;**exclusive**。 | 購買間隔天數&#x200B;**小於** 30 |
| 小於或等於 | 數目比指定的值小&#x200B;**個**。 此值是指定數字的&#x200B;**inclusive**。 | 購物車值&#x200B;**小於或等於** 100 |
| 介於 | 數字是介於&#x200B;**個指定值之間的**。 這些值是指定數字的&#x200B;**包含**。 | 年齡&#x200B;**介於** 25到45之間 |
| 存在 | 值已存在 | 信用分數&#x200B;**存在** |
| 不存在 | 值不存在 | 信用分數&#x200B;**不存在** |
...
| 運算子 | 說明 | 範例 |
| -------- | ----------- | ------- |
| 等於 | 布林值會設定為指定的值（True或False） | 電子郵件選擇加入&#x200B;**等於True** |
| 不等於 | 布林值是&#x200B;**不是**&#x200B;設定為指定的值（True或False） | 已安裝的行動應用程式&#x200B;**不等於True** |
...
| 運算子 | 說明 | 範例 |
| -------- | ----------- | ------- |
| 等於 | 該值等於指定的列舉值 | 性別&#x200B;**等於**&#x200B;女性 |
| 不等於 | 該值不等於指定的列舉值 | 訂單狀態&#x200B;**不等於**&#x200B;已取消 |
| 存在 | 已設定列舉值 | 偏好的語言&#x200B;**存在** |
| 不存在 | 尚未設定列舉值 | 偏好的語言&#x200B;**不存在** |
...
| 運算子 | 說明 | 範例 |
| -------- | ----------- | ------- |
| 今天 | 值發生於今天。 您可以選取&#x200B;**忽略年份**&#x200B;核取方塊，讓比較忽略年份。 | 出生日期&#x200B;**是**&#x200B;今天 |
| 昨天 | 該值發生在昨天。 | 購物車購買&#x200B;**為昨天** |
...
| 早於 | 該值發生在選取的日期之前。 | 會員資格加入日期為&#x200B;**早於** 2025年1月3日 |
| 之後 | 該值發生在選取的日期之後。 | 購買日期為&#x200B;**晚於** 2024年3月14日 |
| 滾動範圍 | 值出現在兩個相對日期之間。 | 上次購買日期在七天前至三天前的滾動範圍內。 |
| 下一 | 此值在下一個選取的時段內發生。 | 未來2天將放棄購買 |

如需時間和日期函式的詳細資訊，請閱讀[時間限制區段](#time-constraints)。
...
如需運算屬性的詳細資訊，包括如何建立運算屬性、可在運算屬性內使用哪些函式，以及如何管理運算屬性，請參閱[運算屬性概觀](/help/profile/computed-attributes/overview.md)。

### 活動

**[!UICONTROL Events]**&#x200B;索引標籤可讓您根據使用[!DNL XDM ExperienceEvent]資料元素發生的事件或動作來建立對象。 您也可在&#x200B;**[!UICONTROL Events]**&#x200B;標籤上找到事件型別，這是常用事件的集合，可讓您更快速地建立區段定義。

除了可以瀏覽[!DNL ExperienceEvent]元素之外，您還可以搜尋事件型別。 事件型別使用與[!DNL ExperienceEvents]相同的編碼邏輯，不需要您搜尋[!DNL XDM ExperienceEvent]類別以尋找正確的事件。 例如，使用搜尋列搜尋「購物車」會傳回事件型別&quot;[!UICONTROL AddCart]&quot;和&quot;[!UICONTROL RemoveCart]&quot;，這是建立區段定義時兩個非常常用的購物車動作。
...
然後您可以輕鬆將[!DNL ExperienceEvents]和&quot;[!UICONTROL Event Types]&quot;拖放到您的區段定義中。

![區段產生器UI的事件區段已反白顯示。](../images/ui/segment-builder/events.png)

依預設，只會顯示資料存放區中填入的結構欄位。 這包含&quot;[!UICONTROL Event Types]&quot;。 如果&quot;[!UICONTROL Event Types]&quot;清單不可見，或您只能選取&quot;[!UICONTROL Any]&quot;作為&quot;[!UICONTROL Event Type]&quot;，請選取&#x200B;**旁的**&#x200B;齒輪圖示&#x200B;**[!UICONTROL Fields]**，然後選取&#x200B;**[!UICONTROL Show full XDM schema]**&#x200B;下的&#x200B;**[!UICONTROL Available Fields]**。 再次選取&#x200B;**齒輪圖示**&#x200B;以返回&#x200B;**[!UICONTROL Fields]**&#x200B;標籤，您現在應該能夠檢視多個「[!UICONTROL Event Types]」和結構描述欄位，無論它們是否包含資料。
...
若要將新規則新增至您的區段定義，請從「**[!UICONTROL Fields]**」標籤拖曳圖磚，並將其拖曳至規則產生器畫布。 接著，會根據要新增的資料型別，為您顯示內容專屬選項。 可用的資料型別包括：字串、日期、[!DNL ExperienceEvents]、「[!UICONTROL Event Types]」和對象。
...
如果您要建立多個值，可以使用大量上傳一次新增所有值。 選取![加號圖示](/help/images/icons/add-circle.png)以顯示&#x200B;**[!UICONTROL Add values in bulk]**&#x200B;彈出視窗。

![加號圖示會反白顯示，顯示您可以選取以存取大量上傳彈出視窗的按鈕。](../images/ui/segment-builder/add-bulk-values.png)

在&#x200B;**[!UICONTROL Add values in bulk]**&#x200B;彈出視窗上，您可以上傳CSV或TSV檔案。
...
您可以從&#x200B;**[!UICONTROL Audience]**&#x200B;標籤將對象拖放到規則產生器畫布上，以參考新區段定義中的對象成員資格。 這可讓您在新的區段定義規則中，以屬性的形式包含或排除對象成員資格。
...
或者，您也可以檢視在[!DNL Segment Builder]中建立的規則之程式碼型版本。 在規則產生器畫布中建立規則後，您可以選取「**[!UICONTROL Code view]**」來將區段定義顯示為PQL。
...
![反白顯示複製代碼按鈕，讓您可以](../images/ui/segment-builder/copy-code.png)

### 彙總函式

[!DNL Segment Builder]中的彙總是資料型別為數字（雙精度或整數）的一組XDM屬性的計算。 區段產生器中支援的四個彙總函式為SUM、AVERAGE、MIN和MAX。

若要建立彙總函式，請從左側邊欄選取事件，然後將其插入[!UICONTROL Events]容器。

![事件區段已反白顯示。](../images/ui/segment-builder/events.png)

將事件放入「事件」容器後，請選取省略符號圖示(...)，然後選取&#x200B;**[!UICONTROL Aggregate]**。
...
若要建立計數函式，請從左側邊欄選取事件，並將其插入至[!UICONTROL Events]容器。

![事件欄位會反白顯示。](../images/ui/segment-builder/events.png)

將事件置於「事件」容器內後，請選取[!UICONTROL At least 1]按鈕。
...
>所有時間限制都是以UTC為基礎。
>
>此外，如果已啟用[!UICONTROL Ignore year]核取方塊，則年份將&#x200B;**不會**&#x200B;作為區段定義評估的一部分進行比較。
...
若要套用卡片層級限制，請選取您要套用時間限制的卡片，接著選取省略符號圖示，然後&#x200B;**[!UICONTROL Apply time rule]**。 這可讓您在&#x200B;**[!UICONTROL Event Rules]**&#x200B;容器中選取時間限制。
...
系統會依列出的順序評估對象。 容器可讓您透過使用巢狀查詢來控制執行順序。

將至少一個圖磚新增至規則產生器畫布後，您就可以開始新增容器。 若要建立新容器，請選取圖磚右上角的省略符號(...)，然後選取&#x200B;**[!UICONTROL Add container]**。

![新增容器按鈕會反白顯示，讓您新增一個容器，做為第一個容器的子系。](../images/ui/segment-builder/add-container.png)

新容器會顯示為第一個容器的子系，但您可以拖曳並移動容器來調整階層。 容器的預設行為是將&quot;[!UICONTROL Include]&quot;提供的屬性、事件或對象。 您可以選取圖磚左上角的[!UICONTROL Exclude]並選取「**[!UICONTROL Include]**」，將規則設定為符合容器條件的「[!UICONTROL Exclude]」設定檔。
...
選取&#x200B;**[!UICONTROL Unwrap container]**&#x200B;後，子容器會移除，條件會內嵌顯示。
...
### 範例 {#container-examples}

您可以在區段產生器中以三種不同的方式使用容器：使用布林邏輯來分組規則、控制是否包含或排除符合容器條件的設定檔，以及定義具有時間限制的事件序列。

+++ 混合布林值邏輯

下列範例在單一運算式中混用&#x200B;**兩者** AND和OR邏輯。 若不使用容器，就無法在單一層級中混合使用AND與OR邏輯。

![顯示如何使用容器混合布林邏輯以及使用包含/排除邏輯的影像。](/help/segmentation/images/ui/segment-builder/mixed-boolean-container.png)
...
![顯示如何使用容器排序事件的影像。](/help/segmentation/images/ui/segment-builder/event-sequence-container.png)

+++

### 最佳做法 {#container-best-practices}

將容器新增至對象時，請記住以下准則：

- 逐步建立您的容器，並使用您新增的每個步驟測試邏輯
   - 如果您使用「排除」邏輯，這一點就特別重要，因為這會顯著變更您的結果
- 清楚地命名您的容器，以瞭解其用途
- 避免有太多巢狀層級的容器，因為它會降低效能
- 請確認容器的順序正確無誤，因為事件順序會大幅影響順序容器
...
若要為您的區段定義選取合併原則，請選取&#x200B;**[!UICONTROL Fields]**&#x200B;標籤上的齒輪圖示，然後使用&#x200B;**[!UICONTROL Merge Policy]**&#x200B;下拉式功能表選取您要使用的合併原則。
...
建立區段定義時，工作區右側的&#x200B;**[!UICONTROL Audience properties]**&#x200B;區段會顯示結果區段定義的大小預估值，好讓您在建立受眾本身之前根據需要調整區段定義。

**[!UICONTROL Qualified Profiles]**&#x200B;表示符合區段定義規則的&#x200B;**實際**&#x200B;設定檔數目。 執行區段評估工作之後，此數字每 24 小時更新一次。

合格設定檔的時間戳記表示最近的&#x200B;**批次**&#x200B;區段評估工作，且對於使用串流或邊緣區段評估的區段定義顯示為&#x200B;**not**。 如果您編輯區段定義，則在下次執行區段評估工作之前，合格設定檔的數量將保持不變。

根據&#x200B;**[!UICONTROL Estimated Profiles]**&#x200B;範例工作&#x200B;**，**&#x200B;表示約&#x200B;**個設定檔範圍**。 這表示樣本資料是以更大的設定檔集進行預計，因此會產生可能與合格設定檔實際數量不同的預估數量。預估的設定檔範例具有95%信賴區間。

當您變更對象的規則時，可以選取「**[!UICONTROL Refresh estimate]**」按鈕來檢視預估設定檔的更新計數。 不過，此數字仍以先前的範例工作為基礎。 當客戶資料變更超過3%或最後一個範例作業超過三天時，範例作業會更新。

選取資訊泡泡圖會提供上次執行範例工作的日期和時間。

![對象屬性區段中會強調顯示合格的設定檔和預估的設定檔。](../images/ui/segment-builder/audience-estimates.png)

**[!UICONTROL Audience properties]**&#x200B;區段也是您可以指定對象重要資訊的位置，包括其名稱、說明和評估型別。 名稱是用來在您的組織所定義的區段定義中識別您的區段定義，因此應是描述性、簡潔及唯一的。

當您繼續建立對象時，可以選取&#x200B;**[!UICONTROL View Profiles]**，檢視對象的編頁預覽。
...
[!UICONTROL Evaluation method eligibility]彈出視窗會出現。 此彈出視窗會顯示可用的評估方法，包括批次、串流和邊緣。 彈出視窗會顯示哪些評估方法符合資格和不符合資格。 根據您在區段定義中使用的引數，它可能不符合某些評估方法的資格。 如需每個評估方法需求的詳細資訊，請閱讀[串流區段](../methods/streaming-segmentation.md#query-types)或[邊緣區段](../methods/edge-segmentation.md#query-types)概述。

完成建立區段定義後，您也可以變更區段定義的評估方法。 如果您將評估方法從Edge或串流變更為「批次」，您將&#x200B;**無法**&#x200B;將其變更回Edge或串流。 在彈出視窗中選取&#x200B;**後，對評估方法的變更將**&#x200B;僅&#x200B;**[!UICONTROL Save]**&#x200B;生效。 取消對話方塊將&#x200B;**保留**&#x200B;原始的評估方法。
