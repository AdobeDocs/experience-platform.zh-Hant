...
若要開啟Audience Portal，請選取「細分」區段中的&#x200B;**[!UICONTROL Browse]**&#x200B;標籤。
...
![針對來源為[!UICONTROL Audience composition]的對象顯示快速動作清單。](../images/ui/audience-portal/browse-audience-composition-details.png)

| 動作 | 來源 | 說明 |
| ------ | ------- | ----------- |
| [!UICONTROL Edit] | 細分服務 | 開啟區段產生器以編輯您的對象。 請注意，如果您的對象是透過API建立的，您將&#x200B;**無法**&#x200B;使用區段產生器編輯對象。 如需使用區段產生器的詳細資訊，請參閱[區段產生器UI指南](./segment-builder.md)。 |
| [!UICONTROL Open composition] | 客群構成 | 開啟「對象」構成來檢視對象。 如需對象構成的詳細資訊，請參閱[對象構成UI指南](./audience-composition.md)。 |
| [!UICONTROL Activate to destination] | 細分服務 | 將對象啟用至目的地。 如需將對象啟用至目的地的詳細資訊，請參閱[啟用概觀](../../destinations/ui/activation-overview.md)。 |
| [!UICONTROL Share with partners] | 對象構成、自訂上傳、細分服務 | 與其他Experience Platform使用者共用您的對象。 如需有關此功能的詳細資訊，請閱讀[區段比對總覽](./segment-match/overview.md)。 |
| [!UICONTROL Manage tags] | 對象構成、自訂上傳、細分服務 | 管理屬於對象的使用者定義標籤。 如需有關此功能的詳細資訊，請閱讀[篩選和標籤](#manage-audiences)的章節。 |
| [!UICONTROL Move to folder] | 對象構成、自訂上傳、細分服務 | 管理對象所屬的資料夾。 如需有關此功能的詳細資訊，請閱讀[篩選和標籤](#manage-audiences)的章節。 |
...
切換&#x200B;**[!UICONTROL Schedule all audiences]**&#x200B;將會啟用排定的分段。 如需排程區段的詳細資訊，請參閱本使用手冊[的](#scheduled-segmentation)排程區段區段。

選取&#x200B;**[!UICONTROL Import audience]**&#x200B;可讓您匯入外部產生的對象。 若要深入瞭解匯入對象，請閱讀使用手冊[中有關](#import-audience)匯入對象的章節。

選取&#x200B;**[!UICONTROL Create audience]**&#x200B;可讓您建立對象。 若要深入瞭解如何建立對象，請參閱使用手冊[中有關](#create-audience)建立對象的章節。

![對象瀏覽頁面上的頂端導覽列會醒目提示。 此列包含建立對象的按鈕和匯入對象的按鈕。](../images/ui/audience-portal/browse-audiences-top.png)

您可以選取&#x200B;**[!UICONTROL Evaluation summary]**&#x200B;以顯示圓形圖，顯示對象評估的摘要。
...
| [!UICONTROL Created] | 建立對象的日期和時間(UTC)。 |
| [!UICONTROL Last updated] | 上次更新對象的日期和時間(UTC)。 |
| [!UICONTROL Tags] | 屬於對象的使用者定義標籤。 有關這些標籤的詳細資訊可在標籤[的](#tags)區段中找到。 |
| [!UICONTROL Description] | 對象的說明。 |
| [!UICONTROL Created by] | 建立對象的人員名稱。 |
| [!UICONTROL Access labels] | 對象的存取權標籤。 存取標籤可讓您根據套用至該資料的使用原則來分類資料集和欄位。 這些標籤可隨時套用，提供您選擇控管資料方式的靈活性。 如需存取標籤的詳細資訊，請閱讀[管理標籤](../../access-control/abac/ui/labels.md)的檔案。 |
| [!UICONTROL Breakdown] | 對象的設定檔狀態劃分。 此設定檔狀態劃分的更詳細說明可在下方找到。 |

如果選取劃分，畫面會顯示長條圖，概述屬於下列每個已計算設定檔狀態的設定檔百分比： [!UICONTROL Realized]、[!UICONTROL Existing]和[!UICONTROL Exiting]。 此外，[!UICONTROL Browse]標籤上顯示的劃分是區段定義狀態最精確的劃分。 如果此數字與[!UICONTROL Overview]標籤上所列的數字不同，您應該使用[!UICONTROL Browse]標籤上的數字作為正確的資訊來源，因為[!UICONTROL Overview]標籤編號每天只更新一次。

| 狀態 | 說明 |
| ------ | ----------- |
| [!UICONTROL Realized] | 自上次批次區段作業執行以來，過去24小時內符合對象&#x200B;**資格**&#x200B;的設定檔計數。 |
| [!UICONTROL Existing] | 自上次批次區段作業執行以來，過去24小時內對象中仍有&#x200B;**保留**&#x200B;的設定檔計數。 此欄位是&#x200B;**已計算**，不會顯示在[`segmentMembership`物件](../../xdm/field-groups/profile/segmentation.md)中。 |
| [!UICONTROL Exiting] | 自上次批次區段作業執行以來，在過去24小時內已&#x200B;**退出**&#x200B;對象的設定檔計數。 |

選取要顯示的欄位後，您也可以重新調整顯示欄的寬度。 您可以拖曳欄之間的區域或選取要重新調整大小之欄的![箭頭圖示](/help/images/icons/chevron-down.png)，接著選取&#x200B;**[!UICONTROL Resize column]**&#x200B;來執行此操作。
...
若要新增標籤，請在您要標籤的對象上選取&#x200B;**[!UICONTROL Manage tags]**。

![已針對指定的對象選取[!UICONTROL Manage tags]按鈕。](../images/ui/audience-portal/browse-manage-tags.png)

**[!UICONTROL Manage tags]**&#x200B;彈出視窗會出現。 在此彈出視窗中，您可以選取已分類標籤或未分類標籤。

| 標籤型別 | 說明 |
| -------- | ----------- |
| 已分類 | 由貴組織管理員建立及管理的標籤。 |
| 未分類 | 在[!UICONTROL Manage tags]彈出視窗中建立的標籤。 任何人都能建立或管理這些型別的標籤。 |

![顯示[!UICONTROL Manage tags]彈出視窗。 選擇已分類或未分類的選項會反白顯示。](../images/ui/audience-portal/create-tag.png)

新增您要附加至對象的所有標籤後，請選取「**[!UICONTROL Save]**」。

![在[!UICONTROL Manage tags]彈出視窗上，新增的標籤會反白顯示。](../images/ui/audience-portal/created-tags.png)
...
若要建立資料夾以保留您的對象，請選取&#x200B;**[!UICONTROL Create folder]**。

![已反白顯示[建立資料夾]按鈕。](../images/ui/audience-portal/create-folder.png)

>[!NOTE]
>
>只有在您位於其他資料夾中時，才能建立資料夾。 這表示如果您在左側導覽列上選取&#x200B;**，則您**&#x200B;無法&#x200B;**[!UICONTROL All Audiences]**&#x200B;建立資料夾。

此時會出現彈出視窗，讓您為新建立的資料夾命名。 在命名資料夾後選取&#x200B;**[!UICONTROL Save]**&#x200B;以完成資料夾的建立。 請注意，父資料夾的名稱&#x200B;**必須**&#x200B;是唯一的。

![建立資料夾對話方塊上的[儲存]按鈕會反白顯示。](../images/ui/audience-portal/create-folder-dialog.png)

若要將對象移至資料夾，請在您要移動的對象上選取&#x200B;**[!UICONTROL Move to folder]**。

![已針對特定對象選取[!UICONTROL Move to folder]按鈕。](../images/ui/audience-portal/browse-move-to-folder.png)

出現&#x200B;**將對象移至資料夾**&#x200B;彈出視窗。 選取您要移動對象的資料夾，然後選取&#x200B;**[!UICONTROL Save]**。
...
| [!UICONTROL Created by] | 可讓您根據建立受眾的人員進行篩選。 |
| [!UICONTROL Creation date] | 可讓您根據對象的建立日期進行篩選。 您可以選取日期範圍，以篩選建立對象的時間。 |
| [!UICONTROL Modified date] | 可讓您根據對象的上次修改日期進行篩選。 您可以選擇日期範圍，以篩選上次修改對象的時間。 |
...
若要檢視特定對象的詳細資訊，請在「**[!UICONTROL Browse]**」標籤中選取對象名稱。
...
**[!UICONTROL Audience summary]**&#x200B;區段提供屬性的識別碼、名稱、說明、來源和詳細資訊等資訊。

此外，您可以選擇將對象啟用至目的地、套用存取權標籤或編輯/更新對象。

選取&#x200B;**[!UICONTROL Activate to destination]**&#x200B;可讓您將對象啟用至目的地。 如需將對象啟用至目的地的詳細資訊，請參閱[啟用概觀](../../destinations/ui/activation-overview.md)。

![[啟用至目的地]按鈕已反白顯示。](../images/ui/audience-portal/audience-details-activate.png)

選取&#x200B;**[!UICONTROL Apply access labels]**&#x200B;可讓您管理屬於對象的存取標籤。 如需存取標籤的詳細資訊，請閱讀[管理標籤](../../access-control/abac/ui/labels.md)的檔案。
...
![顯示對象詳細資訊頁面，並反白顯示[!UICONTROL Open composition]按鈕。](../images/ui/audience-portal/audience-details-open-composition.png)

選取&#x200B;**[!UICONTROL Open composition]**&#x200B;可讓您在「對象構成」中檢視對象。 如需對象構成的詳細資訊，請參閱[對象構成UI指南](./audience-composition.md)。
...
這會將資料到期日延長到初始上傳上定義的天數，並使用相同的對象中繼資料。 如果資料過期之前未定義，資料過期時間會延長30天。 按下按鈕後，您就無法再選取20分鐘。
...
![顯示對象詳細資訊頁面，並反白顯示[!UICONTROL Edit audience]按鈕。](../images/ui/audience-portal/audience-details-edit-audience.png)

選取「**[!UICONTROL Edit audience]**」可讓您在「區段產生器」中編輯對象。 如需使用[!DNL Segment Builder]工作區的詳細資訊，請參閱[[!DNL Segment Builder] 使用手冊](./segment-builder.md)。

>[!ENDTABS]

選取&#x200B;**[!UICONTROL Edit properties]**&#x200B;可讓您編輯對象的基本詳細資料，例如名稱、說明和標籤。

![在對象詳細資訊頁面中會醒目提示「編輯屬性」按鈕。](../images/ui/audience-portal/audience-details-edit-properties.png)

### 客群總計 {#audience-total}

針對Experience Platform產生的對象和組合，**[!UICONTROL Audience total]**&#x200B;區段會顯示符合對象資格的設定檔總數。
...
對於來源為&#x200B;**[!UICONTROL Custom upload]**&#x200B;的對象，**[!UICONTROL Ingestion details]**&#x200B;區段會同時顯示設定檔總計以及外部產生的對象所擷取的資料集詳細資料。
...
**[!UICONTROL Activated destinations]**&#x200B;區段會顯示此對象啟用的目的地。
...
使用UI或API可啟用對象以進行排程評估。 在UI中，返回&#x200B;**[!UICONTROL Browse]**&#x200B;內的&#x200B;**[!UICONTROL Audiences]**&#x200B;索引標籤並開啟&#x200B;**[!UICONTROL Schedule all audiences]**。 這會導致根據您的組織設定的排程評估所有對象。
...
您可以選取&#x200B;**[!UICONTROL Create audience]**&#x200B;來建立對象。
...
選取&#x200B;**[!UICONTROL Compose audiences]**&#x200B;會帶您前往對象構成。 此工作區提供直覺式控制項，可讓您建立和編輯對象，例如用來代表不同動作的拖放圖磚。 若要深入瞭解如何建立對象，請參閱[對象組合指南](./audience-composition.md)。

![顯示對象組合工作區。](../images/ui/audience-portal/audience-composition.png)

### 客戶細分工具 {#segment-builder}

選取&#x200B;**[!UICONTROL Build rule]**&#x200B;會帶您前往區段產生器。 此工作區提供直覺式控制項來建置和編輯區段定義，例如用來表示資料屬性的拖放圖磚。 若要深入瞭解如何建立區段定義，請參閱[區段產生器指南](./segment-builder.md)
...
>title="資料期限"
>abstract="客群會籍到期前的天數。此值可以設定為 1 天至 90 天。客群成員資格過期後，評估將停止，所有輪廓都將退出客群成員資格。您可以在客群成員資格剩餘少於 7 天時重新整理期限時程。但是，一旦剩下 0 天，您將無法重新整理期限時程。"

>[!IMPORTANT]
>
>為了匯入外部產生的對象，您&#x200B;**必須**&#x200B;擁有下列許可權： [!UICONTROL View segments]、[!UICONTROL Manage segments]和[!UICONTROL Import audience]。 如需這些許可權的詳細資訊，請閱讀[存取控制總覽](../../access-control/home.md#permissions)。

您可以選取&#x200B;**[!UICONTROL Import audience]**&#x200B;以匯入外部產生的對象。

![在[對象]瀏覽頁面上，[匯入對象]按鈕會反白顯示。](../images/ui/audience-portal/browse-import-audience.png)

**[!UICONTROL Import audience CSV]**&#x200B;工作流程隨即顯示。 您可以選取要匯入為外部產生對象的CSV檔案。

![在[!UICONTROL Import audience CSV]工作流程中，[!UICONTROL Drag and drop files]方塊會反白顯示，顯示您可在何處上傳外部產生的對象。](../images/ui/audience-portal/import-audience-csv.png)
...
選取要匯入的CSV檔案後，將顯示此外部產生對象的範例資料清單。 確認範例資料正確之後，請選取&#x200B;**[!UICONTROL Next]**。

![顯示外部產生對象的範例資料。](../images/ui/audience-portal/import-audience-sample-data.png)

**[!UICONTROL Audience details]**&#x200B;頁面隨即顯示。 您可以新增對象的相關資訊，包括其名稱、說明、主要身分和身分名稱空間值。

匯入外部產生的對象時，您必須選取其中一個欄位作為主要身分欄位，並指定名稱空間值。 請注意，其餘所有欄位都將被視為&#x200B;**裝載屬性**。 這些屬性會視為&#x200B;**非耐用**，因為它們只會為了個人化的目的與此對象相關聯，而且&#x200B;**不會**&#x200B;連線到設定檔。

![顯示[!UICONTROL Audience details]頁面。](../images/ui/audience-portal/import-audience-audience-details.png)
...
>[!NOTE]
>
>自訂資料到期設定設定檔將&#x200B;**保留**&#x200B;在對象成員資格中的天數。 資料到期過後，設定檔將退出對象成員資格。 此值可以設定在1到90天之間。

填寫您的對象詳細資料後，請選取&#x200B;**[!UICONTROL Next]**。

![在[!UICONTROL Next]頁面上反白顯示[!UICONTROL Audience details]按鈕。](../images/ui/audience-portal/import-audience-filled-details.png)

顯示&#x200B;**[!UICONTROL Review]**&#x200B;頁面。 您可以檢閱新匯入的外部產生對象的詳細資料。

![顯示[!UICONTROL Review]頁面，顯示您新匯入的外部產生對象的詳細資料。](../images/ui/audience-portal/import-audience-review-details.png)

確認詳細資料正確後，選取「**[!UICONTROL Finish]**」將外部產生的對象匯入Adobe Experience Platform。
