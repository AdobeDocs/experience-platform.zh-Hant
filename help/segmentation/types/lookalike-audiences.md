...
>title="條件不符"
>abstract="此對象目前不符合相似深入分析的條件，因為其擁有的輪廓數量低於訓練所需的最低數量，或是尚未觸發輪廓匯出。"
...
>title="相似度與觸及"
>abstract="相似度與觸及圖表會繪製相似對象 (由超過指定相似度分數的輪廓組成) 的預期觸及範圍。您可以將游標停留在圖表中的特定一點，以顯示目前醒目標示的點的相似度百分比和預期的輪廓數量。"
