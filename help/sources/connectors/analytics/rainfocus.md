...
[!DNL RainFocus] 是一個平台，可用來宣傳您的事件及建立您的對象。 您可以使用 [!DNL RainFocus] 建立精美的促銷頁面、追蹤促銷活動成效並最佳化註冊轉換。

使用 [!DNL RainFocus] Adobe Experience Platform和Real-time Customer Data Platform中的原始碼，以透過出席者體驗事件即時自動豐富您的客戶資料設定檔。 在啟用後，體驗事件會自動串流到Real-Time CDP中，以便使用下游目的地和應用程式(例如Customer Journey Analytics和Adobe Journey Optimizer)進行強大的受眾細分、資料分析和啟用與會者歷程。
...
>[!IMPORTANT]
>
>Adobe最近宣佈淘汰JWT權杖，改用OAuth。 為配合此變更， [!DNL RainFocus] 來源將在近期移轉至OAuth。

### 收集必要的認證

為了連線 [!DNL RainFocus] 若要Experience Platform，您必須提供下列連線屬性的值 [!DNL RainFocus]：

| 認證 | 說明 | 範例 |
| --- | --- | --- |
| 使用者端ID | 使用者端ID可從Adobe Developer入口網站的Adobe服務帳戶中取得。 | `b9c32a63e7d41a0f87d3e8b52a16e7a2` |
| 使用者端密碼 | 使用者端密碼可從Adobe Developer入口網站的Adobe服務帳戶取得。 | `k1b-p-umplcjtg_arnw-R-Bx44bybu` |
...
為了儲存來自的體驗事件 [!DNL RainFocus] 在Experience Platform中，您必須建立Experience Data Model (XDM)結構描述來說明資料集，該資料集可儲存將傳送自的可能欄位和資料型別 [!DNL RainFocus].

[!DNL RainFocus] 建議使用下列欄位，涵蓋預設傳送的所有可能資料。
...
| `eventType` | 字串 | 工作階段已排程 | 出席者體驗活動的名稱。 |
...
若要為以下專案建立您的結構描述： [!DNL RainFocus] 資料，請參閱以下檔案以瞭解如何使用API或UI建立結構描述的步驟。
...
>* 您必須確保結構描述包含 **主要身分**、和 **已為設定檔啟用**. 如需詳細資訊，請閱讀以下指南： [在UI中定義身分欄位](https://experienceleague.adobe.com/docs/experience-platform/xdm/ui/fields/identity.html)
>* 您可以將身分範例（電子郵件）取代為另一個適當的識別碼，例如sha256電子郵件或ECID。

### 在RainFocus中建立整合設定檔 {#create-an-integration-profile-in-rainfocus}

服務帳戶和XDM結構描述準備就緒後，您現在可以啟用 [!DNL Integration Profile] 透過 [!DNL RainFocus] 平台。 此 [!DNL Integration Profile] 負責將資料串流至Experience Platform。
...
選取 [!DNL Integration Profile] 您剛建立的用來檢視預先定義清單的專案 **推送型別** 已設定。 這些是 [體驗事件](https://experienceleague.adobe.com/docs/experience-platform/xdm/classes/experienceevent.html) 會在發生時傳送至Experience Platform的郵件。

![RainFocus控制面板中預先定義的推送型別清單。](/help/sources/images/tutorials/create/rainfocus/rainfocus_integration-profile-setup.png)

若要擷取範例JSON裝載的復本，請選取 **[!DNL Sample JSON Payload]**. 接下來，反白並複製範例JSON裝載和 **將其儲存在副檔名為.json的新檔案中**. 這稍後將用於Experience Platform [對應設定](../../tutorials/ui/create/analytics/rainfocus.md#mapping).

![RainFocus儀表板中的JSON裝載範例。](/help/sources/images/tutorials/create/rainfocus/rainfocus_integration-profile-json.png)

>[!TIP]
>
>**設定尚未完成**：建立資料流後，您需要返回 [!DNL RainFocus] 儀表板以完成您的 [!DNL Integration Profile] 透過提供您的 **串流端點URL** 和 **資料流ID**.

## 後續步驟

閱讀本檔案後，您已完成從您的伺服器串流資料所需的先決條件設定。 [!DNL RainFocus] 要Experience Platform的帳戶。 您現在可以在以下位置繼續參閱指南： [正在連線 [!DNL RainFocus] 以使用使用者介面Experience Platform](../../tutorials/ui/create/analytics/rainfocus.md).