...
Adobe Campaign Managed Cloud Services為設計跨頻道客戶體驗提供受管理的平台，可支援視覺化促銷活動協調、即時互動管理和跨頻道執行。 如需詳細資訊，請參閱[Adobe Campaign v8檔案](https://experienceleague.adobe.com/docs/campaign/campaign-v8/campaign-home.html?lang=zh-Hant)。

Adobe Campaign Managed Cloud Services來源聯結器可讓您將來自Adobe Campaign v8的傳遞和追蹤記錄資料擷取到Adobe Experience Platform。 此聯結器可作為平台內的批次來源。
...
## Adobe Campaign Managed Cloud Services來源的預期延遲 {#latency}

假設資料量正常且沒有待處理專案，在Experience Platform中，從Campaign事件到資料可用性的端對端延遲在標準設定中通常為15至30分鐘（包括15分鐘的複製、微批次匯出和已排程的Experience Platform資料流）。 這是透過排程的微批次同步（通常為數十分鐘）實現的近乎即時流程，但並非持續串流。

| 藍本 | 詳細資料 | 預期延遲 |
| --- | --- | --- |
| 行銷活動事件會在中間來源/訊息中心例項中產生 | 在Campaign v8執行（中間/訊息中心）節點上發生傳遞或追蹤事件（傳送、開啟、點按等）。 | Campaign執行階段中的即時（目前在Experience Platform中不可見）。 |
| 從執行階段復寫至Campaign行銷資料庫 | 事件資料從中間/訊息中心復寫到Campaign行銷資料庫（[!DNL Snowflake]或[!DNL Postgres]，視客戶規模而定）。 標準整合模式會採用一般復寫作業。 | 約15分鐘，根據標準的15分鐘複製節奏。 |
| 從Campaign行銷資料庫匯出至登陸區域（例如[!DNL Data Landing Zone]、[!DNL Amazon S3]或[!DNL Azure Blob]） | Campaign中的匯出工作流程（匯出服務）會依排程執行，以擷取新的/已變更的傳送和追蹤記錄，並將其以微批次形式寫入檔案型登陸區域。 | 分鐘，加上匯出排程間隔。 |
| Experience Platform來源資料流會挑選匯出的檔案 | Adobe Campaign Managed Cloud Services來源已設定為Experience Platform [!DNL Flow Service]中的批次資料流。 它會定期掃描登入區域、擷取新檔案，並將它們寫入已設定的ExperienceEvent資料集中。 監視會公開「成功的批次」和「失敗的批次」。 | 分鐘，加上資料流排程間隔。 |
| 資料湖和即時客戶個人檔案中可用的資料 | 擷取批次後，記錄會著陸資料湖中，並（如果資料集已啟用設定檔）更新插入即時客戶設定檔中。 適用於批次擷取和設定檔擷取的標準Experience Platform SLA。 | 與資料流相同的執行視窗內，即在批次執行完成後不久。 記錄通常會在幾分鐘內提供給下游服務。 |
