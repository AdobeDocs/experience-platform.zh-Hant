...
>[!NOTE]
>
>您可以在Adobe Experience Platform UI中使用範本，快速建立B2B和B2C資料的資產。 如需詳細資訊，請閱讀[在Experience Platform UI中使用範本](../../../tutorials/ui/templates.md)的指南。
...
| `TECHNICAL_ACCOUNT_ID` | 用來整合至[Adobe Developer Console](https://developer.adobe.com/)的認證。 請依照下列步驟擷取您的技術帳戶ID：<ul><li>導覽至[Adobe Developer Console](https://developer.adobe.com/)並開啟您的專案。</li><li>開啟使用&#x200B;**服務帳戶(JWT)**&#x200B;的&#x200B;_認證_。 **注意**：在舊版UI中，這可以稱為「企業」。</li><li>憑證詳細資料應顯示您的使用者端ID、技術帳戶ID、組織ID和其他JWT中繼資料。</li></ul>。 | `D42AEVJZTTJC6LZADUBVPA15@techacct.adobe.com` |
...
| `PLATFORM_URL` | 您對其進行API呼叫的URL端點。 此值是固定的，且一律設為： `platform.adobe.io/`。 | `platform.adobe.io/` |
