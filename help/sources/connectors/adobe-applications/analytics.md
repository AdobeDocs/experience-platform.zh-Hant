...
| AACUSTOMID | AACUSTOMID是一個單獨的識別碼欄位，根據`s.VisitorID`實作中[!DNL Analytics]變數的使用情況在Adobe Analytics中填入。 AACUSTOMID由`cust_visid`資料摘要[[!DNL Analytics] 中的](https://experienceleague.adobe.com/docs/analytics/export/analytics-data-feed/data-feed-contents/datafeeds-reference.html)資料行表示。 如果AACUSTOMID存在，則AAID將會以AACUSTOMID為基礎，因為AACUSTOMID會勝過[ID [!DNL Analytics] 的](https://experienceleague.adobe.com/docs/id-service/using/reference/analytics-reference/analytics-order-of-operations.html)作業順序所定義的所有其他識別碼。 |
