...
您可以使用[!DNL HTTP API]來源聯結器將您的資料串流到Platform。 [!DNL Data Prep]函式支援[!DNL HTTP API]來源，可讓您將任何不符合XDM的資料對應到符合XDM的資料集。

以下檔案提供有關如何使用API或使用者介面建立HTTP API串流聯結器以與[!DNL Platform]連線的資訊：
