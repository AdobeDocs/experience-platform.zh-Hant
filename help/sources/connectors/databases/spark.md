...
# [!DNL Azure HDInsights]聯結器上的(Beta) [!DNL Apache Spark]
...
[!DNL Experience Platform]支援從協力廠商資料庫擷取資料。 [!DNL Experience Platform]可以連線到不同型別的資料庫，例如關聯式、NoSQL或資料倉儲。 支援的資料庫提供者包括[!DNL Azure HDInsights]上的[!DNL Apache Spark]。

## IP位址允許清單

使用來源聯結器之前，必須將IP位址清單新增至允許清單。 未能將您區域特定的IP位址新增到允許清單可能會導致使用來源時的錯誤或效能不佳。 如需詳細資訊，請參閱[IP位址允許清單](../../ip-address-allow-list.md)頁面。

以下檔案提供有關如何使用API或使用者介面將[!DNL Azure HDInsights]上的[!DNL Apache Spark]連線到[!DNL Experience Platform]的資訊：

## 使用API將[!DNL Azure HDInsights]上的[!DNL Apache Spark]連線至[!DNL Experience Platform]
...
## 使用使用者介面將[!DNL Azure HDInsights]上的[!DNL Apache Spark]連線至[!DNL Experience Platform]
