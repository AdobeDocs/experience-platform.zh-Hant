---
keywords: Experience Platform;home;popular topics
solution: Experience Platform
title: GreenPlum連接器
topic: overview
translation-type: tm+mt
source-git-commit: b92f33627636b351dbd7b490e461d4f6b2866310
workflow-type: tm+mt
source-wordcount: '142'
ht-degree: 0%

---


# (Beta)GreenPlum連接器

>[!NOTE]
>GreenPlum連接器為beta版。 如需使用 [測試版標籤連接器的詳細資訊](../../home.md#terms-and-conditions) ，請參閱來源概觀。

Adobe Experience Platform為Microsoft、MySQL和Azure等資料庫提供者提供原生連接。 您可以將這些系統的資料匯入平台。

支援不同類型的第三方資料庫，包括關聯式、NoSQL或資料倉庫。 支援資料庫提供者，包括GreenPlum。

以下檔案提供如何使用API或使用者介面將GreenPlum連接至平台的資訊：

## 使用API將GreenPlum連接到平台

- [使用Flow Service API建立GreenPlum連接器](../../tutorials/api/create/databases/greenplum.md)
- [使用Flow Service API探索資料庫系統](../../tutorials/api/explore/database-nosql.md)
- [使用Flow Service API從資料庫收集資料](../../tutorials/api/collect/database-nosql.md)

## 使用UI將GreenPlum連接到平台

- [在UI中建立GreenPlum來源連接器](../../tutorials/ui/create/databases/greenplum.md)
- [在UI中為資料庫連接器配置資料流](../../tutorials/ui/dataflow/databases.md)