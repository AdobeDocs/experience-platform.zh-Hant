...
description: 了解如何使用API或使用者介面將Microsoft SQL Server連線至Adobe Experience Platform。
...
[!DNL Experience Platform] 支援從協力廠商資料庫擷取資料。[!DNL Platform] 可以連接到不同類型的資料庫，如關係、 NoSQL或資料倉庫。對資料庫提供程式的支援包括[!DNL Microsoft] SQL Server。
...
>[!IMPORTANT]
>
>[!DNL Microsoft] SQL Server源連接器當前不支援與Platform的相同區域連接。 這表示，如果您的Azure執行個體使用與Platform相同的網路區域，則無法建立與Platform來源的連線。 目前僅支援跨地區連線。 如需詳細資訊，請連絡您的Adobe客戶經理。

以下文檔提供了有關如何使用API或用戶介面將[!DNL Microsoft] SQL Server連接到[!DNL Platform]的資訊：

## 使用API將[!DNL Microsoft] SQL Server連接到[!DNL Platform]

- [使用流服務API建立Microsoft SQL Server基本連接](../../tutorials/api/create/databases/sql-server.md)
