---
keywords: Experience Platform;home；熱門主題； MariaDB;mariadb; Maria DB; mariadb
...
Experience Platform支援從協力廠商資料庫擷取資料。 [!DNL Platform] 可以連接到不同類型的資料庫，如關係型、 NoSQL或資料倉庫。支援資料庫提供者包括[!DNL MariaDB]。

## IP位址允許清單

在使用來源連接器之前，必須將IP位址清單新增至允許清單。 若未將您地區專屬的IP位址新增至您的允許清單，在使用來源時可能會導致錯誤或效能不佳。 如需詳細資訊，請參閱[IP位址允許清單](../../ip-address-allow-list.md)頁面。

下面的文檔提供了如何使用API或用戶介面將[!DNL MariaDB]連接到[!DNL Platform]的資訊：

## 使用API將[!DNL MariaDB]連接至[!DNL Platform]

- [使用Flow Service API建立MariaDB來源連線](../../tutorials/api/create/databases/mariadb.md)
...
## 使用UI將[!DNL MariaDB]連接至[!DNL Platform]
