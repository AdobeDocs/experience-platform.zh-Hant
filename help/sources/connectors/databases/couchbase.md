...
# (Beta) [!DNL Couchbase] 聯結器

>[!NOTE]
>
>此 [!DNL Couchbase] 聯結器為測試版。 請參閱 [來源概觀](../../home.md#terms-and-conditions) 以取得使用Beta標籤聯結器的詳細資訊。

Adobe Experience Platform為資料庫提供者提供原生連線，例如 [!DNL Microsoft]、 MySQL和 [!DNL Azure]，可讓您從這些系統帶入資料。 支援不同型別的第三方資料庫，包括關聯式、NoSQL或資料倉儲。 對資料庫提供者的支援包括 [!DNL Couchbase].

## IP位址允許清單

在使用來源聯結器之前，必須將IP位址清單新增至允許清單。 使用來源時，若未將您地區專屬的IP位址新增至允許清單，可能會導致錯誤或效能不佳。 請參閱 [IP位址允許清單](../../ip-address-allow-list.md) 頁面以取得詳細資訊。

以下檔案提供有關如何連線的資訊 [!DNL Couchbase] 至 [!DNL Platform] 使用API或使用者介面：

## Connect [!DNL Couchbase] 至 [!DNL Platform] 使用API

- [使用Flow Service API建立Couchbase基本連線](../../tutorials/api/create/databases/couchbase.md)
- [使用Flow Service API探索資料表](../../tutorials/api/explore/tabular.md)
- [使用流程服務API為資料庫來源建立資料流](../../tutorials/api/collect/database-nosql.md)

## Connect [!DNL Couchbase] 至 [!DNL Platform] 使用UI
