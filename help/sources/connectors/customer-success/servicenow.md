...
[!DNL Experience Platform] 支援從協力廠商客戶成功系統擷取資料。 對客戶成功提供者的支援包括 [!DNL ServiceNow].

## IP位址允許清單

在使用來源聯結器之前，必須將IP位址清單新增至允許清單。 使用來源時，若未將您地區專屬的IP位址新增至允許清單，可能會導致錯誤或效能不佳。 請參閱 [IP位址允許清單](../../ip-address-allow-list.md) 頁面以取得詳細資訊。

以下檔案提供有關如何連線的資訊 [!DNL ServiceNow] 至 [!DNL Platform] 使用API或使用者介面：

## Connect [!DNL ServiceNow] 至 [!DNL Platform] 使用API

- [使用Flow Service API建立ServiceNow基本連線](../../tutorials/api/create/customer-success/servicenow.md)
...
## Connect [!DNL ServiceNow] 至 [!DNL Platform] 使用UI

- [在使用者介面中建立ServiceNow來源連線](../../tutorials/ui/create/customer-success/servicenow.md)
