...
雲端儲存空間來源可將您自己的資料帶入Platform，無需下載、格式化或上傳。 內嵌的資料可以格式化為XDM JSON、XDM Parquet或分隔。 流程的每個步驟都會整合至來源工作流程。 Platform可讓您從匯入資料 [!DNL Event Hubs] 即時。

## 縮放方式 [!DNL Event Hubs]

您的縮放因數 [!DNL Event Hubs] 如果您需要擷取大量資料、增加平行度或提高擷取平台的速度，則必須增加執行個體。

### 匯入較大量的資料

目前，您可從帶入的最大資料量 [!DNL Event Hubs] account to Platform為每秒2000筆記錄。 若要擴充並擷取較大量的資料，請聯絡您的Adobe代表。

### 增加上的平行度 [!DNL Event Hubs] 和Platform

平行度是指在多個處理單元上同時執行相同工作，以提高速度和效能。 您可以增加 [!DNL Event Hubs] 增加分割區或取得更多處理單元 [!DNL Event Hubs] 帳戶。 檢視此 [[!DNL Event Hubs] 縮放檔案](https://docs.microsoft.com/en-us/azure/event-hubs/event-hubs-scalability) 以取得詳細資訊。

若要提高平台端的擷取速度，平台必須增加來源聯結器中要讀取的任務數量 [!DNL Event Hubs] 分割區。 一旦您增加 [!DNL Event Hubs] 請連絡您的Adobe代表，以根據您的新磁碟分割調整平台工作。 目前，此程式尚未自動化。
...
* 選取 [!DNL Event Hubs] 您要帶到Platform的名稱空間、資源群組和訂閱，然後選取「 」 **執行**；
* 在出現的JSON內文中，新增下列平台子網路至 `virtualNetworkRules` 裡面 `properties`：


>[!IMPORTANT]
>
>更新之前，您必須先備份您收到的JSON內文 `virtualNetworkRules` 與Platform子網路搭配使用，因為它包含您現有的IP篩選規則。 否則，會在呼叫後刪除規則。
...
以下檔案提供有關如何連線的資訊 [!DNL Event Hubs] 使用API或使用者介面至Platform：
