...
>[!IMPORTANT]
>
>使用者必須先在SFTP伺服器設定中停用鍵盤互動式驗證，才能連線。 禁用此設定將允許手動輸入密碼，而不是通過服務或程式輸入。 有關鍵盤交互身份驗證的詳細資訊，請參閱[元件Pro文檔](https://doc.componentpro.com/ComponentPro-Sftp/authenticating-with-a-keyboard-interactive-authentication)。

以下檔案提供如何使用API或使用者介面將SFTP伺服器連線至[!DNL Platform]的資訊：
