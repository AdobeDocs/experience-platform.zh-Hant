...
description: 瞭解如何使用API或用戶介面將Azure Blob連接到Adobe Experience Platform。
...
雲儲存源可以將您自己的資料 [!DNL Platform] 無需下載、格式化或上載。 所攝取的資料可以格式化為XDM JSON、XDM Parke或分隔。 流程的每個步驟都整合到「源」工作流中。 [!DNL Platform] 允許您從 [!DNL Azure Blob] 批處理。
...
>[!IMPORTANT]
>
>的 [!DNL Azure Blob] 源不支援與Experience Platform的同一區域連接。 如果Azure實例使用與Experience Platform相同的網路區域，則無法建立到Experience Platform源的連接。 設定時，請不要使用Azure East US 2、Azure West Europe和Azure Australia East區域 [!DNL Azure Blob] 源。 目前，僅支援跨區域連接。
...
## 連接 [!DNL Azure Blob] 至 [!DNL Platform]

以下文檔提供了有關如何使用API或用戶介面將Azure Blob連接到Adobe Experience Platform的資訊：

### 使用API

- [使用流服務API建立Azure Blob基連接](../../tutorials/api/create/cloud-storage/blob.md)
- [使用流服務API瀏覽雲儲存源的資料結構和內容](../../tutorials/api/explore/cloud-storage.md)
- [使用流服務API為雲儲存源建立資料流](../../tutorials/api/collect/cloud-storage.md)

### 使用UI

- [在UI中建立Azure Blob源連接](../../tutorials/ui/create/cloud-storage/blob.md)
