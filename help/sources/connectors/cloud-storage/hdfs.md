...
title: Apache HDFS來源聯結器概述
description: 瞭解如何使用API或使用者介面將Apache HDFS連線到Adobe Experience Platform。
...
Adobe Experience Platform為AWS等雲端服務供應商提供原生連線， [!DNL Google Cloud Platform]、和 [!DNL Azure]，可讓您從這些系統帶入資料。 內嵌的資料可以格式化為JSON、Parquet或分隔。 對雲端儲存提供者的支援包括 [!DNL Apache] HDFS。
...
## Connect [!DNL Apache] HDFS至 [!DNL Platform]

以下檔案提供有關如何連線的資訊 [!DNL Apache] HDFS至 [!DNL Platform] 使用API或使用者介面：
