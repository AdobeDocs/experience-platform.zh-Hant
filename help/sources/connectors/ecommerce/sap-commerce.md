...
[!DNL SAP Commerce]來源可讓您從下列[[!DNL SAP] 訂閱帳單](https://www.sap.com/products/financial-management/subscription-billing.html)商業夥伴API端點將客戶和連絡人資訊擷取到Platform：
...
在將[!DNL SAP Commerce]資料帶入Experience Platform之前，您必須先確定您有以下專案：
...
* [!DNL SAP]服務金鑰。 [!DNL SAP]服務金鑰可讓您透過Experience Platform存取[!DNL SAP Subscription Billing] API。 [!DNL SAP Commerce] 需要下列項目:
   * 用戶端 ID
   * 用戶端密碼
   * URL。 URL模式如下： `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`。 當您使用API [建立基底連線](../../tutorials/api/create/ecommerce/sap-commerce.md#base-connection)或透過Platform UI [連線您的 [!DNL SAP Commerce] 帳戶](../../tutorials/ui/create/ecommerce/sap-commerce.md#connect-account)時，此值稍後將用於取得`region`和`tokenEndpoint`的值。
...
以下檔案提供如何使用API或使用者介面將[!DNL SAP Commerce]連線到Platform的資訊：

* [使用API建立來源連線和資料流，將 [!DNL SAP Commerce] 資料帶入Platform](../../tutorials/api/create/ecommerce/sap-commerce.md)。
* [使用UI](../../tutorials/ui/create/ecommerce/sap-commerce.md)連線您的 [!DNL SAP Commerce] 帳戶以Experience Platform。
