...
#### 支援源中基於屬性的訪問控制 [!BADGE 新功能]

>[!TIP]
>
>基於屬性的訪問控制的工作如下： **角色** 可建立用來分類與您的Platform例項互動的使用者類型。 **標籤** 套用至 **角色** 指定該特定角色的訪問權限。 **標籤** 也會套用至結構欄位和區段等資源。 為了讓使用者能存取特定結構欄位和區段，必須將它們新增至 *具有分配給查詢資源的相同標籤的角色*. 如需詳細資訊，請閱讀 [基於屬性的訪問控制端到端指南](../access-control/abac/end-to-end-guide.md).

- 將標籤套用至結構欄位，以定義對組織中特定結構欄位的存取。 建立特定架構欄位的存取權限後，使用者將只能為其有權存取的欄位建立對應。
- 沒有適當角色的用戶將無法建立或更新包含不可訪問架構欄位的映射的資料流。 此外，未經授權的用戶無法更新、刪除、啟用或禁用具有不可訪問架構欄位的現有資料流。
- 此外，資料流的映射、目標資料集和目標連接中必須具有相同的架構ID和版本。
