...
>[!NOTE]
>
>持續失敗30天的Source擷取資料流會自動停用。 使用[監視儀表板](/help/dataflows/ui/monitor-sources.md)檢閱您的資料流、識別其失敗的原因（例如，認證、許可權或結構描述或對應變更）、套用必要的更新，並在解析後重新啟用資料流。
...
| [[!DNL Oracle]](connectors/databases/oracle.md) | 批次 | Azure、AWS |
