...
目前， [!DNL Sources SDK] 支援OAuth 2重新整理程式碼和基本驗證。 請參閱下表，了解使用OAuth 2重新整理程式碼和基本驗證的指引
...
基本驗證是一種驗證類型，可讓您使用應用程式的主機URL、帳戶使用者名稱和帳戶密碼的組合來存取應用程式。
...
| `authSpec.spec.required` | 指定在Platform中要輸入的必填值所需欄位。 | `host` |
