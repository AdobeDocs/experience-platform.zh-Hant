...
description: 以下檔案提供如何使用流量服務API測試及驗證新來源，以及透過來源SDK整合新來源的步驟。
hide: true
hidefromtoc: true
...
將新來源整合至Adobe Experience Platform的最後一步，使用 [!DNL Sources SDK] 是測試源進行驗證。 成功後，您就可以聯絡Adobe代表，提交新來源。
...
* 若要協助您進行測試和偵錯程式，請下載 [[!DNL Sources SDK] 此處的驗證收集和環境](../assets/sdk-verification.zip) 並遵循下列步驟。

## 測試源

要測試源，必須運行 [[!DNL Sources SDK] 驗證收集和環境](../assets/sdk-verification.zip) on [!DNL Postman] 同時提供與源相關的適當環境變數。

若要開始測試，您必須先在上設定集合和環境 [!DNL Postman]. 接下來，指定要測試的連接規範ID。 此ID應與您使用 [!DNL Sources SDK].
