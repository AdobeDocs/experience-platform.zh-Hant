...
資料流是從源中檢索資料並將資料帶入平台資料集的計畫任務。 本教學課程提供使用行銷自動化帳戶設定新資料流的步驟。
...
![調度](../../../images/tutorials/dataflow/marketing-automation/scheduling.png)
...
在「來 *源* 」工作區中，選取您要在「行銷自動化」類別下檢視的行銷 *自動化來源* 。 選擇 *連接源* ，啟動驗證介面。 要查看現有資料流，請選擇 *現有帳戶* ，然後選擇要訪問的帳戶。
...
遵循本教學課程，您已成功建立資料流，從行銷自動化系統匯入資料，並深入瞭解監控資料集。 現在，下游平台服務（例如即時客戶個人檔案和資料科學工作區）可以使用傳入的資料。 如需詳細資訊，請參閱下列檔案：
