...
title: 在UI中為行銷自動化來源連線設定資料流
topic-legacy: overview
type: Tutorial
description: 資料流是一個排程任務，可從源中檢索並將資料內嵌到 [!DNL Platform] 資料集。 本教學課程提供使用行銷自動化帳戶設定新資料流的步驟。
...
資料流是從源中檢索資料並將資料內嵌到[!DNL Platform]資料集的計畫任務。 本教學課程提供使用行銷自動化帳戶設定新資料流的步驟。
...
此外，本教學課程要求您已建立行銷自動化帳戶。 若需在UI中建立不同行銷自動化連接器的教學課程清單，請參閱[來源連接器概述](../../../home.md)。

## 選擇資料

建立行銷自動化帳戶後，會出現&#x200B;*選取資料*&#x200B;步驟，提供互動式介面供您探索檔案階層。
