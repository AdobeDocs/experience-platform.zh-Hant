...
依照本教學課程中的指示，您已成功建立資料流，將資料從行銷自動化來源帶入Platform。 下游[!DNL Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
