...
title: 在UI中使用同意和偏好設定Source建立資料流
type: Tutorial
description: 資料流是排程的工作，會從來源擷取資料並擷取資料至Experience Platform資料集。 本教學課程提供如何使用Experience Platform UI為同意和偏好設定來源建立資料流的步驟。
...
# 在UI中使用同意和偏好設定來源建立資料流

資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Experience Platform UI為同意和偏好設定來源建立資料流的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有具有[!DNL OneTrust Integration]來源的已驗證帳戶。 如需詳細資訊，請參閱有關[在UI](../../ui/create/consent-and-preferences/onetrust.md)中建立 [!DNL OneTrust Integration] 來源連線的教學課程。
...
建立您的同意和偏好設定來源帳戶後，**[!UICONTROL 新增資料]**&#x200B;步驟隨即顯示，為您提供一個介面，讓您探索同意和偏好設定帳戶的表格階層。
...
依照本教學課程所述，您已成功建立資料流，以將您的同意和偏好設定來源中的資料匯入Experience Platform。 下游[!DNL Experience Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
