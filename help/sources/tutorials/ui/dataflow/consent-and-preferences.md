...
資料流是排程的工作，可從Adobe Experience Platform的來源擷取資料並擷取資料到資料集。 本教學課程提供如何使用Platform UI為同意和偏好設定來源建立資料流的步驟。

>[!NOTE]
>
>為了建立資料流，您必須擁有已驗證的帳戶， [!DNL OneTrust Integration] 來源。 請參閱教學課程，位置如下： [建立 [!DNL OneTrust Integration] ui中的來源連線](../../ui/create/consent-and-preferences/onetrust.md) 以取得詳細資訊。
...
建立您的同意和偏好設定來源帳戶後， **[!UICONTROL 新增資料]** 步驟隨即顯示，提供介面供您探索同意和偏好設定帳戶的表格階層。
...
>[!IMPORTANT]
>
>您無法從將任何動態索引鍵配對值對應為物件 [!DNL OneTrust] 到Platform，並且必須在目標架構中指定這些索引鍵，才能在擷取期間對應您的資料。
...
依照本教學課程所述，您已成功建立資料流，將您的同意和偏好設定來源中的資料匯入Platform。 傳入資料現在可供下游使用 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
