---
keywords: Experience Platform；首頁；熱門主題；設定資料流；廣告聯結器
solution: Experience Platform
title: 在UI中使用Advertising Source建立資料流
type: Tutorial
description: 資料流是排程的工作，會從來源擷取資料並擷取資料至Experience Platform資料集。 本教學課程提供如何使用Experience Platform UI為廣告來源建立資料流的步驟。
...
資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Experience Platform UI為廣告來源建立資料流的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有具有廣告來源的已驗證帳戶。 在[來源概觀](../../../home.md#advertising)中找到在UI中建立不同廣告來源帳戶的教學課程清單。
...
建立廣告來源帳戶後，**[!UICONTROL 新增資料]**&#x200B;步驟隨即顯示，提供介面供您探索廣告來源帳戶的表格階層。
...
依照本教學課程所述，您已成功建立資料流，以將廣告來源的資料匯入Experience Platform。 下游[!DNL Experience Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
