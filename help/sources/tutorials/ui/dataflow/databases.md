...
資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Platform UI為資料庫來源建立資料流的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有具有資料庫來源的已驗證帳戶。 在UI中建立不同資料庫來源帳戶的教學課程清單可在[來源概觀](../../../home.md#database)中找到。
>
>* 為了讓Experience Platform擷取資料，所有以表格為基礎的批次來源的時區都必須設定為UTC。 [[!DNL Snowflake] 來源](../../../connectors/databases/snowflake.md)唯一支援的時間戳記是UTC時間的TIMESTAMP_NTZ。
...
依照本教學課程中的指示，您已成功建立資料流，將資料從您的資料庫來源帶入Platform。 下游[!DNL Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
