...
type: Tutorial
description: 資料流是從源資料集中檢索資料並將資料接收到平台資料集的計畫任務。 本教程提供了使用資料庫帳戶配置新資料流的步驟。
...
資料流是從源資料集中檢索資料並將資料接收到平台資料集的計畫任務。 本教程提供了使用資料庫帳戶配置新資料流的步驟。
...
此外，本教程要求您已經建立了資料庫帳戶。 有關在UI中建立不同資料庫連接器的教程清單，請參見 [源連接器概述](../../../home.md)。

## 添加資料

建立資料庫帳戶後， **[!UICONTROL 添加資料]** 步驟，提供互動式介面以便您瀏覽資料庫層次結構。

- 介面的左半部分是瀏覽器，顯示帳戶的資料表清單。
- 該介面的右半部分允許您預覽多達100行資料。
...
![選擇資料](../../../images/tutorials/dataflow/databases/select-data.png)


## 將資料欄位映射到XDM架構

的 *映射* 步驟，提供交互介面將源資料映射到平台資料集。

為要接收到的入站資料選擇資料集。 可以使用現有資料集或建立新資料集。

### 使用現有資料集

要將資料插入現有資料集，請選擇 **[!UICONTROL 現有資料集]**，然後按一下資料集表徵圖。
...
![新建資料集](../../../images/tutorials/dataflow/databases/new-dataset.png)
...
按照本教程，您成功建立了一個資料流，以從外部資料庫導入資料，並深入瞭解了監視資料集。 傳入資料現在可供下游使用 [!DNL Platform] 服務，如 [!DNL Real-time Customer Profile] 和 [!DNL Data Science Workspace]。 有關詳細資訊，請參閱以下文檔：
...
在 **[!UICONTROL 源]** 工作區，選擇 **[!UICONTROL 資料流]** 頁籤。 接下來，選擇要禁用的資料流。

![](../../../images/tutorials/dataflow/databases/list-of-dataflows.png)

的 **[!UICONTROL 屬性]** 列顯示在螢幕的右側，包括 **[!UICONTROL 已啟用]** 切換按鈕。 選擇切換以禁用資料流。 禁用資料流後，可以使用相同的切換功能重新啟用資料流。
