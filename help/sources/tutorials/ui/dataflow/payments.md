...
title: 在UI中使用支付Source建立資料流
type: Tutorial
description: 資料流是排程的工作，會從來源擷取資料並擷取資料至Experience Platform資料集。 本教學課程提供如何使用Experience Platform UI為支付來源建立資料流的步驟。
...
資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Experience Platform UI為付款來源建立資料流的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有具有付款來源的已驗證帳戶。 在UI中建立不同付款來源帳戶的教學課程清單可在[來源概觀](../../../home.md#payments)中找到。
...
建立您的付款來源帳戶之後，會出現&#x200B;**[!UICONTROL 新增資料]**&#x200B;步驟，為您提供介面以探索您的付款來源帳戶的表格階層。
...
依照本教學課程中的指示，您已成功建立資料流，將資料從您的付款來源帶入Experience Platform。 下游[!DNL Experience Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
