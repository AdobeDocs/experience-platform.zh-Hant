...
資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Platform UI為付款來源建立資料流的步驟。
...
依照本教學課程中的指示，您已成功建立資料流，將資料從您的付款來源帶入Platform。 下游[!DNL Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
