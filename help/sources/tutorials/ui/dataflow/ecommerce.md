...
title: 在UI中為電子商務連接器配置資料流
topic: overview
type: Tutorial
description: 資料流是從源中檢索資料並將資料收錄到資料集的計畫 [!DNL Platform] 任務。 本教學課程提供使用電子商務帳戶配置新資料流的步驟。
...
資料流是從源中檢索資料並將資料收錄到資料集的計畫 [!DNL Platform] 任務。 本教學課程提供使用eCommerce帳戶配置新資料 **[!UICONTROL 流的步]** 驟。
...
此外，本教學課程要求您已建立 **[!UICONTROL 電子商務]** 。 如需在UI中建立不同 **[!UICONTROL 電子商務連接器的教學課程清單]** ，請參閱來源連 [接器概觀](../../../home.md)。

## 選擇資料

在建立您的 **[!UICONTROL 電子商務帳戶後]** ，會出現「選 **[!UICONTROL 取資料]** 」步驟，提供互動式介面供您探索檔案階層。
...
找到源資料後，選擇目錄，然後選擇「下 **[!UICONTROL 一步]**」。
...
按照本教程，您成功建立了一個資料流以導入 **[!UICONTROL eCommerce資料]** ，並獲得了對監控資料集的深入瞭解。 現在，下游服務（例如和）可 [!DNL Platform] 以使用傳入 [!DNL Real-time Customer Profile] 的資料 [!DNL Data Science Workspace]。 如需詳細資訊，請參閱下列檔案：
