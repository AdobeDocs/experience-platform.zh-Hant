...
title: 在UI中使用分析源建立資料流
description: 本教程提供了如何使用平台UI為分析源建立資料流的步驟。
...
資料流是從Adobe Experience Platform的源中檢索資料並將資料接收到資料集的調度任務。 本教程提供了如何使用平台UI為分析源建立資料流的步驟。

>[!NOTE]
>
>為了建立資料流，您必須已使用 [!DNL Mixpanel] 源。 請參閱上的教程 [建立 [!DNL Mixpanel] UI中的源連接](../../ui/create/analytics/mixpanel.md) 的子菜單。
...
通過遵循本教程，您已成功建立了資料流，以將資料從分析源帶到平台。 傳入資料現在可供下游使用 [!DNL Platform] 服務，如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]。 有關詳細資訊，請參閱以下文檔：
