---
keywords: Experience Platform；首頁；熱門主題；分析；mixpanel
solution: Experience Platform
title: 在UI中使用Analytics來源建立資料流
...
資料流是排程的工作，可從Adobe Experience Platform的來源擷取資料並擷取資料到資料集。 本教學課程提供如何使用Platform UI為分析來源建立資料流的步驟。

>[!NOTE]
>
>為了建立資料流，您必須擁有已驗證的帳戶， [!DNL Mixpanel] 來源。 請參閱教學課程，位置如下： [建立 [!DNL Mixpanel] ui中的來源連線](../../ui/create/analytics/mixpanel.md) 以取得詳細資訊。
...
依照本教學課程所述，您已成功建立資料流，將資料從您的分析來源帶入Platform。 傳入資料現在可供下游使用 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
