...
>[!NOTE]
>
>為了建立資料流，您必須擁有已驗證的帳戶， [!DNL Mixpanel] 來源。 請參閱上的教學課程 [建立 [!DNL Mixpanel] ui中的來源連線](../../ui/create/analytics/mixpanel.md) 以取得詳細資訊。
...
>[!IMPORTANT]
>
>您無法從將任何動態索引鍵配對值對應為物件 [!DNL OneTrust] 到Platform，並且必須在目標架構中指定這些索引鍵，以便在擷取期間對應您的資料。
...
依照本教學課程中的指示，您已成功建立資料流，將資料從您的分析來源帶入Platform。 下游現在可以使用傳入的資料 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
