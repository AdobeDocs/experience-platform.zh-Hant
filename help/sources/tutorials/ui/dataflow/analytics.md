...
資料流是一項排程任務，可從來源擷取資料，並內嵌至Adobe Experience Platform中的資料集。 本教學課程提供如何使用Platform UI為分析來源建立資料流的步驟。

>[!NOTE]
>
>若要建立資料流，您必須已擁有已驗證的帳戶，且 [!DNL Mixpanel] 來源。 請參閱 [建立 [!DNL Mixpanel] UI中的源連接](../../ui/create/analytics/mixpanel.md) 以取得更多資訊。
...
依照本教學課程，您已成功建立資料流，將資料從分析來源帶入Platform。 下游現在可以使用傳入的資料 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需詳細資訊，請參閱下列檔案：
