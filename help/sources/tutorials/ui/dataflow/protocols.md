---
keywords: Experience Platform；首頁；熱門主題；通訊協定連接器
solution: Experience Platform
title: 在UI中使用協定源建立資料流
type: Tutorial
description: 資料流是一個排程任務，可從源中檢索資料並將資料內嵌到Platform資料集。 本教學課程提供如何使用Platform UI為通訊協定來源建立資料流的步驟。
...
資料流是一項排程任務，可從來源擷取資料，並內嵌至Adobe Experience Platform中的資料集。 本教學課程提供如何使用Platform UI為通訊協定來源建立資料流的步驟。

>[!NOTE]
>
>要建立資料流，您必須已經擁有一個具有協定源的已驗證帳戶。 若需在UI中建立不同通訊協定來源帳戶的教學課程清單，請參閱 [來源概觀](../../../home.md#protocols).
...
建立通訊協定來源帳戶後， **[!UICONTROL 新增資料]** 步驟，提供介面供您探索通訊協定來源帳戶的表格階層。
...
依照本教學課程，您已成功建立資料流，將資料從通訊協定來源帶入Platform。 下游現在可以使用傳入的資料 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需詳細資訊，請參閱下列檔案：
