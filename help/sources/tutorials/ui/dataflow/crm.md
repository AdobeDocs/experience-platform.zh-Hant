...
type: Tutorial
description: 資料流是從源資料集中檢索資料並將資料接收到平台資料集的計畫任務。 本教程提供了如何使用平台UI為CRM源建立資料流的步驟。
...
資料流是從Adobe Experience Platform的源中檢索資料並將資料接收到資料集的調度任務。 本教程提供了有關如何使用平台UI為CRM（客戶關係管理）源建立資料流的步驟。

>[!NOTE]
>
>要建立資料流，您必須已經擁有具有CRM源的經過身份驗證的帳戶。 有關在UI中建立不同CRM源帳戶的教程清單，請參見 [源概述](../../../home.md#crm)。
...
建立CRM源帳戶後， **[!UICONTROL 添加資料]** 步驟，為您提供一個介面以瀏覽CRM帳戶的表層次結構。
...
>[!NOTE]
>
>如果您使用 [!DNL Salesforce] 作為Adobe Real-time Customer Data PlatformB2B版的一部分，請參閱 [[!DNL Salesforce] 欄位映射文檔](../../../connectors/adobe-applications/mapping/salesforce.md) 關於繪圖的指導 [!DNL Salesforce] 源欄位到其相應的XDM目標欄位。
...
通過遵循本教程，您已成功建立了資料流，以將CRM源中的資料帶到平台。 傳入資料現在可供下游使用 [!DNL Platform] 服務，如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]。 有關詳細資訊，請參閱以下文檔：
