...
資料流是排程的工作，可從Adobe Experience Platform的來源擷取資料並擷取資料到資料集。 本教學課程提供如何使用Platform UI為CRM （客戶關係管理）來源建立資料流的步驟。

>[!NOTE]
>
>為了建立資料流，您必須擁有具有CRM來源的已驗證帳戶。 在UI中建立不同CRM來源帳戶的教學課程清單可在以下網址找到： [來源概觀](../../../home.md#crm).
...
建立您的CRM來源帳戶後， **[!UICONTROL 新增資料]** 步驟隨即顯示，提供您探索CRM帳戶表格階層的介面。
...
>[!NOTE]
>
>如果您使用 [!DNL Salesforce] 作為Adobe Real-time Customer Data Platform B2B版本的一部分，請參閱 [[!DNL Salesforce] 欄位對應檔案](../../../connectors/adobe-applications/mapping/salesforce.md) 以取得對應指引 [!DNL Salesforce] 來源欄位至其適當的XDM目標欄位。
...
依照本教學課程所述，您已成功建立資料流，將您的CRM來源中的資料匯入Platform。 傳入資料現在可供下游使用 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
