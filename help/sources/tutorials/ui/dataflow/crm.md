...
資料流是從源中檢索資料並將資料帶入平台資料集的計畫任務。 本教程提供使用CRM基本連接器配置新資料流的步驟。
...
出現 *「名稱流* 」步驟，您必須在其中為資料流提供名稱和可選說明。 完成後，按&#x200B;**「下一步」**。
...
在「 _來源_ 」工作區中，選取您要在「 *CRM」類別下檢視的CRM來源* 。 選擇 *連接源* ，啟動驗證介面。 要查看現有資料流，請選擇 *現有帳戶* ，然後選擇要訪問的帳戶。
...
通過本教程，您成功建立了一個資料流，以便從CRM中導入資料，並獲得了對監控資料集的深入瞭解。 現在，下游平台服務（例如即時客戶個人檔案和資料科學工作區）可以使用傳入的資料。 如需詳細資訊，請參閱下列檔案：
