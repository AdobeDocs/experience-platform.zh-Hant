...
>[!NOTE]
>
>* 為了建立資料流，您必須擁有具有CRM來源的已驗證帳戶。 在UI中建立不同CRM來源帳戶的教學課程清單位於 [來源概觀](../../../home.md#crm).
...
建立您的CRM來源帳戶後， **[!UICONTROL 新增資料]** 步驟隨即顯示，提供介面供您探索CRM帳戶的表格階層。
...
>[!NOTE]
>
>如果您使用 [!DNL Salesforce] 做為Adobe Real-time Customer Data Platform B2B版本的一部分，請參閱 [[!DNL Salesforce] 欄位對應檔案](../../../connectors/adobe-applications/mapping/salesforce.md) 以取得對應指引 [!DNL Salesforce] 將來源欄位新增至其適當的XDM目標欄位。
...
| 載入增量資料者： | 一個選項，其中包含一組型別、日期或時間的篩選來源結構描述欄位。 此欄位用於區分新資料和現有資料。 將根據所選欄的時間戳記擷取增量資料。 |
...
依照本教學課程中的指示，您已成功建立資料流，將資料從CRM來源帶入Platform。 下游現在可以使用傳入的資料 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
