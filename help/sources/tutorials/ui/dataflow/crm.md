---
keywords: Experience Platform；首頁；熱門主題；crm結構描述；crm；CRM；資料流；資料流
solution: Experience Platform
title: 在UI中使用CRM Source建立資料流
type: Tutorial
description: 資料流是排程的工作，會從來源擷取資料並擷取資料至Experience Platform資料集。 本教學課程提供如何使用Experience Platform UI為CRM來源建立資料流的步驟。
...
資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Experience Platform UI為CRM （客戶關係管理）來源建立資料流的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有具有CRM來源的已驗證帳戶。 在[來源概觀](../../../home.md#crm)中找到在UI中建立不同CRM來源帳戶的教學課程清單。
...
建立CRM來源帳戶後，**[!UICONTROL 新增資料]**&#x200B;步驟隨即顯示，提供介面供您探索CRM帳戶的表格階層。
...
>[!NOTE]
>
>如果您使用[!DNL Salesforce]做為Adobe Real-Time Customer Data Platform B2B edition的一部分，請參閱[[!DNL Salesforce] 欄位對應檔案](../../../connectors/adobe-applications/mapping/salesforce.md)，以取得將[!DNL Salesforce]來源欄位對應到其適當XDM目標欄位的指引。
...
| 載入增量資料者： | 一個選項，其中包含一組型別、日期或時間的篩選來源結構描述欄位。 此欄位是用來區分新資料和現有資料。將根據所選欄的時間戳記擷取增量資料。 |
...
依照本教學課程中的指示，您已成功建立資料流，將資料從CRM來源帶入Experience Platform。 下游[!DNL Experience Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
