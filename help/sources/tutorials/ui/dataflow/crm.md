...
type: Tutorial
description: 資料流是排程的工作，可擷取來源中的資料並將其擷取至Platform資料集。 本教學課程提供如何使用Platform UI為CRM來源建立資料流的步驟。
...
資料流是排程的工作，可擷取來源中的資料並將資料擷取到Adobe Experience Platform中的資料集。 本教學課程提供如何使用Platform UI為CRM （客戶關係管理）來源建立資料流的步驟。
...
>[!NOTE]
>
>如果您使用[!DNL Salesforce]做為Adobe Real-time Customer Data Platform B2B版本的一部分，請參閱[[!DNL Salesforce] 欄位對應檔案](../../../connectors/adobe-applications/mapping/salesforce.md)，以取得將[!DNL Salesforce]來源欄位對應到其適當XDM目標欄位的指引。
...
依照本教學課程中的指示，您已成功建立資料流，將資料從CRM來源帶入Platform。 下游[!DNL Platform]服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
