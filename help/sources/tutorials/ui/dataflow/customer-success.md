...
資料流是從源中檢索資料並將資料收錄到資料集的計畫 [!DNL Platform] 任務。 本教程提供使用客戶成功帳戶配置新資料流的步驟。
...
建立客戶成功連接器後，會出現「選 **[!UICONTROL 取資料]** 」步驟，提供互動式介面供您探索檔案階層。
...
通過本教程，您成功建立了一個資料流，以導入來自客戶成功源的資料並獲得了有關監控資料集的深入見解。 現在，下游服務（例如和）可 [!DNL Platform] 以使用傳入 [!DNL Real-time Customer Profile] 的資料 [!DNL Data Science Workspace]。 如需詳細資訊，請參閱下列檔案：
...
在「驗 **[!UICONTROL 證]** 」螢幕中，選擇與要禁用的資料流關聯的帳戶的名稱。
