...
topic: overview
description: 本教學課程提供使用平台使用者介面建立HubSpot來源連接器的步驟。
...
如果您已有連 [!DNL HubSpot] 接，則可以跳過本文檔的其餘部分，並繼續有關配置資料 [流的教程](../../dataflow/marketing-automation.md)。
...
如需快速入門的詳細資訊，請參閱本 [[!DNL HubSpot] 檔案](https://developers.hubspot.com/docs/methods/oauth2/oauth2-overview)。

## 連線您的帳 [!DNL HubSpot] 戶

收集完所需的認證後，您可依照下列步驟將帳戶連結 [!DNL HubSpot] 至 [!DNL Platform]。
...
在「行銷 **[!UICONTROL 自動化]** 」類別下，選 **[!UICONTROL 取HubSpot]**。 如果這是您第一次使用此連接器，請選擇「配 **[!UICONTROL 置」]**。 否則，請選 **[!UICONTROL 擇「添加資料]** 」(Add data [!DNL HubSpot] )以建立新連接器。
...
如果您使用新認證，請選擇「新 **[!UICONTROL 帳戶」]**。 在出現的輸入表單上，提供名稱、選用說明和您的認 [!DNL HubSpot] 證。 完成後，選擇 **[!UICONTROL Connect]** ，然後為建立新連接留出一些時間。
...
遵循本教學課程，您已建立與帳戶的 [!DNL HubSpot] 連線。 您現在可以繼續下一個教學課程，並設 [定資料流，將行銷自動化系統資料帶入 [!DNL Platform]](../../dataflow/marketing-automation.md)。