...
# 在UI中建立Microsoft Dynamics來源連接器

Adobe Experience Platform中的來源連接器可讓您依計畫吸收外部來源的CRM資料。 本教學課程提供使用Platform使用者介面建立Microsoft Dynamics（以下稱為「Dynamics」）來源連接器的步驟。
...
如果您已經擁有有效的Dynamics帳戶，則可以略過本文檔的其餘部分，然後繼續有關配置資料 [流的教程](../../dataflow/crm.md)。
...
收集完所需憑證後，您可依照下列步驟建立新的Dynamics帳戶以連線至平台。
...
在「數 *[!UICONTROL 據庫]* 」類別下，選擇「動態」( **[!UICONTROL Dynamics]** ) **按一下+表徵圖(+)** ，以建立新的「動態」(Dynamics)連接器。
...
如果您使用新認證，請選擇「新 **[!UICONTROL 帳戶」]**。 在出現的輸入表單上，提供名稱、選用說明和動態憑證的連線。 完成後，選擇 **[!UICONTROL Connect]** ，然後為新帳戶建立留出一些時間。

![連接](../../../../images/tutorials/create/ms-dynamics/new.png)

### 現有帳戶

若要連線現有帳戶，請選取您要連線的Dynamics帳戶，然後選取右上角的 **[!UICONTROL Next]** （下一步）以繼續。

![現有](../../../../images/tutorials/create/ms-dynamics/existing.png)

## 後續步驟

遵循本教學課程，您已建立與Dynamics帳戶的連線。 您現在可以繼續下一個教程，並 [配置資料流以將資料導入平台](../../dataflow/crm.md)。