...
如果您已有連 [!DNL Google AdWords] 接，則可以跳過本文檔的其餘部分，並繼續有關配置資料 [流的教程](../../dataflow/payments.md)
...
| `refreshToken` | 從授權存取AdWords [!DNL Google] 所取得的重新整理Token。 |
...
收集完所需憑證後，您可依照下列步驟建立新的入站基本連線，將帳戶連結 [!DNL Google AdWords] 至 [!DNL Platform]。
...
在「 *[!UICONTROL 廣告]* 」類別下，選取 **[!UICONTROL Google AdWords]** ，以在螢幕右側顯示資訊列。 資訊列提供所選來源的簡短說明，以及與來源連線或檢視其檔案的選項。 要建立新的入站基本連接，請選擇「添 **[!UICONTROL 加資料」]**。
...
如果您使用新認證，請選擇「新 **[!UICONTROL 帳戶」]**。 在出現的輸入表單上，提供基本連線名稱、選用說明和您的認 [!DNL Google AdWords] 證。 完成後，選擇 **[!UICONTROL Connect]** ，然後為建立新的基本連接留出一些時間。
...
在本教學課程中，您已建立了與帳戶的基本連 [!DNL Google AdWords] 接。 您現在可以繼續下一個教學課程，並 [設定資料流，將廣告資料匯入Platform](../../dataflow/advertising.md)。