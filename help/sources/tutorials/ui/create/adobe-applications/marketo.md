---
keywords: Experience Platform；首頁；熱門主題；Marketo源連接器；Marketo連接器；Marketo源；Marketo
solution: Experience Platform
title: 在UI中建立Marketo Engage源連接器
...
>[!IMPORTANT]
>
>的 [!DNL Marketo] 連接器使用批處理攝取來攝取所有歷史記錄，並使用流式攝取來進行即時更新。 這允許連接器在接收任何錯誤記錄時繼續流式傳輸。 啟用 **[!UICONTROL 部分攝取]** 切換，然後設定 [!UICONTROL 錯誤閾值%] 以防止資料流失敗。
