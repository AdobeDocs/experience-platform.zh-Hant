...
Adobe Experience Platform中的Source聯結器可讓您依排程擷取外部來源資料。 本教學課程提供使用[!DNL Experience Platform]使用者介面建立[!DNL PostgreSQL]來源聯結器的步驟。
...
若要在[!DNL Experience Platform]上存取您的[!DNL PostgreSQL]帳戶，您必須提供下列值：
...
收集必要的認證後，您可以依照下列步驟將[!DNL PostgreSQL]帳戶連結至[!DNL Experience Platform]。
...
依照本教學課程中的指示，您已建立與[!DNL PostgreSQL]帳戶的連線。 您現在可以繼續進行下一個教學課程，並[設定資料流以將資料帶入 [!DNL Experience Platform]](../../dataflow/databases.md)。
