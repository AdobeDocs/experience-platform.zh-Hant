...
Adobe Experience Platform中的來源聯結器提供依排程擷取外部來源資料的功能。 本教學課程提供以下步驟來建立IBM DB2 （以下稱為「DB2」）來源聯結器： [!DNL Platform] 使用者介面。
...
如果您正在使用新認證，請選取 **[!UICONTROL 新帳戶]**. 在出現的輸入表單上，提供名稱、選擇性說明和您的DB2認證。 完成後，選取 **[!UICONTROL 連線]** 然後等待一段時間以建立新連線。
...
依照本教學課程中的指示，您已建立與DB2帳戶的連線。 您現在可以繼續進行下一個教學課程及 [設定資料流以將資料帶入 [!DNL Platform]](../../dataflow/databases.md).
