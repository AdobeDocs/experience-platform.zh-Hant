...
>[!NOTE]
>
> 此 [!DNL Couchbase] 聯結器為Beta版。 請參閱 [來源概觀](../../../../home.md#terms-and-conditions) 以取得有關使用Beta標籤聯結器的詳細資訊。
...
| 認證 | 說明 |
| ---------- | ----------- |
| `connectionString` | 用來連線至您的裝置的連線字串 [!DNL Couchbase] 執行個體。 的連線字串模式 [!DNL Couchbase] 是 `Server={SERVER}; Port={PORT};AuthMech=1;CredString=[{\"user\": \"{USER}\", \"pass\":\"{PASS}\"}];`. 如需取得連線字串的詳細資訊，請參閱檔案： [[!DNL Couchbase] 連線](https://docs.Couchbase.com/c-sdk/2.10/client-settings.html#configuring-overview). |

## 連線您的 [!DNL Couchbase] 帳戶

收集必要的認證後，您可以依照下列步驟連結 [!DNL Couchbase] 帳戶至 [!DNL Platform].
...
如果您正在使用新認證，請選取 **[!UICONTROL 新帳戶]**. 在出現的輸入表單上，提供名稱、選擇性說明，以及 [!DNL Couchbase] 認證。 完成後，選取 **[!UICONTROL 連線到來源]** 然後等待一段時間以建立新連線。

![connect](../../../../images/tutorials/create/couchbase/new.png)

### 現有帳戶

若要連線現有帳戶，請選取 [!DNL Couchbase] 您要連線的帳戶，然後選取 **[!UICONTROL 下一個]** 前往右上角繼續。

![現有](../../../../images/tutorials/create/couchbase/existing.png)

## 後續步驟

依照本教學課程中的指示，您已建立與的連線， [!DNL Couchbase] 帳戶。 您現在可以繼續進行下一個教學課程及 [設定資料流以將資料帶入 [!DNL Platform]](../../dataflow/databases.md).
