...
收集完所需的認證後，您可依照下列步驟建立新的ATS帳戶以連線至 [!DNL Platform]。
...
在「資 *[!UICONTROL 料庫]* 」類別下，選取 **[!UICONTROL Azure表格儲存]** ，以在畫面右側顯示資訊列。 資訊列提供所選來源的簡短說明，以及與來源連線或檢視其檔案的選項。 要建立新入站連接，請選擇「添 **[!UICONTROL 加資料」]**。
...
如果您使用新認證，請選擇「新 **[!UICONTROL 帳戶」]**。 在出現的輸入表單上，提供連線名稱、選用說明和您的ATS認證。 完成後，選擇 **[!UICONTROL Connect]** ，然後為新帳戶建立留出一些時間。
