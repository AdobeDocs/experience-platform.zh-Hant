...
>[!NOTE]
>
> HP [!DNL Vertica] 聯結器為Beta版。 請參閱 [來源概觀](../../../../home.md#terms-and-conditions) 以取得有關使用Beta標籤聯結器的詳細資訊。

Adobe Experience Platform中的來源聯結器提供依排程擷取外部來源資料的功能。 本教學課程提供建立HP的步驟 [!DNL Vertica] 來源聯結器使用 [!DNL Platform] 使用者介面。
...
如果您已有有效的HP [!DNL Vertica] 連線時，您可以略過本檔案的其餘部分，並繼續進行上的教學課程 [設定資料流](../../dataflow/databases.md).
...
| 認證 | 說明 |
| ---------- | ----------- |
| `connectionString` | 用來連線您的HP的連線字串 [!DNL Vertica] 執行個體。 HP的連線字串模式 [!DNL Vertica] 是 `Server={SERVER};Port={PORT};Database={DATABASE};UID={USERNAME};PWD={PASSWORD}` |
...
收集必要的認證後，您可以依照下列步驟連結HP [!DNL Vertica] 帳戶至 [!DNL Platform].
...
如果您正在使用新認證，請選取 **[!UICONTROL 新帳戶]**. 在出現的輸入表單上，提供名稱、選擇性說明，以及您的HP [!DNL Vertica] 認證。 完成後，選取 **[!UICONTROL 連線]** 然後等待一段時間以建立新連線。

![connect](../../../../images/tutorials/create/hp-vertica/new.png)

### 現有帳戶

若要連線現有的帳戶，請選取HP [!DNL Vertica] 您要連線的帳戶，然後選取 **[!UICONTROL 下一個]** 前往右上角繼續。

![現有](../../../../images/tutorials/create/hp-vertica/existing.png)

## 後續步驟

依照本教學課程內容，您已建立與HP的連線 [!DNL Vertica] 帳戶。 您現在可以繼續進行下一個教學課程及 [設定資料流以將資料帶入 [!DNL Platform]](../../dataflow/databases.md).
