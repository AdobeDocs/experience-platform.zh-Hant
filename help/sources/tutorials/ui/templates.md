...
description: Adobe Experience Platform提供預先設定的範本，供您加速資料擷取程式。 範本包含自動產生的資產，例如結構、資料集、對應規則、身分命名空間，以及在從來源傳入資料時可使用的資料流。
...
Adobe Experience Platform提供預先設定的範本，供您加速資料擷取程式。 範本包含自動產生的資產，例如結構、資料集、對應規則、身分命名空間，以及在從來源傳入資料時可使用的資料流。

使用範本，您可以：

* 加速建立ML型資產，縮短擷取的時間至價值。
...
此時會出現一個快顯視窗，提供您瀏覽範本或使用現有結構和資料集的選項。 若要使用自動產生的資產，請選取 **[!UICONTROL 瀏覽範本]** 然後選取 **[!UICONTROL 選擇]**.
...
此 [!UICONTROL 檢閱範本資產] 頁面會顯示範本中自動產生的資產。 在本頁面中，您可以檢視與來源連線相關聯的自動產生結構、資料集、身分識別命名空間及資料流。
