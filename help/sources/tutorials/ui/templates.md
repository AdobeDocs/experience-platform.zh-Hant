...
在Experience Platform UI中，從左側導覽選取「**[!UICONTROL Sources]**」以存取[!UICONTROL Sources]工作區，並檢視Experience Platform中可用的來源目錄。

使用&#x200B;*[!UICONTROL Categories]*&#x200B;功能表來依類別篩選來源。 或者，在搜尋列中輸入來源名稱，從目錄中尋找特定來源。

移至[!UICONTROL Adobe applications]類別以檢視[!DNL Marketo Engage]來源卡，然後選取[!UICONTROL Add data]以開始。
...
>[!NOTE]
>
>使用需要變更資料擷取工作流程或支援多個資料模型的來源時，範本可以自動產生關聯式結構描述。 這些結構啟用Data Mirror的即時資料同步功能。\
>將範本與關聯式結構描述搭配使用時，自動產生的資產將會包含必要的主索引鍵、版本識別碼和時間戳記識別碼欄位。\
>如需詳細資訊，請參閱[Data Mirror概觀](../../../xdm/data-mirror/overview.md)和[關聯式結構描述技術參考](../../../xdm/schema/relational.md)。

若要使用自動產生的資產，請選取&#x200B;**[!UICONTROL Browse templates]**，然後選取&#x200B;**[!UICONTROL Select]**。
...
若要使用現有帳戶，請選取[!UICONTROL Existing account]，然後從顯示的清單中選取您要使用的帳戶。

![現有帳戶的選擇頁面，其中包含您可以存取的現有帳戶清單。](../../images/tutorials/templates/existing-account.png)

>[!TAB 建立新帳戶]

若要建立新帳戶，請選取&#x200B;**[!UICONTROL New account]**，然後提供您的來源連線詳細資料和帳戶驗證認證。 完成後，選取「**[!UICONTROL Connect to source]**」並留出一些時間建立新連線。
...
預覽視窗會出現，供您瀏覽及檢查範本中的範例資料。 完成後，選取&#x200B;**[!UICONTROL Got it]**。

![預覽範例資料視窗。](../../images/tutorials/templates/preview-sample-data.png)

接著，從清單中選取您要使用的範本。 您可以選取多個範本，一次建立多個資料流。 不過，每個帳戶只能使用一次範本。 選取範本後，請選取「**[!UICONTROL Finish]**」，讓資產產生一些時間。
...
完成擷取排程的設定後，請選取&#x200B;**[!UICONTROL Finish]**。
...
[!UICONTROL Review template assets]頁面會顯示作為範本的一部分自動產生的資產。 您可以在此頁面檢視與來源連線相關聯的自動產生結構描述、資料集、身分名稱空間和資料流程。 產生所有資產最多可能需要五分鐘的時間。如果您選擇離開頁面，資產完成後您將收到傳回的通知。您可以在產生資產後檢閱資產，並可隨時對資料流進行其他設定。

根據預設，自動產生的資料流會設為草稿狀態，以允許對設定進行進一步自訂，例如對應規則或排程頻率。 選取資料流名稱旁邊的省略符號(`...`)，然後選取&#x200B;**[!UICONTROL Preview mappings]**&#x200B;以檢視為您的草稿資料流建立的對應集。

![已選取預覽對應選項的下拉式視窗。](../../images/tutorials/templates/preview.png)

系統會顯示預覽頁面，讓您檢查來源資料欄位與目標結構描述欄位之間的對應關係。 檢視資料流的對應後。 選取&#x200B;**[!UICONTROL Got it.]**

![對應預覽視窗。](../../images/tutorials/templates/preview-mappings.png)

您可在執行後隨時更新資料流程。 選取資料流名稱旁邊的省略符號(`...`)，然後選取&#x200B;**[!UICONTROL Update dataflow]**。 您會進入來源工作流程頁面，您可以在此頁面更新資料流詳細資訊，包括部分擷取、錯誤診斷和警報通知的設定，以及資料流的對應。
...
>[!TIP]
>
>您可以透過來源工作區中的[!UICONTROL Dataflows]目錄頁面存取草稿資料流。 從頂端標題中選取&#x200B;**[!UICONTROL Dataflows]**，然後從清單中選取您要更新的資料流。
>
>![來源工作區資料流目錄中的現有資料流清單。](../../images/tutorials/templates/dataflows.png)

### 發佈您的資料流

透過瀏覽來源工作流程開始發佈程式。 選取[!UICONTROL Update dataflow]之後，您會進入工作流程的&#x200B;*[!UICONTROL Add data]*&#x200B;步驟。 選取&#x200B;**[!UICONTROL Next]**&#x200B;以繼續。

![草稿資料流的新增資料步驟](../../images/tutorials/templates/continue-draft.png)

接下來，確認您的資料流詳細資料，並設定錯誤診斷、部分擷取和警報通知的設定。 完成後，選取&#x200B;**[!UICONTROL Next]**。

![草稿資料流的資料流詳細資料步驟。](../../images/tutorials/templates/dataflow-detail.png)

>[!NOTE]
>
>您可以隨時選取&#x200B;**[!UICONTROL Save as draft]**&#x200B;以停止並儲存您對資料流所做的變更。
...
最後，檢閱資料流的詳細資料，然後選取&#x200B;**[!UICONTROL Save & ingest]**&#x200B;以發佈您的草稿。
