...
* **使用我現有的資產**：使用您建立的現有資料集和結構描述擷取資料。 如有需要，您也可以建立新資料集和結構描述。

若要使用自動產生的資產，請選取 **[!UICONTROL 瀏覽範本]** 然後選取 **[!UICONTROL 選取]**.

![快顯視窗，其中包含瀏覽範本或使用現有資產的選項。](../../images/tutorials/templates/browse-templates.png)
...
若要建立新帳戶，請選取 **[!UICONTROL 新帳戶]**，然後提供您的來源連線詳細資料和帳戶驗證認證。 完成後，選取 **[!UICONTROL 連線到來源]** 並留出一些時間建立新連線。
...
+++[!DNL Marketo Engage] 範本下表概述可用於的範本 [!DNL Marketo Engage] 來源。
...
| 行銷活動成員 | 「促銷活動成員」範本會擷取個別銷售機會或聯絡人與特定客戶之間的關係 [!DNL Salesforce] 行銷活動。 |
...
根據您選取的企業型別，會出現範本清單。 選取預覽圖示 ![預覽圖示](../../images/tutorials/templates/preview-icon.png) 在範本名稱旁邊，以預覽範本中的範例資料。

![反白顯示預覽圖示的範本清單。](../../images/tutorials/templates/templates.png)

預覽視窗會出現，供您瀏覽及檢查範本中的範例資料。 完成後，選取 **[!UICONTROL 知道了]**.

![預覽範例資料視窗。](../../images/tutorials/templates/preview-sample-data.png)

接著，從清單中選取您要使用的範本。 您可以選取多個範本，一次建立多個資料流。 不過，每個帳戶只能使用一次範本。 選取範本後，選取 **[!UICONTROL 完成]** 並讓資產產生一些時間。
...
使用排程介面來設定資料流程的擷取排程。 將您的擷取頻率設為 **一次** 以建立一次性內嵌。

![Dynamics和Salesforce範本的排程介面。](../../images/tutorials/templates/schedule.png)

或者，您也可以將擷取頻率設為 **分鐘**， **小時**， **日**，或 **周**. 如果排程資料流進行多次擷取，則必須設定間隔，以建立每次擷取之間的時間範圍。 例如，擷取頻率設為 **小時** 而間隔設為 **15** 表示您的資料流已排程每隔 **15小時**.

在此步驟中，您也可以啟用 **回填** 和定義資料增量內嵌欄。 回填是用來擷取歷史資料，而您為增量擷取定義的欄則可區分新資料與現有資料。

完成擷取排程的設定後，請選取「 」 **[!UICONTROL 完成]**.

![已啟用回填之Dynamics和Salesforce範本的排程介面。](../../images/tutorials/templates/backfill.png)
...
系統會顯示預覽頁面，讓您檢查來源資料欄位與目標結構描述欄位之間的對應關係。 檢視資料流的對應後。 選取 **[!UICONTROL 知道了。]**
...
您可以使用結構編輯器檢視，對自動產生的結構描述進行更新。 瀏覽指南： [使用結構描述編輯器](../../../xdm/tutorials/create-schema-ui.md) 以取得詳細資訊。
...
透過瀏覽來源工作流程開始發佈程式。 在您選取之後 [!UICONTROL 更新資料流]，您即被帶到 *[!UICONTROL 新增資料]* 工作流程的步驟。 選取 **[!UICONTROL 下一個]** 以繼續進行。
...
![Platform UI中的通知面板（具有警示失敗資料流的通知）會醒目提示。](../../images/tutorials/templates/notifications.png)

您可以更新範本的警報設定，以接收有關資料流狀態的電子郵件和平台內通知。 如需設定警示的詳細資訊，請閱讀以下指南： [如何訂閱來源資料流程的警報](../ui/alerts.md).