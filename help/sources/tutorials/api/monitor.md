...
>[!NOTE]
>
>本教程要求您具有有效資料流的ID值。 如果您沒有有效的資料流ID，請從 [來源概觀](../../home.md) 並在嘗試本教程之前，按照概述的步驟建立資料流。
...
要查看資料流的狀態，請向 [!DNL Flow Service] API，同時提供資料流的對應流ID。
...
依照本教學課程，您已使用 [!DNL Flow Service] API。 您現在可以根據您的獲取計畫繼續監視資料流，以跟蹤其狀態和獲取率。 有關如何使用用戶介面執行相同任務的資訊，請參閱 [使用用戶介面監視資料流](../ui/monitor.md)
