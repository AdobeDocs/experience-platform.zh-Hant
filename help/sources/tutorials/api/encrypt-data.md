...
1. [使用Experience Platform API建立加密金鑰組](#create-encryption-key-pair)。 加密金鑰組由私密金鑰和公開金鑰組成。 建立後，您可以複製或下載公開金鑰，以及其對應的公開金鑰ID和到期時間。 在此過程中，私密金鑰將由Experience Platform儲存在安全的儲存庫中。 **注意：**&#x200B;回應中的公開金鑰是Base64編碼的，在使用之前必須先解碼。
...
將加密資料擷取至Experience Platform的第一步，是對[!DNL Connectors] API的`/encryption/keys`端點發出POST要求，以建立您的加密金鑰組。
...
| 屬性 | 說明 |
| --- | --- |
| `publicKey` | 公開金鑰是用來加密雲端儲存空間中的資料。 此金鑰對應至在此步驟中建立的私密金鑰。 不過，私密金鑰會立即傳送給Experience Platform。 |
...
若要擷取您組織中的所有加密金鑰，請向`/encryption/keys` endpoit=nt發出GET請求。
...
若要擷取一組特定的加密金鑰，請向`/encryption/keys`端點發出GET要求，並提供您的公開金鑰ID作為標頭引數。
...
若要共用您的公開金鑰，請提供您的加密演演算法和Base64編碼的公開金鑰時，對`/customer-keys`端點提出POST要求。
...
| 屬性 | 說明 |
| --- | --- |
| `publicKeyId` | 此公開金鑰ID會在與Experience Platform共用您的客戶管理金鑰時傳回。 在為已簽署和加密的資料建立資料流時，您可以提供此公開金鑰ID作為簽署驗證金鑰ID。 |

+++

### 擷取客戶管理的金鑰組

若要擷取您的客戶自控金鑰，請向`/customer-keys`端點發出GET請求。
...
## 使用[!DNL Flow Service] API將您的雲端儲存空間來源連線至Experience Platform
...
若要建立資料流，請對[!DNL Flow Service] API的`/flows`端點提出POST要求。 若要內嵌加密的資料，您必須將`encryption`區段新增至`transformations`屬性，並包含在先前步驟中建立的`publicKeyId`。
...
| `sourceConnectionIds` | 來源連線ID。 此ID代表資料從來源傳輸至Experience Platform的過程。 |
| `targetConnectionIds` | 目標連線ID。 此ID代表資料傳入Experience Platform後著陸的位置。 |
...
| 屬性 | 說明 |
| --- | --- |
| `params.signVerificationKeyId` | 簽署驗證金鑰ID與與Experience Platform共用Base64編碼公開金鑰後擷取的公開金鑰ID相同。 |
...
若要刪除您的加密金鑰，請向`/encryption/keys`端點提出DELETE要求，並提供您的公開金鑰ID作為標頭引數。
