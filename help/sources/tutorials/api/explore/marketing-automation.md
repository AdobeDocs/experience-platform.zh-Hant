...
本教學課程使用Flow Service API來探索行銷自動化系統。
...
使用行銷自動化系統的基本連線，您可以執行GET請求來探索資料表。 使用以下調用查找要檢查或裝入平台的表的路徑。
...
成功的回應是從您的行銷自動化系統傳送的一連串表格。 尋找您要帶入Platform的表格，並記下其屬性，因為您必須在下一 `path` 步驟中提供表格以檢查其結構。
...
透過本教學課程，您已探索行銷自動化系統，找到您要匯入平台的表格路徑，並取得其結構的相關資訊。 您可以在下一個教學課程中使用此資 [訊，從行銷自動化系統收集資料並匯入平台](../collect/marketing-automation.md)。
