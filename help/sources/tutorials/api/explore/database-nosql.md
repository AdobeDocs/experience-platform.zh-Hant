...
本教學課程使 [!DNL Flow Service] 用API來探索協力廠商資料庫的內容和檔案結構。
...
以下各節提供您需要知道的其他資訊，以便使用 [!DNL Flow Service] API成功連線至協力廠商資料庫。
...
使用資料庫的連接ID，可以通過執行GET請求來瀏覽資料表。 使用以下調用查找要檢查或裝入的表的路徑 [!DNL Platform]。
...
成功的響應從資料庫返回一組表。 尋找您要放入的表格， [!DNL Platform] 並記下其屬性，因 `path` 為您必須在下個步驟中提供表格以檢查其結構。
...
通過本教程，您已探索了資料庫，找到了要裝入的表的路徑，並獲 [!DNL Platform]取了有關其結構的資訊。 您可以在下一個教學課程中使用此資 [訊，從資料庫收集資料並匯入平台](../collect/database-nosql.md)。