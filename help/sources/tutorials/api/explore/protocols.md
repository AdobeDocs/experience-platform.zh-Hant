...
本教學課程使用 [!DNL Flow Service] 探索通訊協定應用程式的API。
...
以下各節提供您需要了解的其他資訊，以便使用成功連接到協定應用程式 [!DNL Flow Service] API。

### 獲取基本連接

為了探索您的協定系統，請使用 [!DNL Platform] API，您必須擁有有效的基本連線ID。 如果您尚未建立要使用的協定系統的基本連接，則可以通過以下教程建立一個基本連接：
...
使用協定應用程式的連接ID，可以通過執行GET請求來瀏覽資料表。 使用以下調用查找要檢查或將其嵌入的表的路徑 [!DNL Platform].
...
成功的響應從協定應用程式返回表陣列。 找到要放進的桌子 [!DNL Platform] 並注意 `path` 屬性，因為您必須在下一個步驟中提供屬性，以檢查其結構。
...
依照本教程，您已探索了您的協定應用程式，找到了要內嵌到的表的路徑 [!DNL Platform]，並取得其結構的相關資訊。 您可以在下一個教學課程中使用此資訊，以 [從通訊協定應用程式收集資料並匯入Platform](../collect/protocols.md).
