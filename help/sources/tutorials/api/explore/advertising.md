...
使用廣告系統的基本連線，您可以執行GET請求來探索資料表。 使用以下調用查找要檢查或裝入平台的表的路徑。
...
成功的回應是從您的廣告系統傳送的表格陣列。 尋找您要帶入Platform的表格，並記下其屬性，因為您必須在下一 `path` 步驟中提供表格以檢查其結構。
...
在本教學課程中，您已探索廣告系統，找到您要匯入Platform的表格路徑，並取得其結構的相關資訊。 您可以在下一個教學課程中使用此資 [訊，從廣告系統收集資料並匯入Platform](../collect/advertising.md)。
