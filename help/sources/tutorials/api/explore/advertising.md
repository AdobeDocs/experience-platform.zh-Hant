...
本教學課程要求您必須與想要從中擷取資料的第三方廣告應用程式建立有效的連線。 有效連接涉及應用程式的連接規範ID和連接ID。 有關建立廣告連線和擷取這些值的詳細資訊，請參閱將廣告 [來源連接至Platform](../../api/create/advertising/ads.md) 教學課程。
...
使用廣告系統的基本連線，您可以執行GET請求來探索資料表。 使用以下調用查找要檢查或裝入的表的路徑 [!DNL Platform]。
...
成功的回應是從您的廣告系統傳送的表格陣列。 尋找您要放入的表格， [!DNL Platform] 並記下其屬性，因 `path` 為您必須在下個步驟中提供表格以檢查其結構。
...
在本教學課程中，您已探索廣告系統、找到您要匯入的表格路徑，並取得 [!DNL Platform]其結構的相關資訊。 您可以在下一個教學課程中使用此資 [訊，從廣告系統收集資料並匯入Platform](../collect/advertising.md)。
