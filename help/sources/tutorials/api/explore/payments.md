...
本教學課程要求您必須與想要從中擷取資料的第三方付款應用程式建立有效的連線。 有效連接涉及應用程式的連接規範ID和連接ID。 有關建立付款連接和檢索這些值的詳細資訊，請參閱將付 [款源連接到平台教程](../../api/create/payments/paypal.md) 。
...
使用付款系統的連線ID，您可以執行GET請求來探索資料表格。 使用以下調用查找要檢查或裝入平台的表的路徑。
...
成功的回應會從付款系統傳回一系清單格。 尋找您要帶入Platform的表格，並記下其屬性，因為您必須在下一 `path` 步驟中提供表格以檢查其結構。
...
在本教學課程中，您已探索了您的付款系統，找到了要收錄到Platform中的表格路徑，並取得了有關其結構的資訊。 您可以在下一個教學課程中使用此資 [訊，從付款系統收集資料並匯入平台](../collect/payments.md)。