...
以下各節提供您必須知道的其他資訊，以便使用Flow Service API成功連線至CS系統。

### 獲得基本連接

若要使用Platform API來探索您的CS系統，您必須擁有有效的基本連線ID。 如果您尚未建立想要使用之CS系統的基本連線，則可透過下列教學課程來建立一個連線：
...
使用CS系統的基本連線，您可以執行GET請求來探索資料表。 使用以下調用查找要檢查或裝入平台的表的路徑。
...
成功的回應會從CS系統傳回一組表格。 尋找您要帶入Platform的表格，並記下其屬性，因為您必須在下一 `path` 步驟中提供表格以檢查其結構。
...
若要從CS系統檢查表的結構，請在指定表的路徑作為查詢參數時執行GET請求。
...
在本教學課程中，您已探索過CS系統，找到您要加入Platform的表格路徑，並取得其結構的相關資訊。 您可在下一個教學課程中使用此 [資訊，從CS系統收集資料並匯入Platform](../collect/customer-success.md)。