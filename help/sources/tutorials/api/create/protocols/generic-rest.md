...
| `accessToken` | 用來存取應用程式的主要驗證認證。 存取權杖代表應用程式的授權，可存取使用者資料的特定方面。 此值為必填，不能使用略過 `requestParameterOverride`. |
...
POST若要使用OAuth 2重新整理程式碼建立基本連線ID，請向 `/connections` 端點，並提供您的OAuth 2認證。
...
| `auth.params.host` | 用來連線至您的裝置的根URL [!DNL Generic REST API] 來源。 |
...
### 建立 [!DNL Generic REST API] 使用基本驗證的基底連線

若要建立 [!DNL Generic REST API] 使用基本驗證的基礎連線，向發出POST請求 `/connections` 端點 [!DNL Flow Service] API，同時提供您的基本驗證認證。
...
| `auth.params.host` | 用來連線至您的裝置的根URL [!DNL Generic REST API] 來源。 |
| `auth.params.username` | 與您的對應之使用者名稱 [!DNL Generic REST API] 來源。 這是基本驗證的必要專案。 |
| `auth.params.password` | 與您的對應之密碼 [!DNL Generic REST API] 來源。 這是基本驗證的必要專案。 |
