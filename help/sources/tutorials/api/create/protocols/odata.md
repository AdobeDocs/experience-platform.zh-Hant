...
title: 使用Flow Service API建立通用OData連接器
topic: overview
description: 本教學課程使用Flow Service API來引導您完成將Generic OData連接至Experience Platform的步驟。
...
# 使用 [!DNL Generic OData] API建立連 [!DNL Flow Service] 接器
...
本教學課程使 [!DNL Flow Service] 用API來引導您完成連線至的 [!DNL Generic OData] 步驟 [!DNL Experience Platform]。
...
要連接 [!DNL Flow Service] OData，必須為以下連接屬性提供值：

| 憑證 | 說明 |
| ---------- | ----------- |
| url | 服務的根URL [!DNL OData] 。 |
| connectionSpec.id | 建立連線所需的唯一識別碼。 的連接規範ID [!DNL OData] 為： `8e6b41a8-d998-4545-ad7d-c6a9fff406c3` |
...
連接指定源，並包含該源的憑據。 每個帳戶只需要一個連 [!DNL OData] 接，因為它可用於建立多個源連接器以導入不同的資料。
...
要建立連接，必 [!DNL OData] 須在POST請求中提供其唯一連接規範ID。 的連接規範ID [!DNL OData] 為 `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`。
...
| `connectionSpec.id` | 連 [!DNL OData] 接規範ID: `8e6b41a8-d998-4545-ad7d-c6a9fff406c3`. |
...
在本教學課程中，您已使 [!DNL OData] 用 [!DNL Flow Service] API建立連線，並取得連線的唯一ID值。 在下一個教學課程中，您可以使用此ID，學習如何使 [用Flow Service API來探索通訊協定應用程式](../../explore/protocols.md)。