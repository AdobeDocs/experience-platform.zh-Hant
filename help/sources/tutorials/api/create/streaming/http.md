...
若要建立未驗證的基礎連線，請向以下連線發出POST請求： `/connections` 端點，同時提供連線的名稱、資料型別和HTTP API連線規格ID。 此ID為 `bc7b00d6-623a-4dfc-9fdb-f1240aeadaeb`.
...
成功的回應會傳回HTTP狀態201以及新建立連線的詳細資料，包括其唯一識別碼(`id`)。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `auth.params.sourceId` | 建立已驗證的基礎連線時，可使用的其他識別碼。 此引數為選用引數，將使用與相同的值 `name` 屬性（若未提供）。 |
| `auth.params.authenticationRequired` | 此引數會指定串流連線是否需要驗證。 如果 `authenticationRequired` 設為 `true` 則必須為串流連線提供驗證。 如果 `authenticationRequired` 設為 `false` 則不需要驗證。 |
...
成功的回應會傳回HTTP狀態200，其中包含請求連線的詳細資訊。 串流端點URL會使用連線自動建立，並可使用擷取 `inletUrl` 值。
...
目標連線代表與擷取資料著陸目的地之間的連線。 若要建立Target連線，請向發出POST要求 `/targetConnections` 同時為您的目標資料集和目標XDM結構描述提供ID。 在此步驟中，您也必須提供資料湖連線規格ID。 此ID為 `c604ff05-7f1a-43c0-8e18-33bf874cb11c`.
...
成功的回應會傳回HTTP狀態201，連同新建立目標連線的詳細資料，包括其唯一識別碼(`id`)。
...
使用轉換建立資料流時， `name` 無法變更引數。 此值必須一律設為 `Mapping`.
...
成功的回應會傳回HTTP狀態201，其中包含新建立資料流的詳細資訊，包括其唯一識別碼(`id`)。
...
如果串流連線已啟用驗證，使用者端必須新增 `Authorization` 標頭至他們的請求。

如果 `Authorization` 標頭不存在，或傳送了無效/過期的存取Token，將會傳回HTTP 401未授權的回應，並具有類似的回應，如下所示：
