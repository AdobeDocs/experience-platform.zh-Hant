...
## 使用[!DNL Flow Service] API連線[!DNL Customer.io]至Experience Platform {#connect-platform-to-flow-api}

以下概述建立來源連線和資料流，將您的[!DNL Customer.io]事件資料匯入Experience Platform所需執行的步驟。
...
將資料從[!DNL Customer.io]引進Experience Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
