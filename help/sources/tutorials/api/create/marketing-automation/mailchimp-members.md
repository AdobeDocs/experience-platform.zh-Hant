...
下列教學課程將逐步引導您完成建立來源連線和資料流的步驟，以使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將[!DNL Mailchimp Members]資料帶入Experience Platform。
...
擷取[!DNL Mailchimp]驗證認證後，您現在可以開始建立資料流的程式，將[!DNL Mailchimp Members]資料帶入Experience Platform。 建立資料流的第一步是建立基礎連線。
