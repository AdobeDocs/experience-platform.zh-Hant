---
title: 使用流服務API為Chatlio建立源連接和資料流
description: 了解如何使用Flow Service API將Adobe Experience Platform連線至Chatlio。
badge: "Beta"
...
# 為建立源連接和資料流 [!DNL Chatlio] 使用流量服務API

>[!NOTE]
>
>此 [!DNL Chatlio] 來源為測試版。 請閱讀 [來源概觀](../../../../home.md#terms-and-conditions) 以取得使用測試版標籤來源的詳細資訊。

以下教程將逐步引導您完成建立源連接和資料流的步驟 [[!DNL Chatlio]](https://chatlio.com/) 事件資料傳送至Adobe Experience Platform，使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connect [!DNL Chatlio] 到使用 [!DNL Flow Service] API {#connect-platform-to-flow-api}

以下概述建立源連接和資料流以帶來您 [!DNL Chatlio] 事件資料Experience Platform。
...
| `data.format` | 格式 [!DNL Chatlio] 您要擷取的資料。 目前，唯一支援的資料格式是 `json`. |
...
| `data.format` | 格式 [!DNL Chatlio] 您要擷取的資料。 |
...
將資料從 [!DNL Chatlio] 到平台是建立資料流。 您現在已準備下列必要值：
...
成功的響應返回有關資料流的資訊，包括終結點URL，標籤為 `inletUrl`. 請參閱 [設定Webhook](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) 頁面，以取得必要值。
