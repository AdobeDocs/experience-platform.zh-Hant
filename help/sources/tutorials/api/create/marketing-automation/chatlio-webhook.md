...
## 使用[!DNL Flow Service] API連線[!DNL Chatlio]至平台 {#connect-platform-to-flow-api}

以下概述建立來源連線和資料流以Experience Platform[!DNL Chatlio]事件資料所需執行的步驟。
...
將資料從[!DNL Chatlio]引進Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
