---
title: 使用流量服務API為Chatlio建立Source連線和資料流
...
# 使用Flow Service API為[!DNL Chatlio]建立來源連線和資料流

>[!NOTE]
>
>[!DNL Chatlio]來源是測試版。 如需使用Beta版標籤來源的相關資訊，請參閱[來源概觀](../../../../home.md#terms-and-conditions)。

下列教學課程會逐步引導您完成建立來源連線和資料流的步驟，以使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將[[!DNL Chatlio]](https://chatlio.com/)事件資料帶入Adobe Experience Platform。
...
## 使用[!DNL Flow Service] API連線[!DNL Chatlio]至Experience Platform {#connect-platform-to-flow-api}

以下概述建立來源連線和資料流，將您的[!DNL Chatlio]事件資料匯入Experience Platform所需執行的步驟。
...
| `data.format` | 您要擷取的[!DNL Chatlio]資料格式。 目前唯一支援的資料格式為`json`。 |
...
| `data.format` | 您要擷取的[!DNL Chatlio]資料格式。 |
...
將資料從[!DNL Chatlio]引進Experience Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
...
成功的回應會傳回資料流上的資訊，包括標示為`inletUrl`的端點URL。 請參閱[設定Webhook](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url)頁面以取得所需的值。
