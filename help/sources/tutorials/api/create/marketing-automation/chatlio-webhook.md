...
description: 瞭解如何使用流服務API將Adobe Experience Platform連接到Chatlio。
badge: β
...
# 建立源連接和資料流 [!DNL Chatlio] 使用流服務API

>[!NOTE]
>
>的 [!DNL Chatlio] 源為beta。 請閱讀 [源概述](../../../../home.md#terms-and-conditions) 的子菜單。

以下教程將指導您完成建立源連接和資料流的步驟，以便 [[!DNL Chatlio]](https://chatlio.com/) 事件資料到Adobe Experience Platform [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)。
...
## 連接 [!DNL Chatlio] 到使用 [!DNL Flow Service] API {#connect-platform-to-flow-api}

下面概述了建立源連接和資料流以使用 [!DNL Chatlio] 事件資料到Experience Platform。
...
| `data.format` | 格式 [!DNL Chatlio] 要攝取的資料。 目前，唯一支援的資料格式是 `json`。 |
...
| `data.format` | 格式 [!DNL Chatlio] 要攝取的資料。 |
...
向從 [!DNL Chatlio] 平台是建立資料流。 現在，您準備了以下必需值：
...
成功的響應將返回有關資料流的資訊，包括標籤為的終結點URL `inletUrl`。 請參閱 [設定網頁掛接](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) 的子菜單。
