...
閱讀下列教學課程，瞭解如何使用[!DNL Oracle NetSuite Activities Entities]API[[!DNL Flow Service] 將連絡人和客戶資料從您的](https://www.adobe.io/experience-platform-apis/references/flow-service/)帳戶帶入Adobe Experience Platform。
...
下列章節提供您需瞭解的其他資訊，才能使用[!DNL Oracle NetSuite Entities] API成功連線到[!DNL Flow Service]。
...
## 使用[!DNL Oracle NetSuite Entities] API連線[!DNL Flow Service]至Experience Platform
...
若要建立基底連線ID，請在提供您的`/connections`驗證認證作為要求內文的一部分時，對[!DNL Oracle NetSuite Entities]端點提出POST要求。
