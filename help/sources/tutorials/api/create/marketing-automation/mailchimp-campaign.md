...
擷取[!DNL Mailchimp]驗證認證後，您現在可以開始建立資料流的程式，將[!DNL Mailchimp Campaign]資料帶入Experience Platform。 建立資料流的第一步是建立基礎連線。
...
| `auth.params.authorizationTestUrl` | （選擇性）建立基本連線時，會使用授權測試URL來驗證認證。 如果未提供，則會在來源連線建立步驟期間自動檢查認證。 |
...
使用您在上一步中產生的基本連線ID，您可以執行GET要求來探索檔案和目錄。 執行GET請求以探索來源的檔案結構和內容時，您必須包含下表列出的查詢引數：
...
>[!TIP]
>
>若要擷取`{SOURCE_PARAMS}`接受的格式型別，您必須以base64編碼整個`campaignId`字串。 例如，以base64編碼的`{"campaignId": "c66a200cda"}`等於`eyJjYW1wYWlnbklkIjoiYzY2YTIwMGNkYSJ9`。
...
| `params.campaignId` | [!DNL Mailchimp]行銷活動識別碼會識別特定的[!DNL Mailchimp]行銷活動，然後可讓您傳送電子郵件給清單/對象。 |
...
| `data.format` | 您要帶入Experience Platform的[!DNL Mailchimp]資料格式。 |
...
將[!DNL Mailchimp]資料引進Experience Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
...
若要排程內嵌，您必須先將開始時間值設為以秒為單位的epoch時間。 然後，您必須將頻率值設定為下列五個選項之一： `once`、`minute`、`hour`、`day`或`week`。 間隔值會指定兩個連續擷取之間的期間，而建立一次性擷取(`once`)不需要設定間隔。 對於所有其他頻率，間隔值必須設定為等於或大於`15`。
