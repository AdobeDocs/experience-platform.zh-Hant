...
以下教程將逐步引導您建立源連接和資料流，以使用[[!DNL Flow Service]  API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將[!DNL MailChimp Campaign]資料帶入Platform。
...
檢索到[!DNL MailChimp]身份驗證憑據後，您現在可以開始建立資料流以將[!DNL MailChimp Campaign]資料帶到Platform的過程。 建立資料流的第一步是建立基本連接。
...
| `auth.params.username` | 與您的[!DNL MailChimp]帳戶對應的使用者名稱。 這是基本驗證的必要條件。 |
...
| `{SOURCE_PARAMS}` | `campaign_id`的base64編碼字串。 |

>[!TIP]
>
>若要擷取`{SOURCE_PARAMS}`接受的格式類型，您必須以base64對整個`campaignId`字串進行編碼。 例如，以base64編碼的`{"campaignId": "c66a200cda"}`等於`eyJjYW1wYWlnbklkIjoiYzY2YTIwMGNkYSJ9`。
...
成功的響應返回新目標連接的唯一標識符(`id`)。 This ID is required in later steps.
...
>[!IMPORTANT]
>
>[!DNL MailChimp Campaign]目前不支援資料準備函式。
...
To schedule an ingestion, you must first set the start time value to epoch time in seconds. 然後，您必須將頻率值設定為以下五個選項之一：`once`、`minute`、`hour`、`day`或`week`。 間隔值指定兩個連續擷取和建立一次性擷取(`once`)之間的期間，不需要設定間隔。 對於所有其他頻率，間隔值必須設定為等於或大於`15`。
...
A successful response returns your flow ID and an updated etag. 您可以在提供流程ID時，向[!DNL Flow Service] API提出GET要求，以驗證更新。
...
A successful response returns HTTP status 204 (No Content) and a blank body. 您可以嘗試對資料流執行查閱(GET)請求以確認刪除。 API會傳回HTTP 404（找不到）錯誤，指出資料流已刪除。
