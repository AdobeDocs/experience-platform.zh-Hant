...
description: 了解如何使用Flow Service API將Adobe Experience Platform連線至SugarCRM事件。
...
# （測試版）建立源連接和資料流 [!DNL SugarCRM Events] 使用流量服務API

>[!NOTE]
>
>此 [!DNL SugarCRM Events] 來源為測試版。 請參閱 [來源概觀](../../../../home.md#terms-and-conditions) 以取得使用測試版標籤來源的詳細資訊。

以下教學課程會逐步引導您完成建立 [!DNL SugarCRM Events] 源連接和建立資料流，以便 [[!DNL SugarCRM]](https://www.sugarcrm.com/) 事件資料傳送至Adobe Experience Platform，使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connect [!DNL SugarCRM Events] 到使用 [!DNL Flow Service] API

以下概述驗證您的 [!DNL SugarCRM] 源、建立源連接和建立資料流，以將事件資料Experience Platform。

### 建立基本連接 {#base-connection}

基本連接在源和平台之間保留資訊，包括源的驗證憑據、連接的當前狀態和唯一基本連接ID。 基本連線ID可讓您從來源探索和導覽檔案，並識別您要擷取的特定項目，包括其資料類型和格式的相關資訊。

若要建立基本連線ID，請向 `/connections` 端點提供 [!DNL SugarCRM Events] 請求內文的驗證憑證。
...
| `{SOURCE_PARAMS}` | 定義要帶入Platform的源檔案的參數。 要檢索接受的格式類型，請執行以下操作 `{SOURCE_PARAMS}`，則必須對base64中的整個字串進行編碼。 <br> [!DNL SugarCRM Events] 不需要任何裝載。 的值 `{SOURCE_PARAMS}` 傳遞為 `{}`，以base64編碼相當於 `e30=` 如下所示。 |
...
成功的回應會傳回查詢的檔案結構。 *有些記錄已移除，以提供更佳的簡報。*
...
以下請求將建立源連接 [!DNL SugarCRM Events]:
...
| `baseConnectionId` | 基本連接ID為 [!DNL SugarCRM Events]. 此ID是在先前的步驟中產生。 |
| `connectionSpec.id` | 與源對應的連接規範ID。 |
| `data.format` | 格式 [!DNL SugarCRM Events] 您要擷取的資料。 目前，唯一支援的資料格式是 `json`. |
...
| `data.format` | 格式 [!DNL SugarCRM Events] 您要擷取的資料。 |
...
將資料從 [!DNL SugarCRM Events] 到平台是建立資料流。 您現在已準備下列必要值：
