...
為了建立HubSpot連接，Flow Service中必須有一組HubSpot連接規範。 將Platform連接到HubSpot的第一步是檢索這些規範。

**API格式**

每個可用源都有其唯一的連接規範集，用於描述連接器屬性（如驗證要求）。 向端點發送GET請求 `/connectionSpecs` 將返回所有可用源的連接規範。 您也可以包含查詢， `property=name=="hubspot"` 以取得HubSpot的專屬資訊。
...
成功的響應返回HubSpot的連接規範，包括其唯一標識符(`id`)。 在下個步驟中，此ID是建立API連線的必要項。
...
API的連線會指定來源，並包含該來源的認證。 每個HubSpot帳戶只需要一個API連線，因為它可用於建立多個來源連接器，以匯入不同的資料。
...
在本教程中，您已使用Flow Service API建立HubSpot連線，並取得連線的唯一ID值。 在下一個教學課程中，您可以使用此連線ID，學習如 [何使用Flow Service API來探索CRM系統](../../explore/crm.md)。