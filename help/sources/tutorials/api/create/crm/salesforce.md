...
API的連線會指定來源，並包含該來源的認證。 每個帳戶只需要一個API連線， [!DNL Salesforce] 因為它可用於建立多個來源連接器，以匯入不同的資料。
...
在本教學課程中，您已使用API建立您 [!DNL Salesforce] 帳戶的連線，並且已取得唯一ID作為回應內文的一部分。 在下一個教學課程中，您可以使用此連線ID，學習如 [何使用Flow Service API來探索CRM系統](../../explore/crm.md)。