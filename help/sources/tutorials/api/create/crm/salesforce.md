...
本教學課程會逐步引導您使用[[!DNL Flow Service] API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml)建立[!DNL Salesforce]基本連線的步驟。
...
以下各節提供您需要了解的其他資訊，以便使用[!DNL Flow Service] API成功將[!DNL Platform]連接到[!DNL Salesforce]帳戶。
...
| 憑據 | 說明 |
| ---------- | ----------- |
| `environmentUrl` | [!DNL Salesforce]源實例的URL。 |
| `username` | [!DNL Salesforce]使用者帳戶的使用者名稱。 |
| `password` | [!DNL Salesforce]用戶帳戶的密碼。 |
| `securityToken` | [!DNL Salesforce]使用者帳戶的安全權杖。 |
| `connectionSpec.id` | 連接規範返回源的連接器屬性，包括與建立基連接和源連接相關的驗證規範。 [!DNL AdWords]的連接規範ID為：`cfc0fee1-7dc0-40ef-b73e-d8b134c436f5`。 |
...
若要建立基本連線ID，請在提供[!DNL Salesforce]驗證憑證作為請求參數的一部分時，向`/connections`端點提出POST請求。
...
| `auth.params.password` | 與[!DNL Salesforce]帳戶相關聯的密碼。 |
| `auth.params.securityToken` | 與您的[!DNL Salesforce]帳戶相關聯的安全令牌。 |
| `connectionSpec.id` | [!DNL Salesforce]連接規範ID:`cfc0fee1-7dc0-40ef-b73e-d8b134c436f5`。 |
...
依照本教學課程，您已使用[!DNL Flow Service] API建立[!DNL Salesforce]連線，並取得連線的唯一ID值。 在您了解如何使用流量服務API](../../explore/crm.md)探索CRM系統時，您可以在下一個教學課程中使用此ID。[
