...
## 將[!DNL Salesforce]連線至[!DNL Azure]上的Experience Platform {#azure}

請閱讀下列步驟，以瞭解如何將[!DNL Salesforce]來源連線至[!DNL Azure]上的Experience Platform。
...
若要建立基底連線並將您的[!DNL Salesforce]帳戶連線到[!DNL Azure]上的Experience Platform，請對`/connections`端點發出POST要求，並在要求內文中提供您的[!DNL Salesforce]驗證認證。
...
請閱讀下列步驟，以瞭解如何將[!DNL Salesforce]來源連線至AWS上的Experience Platform。

### 先決條件

如需如何設定您的[!DNL Salesforce]帳戶以便在AWS上連線到Experience Platform的詳細資訊，請閱讀[[!DNL Salesforce] 總覽](../../../../connectors/crm/salesforce.md#aws)。

### 在AWS上的Experience Platform上為[!DNL Salesforce]建立基礎連線

若要建立基礎連線並將您的[!DNL Salesforce]帳戶連線至AWS上的Experience Platform，請對`/connections`端點發出POST要求，並提供您認證的適當值。
...
下列請求會在AWS上的Experience Platform中建立[!DNL Salesforce]來源的基礎連線。
...
若要驗證您的連線狀態，請向`/connections`端點發出GET請求，並提供在建立步驟中產生的基本連線ID。
