---
title: 使用Flow Service API為Shopify資料建立串流Source連線和資料流
description: 瞭解如何使用Flow Service API為Shopify資料建立串流來源連線和資料流。
badge: Beta
...
# 使用Flow Service API為[!DNL Shopify]資料建立串流來源連線和資料流

>[!NOTE]
>
>[!DNL Shopify]串流來源是測試版。 如需使用Beta版標籤來源的相關資訊，請參閱[來源概觀](../../../../home.md#terms-and-conditions)。

下列教學課程提供如何使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)建立串流來源連線和資料流，將資料從[[!DNL Shopify]](https://www.shopify.com/)串流到Adobe Experience Platform的步驟。
...
## 使用流程服務API將[!DNL Shopify]資料串流到Experience Platform

以下概述建立來源連線和資料流以將您的[!DNL Shopify]資料串流到Experience Platform所需的步驟。
...
| `data.format` | 您要擷取的[!DNL Shopify]資料格式。 目前唯一支援的資料格式為`json`。 |
...
您現在擁有唯一識別碼、目標結構描述、目標資料集，以及與Data Lake的連線規格ID。 使用這些識別碼，您可以使用[!DNL Flow Service] API建立目標連線，以指定將包含傳入來源資料的資料集。
...
| `data.format` | 您要帶入Experience Platform的[!DNL Shopify]資料格式。 |
...
將資料從[!DNL Shopify]引進Experience Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
...
| 屬性 | 說明 |
| --- | --- |
| `name` | 資料流的名稱。 確保資料流的名稱為描述性，因為您可以使用此名稱來查詢資料流上的資訊。 |
...
提供資料流的ID時，藉由向[!DNL Flow Service] API的`/flows`端點發出PATCH要求，更新資料流的詳細資訊，例如其名稱和說明，以及其執行排程和相關聯的對應集。 發出PATCH請求時，您必須在`If-Match`標頭中提供資料流的唯一`etag`。 如需完整的API範例，請閱讀[使用API更新來源資料流的指南](https://experienceleague.adobe.com/docs/experience-platform/sources/api-tutorials/update-dataflows.html?lang=zh-Hant)
