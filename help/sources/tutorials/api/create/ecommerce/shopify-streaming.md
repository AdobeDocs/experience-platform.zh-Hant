...
## 使用流程服務API將[!DNL Shopify]資料串流到Platform

以下概述建立來源連線和資料流以將您的[!DNL Shopify]資料串流到Platform所需的步驟。
...
| `data.format` | 您要帶到Platform的[!DNL Shopify]資料格式。 |
...
將資料從[!DNL Shopify]引進Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
