...
# 使用Flow Service API建立Amazon S3連接器

Flow Service用於收集和集中Adobe Experience Platform內不同來源的客戶資料。 該服務提供用戶介面和REST風格的API，所有支援的源都可從中連接。

本教學課程使用Flow Service API來引導您完成將Experience Platform連接至Amazon S3（以下稱為「S3」）儲存空間的步驟。
...
以下各節提供您需要瞭解的其他資訊，以便使用Flow Service API成功連線至S3儲存空間。

### 收集必要的認證

為了讓流服務與您的S3儲存連接，您必須為以下連接屬性提供值：
...
在將平台連接到S3儲存器之前，必須驗證S3的連接規範是否存在。 如果連接規範不存在，則無法建立連接。

每個可用源都有其唯一的連接規範集，用於描述連接器屬性（如驗證要求）。 您可以執行GET請求並使用查詢參數來查找S3的連接規範。

**API格式**

傳送不含查詢參數的GET請求時，會傳回所有可用來源的連線規格。 您可以包含查詢 `property=name=="amazon-s3"` 以取得S3的特定資訊。
...
成功的回應會傳回S3的連線規格，包括其唯一識別碼(`id`)。 在下個步驟中需要此ID才能建立基本連線。
...
基本連接指定源，並包含該源的憑據。 每個S3帳戶只需要一個基本連線，因為它可用來建立多個來源連接器，以匯入不同的資料。
...
| `connectionSpec.id` | 在上一步 `id` 中檢索的S3儲存的連接規範。 |
