...
topic: overview
type: Tutorial
description: 本教學課程使用Flow Service API來引導您完成將Apache Hadoop分佈式檔案系統（以下稱為「HDFS」）連接至Experience Platform的步驟。
...
# 使用[!DNL Flow Service] API建立[!DNL Apache] HDFS連接器
...
按照本教程，您已使用[!DNL Flow Service] API建立了HDFS連接，並獲取了該連接的唯一ID值。 您可在下一個教學課程中使用此ID，同時學習如何使用Flow Service API[來探索協力廠商雲端儲存空間。](../../explore/cloud-storage.md)
