...
| 認證 | 說明 |
| ---------- | ----------- |
| `accessKeyId` | 用於向Platform驗證您的[!DNL Google Cloud Storage]帳戶的61個字元英數字串。 |
| `secretAccessKey` | 用於向Platform驗證您的[!DNL Google Cloud Storage]帳戶的40個字元Base-64編碼字串。 |
...
若要建立基底連線ID，請在提供[!DNL Google Cloud Storage]驗證認證作為要求引數的一部分時，向`/connections`端點提出POST要求。
