...
# 使用[!DNL Flow Service] API為[!DNL Zendesk]建立資料流

下列教學課程會逐步引導您完成建立來源連線和資料流的步驟，以使用[[!DNL Flow Service] API](<https://www.adobe.io/experience-platform-apis/references/flow-service/>)將[!DNL Zendesk]資料帶入Platform。
...
下列章節提供您需瞭解的其他資訊，才能使用[!DNL Flow Service] API成功連線到[!DNL Zendesk]。
...
## 使用[!DNL Flow Service] API連線[!DNL Zendesk]至平台

下列教學課程會逐步引導您完成建立[!DNL Zendesk]來源連線與建立資料流的步驟，以使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將[!DNL Zendesk]資料帶入Platform。

### 建立基礎連線 {#base-connection}

基礎連線會保留您的來源和平台之間的資訊，包括來源的驗證認證、連線的目前狀態，以及您唯一的基本連線ID。 基礎連線ID可讓您從來源內部探索及導覽檔案，並識別您要擷取的特定專案，包括其資料型別和格式的資訊。

若要建立基底連線ID，請在提供您的[!DNL Zendesk]驗證認證作為要求內文的一部分時，向`/connections`端點提出POST要求。
...
| `{SOURCE_PARAMS}` | 定義您要帶到Platform之來源檔案的引數。 若要擷取`{SOURCE_PARAMS}`接受的格式型別，您必須以base64編碼整個`parameter`字串。 在下列範例中，以base64編碼的`"{}"`等於`e30`。 |
...
成功的回應會傳回查詢檔案的結構。 在以下範例中，``data[]``裝載中只顯示單一記錄，但可能有多個記錄。
...
| `baseConnectionId` | [!DNL Zendesk]的基本連線識別碼。 此ID是在先前步驟中產生的。 |
| `connectionSpec.id` | 與您的來源對應的連線規格ID。 |
| `data.format` | 您要擷取的[!DNL Zendesk]資料格式。 目前唯一支援的資料格式為`json`。 |
...
| `data.format` | 您要帶到Platform的[!DNL Zendesk]資料格式。 |
