...
# 建立資料流，用於 [!DNL Zendesk] 使用 [!DNL Flow Service] API

以下教學課程將逐步引導您完成建立來源連線和資料流的步驟，以便 [!DNL Zendesk] 使用將資料傳送至Platform [[!DNL Flow Service] API](<https://www.adobe.io/experience-platform-apis/references/flow-service/>).
...
以下小節提供成功連線所需的其他資訊 [!DNL Zendesk] 使用 [!DNL Flow Service] API。
...
## 連線 [!DNL Zendesk] 至平台，使用 [!DNL Flow Service] API
...
若要建立基本連線ID，請向以下連線ID發出POST請求： `/connections` 端點，同時提供 [!DNL Zendesk] 要求內文中的驗證認證。
...
| `{SOURCE_PARAMS}` | 定義您要帶到Platform之來源檔案的引數。 擷取接受的格式型別 `{SOURCE_PARAMS}`，您必須將整個 `parameter` 字串（以base64為單位）。 在以下範例中， `"{}"` 以base64編碼等於 `e30`. |
...
| `baseConnectionId` | 的基礎連線ID： [!DNL Zendesk]. 此ID是在先前步驟中產生的。 |
