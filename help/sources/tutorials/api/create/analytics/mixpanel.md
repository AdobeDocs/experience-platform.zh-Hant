...
若要建立基底連線ID，請在提供您的[!DNL Mixpanel]驗證認證作為要求內文的一部分時，向`/connections`端點提出POST要求。
...
| `{SOURCE_PARAMS}` | 定義您要帶到Platform之來源檔案的引數。 若要擷取`{SOURCE_PARAMS}`接受的格式型別，您必須以base64編碼整個`{"projectId":"2671127","timezone":"Pacific Standard Time"}`字串。 **附註**：在下列範例中，以base64編碼的`"{"projectId":"2671127","timezone":"Pacific Standard Time"}"`等於`eyJwcm9qZWN0SWQiOiIyNjcxMTI3IiwidGltZXpvbmUiOiJQYWNpZmljIFN0YW5kYXJkIFRpbWUifQ==`。 |
...
| `data.format` | 您要帶到Platform的[!DNL Mixpanel]資料格式。 |
...
將資料從[!DNL Mixpanel]引進Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
