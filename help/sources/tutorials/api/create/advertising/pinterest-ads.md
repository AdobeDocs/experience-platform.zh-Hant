...
## 使用[!DNL Flow Service] API連線[!DNL Pinterest Ads]至平台 {#connect-platform-to-flow-api}

以下概述連線[!DNL Pinterest Ads]至Experience Platform要採取的步驟。

### 建立基礎連線 {#base-connection}

基礎連線會保留您的來源和平台之間的資訊，包括來源的驗證認證、連線的目前狀態，以及您唯一的基本連線ID。 基礎連線ID可讓您從來源內部探索及導覽檔案，並識別您要擷取的特定專案，包括其資料型別和格式的資訊。

若要建立基底連線ID，請在提供您的[!DNL Pinterest Ads]驗證認證作為要求內文的一部分時，向`/connections`端點提出POST要求。
...
| `{SOURCE_PARAMS}` | 定義您要帶到Platform之來源檔案的引數。 若要擷取`{SOURCE_PARAMS}`接受的格式型別，您必須以base64編碼整個`{"ad_account_id":"{PINTEREST_AD_ACCOUNT_ID}","object_ids":"{COMMA_SEPERATED_OBJECT_IDS}","object_type":"{OBJECT_TYPE}}"}`字串。 |
...
您可以向[!DNL Flow Service] API發出POST要求，以建立來源連線。 來源連線由基礎連線ID、來源資料檔案的路徑以及連線規格ID組成。
...
| `data.format` | 您要帶到Platform的[!DNL Pinterest Ads]資料格式。 |
...
將資料從[!DNL Pinterest Ads]引進Platform的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
