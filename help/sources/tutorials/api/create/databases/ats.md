...
# 使用 [!DNL Azure Table Storage] API建立連 [!DNL Flow Service] 接器
...
本教學課程使 [!DNL Flow Service] 用API來引導您完成連接 [!DNL Azure Table Storage] （以下稱為「ATS」）至的步驟 [!DNL Experience Platform]。
...
以下各節提供您必須知道的其他資訊，才能使用 [!DNL Flow Service] API成功連線至ATS。

### 收集必要的認證

若要連 [!DNL Flow Service] 接ATS，您必須提供下列連線屬性的值：
...
在本教學課程中，您已使用 [!DNL Flow Service] API建立ATS連線，並取得連線的唯一ID值。 在下一個教學課程中，您可以使用此ID來學習如何使 [用Flow Service API來探索資料庫](../../explore/database-nosql.md)。
