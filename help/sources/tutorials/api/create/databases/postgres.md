...
topic: overview
description: 本教學課程使用Flow Service API來引導您完成將Experience Platform連接至PostgreSQL（以下稱為「PSQL」）的步驟。
...
# 使用 [!DNL PostgreSQL] API建立連 [!DNL Flow Service] 接器
...
本教學課程使 [!DNL Flow Service] 用API來引導您完成連線至的 [!DNL Experience Platform][!DNL PostgreSQL] 步驟（以下稱為「PSQL」）。
...
以下各節提供您需要知道的其他資訊，以便使用 [!DNL Flow Service] API成功連線至PSQL。

### 收集必要的認證

要與PSQL [!DNL Flow Service] 連接，必須提供以下連接屬性：
...
有關獲取連接字串的詳細資訊，請參 [閱此PSQL文檔](https://www.postgresql.org/docs/9.2/app-psql.html)。
...
在本教程中，您使用 [!DNL Flow Service] API建立了PSQL連接，並獲取了該連接的唯一ID值。 在下一個教程中，您可以使用此連接ID來學習如何使 [用流服務API來探索資料庫或NoSQL系統](../../explore/database-nosql.md)。
