...
# 使用Flow Service API建立Azure資料總管連接器
...
本教學課程使用Flow Service API來引導您完成將Azure資料總管（以下稱為「資料總管」）連接至Experience Platform的步驟。
...
以下各節提供您需要知道的其他資訊，以便使用Flow Service API成功連線至資料總管。

### 收集必要的認證

為了讓流服務與資料瀏覽器連接，您必須為以下連接屬性提供值：
...
| `connectionSpec.id` | 建立連線所需的唯一識別碼。 「資料瀏覽器」的連接規範ID為 `0479cc14-7651-4354-b233-7480606c2ac3`。 |
...
連接指定源，並包含該源的憑據。 每個資料總管帳戶只需要一個連接器，因為它可用於建立多個來源連接器以匯入不同的資料。
...
為了建立資料總管連線，其唯一的連線規格ID必須作為POST要求的一部分提供。 「資料瀏覽器」的連接規範ID為 `0479cc14-7651-4354-b233-7480606c2ac3`。
...
| 參數 | 說明 |
| --------- | ----------- |
| `auth.params.endpoint` | Data Explorer伺服器的端點。 |
| `auth.params.database` | Data Explorer資料庫的名稱。 |
| `auth.params.tenant` | 用來連線至資料總管資料庫的唯一租用戶ID。 |
| `auth.params.servicePrincipalId` | 用於連接到Data Explorer資料庫的唯一服務承擔者ID。 |
| `auth.params.servicePrincipalKey` | 用於連接到Data Explorer資料庫的唯一服務主體鍵。 |
| `connectionSpec.id` | Data Explorer連接規範ID: `0479cc14-7651-4354-b233-7480606c2ac3`. |
...
在本教學課程中，您已使用Flow Service API建立資料總管連線，並已取得連線的唯一ID值。 在下一個教學課程中，您可以使用此ID來學習如何使 [用Flow Service API來探索資料庫](../../explore/database-nosql.md)。
