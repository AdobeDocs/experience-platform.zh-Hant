...
title: 使用流量服務API建立客戶成功來源的資料流
type: Tutorial
description: 本教學課程涵蓋從客戶成功系統擷取資料，以及使用來源聯結器和API擷取資料到Platform的步驟。
...
# 使用建立客戶成功來源的資料流 [!DNL Flow Service] API

本教學課程涵蓋從客戶成功來源擷取資料，以及使用將資料帶到Platform的步驟 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>為了建立資料流，您必須擁有具有客戶成功來源的有效基本連線ID。 如果您沒有此ID，請參閱 [來源概觀](../../../home.md#customer-success) 以取得您可以建立基礎連線的客戶成功來源清單。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 您存取的協力廠商客戶成功系統的唯一連線ID。 |
| `params.path` | 來源檔案的路徑。 |
| `connectionSpec.id` | 與您特定的協力廠商客戶成功系統相關聯的連線規格ID。 請參閱 [附錄](#appendix) 以取得連線規格ID的清單。 |
...
資料流負責從來源收集資料，並將資料匯入Platform。 若要建立資料流，您必須先透過對流程服務API執行GET請求來取得資料流規格。 資料流規格負責從協力廠商客戶成功系統收集資料。
...
| `transformations.params.deltaColum` | 用來區分新資料和現有資料的指定欄。 將根據所選欄的時間戳記擷取增量資料。 支援的日期格式 `deltaColumn` 是 `yyyy-MM-dd HH:mm:ss`. |
...
依照本教學課程，您已建立來源聯結器，以依排程從客戶成功系統收集資料。 傳入資料現在可供下游使用 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
