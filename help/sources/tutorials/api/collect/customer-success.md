...
# 使用為客戶成功源建立資料流 [!DNL Flow Service] API

本教學課程涵蓋從客戶成功來源擷取資料，以及使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>為了建立資料流，您必須具有與客戶成功源的有效基本連接ID。 如果您沒有此ID，請參閱 [來源概觀](../../../home.md#customer-success) 以取得可建立基本連線的客戶成功來源清單。
...
| `params.path` | 源檔案的路徑。 |
| `connectionSpec.id` | 與特定第三方客戶成功系統相關聯的連線規格ID。 請參閱 [附錄](#appendix) 以獲取連接規範ID的清單。 |
...
依照本教學課程，您已建立來源連接器，以依排程從客戶成功系統收集資料。 下游現在可以使用傳入的資料 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需詳細資訊，請參閱下列檔案：
