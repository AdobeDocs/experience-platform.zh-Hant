...
title: 使用流量服務API建立Advertising Sources的資料流
type: Tutorial
...
# 使用為廣告來源建立資料流 [!DNL Flow Service] API

Adobe Experience Platform本教學課程涵蓋從協力廠商廣告應用程式擷取資料，以及透過來源聯結器和 [[!DNL Flow Service]](https://www.adobe.io/experience-platform-apis/references/flow-service/) API。

>[!NOTE]
>
>為了建立資料流，您必須擁有包含廣告來源的有效基本連線ID。 如果您沒有此ID，請參閱 [來源概觀](../../../home.md#advertising) 以取得可用來建立基本連線的廣告來源清單。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 您正在存取的廣告應用程式的唯一連線ID。 |
| `params.path` | 來源檔案的路徑。 |
...
資料流負責從來源收集資料，並將資料匯入Platform。 若要建立資料流，您必須先取得負責收集廣告資料的資料流規格。
...
收集廣告資料的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
...
依照本教學課程所述，您已建立來源聯結器，以依排程從廣告系統收集資料。 傳入資料現在可供下游平台服務使用，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
