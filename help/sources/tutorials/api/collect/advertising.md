...
本教學課程要求您具備要匯入平台之檔案的相關資訊，包括檔案的路徑和結構。 如果您沒有此資訊，請先參閱[教學課程，在嘗試本教學課程之前，先使用Flow Service API](../../api/create/advertising/ads.md)探索廣告應用程式。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 您所存取之廣告應用程式的唯一連線ID。 |
| `params.path` | 源檔案的路徑。 |
...
資料流負責從源收集資料，並將其引入平台。 要建立資料流，必須首先獲取負責收集廣告資料的資料流規範。
...
在本教學課程中，您已建立來源連接器，以依計畫從廣告系統收集資料。 現在，下游平台服務（如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）可以使用傳入的資料。 如需詳細資訊，請參閱下列檔案：
