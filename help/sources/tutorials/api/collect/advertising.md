...
本教學課程需要您具備要帶入Platform之檔案的相關資訊，包括檔案的路徑和結構。 若您沒有此資訊，請參閱[的教學課程，在嘗試本教學課程之前，先使用流量服務API](../../api/create/advertising/ads.md)探索廣告應用程式。
...
依照本教學課程，您已建立來源連接器，以依排程從廣告系統收集資料。 下游Platform服務（例如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用傳入的資料。 如需詳細資訊，請參閱下列檔案：
