...
title: 使用流程服務API建立付款來源的資料流
type: Tutorial
description: 本教學課程涵蓋從支付應用程式擷取資料，以及使用來源聯結器和API擷取資料到Platform的步驟。
...
# 使用建立付款來源的資料流 [!DNL Flow Service] API

本教學課程涵蓋從付款來源擷取資料，並使用將資料帶入Platform的步驟 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>為了建立資料流，您必須擁有付款來源的有效基本連線ID。 如果您沒有此ID，請參閱 [來源概觀](../../../home.md#payments) 以取得可建立基礎連線的付款來源清單。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 您所存取之第三方支付應用程式的唯一連線ID。 |
| `params.path` | 來源檔案的路徑。 |
...
依照本教學課程，您已建立來源聯結器，以依排程從付款應用程式收集資料。 傳入資料現在可供下游平台服務使用，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
