...
# 使用[!DNL Flow Service] API建立付款來源的資料流

本教學課程涵蓋從付款來源擷取資料，以及使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將資料帶到Platform的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有付款來源的有效基底連線ID。 如果您沒有此ID，請參閱[來源概觀](../../../home.md#payments)，以取得您可以建立基礎連線的付款來源清單。
...
資料流負責從來源收集資料，並將這些資料匯入Platform。 若要建立資料流，您必須先對[!DNL Flow Service] API執行GET要求，以取得資料流規格。 資料流規格負責從外部資料庫或NoSQL系統收集資料。
...
依照本教學課程，您已建立來源聯結器，以依排程從付款應用程式收集資料。 下游Platform服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用傳入的資料。 如需更多詳細資訊，請參閱下列檔案：
