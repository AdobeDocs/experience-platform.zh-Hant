...
本教程介紹了從支付源檢索資料並使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)。

>[!NOTE]
>
>為了建立資料流，您必須已具有有效的付款源基本連接ID。 如果您沒有此ID，請查看 [源概述](../../../home.md#payments) 用於建立基本連接的付款來源清單。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 您正在訪問的第三方付款應用程式的唯一連接ID。 |
| `params.path` | 源檔案的路徑。 |
| `connectionSpec.id` | 付款申請的連接規範ID。 |
...
按照本教程，您建立了源連接器，以按計畫從付款應用程式收集資料。 現在，下游平台服務(如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]。 有關詳細資訊，請參閱以下文檔：
