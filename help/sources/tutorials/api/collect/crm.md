...
本教程介紹從CRM源檢索資料並使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)。
...
| 屬性 | 說明 |
| --- | --- |
| `baseConnectionId` | 您正在訪問的第三方CRM系統的唯一連接ID。 |
| `params.path` | 源檔案的路徑。 |
| `connectionSpec.id` | 與特定第三方CRM系統關聯的連接規範ID。 查看 [附錄](#appendix) 連接規範ID清單。 |
...
資料流負責從源收集資料，並將其引入平台。 要建立資料流，必須首先獲得負責收集CRM資料的資料流規範。
...
收集CRM資料的最後一步是建立資料流。 現在，您準備了以下必需值：
...
| `transformations.params.deltaColum` | 用於區分新資料和現有資料的指定列。 增量資料將根據選定列的時間戳進行接收。 支援的格式 `deltaColumn` 是 `yyyy-MM-dd HH:mm:ss`。 如果使用的是Microsoft動態，則支援的格式 `deltaColumn` 是 `yyyy-MM-ddTHH:mm:ssZ`。 |
...
按照本教程，您已建立了源連接器以按計畫從CRM系統收集資料。 現在，下游平台服務(如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]。 有關詳細資訊，請參閱以下文檔：
