...
# 使用CRM來源建立資料流 [!DNL Flow Service] API

本教學課程涵蓋從CRM來源擷取資料，以及使用將資料帶到Platform的步驟 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `transformations.params.deltaColum` | 用來區分新資料和現有資料的指定欄。 將根據所選欄的時間戳記擷取增量資料。 支援的格式 `deltaColumn` 是 `yyyy-MM-dd HH:mm:ss`. 如果您使用Microsoft Dynamics，則受支援的 `deltaColumn` 是 `yyyy-MM-ddTHH:mm:ssZ`. |
