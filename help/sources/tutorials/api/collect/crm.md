...
type: Tutorial
description: 本教學課程涵蓋從協力廠商CRM系統擷取資料，以及使用來源聯結器和API將資料帶入Experience Platform的步驟。
...
# 使用[!DNL Flow Service] API建立CRM來源的資料流

本教學課程涵蓋從CRM來源擷取資料，以及使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將資料帶入Experience Platform的步驟。
...
| `params.path` | 來源檔案的路徑。 |
| `connectionSpec.id` | 與您特定的協力廠商CRM系統相關聯的連線規格ID。 如需連線規格ID的清單，請參閱[附錄](#appendix)。 |
...
資料流負責從來源收集資料，並將資料匯入Experience Platform。 為了建立資料流，您必須先取得負責收集CRM資料的資料流規格。
...
收集CRM資料的最後一步是建立資料流。 到現在為止，您已準備下列必要值：
...
| `transformations.params.deltaColum` | 用來區分新資料和現有資料的指定欄。 將根據所選欄的時間戳記擷取增量資料。 `deltaColumn`支援的格式為`yyyy-MM-dd HH:mm:ss`。 如果您使用Microsoft Dynamics，`deltaColumn`支援的格式為`yyyy-MM-ddTHH:mm:ssZ`。 |
...
按照本教學課程，您已建立來源聯結器，以依排程從CRM系統收集資料。 下游Experience Platform服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
