...
# 使用為CRM源建立資料流 [!DNL Flow Service] API

本教學課程涵蓋從CRM來源擷取資料，以及使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>要建立資料流，您必須具有與CRM源的有效基本連接ID。 如果您沒有此ID，請參閱 [來源概觀](../../../home.md#customer-relationship-management) ，以取得您可以建立基本連線的CRM來源清單。
...
| `params.path` | 源檔案的路徑。 |
| `connectionSpec.id` | 與您特定的第三方CRM系統相關聯的連線規格ID。 請參閱 [附錄](#appendix) 以獲取連接規範ID的清單。 |
...
資料流負責收集來源的資料，並將其導入Platform。 要建立資料流，必須首先獲取負責收集CRM資料的資料流規範。
...
| `transformations.params.deltaColum` | 用於區分新資料和現有資料的指定列。 將根據所選欄的時間戳記擷取增量資料。 支援的格式 `deltaColumn` is `yyyy-MM-dd HH:mm:ss`. 如果您使用Microsoft Dynamics，則支援的格式 `deltaColumn` is `yyyy-MM-ddTHH:mm:ssZ`. |
...
依照本教學課程，您已建立來源連接器，以排程從CRM系統收集資料。 下游Platform服務(例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需詳細資訊，請參閱下列檔案：
