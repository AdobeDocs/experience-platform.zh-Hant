...
本教學課程要求您透過有效連線和您要匯入Platform之表格的相關資訊（包括表格的路徑和結構），來存取協力廠商CRM系統。 如果您沒有此資訊，請在嘗試本教學課程之前，參閱有關[使用流量服務API](../explore/crm.md)探索CRM系統的教學課程。
...
以下各節提供您需要知道的其他資訊，以便使用[!DNL Flow Service] API成功連接到CRM系統。
...
| 屬性 | 說明 |
| --- | --- |
| `baseConnectionId` | 您正在存取之協力廠商CRM系統的唯一連線ID。 |
| `params.path` | 源檔案的路徑。 |
| `connectionSpec.id` | 與您特定的第三方CRM系統相關聯的連線規格ID。 有關連接規範ID的清單，請參見[附錄](#appendix)。 |
...
若要將來源資料內嵌至目標資料集，必須先將其對應至目標資料集所遵守的目標架構。 這是透過向轉換服務API執行POST要求（與要求裝載中定義的資料對應）來達成。
...
資料流負責收集來源的資料，並將其導入Platform。 要建立資料流，必須首先獲得負責收集CRM資料的資料流規範。
...
收集CRM資料的最後一步是建立資料流。 您現在已準備下列必要值：
...
| `transformations.params.deltaColum` | 用於區分新資料和現有資料的指定列。 將根據所選欄的時間戳記擷取增量資料。 `deltaColumn`支援的格式為`yyyy-MM-dd HH:mm:ss`。 如果您使用Microsoft Dynamics,`deltaColumn`的支援格式為`yyyy-MM-ddTHH:mm:ssZ`。 |
...
依照本教學課程，您已建立來源連接器，以排程從CRM系統收集資料。 下游Platform服務（例如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用傳入的資料。 如需詳細資訊，請參閱下列檔案：
