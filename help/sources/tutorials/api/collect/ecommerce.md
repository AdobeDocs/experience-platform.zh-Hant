...
本教學課程涵蓋從電子商務來源擷取資料，並使用將其帶至Platform的步驟 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md)：Experience Platform組織客戶體驗資料的標準化架構。
   ...
   * [結構描述登入API](../../../../xdm/api/getting-started.md)：瞭解如何成功執行對結構描述登入API的呼叫。 這包括您的 `{TENANT_ID}`、「容器」的概念，以及發出請求所需的標頭（請特別注意Accept標頭及其可能的值）。
...
成功的回應會傳回唯一識別碼(`id`)。 在後續步驟中，建立目標連線時需要此ID。
...
資料流負責從來源收集資料並將收集到的資料匯入 [!DNL Platform]. 若要建立資料流，您必須先透過對以下專案執行GET要求來取得資料流規格： [!DNL Flow Service] API。 資料流規格負責從電子商務來源收集資料。
...
| `scheduleParams.interval` | 間隔會指定兩個連續資料流執行之間的期間。 間隔的值應為非零整數。 在以下情況下不需要間隔： `frequency` 設為 `once` 且應大於或等於 `15` 針對其他 `frequency` 值。 |
...
依照本教學課程中的指示，您已建立來源聯結器，以依排程收集資料電子商務。 下游現在可以使用傳入的資料 [!DNL Platform] 服務，例如 [!DNL Real-Time Customer Profile] 和 [!DNL Data Science Workspace]. 如需更多詳細資訊，請參閱下列檔案：
