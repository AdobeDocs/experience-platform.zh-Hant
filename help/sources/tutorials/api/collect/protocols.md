...
本教學課程涵蓋從協力廠商通訊協定應用程式擷取資料，並透過來源連接器和[[!DNL Flow Service]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml) API擷取資料至Adobe Experience Platform的步驟。

## 快速入門

本教學課程要求您透過有效的基本連線存取通訊協定系統，並了解您要匯入Platform的檔案相關資訊，包括表格的路徑和結構。 如果您沒有此資訊，請在嘗試本教學課程之前，先參閱有關[使用流服務API](../explore/protocols.md)探索通訊協定系統的教學課程。
...
以下各節提供了您需要了解的其他資訊，以便使用[!DNL Flow Service] API成功連接到協定應用程式。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 協定應用程式的連接ID |
| `params.path` | 源檔案的路徑。 |
| `connectionSpec.id` | 協定應用程式的連接規範ID。 |
...
資料流負責從源收集資料並將其導入Platform。 要建立資料流，必須首先獲得負責從協定應用程式收集資料的資料流規範。
...
| `transformations.params.deltaColum` | 用於區分新資料和現有資料的指定列。 將根據所選欄的時間戳記擷取增量資料。 使用一般OData時，`deltaColumn`支援的格式為`yyyy-MM-ddTHH:mm:ssZ`。 |
...
依照本教學課程，您已建立來源連接器，以依排程從通訊協定應用程式收集資料。 下游Platform服務（例如[!DNL Real-time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用傳入的資料。 如需詳細資訊，請參閱下列檔案：
