---
keywords: Experience Platform；首頁；熱門主題；收集通訊協定資料；通訊協定資料
solution: Experience Platform
title: 使用流程服務API建立通訊協定來源的資料流
type: Tutorial
description: 本教學課程涵蓋從通訊協定應用程式擷取資料，以及使用來源聯結器和API將其擷取至Experience Platform的步驟。
...
# 使用[!DNL Flow Service] API建立通訊協定來源的資料流

本教學課程涵蓋從通訊協定來源擷取資料，以及使用[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/)將這些資料帶到Experience Platform的步驟。

>[!NOTE]
>
>* 為了建立資料流，您必須擁有包含通訊協定來源的有效基底連線ID。 如果您沒有此ID，請參閱[來源概觀](../../../home.md#protocols)，以取得您可以建立基礎連線的通訊協定來源清單。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `baseConnectionId` | 您通訊協定應用程式的連線ID |
| `params.path` | 來源檔案的路徑。 |
| `connectionSpec.id` | 您通訊協定應用程式的連線規格ID。 |
...
資料流負責從來源收集資料，並將這些資料匯入Experience Platform。 為了建立資料流，您必須先取得負責從通訊協定應用程式收集資料的資料流規格。
...
| `transformations.params.deltaColum` | 用來區分新資料和現有資料的指定欄。 將根據所選欄的時間戳記擷取增量資料。 使用Generic OData時`deltaColumn`支援的格式為`yyyy-MM-ddTHH:mm:ssZ`。 |
...
依照本教學課程中的指示，您已建立來源聯結器，以依排程從通訊協定應用程式收集資料。 下游Experience Platform服務（例如[!DNL Real-Time Customer Profile]和[!DNL Data Science Workspace]）現在可以使用內送資料。 如需更多詳細資訊，請參閱下列檔案：
