...
# 為協定源建立資料流使用 [!DNL Flow Service] API

本教學課程涵蓋從通訊協定來源擷取資料，以及使用 [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>要建立資料流，您必須已具有與協定源的有效基本連接ID。 如果您沒有此ID，請參閱 [來源概觀](../../../home.md#protocols) 以獲取可建立基本連接的協定源清單。
...
| `transformations.params.deltaColum` | 用於區分新資料和現有資料的指定列。 將根據所選欄的時間戳記擷取增量資料。 支援的格式 `deltaColumn` 使用一般OData時為 `yyyy-MM-ddTHH:mm:ssZ`. |
