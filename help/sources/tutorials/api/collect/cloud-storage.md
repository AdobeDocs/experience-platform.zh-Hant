...
本教學課程要求您透過有效的連線存取協力廠商雲端儲存空間，並瞭解您要匯入平台的檔案，包括檔案的路徑和結構。 如果您沒有此資訊，請先參閱教學課程， [在嘗試本教學課程之前，使用Flow Service API探索協力廠商雲端儲存空間](../explore/cloud-storage.md) 。
...
成功的響應返回負責將雲儲存資料帶入平台的資料流規範的詳細資訊。 響應包括唯一流規範ID。 在下一步中需要此ID才能建立新的資料流。
