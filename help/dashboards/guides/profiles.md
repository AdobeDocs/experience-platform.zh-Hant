...
[!UICONTROL Profiles]控制面板顯示您的組織在「設定檔存放區」內Experience Platform的屬性（記錄）資料的快照。 快照不包含任何事件（時間系列）資料。
...
**[!UICONTROL 設定檔計數]**&#x200B;介面工具集顯示建立快照時設定檔存放區內合併的設定檔總數。 此數字是您的設定檔資料所套用的所選合併原則的結果，以便將設定檔片段合併在一起，以便為每個個人建立單一設定檔。

請參閱本檔案前面有關合併原則的[章節，以了解更多資訊。](#merge-policies)

>[!NOTE]
>
>出於多種原因，[!UICONTROL 設定檔計數]介面工具集可能會顯示與UI [!UICONTROL Profiles]區段中[!UICONTROL Browse]標籤上顯示的設定檔計數不同的數字。 最常見的原因是，[!UICONTROL Browse]標籤會根據您組織的預設合併原則參考合併的設定檔總數，而[!UICONTROL 設定檔計數]介面工具集則會根據您選取要在控制面板中檢視的合併原則，參考合併的設定檔總數。
>
>另一個常見原因是，拍攝儀表板快照的時間與為[!UICONTROL Browse]頁簽運行示例作業的時間之間存在差異。 您可以查看介面工具集上的時間戳記，查看[!UICONTROL 設定檔計數]介面工具集上次更新的時間，並進一步了解範例工作在[!UICONTROL 瀏覽]標籤上的觸發方式，請參閱即時客戶設定檔UI指南](https://experienceleague.adobe.com/docs/experience-platform/profile/ui/user-guide.html?lang=en#profile-count)中的[設定檔計數區段。

![](../images/profiles/profile-count.png)

### [!UICONTROL 新增的設定檔] {#profiles-added}

**[!UICONTROL 新增的設定檔]**&#x200B;介面工具集會顯示自上次拍攝快照起，已新增至設定檔存放區的合併設定檔總數。 此數字是您的設定檔資料所套用的所選合併原則的結果，以便將設定檔片段合併在一起，以便為每個個人建立單一設定檔。 您可以使用下拉式選取器，檢視過去30天、90天或12個月內新增的設定檔。

>[!NOTE]
>
>[!UICONTROL 已新增的設定檔]介面工具集會反映設定檔存放區設定及擷取設定檔後新增的設定檔數量。 換言之，如果您的組織在第1天設定設定檔存放區並擷取4,000,000，則24小時內即可使用控制面板，但新增的[!UICONTROL 設定檔]介面工具集將設為0。 這麼做是為了避免初次將設定檔擷取至系統時產生的尖峰。 在接下來的30天內，您的組織會將額外1,000,000個設定檔擷取至設定檔存放區。 建立下一個快照後，新增的[!UICONTROL 描述檔]介面工具集將顯示總共1,000,000個已新增描述檔，而[!UICONTROL 描述檔計數]介面工具集將顯示5,000,000個描述檔。

![](../images/profiles/profiles-added.png)

### [!UICONTROL 設定檔計數趨勢] {#profiles-count-trend}

**[!UICONTROL 設定檔計數趨勢]**&#x200B;介面工具集顯示過去30天、90天或12個月內，每天新增至設定檔存放區的合併設定檔總數。 拍攝快照時每天都會更新此數字，因此，如果您要將設定檔內嵌至Platform，則在拍攝下一個快照前不會反映設定檔數。 新增的設定檔計數是將選取的合併原則套用至您的設定檔資料的結果，以便將設定檔片段合併在一起，以便為每個個人建立單一設定檔。
...
**[!UICONTROL 身分重疊]**&#x200B;介面工具集會顯示Venn圖表或設定圖表，顯示包含多個身分的設定檔存放區中設定檔的重疊。
...
如需設定檔片段的詳細資訊，請先閱讀即時客戶設定檔概覽中[設定檔片段與合併設定檔](https://experienceleague.adobe.com/docs/experience-platform/profile/home.html?lang=en#profile-fragments-vs-merged-profiles)上的區段。
