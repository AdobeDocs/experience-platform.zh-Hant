...
## [!UICONTROL Destinations]個儀表板資料 {#destinations-dashboard-data}
...
## 探索[!UICONTROL Destinations]儀表板 {#explore}

若要導覽至Experience Platform UI中的目的地儀表板，請在左側邊欄中選取「**[!UICONTROL Destinations]**」，然後選取「**[!UICONTROL Overview]**」標籤以顯示儀表板。

最近快照的日期和時間會顯示在目的地下拉式清單旁的[!UICONTROL Overview]頂端。 截至該日期和時間，所有Widget資料都是準確的。 快照的時間戳記會以UTC提供；而不是在個別使用者或組織的時區中。

>[!NOTE]
>
>如果您的組織剛開始使用Experience Platform，但尚未擁有作用中的目的地，則不會顯示目的地儀表板和[!UICONTROL Overview]索引標籤。 相反地，從左側導覽選取[!UICONTROL Destinations]會顯示[!UICONTROL Catalog]標籤。 若要進一步瞭解[!UICONTROL Catalog]標籤，請參閱[[!UICONTROL Destinations]工作區指南](../../destinations/ui/destinations-workspace.md)。

![反白顯示最近快照的Experience Platform UI目的地概觀。](../images/destinations/snapshot-timestamp.png)

### 修改[!UICONTROL Destinations]儀表板 {#modify}

選取&#x200B;**[!UICONTROL Modify dashboard]**&#x200B;以變更目的地儀表板的外觀。 控制面板的變更是按使用者進行，而非整個組織。 您可以從儀表板移動、新增、調整大小及移除Widget，並存取Widget程式庫來自訂儀表板。 從Widget程式庫，您可以探索可用的Widget並為您的組織建立自訂Widget。
...
>[!NOTE]
>
>自2023年7月26日起， [!UICONTROL Profiles]、[!UICONTROL Audiences]和[!UICONTROL Destinations]概觀儀表板已重設為未在前六個月修改其檢視的所有使用者的新預設Widget載出。
...
Adobe提供多種標準Widget，可用來視覺化與目的地相關的不同量度，並評估資料分析可用對象的完整性。 您也可以使用[!UICONTROL Widget library]建立要與您的組織共用的自訂Widget。 若要深入瞭解如何建立自訂Widget，請先閱讀[Widget程式庫概觀](../customize/widget-library.md)。
...
* **啟用**：啟用是使用者將對象或設定檔對應到目的地(例如Oracle Eloqua、Google或Salesforce Marketing Cloud)所採取的動作。
...
>title="最常用的目的地"
>abstract="此小工具會依據對應的對象數顯示貴組織最活躍的目的地。這些數字在上次快照時是準確的。此排名會提供對目前使用最多的目的地的洞察，同時會強調可能未受到充分利用的目的地。"

**[!UICONTROL Most used destinations]** Widget會依對應對象數目（截至上次快照為止）顯示您組織的主要目的地。 此排名可讓insight瞭解哪些目的地正在使用中，同時也可能顯示那些可能未充分利用的目的地。

例如，如果您昨天設定了目的地，但尚未將任何對象對應至該目的地，您便可看到該目的地目前利用不足。

[!UICONTROL Audience count]欄中顯示的對應對象數目自上次每日快照起是準確的。 將新對象對應到目的地不會更新計數，直到拍攝下一個快照為止。

從Widget上顯示的清單中選取目的地的名稱，即可導覽至該特定目的地的目的地詳細資料。 您也可以選取「**[!UICONTROL View All]**」以導覽至「**[!UICONTROL Browse]**」標籤，然後選取目的地名稱以檢視其詳細資料。
...
**[!UICONTROL Recently created destinations]** Widget可讓您檢視貴組織最近設定之目的地的清單。

顯示的建立日期與上次的每日快照相符。 換言之，如果您建立新的目的地，則在拍攝下一個快照之前，它不會出現在清單中。

從Widget上顯示的清單中選取目的地的名稱，會帶您前往從&#x200B;**[!UICONTROL Browse]**&#x200B;標籤連結的目的地詳細資料。 您也可以選取「**[!UICONTROL View All]**」以導覽至「**[!UICONTROL Browse]**」標籤，然後選取目的地名稱以檢視其詳細資料。
...
>title="最近啟動的對象"
>abstract="此小工具會提供最近對應至目的地的對象清單。此清單會提供系統中正積極使用的對象和目的地的快照，能有助於對任何錯誤的對應進行移難排解。"

**[!UICONTROL Recently activated audiences]** Widget提供最近對應至目的地的對象清單。 此清單會提供系統中正積極使用的對象和目的地的快照，能有助於對任何錯誤的對應進行移難排解。

顯示的[!UICONTROL Updated]日期顯示上次將對象啟動至目的地的時間，而且準確顯示上次的每日快照。 換言之，如果您對目的地啟用對象，則更新日期要等到拍攝下一個快照之後才會變更。

從Widget上顯示的清單中選取對象名稱，可帶您前往對象詳細資料。 您也可以選取「**[!UICONTROL View All]**」以導覽至「[!UICONTROL Audiences]」[!UICONTROL Browse]標籤，然後選取對象名稱以檢視其詳細資料。
...
>title="最近啟動的對象 (依目的地)"
>abstract="此小工具會根據在概觀下拉選單中選擇的目的地以遞減順序顯示最近啟動的前五個對象。"

**[!UICONTROL Recently activated audiences by destination]** Widget會根據在總覽下拉式清單中選取的目的地，以遞減順序顯示最近啟用的前五個對象。 它類似於[!UICONTROL Recently activated audiences] Widget，但顯示的資料&#x200B;**僅**&#x200B;適用於選取的目的地。
...
>title="對象規模趨勢"
>abstract="此小工具會說明對象中包含的設定檔數量，這會每天傳送到目的地帳戶。第一個下拉選單會調整對象趨勢的時段。第二個小工具下拉選單會選取要分析的對象。可從概觀的下拉選單中選擇目的地。"

**[!UICONTROL Audience size trend]** Widget針對已對應至該目的地帳戶的對象，描述一段時間內設定檔計數的關係。 Widget使用折線圖來說明對象中包含且每日傳送至目的地帳戶的設定檔數量。
...
**[!UICONTROL Audience size trend]** Widget在Widget的右上角提供[!UICONTROL Captions]按鈕。 選取&#x200B;**[!UICONTROL Captions]**&#x200B;以開啟自動註解對話方塊。 機器學習模型會藉由分析圖表和受眾資料來自動產生標題，以說明關鍵趨勢和重要事件。
...
>title="未對應對象 (依身分識別)"
>abstract="此小工具會針對特定目的地和身分識別列出依遞減的身分識別計數排名的前五個&#x200B;**未對應**&#x200B;對象。小工具下拉選單中列出的篩選器 ID 會根據在概觀頁面頂端選取的目的地帳戶而變更。"

**[!UICONTROL Unmapped audiences by identity]** Widget列出前五名&#x200B;**未對應**&#x200B;對象，依指定目的地和身分的遞減身分計數排名。 它會根據所選的ID反白標示對對應至所選目的地帳戶最有利的受眾。
...
>title="已對應對象 (依身分識別)"
>abstract="此小工具會提供前五個&#x200B;**已對應**&#x200B;對象的清單。該清單會根據對象中包含的來源 ID 的數量由高至低排序。要計算的目的地 ID 會從小工具標題下方的下拉選單中選取。此小工具下拉選單中可用的目的地 ID 會依據在概觀儀表板頂部選擇的目的地而定。"

此小工具會提供前五個&#x200B;**已對應**&#x200B;對象的清單。該清單會根據對象中包含的來源 ID 的數量由高至低排序。要計算的目的地 ID 會從小工具標題下方的下拉選單中選取。Widget中下拉式清單可用的目的地ID會根據在總覽儀表板頂端選取的目的地帳戶篩選器而變更。

![依身分識別介面工具集的對應對象。](../images/destinations/mapped-audiences-by-identity.png)

**[!UICONTROL Mapped audiences by identity]** Widget醒目提示在選定目的地內成功鎖定促銷活動之設定檔商機的可能性。 有效率的鎖定目標行銷活動不取決於傳送至目的地的設定檔數量，而是取決於與目的地ID可能相符的來源ID數量，藉以提供有用且可操作的資料。
...
>title="常見對象"
>abstract="此小工具會提供在頁面頂端選擇的目的地帳戶中啟動的前五個對象的清單，以及在小工具下拉選單中選取的目的地。對象清單會根據啟動的時間排序。啟動時間最近的對象會顯示在頂端。"

**[!UICONTROL Common audiences]** Widget提供在頁面頂端選取的目的地帳戶中啟用的前五個對象的清單，以及在下拉式Widget中選取的目的地。 對象清單會根據啟動的時間排序。啟動時間最近的對象會顯示在頂端。
...
[!UICONTROL Mapped audiences] Widget會顯示對應對象的總數，這些對象可以啟動至頁面頂端選取的目的地。

選取「**[!UICONTROL Audiences]**」以導覽至「對象」儀表板[!UICONTROL Browse]標籤。 此工作區會顯示貴組織的所有區段定義清單。
...
>title="已對應對象的健康情況"
>abstract="此小工具會提供最多包含 20 個已對應對象的清單，這些對象的總設定檔計數和對應至該目的地的 30 天平均對象規模至少有一個標準差的因素偏差。這會為過去 30 天內對象規模和平均值的差異提供計算量度。對象規模的排序為由高至低。"
...
如果[!UICONTROL Mapped audience health] Widget中的對象發生大幅偏差，您應該參考對象人數趨勢圖表，並找出異常對象。 此趨勢可進一步提供insight來瞭解您對象的健康情況。
...
>title="目的地計數"
>abstract="此小工具會提供可以在系統內啟動和傳遞對象的可用端點總數。此數字包括使用中和非使用中的目的地。"

[!UICONTROL Destinations count] Widget提供可在系統中啟用及傳遞對象的可用端點總數。 此數字包括使用中和非使用中的目的地。

在總計數下，選取&#x200B;**[!UICONTROL Destinations]**&#x200B;以導覽至目的地瀏覽標籤。 此頁面列出您目前為止已建立連線的所有目的地。

![目的地計數Widget.](../images/destinations/destinations-count.png)

### [!UICONTROL Destination status] {#destination-status}

[!UICONTROL Destination status] Widget會將已啟用的目的地總數顯示為單一量度，並使用環形圖來說明已啟用與已停用目的地之間的比例差異。
...
[!UICONTROL Activated audiences across all destinations] Widget提供在單一量度中，針對所有目的地啟用的對象總數。 此數字與最近的快照相符。

![所有目的地Widget中的啟用對象。](../images/destinations/activated-audiences-across-all-destinations.png)

選取「**[!UICONTROL Audiences]**」以導覽至目的地[!UICONTROL Browse]標籤。 此頁面提供所有已啟用目的地的清單以及各種相關量度。 請參閱檔案以取得有關[[!UICONTROL Browse]標籤](../../destinations/ui/destinations-workspace.md#browse)的詳細資訊。
...
選取「**[!UICONTROL Audiences]**」以導覽至目的地儀表板的詳細資訊頁面。 [!UICONTROL Activation data]索引標籤會顯示已對應到目的地的對象清單，包括其開始日期和結束日期（如果適用），以及資料匯出的其他相關資訊，例如匯出型別、排程和頻率。 若要檢視特定對象的詳細資料，請從[!UICONTROL Audience Name]欄中選取其名稱。
