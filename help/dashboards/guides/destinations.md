...
* [[!UICONTROL 對象人數趨勢]](#audience-size-trend)
* [[!UICONTROL 依身分割槽分的未對應對象]](#unmapped-audiences-by-identity)
* [[!UICONTROL 依身分對應的對象]](#mapped-audiences-by-identity)
...
>title="最常使用的目的地"
>abstract="此Widget會依對應的對象數量顯示貴組織最活躍的目的地。 這些數字在上次快照時是準確的。 此排名可讓您深入瞭解目前最常使用哪些目的地，並著重顯示可能未充分利用的目的地。"
...
>title="最近建立的目的地"
>abstract="此Widget會顯示貴組織內最近設定之目的地的清單。"
...
>title="最近啟用的對象"
>abstract="此Widget提供最近對應至目的地的對象清單。 此清單提供系統中目前使用中的對象和目的地的快照，可協助疑難排解任何錯誤的對應。"

此 **[!UICONTROL 最近啟用的對象]** widget提供最近對應至目的地的對象清單。 此清單提供系統中目前使用中的對象和目的地的快照，可協助疑難排解任何錯誤的對應。
...
>title="依目的地區分的最近啟用的對象"
>abstract="此Widget會根據在概述下拉式清單中選取的目的地，以遞減順序顯示最近啟用的前五個受眾。"
...
>title="對象人數趨勢"
>abstract="此Widget說明對象中包含的設定檔數量，這些設定檔數量每天都會傳送至目的地帳戶。 第一個下拉式功能表會調整對象趨勢的時段。 第二個Widget下拉式選單會選取要分析的對象。 從概述下拉式選單中選擇目的地。"
...
>title="依身分區分的未對應對象"
>abstract="此Widget列出前五名 **未對應** 依指定目的地和身分的遞減身分計數排名的對象。 Widget下拉式清單中列出的篩選器ID會隨著在概覽頁面頂端選取的目的地帳戶而變更。"
...
>title="依身分區分的已對應對象"
>abstract="此Widget提供前五個清單： **已對應** 對象。 此清單會根據對象內包含的來源ID數量，從高到低排序。 可從Widget標題下方的下拉式選單中選取要計數的目的地ID。 Widget下拉式清單中可用的目的地ID取決於在總覽儀表板頂端選擇的目的地。"

此Widget提供前五個清單： **已對應** 對象。 此清單會根據對象內包含的來源ID數量，從高到低排序。 可從Widget標題下方的下拉式選單中選取要計數的目的地ID。 Widget中下拉式清單可用的目的地ID會根據在總覽儀表板頂端選取的目的地帳戶篩選器而變更。
...
### 通用對象 {#common-audiences}

>[!CONTEXTUALHELP]
>id="platform_dashboards_destinations_commonaudiences"
>title="通用對象"
>abstract="此Widget提供在頁面頂端所選目的地帳戶，以及在下拉式Widget中選取的目的地啟用的前五個對象清單。 對象清單是根據最近啟用的方式排序的。 最近啟用的對象會顯示在頂端。"

此 **[!UICONTROL 通用對象]** widget提供在頁面頂端所選目的地帳戶中啟用的前五個對象的清單，以及在介面工具下拉式清單中選取的目的地。 對象清單是根據最近啟用的方式排序的。 最近啟用的對象會顯示在頂端。

此 [!UICONTROL 對象人數] 欄提供每個列出對象的設定檔總數。

![通用對象Widget。](../images/destinations/common-audiences.png)

### 已對應的對象 {#mapped-audiences}
...
### 對應的對象健康狀況 {#mapped-audience-health}

>[!CONTEXTUALHELP]
>id="platform_dashboards_destinations_mappedaudiencehealth"
>title="對應的對象健康狀況"
>abstract="此Widget提供最多20個對應對象的清單，這些對應對象的總設定檔計數與對應至該目的地的30天平均對象人數至少有一個標準差。 它提供過去30天平均值之受眾人數分散的計算量度。 對象人數會從高到低排序。"
...
>title="目的地計數"
>abstract="此Widget提供可在系統中啟用及傳遞對象的可用端點總數。 此數字包含使用中及非使用中的目的地。"

此 [!UICONTROL 目的地計數] widget提供可在系統中啟用及傳遞對象的可用端點總數。 此數字包含使用中及非使用中的目的地。
