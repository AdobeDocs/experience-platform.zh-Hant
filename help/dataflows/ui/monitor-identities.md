...
監視儀表板可讓您以視覺化方式呈現身分中的資料活動，包括資料身分的狀態。 本教學課程提供如何使用Experience Platform使用者介面監控資料身分的相關指示，讓您追蹤身分處理狀態。
...
若要存取&#x200B;**[!UICONTROL Identities]**&#x200B;儀表板，請在左側導覽中選取&#x200B;**[!UICONTROL Monitoring]**。 在&#x200B;**[!UICONTROL Monitoring]**&#x200B;頁面上，選取&#x200B;**[!UICONTROL Identities]**&#x200B;卡片。

![身分卡。 顯示關於已接收記錄數、已擷取記錄數以及成功率的資訊。](../assets/ui/monitor-identities/focus-card.png)

在主要&#x200B;**[!UICONTROL Identities]**&#x200B;儀表板上，**[!UICONTROL Identities]**&#x200B;卡片會顯示所接收記錄總數、所擷取的記錄數，以及記錄擷取成功率的相關資訊。

儀表板本身包含有關身分處理的量度。 預設情況下，儀表板將顯示過去24小時內您組織來源的身分處理詳細資訊。

![身分儀表板。 顯示每個來源接收之記錄數目的相關資訊。](../assets/ui/monitor-identities/sources.png)

[!UICONTROL Identity processing]頁面包含擷取至[!DNL Identity Service]的記錄資訊，包括新增的身分數目、建立的圖表和更新的圖表。
...
| **[!UICONTROL Records failed]** | 由於資料錯誤而未擷取至Experience Platform的記錄數。 |
| **[!UICONTROL Records skipped]** | 已擷取但不納入[!DNL Identity Service]的記錄數，因為記錄列只有一個識別碼。 |
| **[!UICONTROL Records ingested]** | 擷取到[!DNL Identity Service]的記錄數。 |
| **[!UICONTROL Identities added]** | 新增到[!DNL Identity Service]的網路新識別碼數目。 |
| **[!UICONTROL Graphs created]** | 在[!DNL Identity Service]中建立的淨新身分圖表數目。 |
| **[!UICONTROL Graphs updated]** | 以新邊更新的現有身分圖表數目。 |
...
或者，您可以在切換時選取「**[!UICONTROL Dataflows]**」，以檢視貴組織過去24小時資料流程的身分處理詳細資料。
...
| **[!UICONTROL Records skipped]** | 已擷取但不納入[!DNL Identity Service]的記錄數，因為記錄列只有一個識別碼。 |
| **[!UICONTROL Records ingested]** | 擷取到[!DNL Identity Service]的記錄數。 |
| **[!UICONTROL Total records]** | 所有記錄的總數，包括失敗的記錄、略過的記錄、新增的身分和重複的記錄。 |
| **[!UICONTROL Identities added]** | 新增到[!DNL Identity Service]的網路新識別碼數目。 |
| **[!UICONTROL Graphs created]** | 在[!DNL Identity Service]中建立的淨新身分圖表數目。 |
| **[!UICONTROL Graphs updated]** | 以新邊更新的現有身分圖表數目。 |
| **[!UICONTROL Total failed dataflows]** | 失敗的資料流執行次數。 |
...
[!UICONTROL Dataflow run details]頁面會顯示您[!DNL Identity]資料流執行的詳細資訊，包括其組織ID和資料流執行ID。 此頁面也會顯示[!DNL Identity Service]提供的對應錯誤碼和錯誤訊息（如果擷取程式發生任何錯誤）。
