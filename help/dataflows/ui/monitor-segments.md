...
分段服務允許您從Adobe Experience Platform的即時客戶概要資訊資料中建立段和受眾。 平台提供資料流，以透明地跟蹤從源到目標的資料流。
...
>title="區段"
>abstract="段視圖包含有關IMS組織所有段的資訊，以及有關其激活和評估作業的進一步資訊。"
...
![段操控板。 將顯示有關IMS組織和沙箱中不同段的資訊。](../assets/ui/monitor-segments/segment-monitoring-dashboard.png)
...
| **[!UICONTROL 配置檔案計數]** | 段作業正在評估的配置檔案數。 每個用戶應具有唯一的配置檔案。 |
...
| 量度 | 說明 |
| ------ | ----------- |
| **[!UICONTROL 收到的配置檔案]** | 在激活流中接收的配置檔案總數。 |
| **[!UICONTROL 已激活身份]** | 根據收到的配置檔案成功激活到目標的標識總數。 |
| **[!UICONTROL 排除的身份]** | 根據接收的配置檔案，被排除為無法激活到目標的標識總數。 由於缺少屬性或違反同意，這些身份可能被排除。 |
...
| **[!UICONTROL IMS組織ID]** | 激活作業所屬的IMS組織的ID。 |
...
| **[!UICONTROL IMS組織ID]** | 評估作業所屬IMS組織的ID。 |
