...
在UI中 [!DNL Adobe Experience Platform] ，按一下左 **[!UICONTROL 側導覽]** 中的「服務」。 「服 **[!UICONTROL 務]** 」瀏覽器隨即出現，並顯示可用的Adobe Intelligent Services。 在「歸因AI」的容器中，按一下「 **[!UICONTROL 開啟]**」。
...
要查看服務實例的其他詳細資訊，請單 **[!UICONTROL 擊右上角]** 的「顯示更多」。
...
此時將出現編輯對話框，允許您編輯實例的說明和計分頻率。 若要確認變更並關閉對話方塊，請按一 **[!UICONTROL 下右下]** 角的「編輯」。

![編輯跨欄](./images/insights/edit-popover.png)

### 更多動作 {#more-actions}

「更 **[!UICONTROL 多動作]** 」按鈕位於「編輯」旁的右上導 *覽中*。 按一 **[!UICONTROL 下「更多動作]** 」會開啟下拉式清單，供您選取下列其中一個作業：
...
>[!NOTE]
>
>依預設，除了「歸因模型」篩選器（設為「增量和受影響的歸因轉換」）外，每個篩選器都會設為「全部」。
...
按一 **[!UICONTROL 下「歸因模型]** 」會開啟下拉式清單，其中包含所有可用的不同歸因模型。 您可以選取多個模型來比較結果。 如需不同歸因模型及其運作方式的詳細資訊，請造訪 [Attribution AI](./overview.md) overview，其中包含一個表格，其中包含各模型的相關資訊。

![歸因模型](./images/insights/attribution-model.png)

### 產品

「產 **[!UICONTROL 品]** 」(Product)篩選器允許您從建立實例時最初吸收的任何產品中進行選擇。 按一下下拉式清單，然後使用搜尋功能快速選取您要比較的所有產品。

![產品篩選](./images/insights/product-filter.png)

### Geography

「地 **[!UICONTROL 理位置]** 」篩選器會根據地區模型填入國家代碼。 根據您的資料，此篩選器可能存在或不存在。
...
按一下 **[!UICONTROL 渠道篩選]** ，會顯示包含所有可用行銷渠道的下拉式清單。 您可以選取多個渠道來比較。
...
「熱 **[!UICONTROL 門促銷活動]** 」卡片會顯示促銷活動的概述，以及促銷活動在每個通道中的執行情形。 此資訊卡可協助您的團隊瞭解特定通路的特定促銷活動成效，並提供進一步投資的深入資訊。
