...
| eventType（字串） | True | 此時間系列記錄的主要事件類型。 <br> **示例：** &quot;訂單&quot;、&quot;採購&quot;、&quot;訪問&quot; |
| eventMergeId（字串） | True | 要關聯或合併多個 [!DNL Experience Events] 本質上是同一事件或應該合併的。 這將在接收之前由資料生成器填充。 <br> **示例：** 575525617716-0-edc2ed37-1ab-4750-a820-1c2b3844b8c4 |
| _id（字串） | False | 時間系列事件的唯一標識符。 <br> **示例：** 4461-edc2ed37-1ab-4750-a820-1c2b3844b8c4 |
| _tenantId（對象） | False | 與您的堅定ID對應的頂級對象容器。 <br> **示例：** _atsdsnrmmsv2 |
| your_schema_name（對象） | False | 使用轉換事件對與其關聯的所有觸點事件及其元資料進行分數行。 <br> **示例：** Attribution AI分數 — 型號名稱__2020 |
| 分段（字串） | True | 轉換段，例如建立模型所針對的地理分段。 如果沒有段，則段與conversionName相同。 <br> **示例：** 訂單(_U) |
| conversionName（字串） | True | 在安裝過程中配置的轉換的名稱。 <br> **示例：** 訂單、線索、訪問 |
| 轉換（對象） | False | 轉換元資料列。 |
| 資料源（字串） | True | 資料源的全局唯一標識。 <br> **示例：** Adobe Analytics |
| eventSource（字串） | True | 實際事件發生時的源。 <br> **示例：** Adobe |
| eventType（字串） | True | 此時間系列記錄的主要事件類型。 <br> **示例：** 訂單 |
| geo（字串） | True | 轉換所在的地理位置 `placeContext.geo.countryCode`。 <br> **示例：** 美國 |
| 價格合計（雙） | True | 透過轉換可換股債券 <br> **示例：** 九十九點九 |
| product（字串） | True | 產品本身的XDM標識符。 <br> **示例：** RX 1080ti |
| productType（字串） | True | 此產品視圖向用戶顯示的產品顯示名稱。 <br> **示例：** 格普斯 |
| 數量（整數） | True | 轉換期間採購的數量。 <br> **示例：** 小行星1080 |
| receivedTimestamp(DateTime) | True | 收到轉換的時間戳。 <br> **示例：** 2020-06-09T00:01:51.000Z |
| skuId（字串） | True | 庫存保管單位(SKU)，供應商定義的產品的唯一標識符。 <br> **示例：** MJ-03-XS — 黑色 |
| 時間戳(DateTime) | True | 轉換的時間戳。 <br> **示例：** 2020-06-09T00:01:51.000Z |
| 傳遞（對象） | True | 配置模型時用戶指定的其他分數資料集列。 |
| commerce_order_purchaseCity（字串） | True | 其他得分資料集列。 <br> **示例：** 城市：聖何塞 |
| customerProfile（對象） | False | 用於構建模型的用戶的標識詳細資訊。 |
| 標識（對象） | False | 包含用於構建模型的用戶的詳細資訊，如 `id` 和 `namespace`。 |
| id（字串） | True | 用戶的標識ID，如cookie ID、Adobe AnalyticsID(AAID)或Experience CloudID（ECID，又稱MCID或訪問者ID）等。 <br> **示例：** 173487627254086563468320891369597404 |
| 命名空間（字串） | True | 用於生成路徑和模型的標識命名空間。 <br> **示例：** 救 |
| 觸點詳細資訊（對象陣列） | True | 導至轉換的觸點詳細資訊清單，按 | 觸點事件或時間戳。 |
| touchpointName（字串） | True | 在安裝過程中配置的觸點名稱。 <br> **示例：** 已付搜索按一下 |
| 分數（對象） | True | 此轉換的觸點貢獻為分數。 有關在此對象中生成的分數的詳細資訊，請參見 [聚合屬性分析](#aggregated-attribution-scores) 的子菜單。 |
| touchPoint（對象） | True | Touchpoint元資料。 有關在此對象中生成的分數的詳細資訊，請參見 [聚合分數](#aggregated-scores) 的子菜單。 |
...
| 列名 | 約束 | 可為Null | 說明 |
| --- | --- | --- | --- |
| customerevents_date(DateTime) | 自定義格式和固定格式 | False | YYYY-MM-DD格式的客戶事件日期。 <br> **示例**:2016-05-02 |
| mediatouchpoints_date（日期時間） | 自定義格式和固定格式 | True | YYYY-MM-DD格式的媒體觸點日期 <br> **示例**:2017-04-21 |
| 段（字串） | 已計算 | False | 轉換段，例如建立模型時所依據的地理分段。 如果沒有段，則段與conversion_scope相同。 <br> **示例**:順序(_A) |
| conversion_scope（字串） | 用戶定義的 | False | 用戶配置的轉換的名稱。 <br> **示例**:訂單 |
| touchpoint_scope（字串） | 用戶定義的 | True | 用戶配置的Touchpoint的名稱 <br> **示例**:已付搜索按一下 |
| product（字串） | 用戶定義的 | True | 產品的XDM標識符。 <br> **示例**:抄送 |
| product_type（字串） | 用戶定義的 | True | 此產品視圖向用戶顯示的產品顯示名稱。 <br> **示例**:gpus，筆記型電腦 |
| geo（字串） | 用戶定義的 | True | 傳遞轉換的地理位置(placeContext.geo.countryCode) <br> **示例**:美國 |
| event_type（字串） | 用戶定義的 | True | 此時間系列記錄的主要事件類型 <br> **示例**:付款轉換 |
| media_type（字串） | 枚舉 | False | 描述介質類型是付費、擁有還是獲得。 <br> **示例**:已付、自有 |
| 通道（字串） | 枚舉 | False | 的 `channel._type` 屬性，用於提供具有相似屬性的通道的粗略分類 [!DNL Consumer Experience Event] XDM <br> **示例**:搜索 |
| 操作（字串） | 枚舉 | False | 的 `mediaAction` 屬性用於提供體驗事件媒體操作的類型。 <br> **示例**:按一下 |
| campaign_group（字串） | 用戶定義的 | True | 將多個市場活動組合在一起的市場活動組的名稱，如「50%_DISCOUNT」。 <br> **示例**:商業 |
| campaign_name（字串） | 用戶定義的 | True | 用於標識「50%_DISCOUNT_USA」或「50%_DISCOUNT_ASIA」等市場營銷活動的市場活動的名稱。 <br> **示例**:感恩節大促銷 |
