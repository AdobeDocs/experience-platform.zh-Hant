...
Attribution AI支援兩類分數：演算法和規則型。 演算法分數包含增量和受影響的分數。 規則型分數包括首次接觸、上次接觸、線性、U形和時間耗損。
...
- **預算分配**:在行銷管道中通知預算分配決策。
- **促銷活動最佳化**:在每個管道內，了解哪些促銷活動、創作元素和關鍵字在哪些SKU或Geos中效果最佳。這可讓您查看每個管道，讓行銷團隊能最佳化其策略。
- **完整漏斗歸因**:了解行銷對整個客戶歷程的影響。例如，免費帳戶註冊到付費轉換及更新。
- **合作夥伴評估**:根據歸因結果評估機構和合作夥伴的效力。

### 其他功能

Attribution AI也提供與其他Adobe解決方案（例如[!DNL Adobe Analytics]）的整合。 這可讓您使用這些解決方案，運用可自訂的演算法模型來評估媒體效能，並提供分析分析。

## 後續步驟

您可以依照[快速入門](./getting-started.md)指南開始操作。 本指南會逐步引導您設定所有必要的Attribution AI預先請求。 如果您已準備好認證和資料，請訪問[Attribution AI使用手冊](./user-guide.md)。 本指南會逐步引導您建立執行個體，並提交以進行訓練和計分。
