---
keywords: 體驗平台；洞察；客戶智慧；熱門主題；客戶智慧
...
在[!DNL Adobe Experience Platform] UI中，按一下左側導覽中的&#x200B;**[!UICONTROL 服務]**。 出現&#x200B;*Services*&#x200B;瀏覽器並顯示可用的智慧服務。 在客戶AI的容器中，按一下&#x200B;**[!UICONTROL Open]**。
...
- **[!UICONTROL 克隆]**:選擇 **** 克隆操作當前選定的服務實例設定。然後，您可以修改工作流程，進行微調，並將其重新命名為新例項。
...
檢視服務例項其他詳細資訊的第二個方法位於前瞻分析頁面。 您可以按一下右上角的&#x200B;**[!UICONTROL 顯示更多]**&#x200B;來填入下拉式清單。 詳細資訊會列出，例如分數定義、建立時間和傾向類型。 有關所列任何屬性的更多資訊，請訪問[配置客戶AI實例](./configure.md)。
...
要編輯實例，請按一下右上角導航中的&#x200B;**[!UICONTROL 編輯]**。

![按一下「編輯」按鈕](../images/insights/edit-button.png)

此時將出現編輯對話框，允許您編輯實例的名稱、說明、狀態和計分頻率。 要確認更改並關閉對話框，請在右下角選擇&#x200B;**[!UICONTROL 保存]**。

![編輯跨欄](../images/insights/edit-instance.png)

### 更多動作

**[!UICONTROL 更多操作]**&#x200B;按鈕位於&#x200B;**[!UICONTROL 編輯]**&#x200B;旁的右上導航中。 按一下&#x200B;**[!UICONTROL 更多操作]**&#x200B;開啟一個下拉菜單，允許您選擇以下操作之一：

- **[!UICONTROL 克隆]**:選擇 **** 克隆操作服務實例設定。然後，您可以修改工作流程，進行微調，並將其重新命名為新例項。
- **[!UICONTROL 刪除]**:刪除實例。
- **[!UICONTROL 存取分數]**:選取 **[!UICONTROL 存]** 取分數會開啟對話方塊，提供客戶 [](./download-scores.md) AI教學課程下載分數的連結，該對話方塊也提供進行API呼叫所需的資料集ID。
...
**[!UICONTROL 分數分佈]**&#x200B;卡會根據分數提供人口的視覺化摘要。 您在[!UICONTROL 分數分佈]卡片中看到的顏色代表產生的傾向分數類型。 將滑鼠指標暫留在任何計分分配上，可提供屬於該分配的確切計數。
...
針對低、中和高傾向重新導向您至區段產生器，在任何區段中選擇「建立區段」按鈕。****

>[!NOTE]
>
>**[!UICONTROL 建立區段]**&#x200B;按鈕只有在資料集啟用即時客戶設定檔時才可用。 有關如何啟用即時客戶配置檔案的詳細資訊，請訪問[即時客戶配置檔案概述](../../../rtcdp/overview.md)。

![按一下建立區段](../images/insights/influential-factors-create-segment.png)

![建立區段](../images/insights/create-segment.png)

區段產生器可用來定義區段。 從「前瞻分析」頁面選擇「建立區段」時，客戶AI會自動將選取的區段資訊新增至區段。 ****&#x200B;若要完成區段的建立，只需填入位於區段產生器使用者介面右側導軌的&#x200B;*Name*&#x200B;和&#x200B;*Description*&#x200B;容器。 在為區段指定名稱和說明後，按一下右上角的&#x200B;**[!UICONTROL Save]**。
...
若要在平台UI中檢視新區段，請按一下左側導覽中的&#x200B;**[!UICONTROL 區段]**。 此時會顯示&#x200B;**[!UICONTROL 瀏覽]**&#x200B;頁面，並顯示所有可用區段。

![您的所有區段](../images/insights/Segments-dashboard.png)

## 後續步驟

本檔案概述了客戶AI服務實例提供的見解。 您現在可以繼續[教學課程，在客戶AI](./download-scores.md)中下載分數，或瀏覽其他提供的[Adobe智慧型服務](../../home.md)指南。
