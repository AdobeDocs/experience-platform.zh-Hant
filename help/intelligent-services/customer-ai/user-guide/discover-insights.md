...
![在Adobe Experience Platform UI中導覽至Customer AI服務執行個體。](../images/insights/navigate-to-service.png)
...
![Customer AI儀表板顯示服務執行個體清單及其詳細資料。](../images/insights/dashboard.png)

選取服務執行處理名稱以開始。

![熒幕擷圖顯示在Customer AI儀表板中選取服務執行個體名稱的程式。](../images/insights/click-the-name.png)
...
![Customer AI深入分析登陸頁面顯示各種視覺效果和探索深入分析的選項。](../images/insights/landing_page_insights.png)
...
![顯示名稱、說明、評分頻率和其他組態選項等詳細資訊的服務執行個體面板。](../images/user-guide/service-instance-panel.png)

>[!NOTE]
>
>如果評分回合失敗，則會提供錯誤訊息。 錯誤訊息會列在右側邊欄的&#x200B;**上次執行詳細資料**&#x200B;下，只有失敗的執行才看得見。

![針對Customer AI中失敗的評分回合顯示的錯誤訊息。](../images/insights/failed-run.png)
...
![Customer AI資料集預覽彈出視窗，使用顏色編碼的索引鍵顯示多個資料集以方便識別。](../images/insights/landing-show-more.png)

### Customer AI資料集預覽彈出視窗

如果Customer AI使用一個以上的資料集，則會提供標示為&#x200B;**[!UICONTROL Multiple]**&#x200B;的超連結，後面接著括弧`()`中的資料集數目。

![多個資料集預覽會顯示有顏色編碼的索引鍵，以便輕鬆識別Customer AI中使用的資料集。](../images/insights/insights-multi-datasets.png)

選取多個資料集連結會開啟Customer AI資料集預覽彈出視窗。 預覽中的每個顏色代表資料集，如資料集欄左側的顏色鍵所示。 在此範例中，您可以看到只有&#x200B;**資料集1**&#x200B;包含`PROP1`欄。

![資料集預覽彈出視窗顯示所選執行個體的其他詳細資料。](../images/insights/dataset-preview.png)

### 編輯例項

若要編輯執行個體，請選取右上方導覽列中的&#x200B;**[!UICONTROL 編輯]**。

Customer AI介面中的![編輯按鈕。](../images/insights/edit-button.png)

「編輯」對話方塊會出現，讓您編輯執行個體的名稱、說明、狀態和評分頻率。 若要確認變更並關閉對話方塊，請選取右下角的&#x200B;**[!UICONTROL 儲存]**。

![編輯執行個體彈出視窗，顯示用於修改Customer AI執行個體的名稱、說明、狀態和評分頻率的選項。](../images/insights/edit-instance.png)
...
![其他動作下拉式清單，顯示[複製]、[刪除]、[存取]和[檢視執行記錄]等選項。](../images/insights/more-actions.png)
...
![已評分的個人檔案視覺化，分為高、中及低傾向值區，每個區段以不同顏色表示。](../images/insights/scoring-summary.png)

您可以將滑鼠停留在鈴鐺上的任何顏色上以檢視其他資訊，例如屬於貯體的設定檔百分比和總數。

![顯示高、中、低傾向值區間個人檔案分佈的評分環視覺效果。](../images/insights/scoring-ring.png)

## 分數的分佈

**[!UICONTROL 分數分佈]**&#x200B;卡片會根據分數為您提供母體的視覺摘要。 您在[!UICONTROL 分數分佈]卡片中看到的顏色代表產生的傾向分數型別。 暫留在任何評分分配上，可提供屬於該分配的精確計數。

![顯示Customer AI中不同傾向值區之分數分佈的視覺效果。](../images/insights/distribution-of-scores.png)

## 影響因素

對於每個分數貯體，會產生一張卡片，顯示該貯體的前10個影響因素。 影響因素會提供您更多詳細資訊，讓您瞭解客戶為何屬於各種分數貯體。

![每個傾向性貯體的影響因素視覺化，強調影響客戶行為的前10個因素。](../images/insights/influential-factors.png)

### 影響因子明細

將游標暫留在任何主要影響因素上，可以進一步劃分資料。 已針對為何某些設定檔屬於傾向性貯體提供概覽。 根據係數，您可以獲得數字、分類或布林值。 下列範例會依地區顯示分類值。

![針對選取的傾向性貯體，顯示影響因素詳細劃分的深入分析視覺效果。](../images/insights/drilldown.png)

此外，透過深入分析，您可以比較分佈因數（如果分佈因數出現在兩個或多個傾向性貯體中），並使用這些值建立更具體的區段。 下列範例說明第一個使用案例：

![跨傾向值區的分佈因素比較，強調影響因素的差異。](../images/insights/drilldown-compare.png)
...
![按鈕，可從Customer AI深入分析中的影響因素建立區段。](../images/insights/influential-factors-create-segment.png)

![按鈕，可從Customer AI深入分析中的影響因素建立區段。](../images/insights/create-segment.png)
...
![個別評分回合結果的視覺化，顯示跨流失/轉換和未流失/未轉換類別的設定檔分佈。](../images/insights/scoring_tab.png)
...
![提升圖顯示預測性模型優於隨機鎖定目標。 較高的早期十等分提升值表示模型較強。](../images/user-guide/lift-chart.png)
...
![收益圖說明透過鎖定高傾向使用者與隨機鎖定目標所取得的積極結果的累計百分比。](../images/user-guide/gains-chart.png)
