...
Customer AI指南需要實際瞭解使用Customer AI的各種各樣的Experience Platform服務。 開始之前，請檢閱下列檔案：
...
- [區段產生器使用手冊](../../segmentation/tutorials/create-a-segment.md)： Experience Platform可讓您輕鬆建立及存取區段，並使用不同的建置區塊來進一步表示您的區段特性。
...
下載Customer AI分數是透過API呼叫的組合完成。 若要呼叫Experience Platform API，您必須先完成[驗證教學課程](https://www.adobe.com/go/platform-api-authentication-en)。 完成驗證教學課程，在所有Experience Platform API呼叫中提供每個必要標題的值，如下所示：
