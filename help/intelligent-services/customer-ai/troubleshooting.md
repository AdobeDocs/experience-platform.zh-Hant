...
當模型訓練、評分和設定失敗時，Customer AI會顯示錯誤。 在&#x200B;**[!UICONTROL Service instances]**&#x200B;區段中，**[!UICONTROL LAST RUN STATUS]**&#x200B;的欄會顯示下列其中一個訊息： **[!UICONTROL Success]**、**[!UICONTROL Training issue]**&#x200B;和&#x200B;**[!UICONTROL Failed]**。

![顯示成功、訓練問題或失敗的上次執行狀態。](./images/errors/last-run-status.png)

在顯示&#x200B;**[!UICONTROL Failed]**&#x200B;或&#x200B;**[!UICONTROL Training issue]**&#x200B;的事件中，您可以選取執行狀態以開啟側面板。 側面板包含您的&#x200B;**[!UICONTROL Last run status]**&#x200B;和&#x200B;**[!UICONTROL Last run details]**。 **[!UICONTROL Last run details]**&#x200B;包含執行失敗原因的資訊。 如果Customer AI無法提供您錯誤的詳細資訊，請聯絡支援人員並提供錯誤代碼。
...
如果您收到錯誤&quot;[!UICONTROL Model Quality is poor. We recommend creating a new app with the modified configuration]&quot;。 請依照下列建議的步驟進行疑難排解。
...
| 408 | 沒有符合資格的最近資料 | 在`{{data_days}}`之前的`{{etl_window_end}}`天內，沒有符合資格的使用者的使用者行為資料。 請檢查資料集，確保會定期更新。 （錯誤碼：VALIDATION-408 NO_RECENT_DATA_FOR_ELIGIBLE_POPULATION） | 2021-04-01之前60天內沒有符合資格的使用者的使用者行為資料。 請檢查資料集，確保會定期更新。 （錯誤碼：VALIDATION-408 NO_RECENT_DATA_FOR_ELIGIBLE_POPULATION） |
