...
| mobileplaceid | placeContext.POIinteraction.POIDetail.POID | 字串 | 從上下文資料變數a.loc.id收集。 指定興趣點的識別碼。 |
...
| mobilecrashes | application.cranses | 物件 | <!-- MISSING --> | {id（字串），值（數字）} |
...
| videocomplete | media.mediaTimed.comples | 物件 | <!-- MISSING --> | {id（字串），值（數字）} |
...
| videoadcomplete | advertising.comples | 物件 | <!-- MISSING --> | {id（字串），值（數字）} |
...
| videochaptercomplete | media.mediaTimed.mediaChapter.comples | 物件 | <!-- MISSING --> | {id（字串），值（數字）} |
...
| videopausecount | media.mediaTimed.anstuints | 物件 | <!-- MISSING --> | {id（字串），值（數字）} |
