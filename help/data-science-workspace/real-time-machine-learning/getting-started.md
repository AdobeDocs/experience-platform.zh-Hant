...
若要開始使用Real-time Machine Learning，您必須有權存取資料集。 您可選擇使用外部資料集並將其上傳至您的[!DNL JupyterLab]環境，或在Experience Platform中建立新資料集（如果尚未這麼做的話）。
...
若要建立用於即時機器學習的新資料集，您需要資料集的資料結構描述。 接下來，您需要使用您建立的結構描述內嵌資料。 使用下列教學課程來建立和填入[!DNL Experience Platform]的資料集：
