...
>[!TIP]
>
>在與層次結構交互時，如果子屬性具有句點(`.`)，必須使用反斜線(`\`)以轉義特殊字元。 有關詳細資訊，請閱讀上的指南 [轉義特殊字元](home.md#escape-special-characters)。
...
| 匹配_regex | 檢查字串是否與輸入的規則運算式匹配。 | <ul><li>字串： **必需** 您正在檢查的字串與規則運算式匹配。</li><li>規則運算式： **必需** 您所比較的規則運算式。</li></ul> | matches_regex(STRING, REGEX) | matches_regex(&quot;E259,E259B_009,1_1&quot;, &quot;([^,]+),[^,]*,([^,]+)」) | true |
...
| get_url_encoded | 此函式以URL為輸入，用ASCII字元替換或編碼特殊字元。 有關特殊字元的詳細資訊，請閱讀 [特殊字元清單](#special-characters) 的下一頁。 | <ul><li>URL: **必需** 要替換或用ASCII字元編碼的特殊字元的輸入URL。</li></ul> | get_url_encoded(URL) | get_url_encoded(&quot;https</span>://example.com/partneralliance_asia-pacific_2022&quot;) | https%3A%2F%2Fexample.com%2Fpartneralliance_asia-pacific_2022 |
| get_url_decoded | 此函式將URL作為輸入，並將ASCII字元解碼為特殊字元。  有關特殊字元的詳細資訊，請閱讀 [特殊字元清單](#special-characters) 的下一頁。 | <ul><li>URL: **必需** 要解碼為特殊字元的ASCII字元的輸入URL。</li></ul> | get_url_decoded(URL) | get_url_decoded(&quot;https%3A%2F%2Fexample.com%2Fpartneralliance_asia-pacific_2022&quot;) | htps</span>://example.com/partneralliance_asia-pacific_2022 |
...
| 日期/部分 | 檢索日期的部分。 支援以下元件值： <br><br>&quot;年&quot;<br>&quot;yyyy&quot;<br>&quot;yy&quot;<br><br>&quot;季度&quot;<br>&quot;qq&quot;<br>&quot;q&quot;<br><br>&quot;月&quot;<br>&quot;mm&quot;<br>&quot;m&quot;<br><br>&quot;日常年&quot;<br>&quot;dy&quot;<br>&quot;y&quot;<br><br>&quot;天&quot;<br>&quot;dd&quot;<br>&quot;d&quot;<br><br>&quot;周&quot;<br>&quot;w&quot;<br>&quot;w&quot;<br><br>&quot;工作日&quot;<br>&quot;dw&quot;<br>&quot;w&quot;<br><br>&quot;小時&quot;<br>&quot;hh&quot;<br>&quot;hh24&quot;<br>&quot;hh12&quot;<br><br>&quot;分鐘&quot;<br>&quot;mi&quot;<br>&quot;n&quot;<br><br>&quot;第二&quot;<br>&quot;ss&quot;<br>&quot;s&quot;<br><br>&quot;毫秒&quot;<br>&quot;SSS&quot; | <ul><li>元件： **必需** 表示日期部分的字串。 </li><li>日期： **必需** 日期，採用標準格式。</li></ul> | date_part&#x200B;（元件，日期） | date_part(&quot;MM&quot;, date(&quot;2019-10-17 11:55:12英吋)) | 10 |
...
| 函數 | 說明 | 參數 | 語法 | 運算式 | 示例輸出 |
| -------- | ----------- | ---------- | -------| ---------- | ------------- |
| 為空 | 檢查對象是否為空。 | <ul><li>輸入： **必需** 您嘗試檢查的對象為空。</li></ul> | is_empty(INPUT) | `is_empty([1, null, 2, 3])` | false |
| 陣列_到對象 | 建立對象清單。 | <ul><li>輸入： **必需** 鍵對和陣列對的分組。</li></ul> | arrays_to_object(INPUT) | `arrays_to_objects('sku', explode("id1\|id2", '\\\|'), 'price', [22.5,14.35])` | ```[{ "sku": "id1", "price": 22.5 }, { "sku": "id2", "price": 14.35 }]``` |
...
| str_to_object | 從輸入字串建立對象。 | <ul><li>字串： **必需** 正在分析以建立對象的字串。</li><li>VALUE_DELIMITER: *可選* 分隔欄位與值的分隔符。 預設分隔符為 `:`。</li><li>欄位分隔符： *可選* 分隔欄位值對的分隔符。 預設分隔符為 `,`。</li></ul> | str_to_object &#x200B;(STRING, VALUE_DELIMITER, FIELD_DELIMITER) **注釋**:您可以使用 `get()` 與 `str_to_object()` 以檢索字串中鍵的值。 | <ul><li>示例#1:str_to_object(&quot;firstName - John;姓氏 — ;- 123 345 7890&quot;、&quot;-&quot;、&quot;;&quot;)</li><li>示例#2:str_to_object(&quot;firstName - John;姓氏 — ;phone - 123 456 7890&quot;, &quot;-&quot;, &quot;;&quot;)。get(&quot;firstName&quot;)</li></ul> | <ul><li>示例#1:`{"firstName": "John", "lastName": "Doe", "phone": "123 456 7890"}`</li><li>示例#2:&quot;約翰&quot;</li></ul> |
| 包含鍵 | 檢查源資料中是否存在對象。 **注：** 此函式替換已棄用 `is_set()` 的子菜單。 | <ul><li>輸入： **必需** 如果源資料中存在要檢查的路徑。</li></ul> | contains_key(INPUT) | contains_key(&quot;evars.evar.field1&quot;) | true |
...
| `fpid_to_ecid ` | 此函式採用FPID字串，並將其轉換為ECID，以用於Adobe Experience Platform和Adobe Experience Cloud應用程式。 | <ul><li>字串： **必需** 要轉換為ECID的FPID字串。</li></ul> | `fpid_to_ecid(STRING)` | `fpid_to_ecid("4ed70bee-b654-420a-a3fd-b58b6b65e991")` | `"28880788470263023831040523038280731744"` |

{style="table-layout:auto"}

### 用戶代理函式 {#user-agent}

下表中包含的任何用戶代理函式都可以返回以下值之一：

* 電話 — 帶小螢幕的移動設備（通常&lt; 7英吋）
* 移動 — 尚未識別的移動設備。 該移動設備可以是電子閱讀器、平板電腦、電話、手錶等。

有關設備欄位值的詳細資訊，請閱讀 [設備欄位值清單](#device-field-values) 的下一頁。
...
以下提供了有關使用資料準備映射功能的其他資訊

### 特殊字元 {#special-characters}

下表概述了保留字元及其相應編碼字元的清單。
...
| --- | --- |
| 桌面 | 台式機或筆記型電腦類型的設備。 |
| 匿名 | 匿名設備。 有些情況下， `useragents` 已經由匿名化軟體修改的。 |
| 未知 | 未知設備。 這些通常 `useragents` 沒有設備資訊。 |
| 行動 | 一種尚未識別的移動設備。 該移動設備可以是電子閱讀器、平板電腦、電話、手錶等。 |
| 平板電腦 | 具有大螢幕（通常> 7英吋）的移動設備。 |
| 電話 | 一種具有小螢幕（通常&lt; 7&quot;）的移動設備。 |
| 監視 | 一種帶小螢幕（通常&lt; 2英吋）的移動設備。 這些設備通常作為電話/平板電腦類型設備的附加螢幕操作。 |
| 增強現實 | 一種具有AR能力的移動設備。 |
| 虛擬現實 | 一種具有VR功能的移動設備。 |
| 電子閱讀器 | 一種類似平板電腦的設備，但通常 [!DNL eInk] 的上界。 |
| 機頂盒 | 一種允許通過電視大小的螢幕進行交互的連接設備。 |
| TV | 與機頂盒類似的設備，但內置在電視中。 |
| 家用設備 | （通常是大型）家電，如冰箱。 |
| 遊戲控制台 | 固定遊戲系統 [!DNL Playstation] 或 [!DNL XBox]。 |
| 掌上遊戲機 | 移動遊戲系統 [!DNL Nintendo Switch]。 |
| 聲音 | 聲音驅動裝置 [!DNL Amazon Alexa] 或 [!DNL Google Home]。 |
| 車 | 一種基於車輛的瀏覽器。 |
| 機器人 | 訪問網站的機器人。 |
| 移動機器人 | 訪問網站但表示希望被視為移動訪問者的機器人。 |
| 機器人模擬器 | 訪問網站的機器人，假裝這是機器人 [!DNL Google]但事實並非如此。 **注釋**:在大多數情況下，機器人模仿者確實是機器人。 |
| Cloud | 基於雲的應用程式。 這些既不是機器人，也不是駭客，而是需要連接的應用程式。 這包括 [!DNL Mastodon] 伺服器。 |
| 駭客 | 如果在 `useragent` 的下界。 |
