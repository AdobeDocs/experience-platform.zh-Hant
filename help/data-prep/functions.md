...
| 函數 | 說明 | 參數 | 語法 | 運算式 | 範例輸出 |
-------- | ----------- | ---------- | -------| ---------- | -------------
| min | 返回給定參數的最小值。 使用自然排序。 | <ul><li>選項：**Required**&#x200B;一個或多個可相互比較的對象。</li></ul> | min(OPTIONS) | min(3, 1, 4) | 1 |
| max | 返回給定參數的最大值。 使用自然排序。 | <ul><li>選項：**Required**&#x200B;一個或多個可相互比較的對象。</li></ul> | max(OPTIONS) | max(3, 1, 4) | 4 |
