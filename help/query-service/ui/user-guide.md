...
在[!DNL Experience Platform] UI中，選取左側導覽功能表中的&#x200B;**[!UICONTROL Queries]**&#x200B;以開啟查詢服務工作區。 接下來，若要開始撰寫查詢，請選取畫面右上方的&#x200B;**[!UICONTROL Create Query]**。 此連結可從查詢服務工作區中的任何頁面使用。
...
>[!TIP]
>
>您可以在編寫查詢時進行[!UICONTROL Disable syntax auto complete]，而不會失去進度。
...
[!UICONTROL Run selected query]圖示。 在編輯器中選取查詢語法之前，此圖示預設為停用。

![反白顯示[!UICONTROL Run selected query]圖示的查詢編輯器。](../images/ui/query-editor/run-selected-query.png)

### 取消查詢編輯器工作階段 {#cancel-query}

取消長時間執行的查詢，以控制查詢執行並提高生產力。 此動作會在查詢執行期間清除查詢編輯器。 請注意，查詢會在背景中繼續執行。 如果是CTAS查詢，仍會產生輸出資料集。 若要取消編輯器中的執行並繼續構成SQL陳述式，請在執行查詢後選取&#x200B;**[!UICONTROL Cancel query]**。

![反白顯示[!UICONTROL Cancel query]的查詢編輯器。](../images/ui/query-editor/cancel-query-run.png)

確認對話方塊隨即顯示。 選取&#x200B;**[!UICONTROL Confirm]**&#x200B;以取消查詢執行。

![取消查詢確認對話方塊中反白顯示[確認]。](../images/ui/query-editor/cancel-query-confirmation-dialog.png)

### 結果計數 {#result-count}

「查詢編輯器」有最多50,000列的輸出。 您可以選擇在查詢編輯器主控台中一次顯示的列數。 若要變更主控台中顯示的列數，請選取&#x200B;**[!UICONTROL Result count]**&#x200B;下拉式清單，然後選取50、100、150、300、500和1000選項。
...
[!UICONTROL Query Editor]已妥善組織，以便儘可能輕鬆撰寫查詢。 下面的熒幕擷圖顯示編輯器在UI中的顯示方式，SQL輸入欄位和&#x200B;**Play**&#x200B;會強調顯示。
...
[!UICONTROL Format text]功能新增標準化語法樣式，讓您的查詢更易讀取。 選取&#x200B;**[!UICONTROL Format text]**&#x200B;以標準化查詢編輯器中的所有文字。

>[!NOTE]
>
>[!UICONTROL Format text]功能不適用於匿名區塊。 若要瞭解如何循序連結一或多個SQL陳述式，請參閱[匿名區塊檔案](../key-concepts/anonymous-block.md)。

![含有[!UICONTROL Format text]和SQL敘述句的查詢編輯器。](../images/ui/query-editor/format-text.png)
...
若要在查詢編輯器中檢視查詢，請從[!UICONTROL Templates]索引標籤中選取任何已儲存的範本。 查詢詳細資訊面板提供管理所選查詢的更多資訊和工具。 此外，也會顯示有用的中繼資料，例如上次修改查詢的時間以及修改者（如適用）。

>[!NOTE]
>
>[!UICONTROL View schedule]、[!UICONTROL Add schedule]和[!UICONTROL Delete query]選項只有在查詢已儲存為範本後才能使用。 [!UICONTROL Add schedule]選項會直接將您從查詢編輯器帶往排程產生器。 [!UICONTROL View schedule]選項會直接將您帶到該查詢的排程詳細目錄。 請參閱查詢排程檔案，瞭解如何在UI[中](./query-schedules.md#create-schedule)建立查詢排程。

![查詢詳細資料面板反白顯示的查詢編輯器。](../images/ui/query-editor/query-details.png)

從詳細資訊面板，您可以直接從UI產生輸出資料集、刪除或命名顯示的查詢、檢視查詢執行排程，以及將查詢新增到排程。

若要產生輸出資料集，請選取&#x200B;**[!UICONTROL Run as CTAS]**。 **[!UICONTROL Enter output dataset details]**&#x200B;對話方塊隨即顯示。 輸入名稱和說明，然後選取&#x200B;**[!UICONTROL Run as CTAS]**。 新資料集會顯示在&#x200B;**[!UICONTROL Datasets]**&#x200B;瀏覽標籤中。 請參閱[檢視資料集檔案](../../catalog/datasets/user-guide.md#view-datasets)，深入瞭解貴組織的可用資料集。

>[!NOTE]
>
>[!UICONTROL Run as CTAS]選項只有在查詢有&#x200B;**未**&#x200B;排程時才可用。

![ [!UICONTROL Enter output dataset details]對話方塊。](../images/ui/query-editor/output-dataset-details.png)

在您執行&#x200B;**[!UICONTROL Run as CTAS]**&#x200B;動作後，會彈出一則確認訊息，通知您動作成功。 此快顯訊息包含一個連結，提供導覽至查詢記錄工作區的一個便利方式。 如需查詢記錄檔的詳細資訊，請參閱[查詢記錄檔](./query-logs.md)。

### 正在儲存查詢 {#saving-queries}

「查詢編輯器」提供儲存功能，可讓您儲存查詢並稍後處理。 若要儲存查詢，請選取[查詢編輯器]右上角的&#x200B;**[!UICONTROL Save]**。 必須使用&#x200B;**[!UICONTROL Query Details]**&#x200B;面板提供查詢的名稱，才能儲存查詢。

>[!NOTE]
>
>使用查詢編輯器命名並儲存在中的查詢，可在查詢儀表板[!UICONTROL Templates]索引標籤中作為範本使用。 如需詳細資訊，請參閱[範本檔案](./query-templates.md)。
...
>[!NOTE]
>
>排定的查詢若連續執行10次失敗，就會自動置於[!UICONTROL Quarantined]狀態。 具有此狀態的查詢需要您的介入，才能進行任何進一步的執行。 如需詳細資訊，請參閱[隔離的查詢](./monitor-queries.md#quarantined-queries)檔案。

請參閱查詢排程檔案，瞭解如何在UI[中](./query-schedules.md)建立查詢排程。 或者，若要瞭解如何使用API新增排程，請閱讀[排程查詢端點指南](../api/scheduled-queries.md)。

任何排定的查詢都會新增到[!UICONTROL Scheduled queries]索引標籤中的清單。 您可以從該工作區透過UI監視所有已排程查詢工作的狀態。 在[!UICONTROL Scheduled queries]標籤上，您可以找到有關查詢執行及訂閱警示的重要資訊。 可用的資訊包括狀態、排程詳細資料，以及執行失敗時的錯誤訊息/代碼。 如需詳細資訊，請參閱[監視排程查詢檔案](./monitor-queries.md)。


### 如何尋找先前的查詢 {#previous-queries}

從「查詢編輯器」執行的所有查詢都會在「記錄」表格中擷取。 您可以使用&#x200B;**[!UICONTROL Log]**&#x200B;標籤中的搜尋功能來尋找查詢執行。 已儲存的查詢列在&#x200B;**[!UICONTROL Templates]**&#x200B;索引標籤中。

如果已排程查詢，則[!UICONTROL Scheduled Queries]索引標籤會透過UI改善這些查詢作業的可見度。 如需詳細資訊，請參閱[查詢監視檔案](./monitor-queries.md)。
...
若要使用物件瀏覽器瀏覽您的資料庫，請在搜尋欄位中輸入表格名稱，或選取&#x200B;**[!UICONTROL Tables]**&#x200B;以展開可用資料集和表格的清單。 使用搜尋欄位時，會根據您的輸入動態篩選可用表格清單。
...
若要在查詢編輯器中執行查詢，您可以在編輯器中輸入SQL，或從&#x200B;**[!UICONTROL Log]**&#x200B;或&#x200B;**[!UICONTROL Templates]**&#x200B;索引標籤載入先前的查詢，然後選取&#x200B;**播放**。 查詢執行的狀態會顯示在下方的&#x200B;**[!UICONTROL Console]**&#x200B;標籤中，而輸出資料會顯示在&#x200B;**[!UICONTROL Results]**&#x200B;標籤中。
...
完成查詢後，結果會顯示在&#x200B;**[!UICONTROL Results]**&#x200B;索引標籤旁邊的&#x200B;**[!UICONTROL Console]**&#x200B;索引標籤中。 此檢視顯示您查詢的表格輸出，根據您選擇的[結果計數](#result-count)顯示50到1000列結果。 此檢視可讓您驗證您的查詢是否產生預期的輸出。 若要使用您的查詢產生資料集，請移除傳回資料列的限制，然後使用`CREATE TABLE tablename AS SELECT`執行查詢，以使用輸出產生資料集。 請參閱[產生資料集教學課程](./create-datasets.md)，瞭解如何從查詢編輯器中的查詢結果產生資料集的指示。
...
若要下載您的查詢結果，請選取[查詢編輯器]索引標籤&#x200B;**[!UICONTROL Download]**&#x200B;右上角的&#x200B;**[!UICONTROL Result]**。 然後從下拉式功能表中選擇&#x200B;**[!UICONTROL CSV]**、**[!UICONTROL XLSX]**&#x200B;或&#x200B;**[!UICONTROL JSON]**。 檔案會自動下載到您的本機電腦。 選擇適合您使用案例的格式、CSV用於輕量型匯出、XLSX用於格式化試算表，或JSON用於結構化資料處理。

>[!NOTE]
>
>如果&#x200B;**[!UICONTROL Download]**&#x200B;按鈕遺失，請檢查您的查詢結果。 只有在傳回記錄時，按鈕才會出現。 如果未傳回任何記錄，**[!UICONTROL Result]**&#x200B;索引標籤會顯示「沒有結果」訊息，而且會停用下載選項。
...
成功執行查詢後，在&#x200B;**[!UICONTROL View results]**&#x200B;索引標籤中選取&#x200B;**[!UICONTROL Result]**&#x200B;以開啟您結果的表格化全熒幕檢視。
...
您可以從[!UICONTROL Result]標籤或全熒幕結果預覽中複製查詢結果。 從&#x200B;**[!UICONTROL Result]**&#x200B;索引標籤中，選取復製圖示(![復製圖示。](../../images/icons/copy.png))以複製所有查詢結果到剪貼簿。 若要啟用復製圖示，請先選取一列。 您可以選取個別列，或使用頂端的核取方塊來一次選取所有列。

![查詢編輯器的結果索引標籤，反白顯示復本圖示。](../images/ui/overview/query-editor-copy-icon.png)

或者，選取&#x200B;**[!UICONTROL View results]**&#x200B;以開啟全熒幕預覽。 在此對話方塊中，選取個別列，或使用左上角的核取方塊來選取所有列，然後選取復製圖示（![復製圖示）。](../../images/icons/copy.png))以複製選取的資料。
...
## 監視和管理並行工作階段 {#monitor-manage-sessions}

使用工作階段管理來檢視沙箱中的作用中查詢編輯器工作階段、識別閒置工作階段，並結束這些工作階段以釋放共用容量。 您無法中斷正在執行查詢的工作階段。 此功能僅適用於管理員，且需要&#x200B;**[!UICONTROL Manage Query Session]**&#x200B;許可權。

若要存取工作階段管理，請選取查詢服務工作區中的&#x200B;**[!UICONTROL Admin]**&#x200B;索引標籤。 如需檢視工作階段詳細資料、解譯工作階段狀態以及結束閒置工作階段的逐步指示，請參閱[管理查詢服務工作階段](session-management.md)。
