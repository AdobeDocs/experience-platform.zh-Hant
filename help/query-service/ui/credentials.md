...
>[!NOTE]
>
>此認證面板不會自動提供給所有使用者使用。如果您需要，請聯絡您的Adobe客戶團隊以請求將[!UICONTROL Credentials]索引標籤包含在查詢服務工作區中。 如有要求，此變更將適用於整個組織，並由Adobe的工程團隊執行。 這不是由使用者控制的設定。
...
**[!UICONTROL Expiring credentials]**&#x200B;區段提供下列資訊：

- **[!UICONTROL Host]**：要連線使用者端的主機名稱。 這會合併您的組織名稱，如同在Experience Platform UI的頂端功能區中所示。
- **[!UICONTROL Port]**：要連線之主機的連線埠號碼。
- **[!UICONTROL Database]**：要連線使用者端的資料庫名稱。
- **[!UICONTROL Username]**：用來連線到查詢服務的使用者名稱。
- **[!UICONTROL Password]**：用來連線到查詢服務的密碼。 UI中的密碼已進行雜湊處理，以提高安全性。 選取復製圖示(![復製圖示。](/help/images/icons/copy.png))，將您完整的未雜湊認證複製到剪貼簿。
- **[!UICONTROL PSQL command]**：自動插入所有相關資訊的命令，讓您在命令列上使用PSQL連線到查詢服務。
- **[!UICONTROL Expires]**：到期認證的到期日期與時間。 權杖的預設有效期間為24小時，但可在Admin Console的進階設定中變更。
...
若要在Power BI或Tableau中存取您的Customer Journey Analytics資料，請選取[!UICONTROL Database]下拉式功能表，然後從可用選項中選取`prod:cja`。 接著，複製您的[!DNL Postgres]認證引數（主機、連線埠、資料庫、使用者名稱及其他），以用於您的Power BI或Tableau設定。
...
若要建立一組不會到期的認證，請返回Experience Platform UI並從左側導覽選取「**[!UICONTROL Queries]**」以存取[!UICONTROL Queries]工作區。 接著，選取&#x200B;**[!UICONTROL Credentials]**&#x200B;標籤，接著選取&#x200B;**[!UICONTROL Generate credentials]**。

![顯示[認證]索引標籤和[產生認證]的查詢儀表板。](../images/ui/credentials/generate-credentials.png)

會出現一個對話方塊，讓您產生認證。 若要建立不會到期的認證，您必須提供下列詳細資料：

- **[!UICONTROL Name]**：您正在產生的認證名稱。
- **[!UICONTROL Description]**： （選用）您產生的認證說明。
- **[!UICONTROL Assigned to]**：將指派認證的使用者。 此值應為建立認證之使用者的電子郵件地址。
- **[!UICONTROL Password]** （選擇性）您認證的選擇性密碼。 如果未設定密碼，Adobe會自動為您產生密碼。

提供所有必要的詳細資料後，請選取&#x200B;**[!UICONTROL Generate credentials]**&#x200B;以產生您的認證。

![[產生認證]對話方塊已反白顯示。](../images/ui/credentials/create-account.png)

>[!IMPORTANT]
>
>選取&#x200B;**[!UICONTROL Generate credentials]**&#x200B;時，會將組態JSON檔案下載至您的本機電腦。 由於Adobe **不會**&#x200B;記錄產生的認證，因此您必須安全地儲存下載的檔案，並保留認證的記錄。
>
...
儲存您產生的認證之後，請選取&#x200B;**[!UICONTROL Close]**。 您現在可以看到所有不會到期的認證清單。
...
提供所有必要的詳細資料後，請選取&#x200B;**[!UICONTROL Update account]**&#x200B;以完成認證的更新。
...
>[!NOTE]
>
>使用不會到期的認證連線到主機時，仍需使用[!UICONTROL EXPIRING CREDENTIALS]區段中列出的所有引數，但密碼和使用者名稱除外。
>輸入使用者名稱與密碼的格式使用冒號分隔值，如本範例`username:{your_username}`和`password:{password_string}`所示。

| 參數 | 說明 | 範例 |
|---|---|---|
| **伺服器/主機** | 您連線的伺服器/主機名稱。 <ul><li>此值同時用於即將到期的認證和不即將到期的認證，並且採用`server.adobe.io`的形式。 在&#x200B;**[!UICONTROL Host]**&#x200B;區段的[!UICONTROL EXPIRING CREDENTIALS]下找到值。</ul></li> | `acme.platform.adobe.io` |
| **連線埠** | 您連線的伺服器/主機連線埠。 <ul><li>此值同時用於即將到期的認證和不即將到期的認證，可以在&#x200B;**[!UICONTROL Port]**&#x200B;區段的[!UICONTROL EXPIRING CREDENTIALS]下找到。</ul></li> | `80` |
| **資料庫** | 您正在連線的資料庫。 <ul><li>此值同時用於即將到期的認證和不即將到期的認證，並在&#x200B;**[!UICONTROL Database]**&#x200B;區段的[!UICONTROL EXPIRING CREDENTIALS]下找到。 </ul></li> | `prod:all` |
| **使用者名稱** | 連線到外部使用者端的使用者使用者名稱。 <ul><li>此值會用於即將到期的認證和不即將到期的認證。 它採用`@AdobeOrg`之前的英數字串形式。 在&#x200B;**[!UICONTROL Username]**&#x200B;下找到此值。</li></ul> | `ECBB80245ECFC73E8A095EC9@AdobeOrg` |
| **密碼** | 連線到外部使用者端的使用者密碼。 <ul><li>如果您使用即將到期的認證，可在&#x200B;**[!UICONTROL Password]**&#x200B;區段的[!UICONTROL EXPIRING CREDENTIALS]下找到此認證。</li><li>如果您使用不會到期的認證，此值是來自technicalAccountID的串連引數，以及從設定JSON檔案取得的認證。 密碼值的格式為： `{technicalAccountId}:{credential}`。</li></ul> | <ul><li>即將到期的認證密碼超過一千個字元的英數字串。 將不會提供範例。</li><li>不會到期的認證密碼如下： <br>`4F2611B8613DK3670V495N55:3d182fa9e0b54f33a7881305c06203ee`</li></ul> |
