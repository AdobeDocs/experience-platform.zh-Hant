...
通過向IMS組織發出GET請求，您可以檢索IMS組織的所有查詢清單 `/queries` 端點。
...
成功的響應返回HTTP狀態200，其中列出了指定IMS組織的查詢清單(JSON)。 以下響應返回為IMS組織建立的最新查詢。
...
您可以請求刪除指定的查詢，方法是向 `/queries` 終結點和提供查詢 `id` 值。
