...
在Adobe Experience Platform Query Service中，帳戶用於建立非到期憑證，以便與外部SQL用戶端搭配使用。 您可以在查詢服務API中使用`/accounts`端點，這可讓您以程式設計方式建立、擷取、編輯和刪除您的查詢服務整合帳戶（也稱為技術帳戶）。

## 快速入門

本指南中使用的端點是查詢服務API的一部分。 繼續之前，請檢閱[快速入門手冊](./getting-started.md)，以取得您需要了解的重要資訊，以成功呼叫API，包括必要的標題和如何讀取範例API呼叫。

## 建立帳戶

您可以向`/accounts`端點提出POST請求，以建立查詢服務整合帳戶。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `accountName` | **** 必要查詢服務整合帳戶的名稱。 |
| `assignedToUser` | **** 必要：將為其建立查詢服務整合帳戶的Adobe ID。 |
| `credential` | *（選用）* 用於查詢服務整合的憑證。如果未指定，系統將自動為您生成憑據。 |
| `description` | *（選用）* 查詢服務整合帳戶的說明。 |
...
| `technicalAccountId` | 查詢服務整合帳戶的ID。 這與`credential`一起構成您帳戶的密碼。 |
| `credential` | 查詢服務整合帳戶的憑據。 這與`technicalAccountId`一起構成您帳戶的密碼。 |

## 更新帳戶

您可以向`/accounts`端點提出PUT請求，以更新Query Service整合帳戶。
...
| 屬性 | 說明 |
| -------- | ----------- |
| `accountName` | *（選用）* 查詢服務整合帳戶的更新名稱。 |
| `assignedToUser` | *（選用）* Query Service整合帳戶所連結的更新Adobe ID。 |
| `credential` | *（選用）* 查詢服務帳戶的更新憑證。 |
| `description` | *（選用）* 查詢服務整合帳戶的更新說明。 |
...
您可以向`/accounts`端點提出GET請求，以檢索所有查詢服務整合帳戶的清單。
...
您可以向`/accounts`端點提出DELETE請求，以刪除您的Query Service整合帳戶。
