...
+++回答
可以，您可以使用[!DNL Postman] （免費的協力廠商應用程式）將所有Adobe API服務視覺化並與其互動。 請觀看[[!DNL Postman] 設定指南](https://video.tv.adobe.com/v/28832)，瞭解如何在Adobe Developer Console中設定專案及取得與[!DNL Postman]搭配使用所需的所有認證的逐步指示。 請參閱有關啟動、執行和共用[集合 [!DNL Postman] 的](https://learning.postman.com/docs/running-collections/intro-to-collection-runs/)指南的官方檔案。
+++
...
+++回答
不可以。 資料大小沒有限制，但互動式工作階段有10分鐘的查詢逾時限制。 如果查詢是以批次CTAS方式執行，則10分鐘逾時不適用。 如需詳細資訊，請參閱[互動式查詢執行](./best-practices/writing-queries.md#interactive-query-execution)的指南。
+++
...
- [將查詢轉換為CTAS查詢](./sql/syntax.md#create-table-as-select)並排程執行。 排程執行可透過UI[或](./ui/user-guide.md#scheduled-queries)API[完成](./api/scheduled-queries.md#create)。
...
+++回答
不可以。 查詢服務具有自動縮放功能，可確保同時進行的查詢不會明顯影響服務的效能。
+++
...
- 登入Experience Platform後，在使用者介面的左側導覽中選取「**[!UICONTROL Datasets]**」以導覽至[!UICONTROL Datasets]儀表板。
- 資料集[!UICONTROL Browse]索引標籤隨即開啟。 您可以使用搜尋列來調整可用選項。 從顯示的清單中選取資料集。

![Experience Platform UI中的資料集控制面板，其搜尋列和資料集已反白顯示。](./images/troubleshooting/dataset-selection.png)

- [!UICONTROL Datasets activity]畫面會出現。 選取&#x200B;**[!UICONTROL Preview dataset]**&#x200B;以開啟XDM結構描述的對話方塊，以及所選資料集中平面化資料的表格檢視。 如需詳細資訊，請參閱[預覽資料集檔案](../catalog/datasets/user-guide.md#preview-a-dataset)
...
- 在頂端導覽列的右側，選取&#x200B;**[!UICONTROL Sign In]**。

![已登入的Adobe網站強調顯示。](./images/troubleshooting/adobe-sign-in.png)

- 使用已在您的Adobe ID授權中註冊的Adobe和密碼。
- 從頂端導覽列中選取&#x200B;**[!UICONTROL Help & Support]**。

![上方導覽列的下拉式功能表，包含[說明及支援]、[企業支援]及[連絡我們]等標示專案。](./images/troubleshooting/help-and-support.png)

此時會出現包含[!UICONTROL Help and support]區段的下拉式橫幅。 選取&#x200B;**[!UICONTROL Contact us]**&#x200B;以開啟Adobe客戶服務虛擬助理，或選取&#x200B;**[!UICONTROL Enterprise support]**&#x200B;以取得大型組織的專用說明。
+++
...
+++回答
Query Service提供數種內建的SQL Helper函式，以擴充SQL功能。 請參閱檔案，以取得查詢服務[所支援](./sql/spark-sql-functions.md)SQL函式的完整清單。
+++

### 是否支援所有原生[!DNL Spark SQL]函式，或使用者是否僅限使用Adobe提供的包裝函式[!DNL Spark SQL]函式？

+++回答
尚未在資料湖資料上測試所有開放原始碼[!DNL Spark SQL]函式。 測試並確認後，會將其新增至支援清單。 請參考支援的[函式 [!DNL Spark SQL] 的](./sql/spark-sql-functions.md)清單，以檢查特定函式。
+++
...
您也應該檢視檔案以瞭解如何在UI[中及透過](./ui/user-guide.md#scheduled-queries)API[執行](./api/scheduled-queries.md)已排程的查詢。
...
再舉一例，如果特定時間戳記是`2021-07-14 12:40:00.0`時區的`Asia/Seoul`，則傳回的UTC時間戳記將是`2021-07-14 03:40:00.0`
...
`from_utc_timestamp()`方法會從您當地時區&#x200B;**的時間戳記中解譯指定的引數**，並以UTC格式提供所需區域的對等時間戳記。 在以下範例中，小時是使用者的當地時區中的2:40PM。 以變數傳遞的首爾時區比當地時區早九小時。
...
您可以使用`NOT IN`或`NOT EXISTS`，而不使用`LEFT OUTER JOIN`。
...
+++回答
可以。不過，某些協力廠商使用者端（例如DbVisualizer）在SQL區塊之前和之後可能需要個別的識別碼，以表示指令碼的部分應作為單一陳述式處理。 如需詳細資訊，請參閱[匿名區塊檔案](./key-concepts/anonymous-block.md)或[正式的DbVisualizer檔案](https://confluence.dbvis.com/display/UG120/Executing+Complex+Statements#ExecutingComplexStatements-UsinganSQLDialect)。
+++

## TLS、連線埠存取和加密 {#tls-port-questions}

### 在連線埠80上建立的連線是否仍使用HTTPS和TLS加密？

+++回答
可以。連線埠80上的連線會使用TLS加密來保護，服務必須強制使用TLS。 不接受純HTTP連線。 連線埠80支援是為了配合某些客戶網路原則。 如果您的組織封鎖連線埠80，請改用連線埠5432。 兩個連線埠都需要TLS並提供相同的安全性狀態。
+++

### Adobe的查詢服務是否透過未加密的HTTP （連線埠80）公開資料？

+++回答
不可以。 連線埠80上的連線需要TLS，而且任何純文字HTTP請求都會在伺服器端遭到拒絕。 也支援連線埠5432，且已進行TLS加密。
+++

### 使用連線埠80進行查詢服務和Data Distiller是否為舊有設定？

+++回答
不可以。 具有強制TLS的連線埠80是受支援的組態，專為具有特定網路需求的客戶所設計。 這並非舊版或不安全模式。 如果您的環境限制連線埠80上的輸出連線，請改用連線埠5432；兩個連線埠都會強制執行TLS。
+++

### 從Power BI使用者端到查詢服務的所有連線皆使用TLS 1.2嗎？

+++回答
可以。傳輸中的資料一律會使用HTTPS來保護，目前支援的版本為TLS 1.2。所有Power BI與查詢服務的連線都需要加密的傳輸。
+++

### 搭配Data Distiller使用連線埠80時是否未加密？

+++回答
不可以。 Data Distiller會在連線埠80上強制執行TLS並拒絕任何純文字HTTP請求。 也支援連線埠5432，且已進行TLS加密。
+++

### 搭配查詢服務或資料Distiller使用連線埠80時，是否有任何風險或限制？

+++回答
可以。TLS會在連線埠80上強制執行，而且不支援未加密的連線。 由於原則限制，某些組織會封鎖連線埠80上的傳出流量。 如果適用於您的網路，請改用連線埠5432。 兩個連線埠提供相同等級的保全性，因為所有情況下都需要TLS。
+++
...
### 資料Distiller在開發沙箱和生產沙箱之間的效能是否有所不同？

+++回答
當您在開發和生產沙箱中執行查詢時，可以預期類似的效能。 這兩種環境的設計目的都是為了提供相同等級的處理能力。 不過，計算時數的差異可能會發生，具體取決於您處理的資料量以及執行查詢時的整體系統活動。

在Experience Platform UI的[授權使用量儀表板](../dashboards/guides/license-usage.md)中追蹤您的計算小時使用量。
+++
...
+++回答
可以。資料可從查詢服務中擷取，也可選擇透過SQL命令將結果儲存為CSV格式。
...
+++回答
可以，協力廠商案頭使用者端可透過一次性設定不會到期的憑證來連線至查詢服務。 未過期的認證可由授權的使用者產生，並透過自動下載至其本機電腦的JSON檔案接收。 在檔案中可找到有關如何建立和下載不會到期的認證[的完整](./ui/credentials.md#non-expiring-credentials)指南。
+++
...
### 對於不會到期的認證密碼的特殊字元是否有任何限制？

+++回答
可以。當您設定不會到期的認證密碼時，必須至少包含一個數字、一個小寫字母、一個大寫字母和一個特殊字元。 不支援美元符號($)。 請改用特殊字元，例如！、@、#、^或&amp;。
+++
...
+++回答
是，您可以將Power BI連線至查詢服務。 請參閱檔案，以取得有關將Power BI案頭應用程式連線至查詢服務[的](./clients/power-bi.md)指示。
+++
