...
| 權限 | 說明 |
|---|---|
| [!UICONTROL Manage Queries] | 此許可權可讓使用者執行資料探索和批次查詢，以便讀取現有資料集或寫入資料集上的資料。 這包含`CREATE TABLE AS SELECT` (`CTAS`)和`INSERT INTO AS SELECT` (`ITAS`)查詢。 |
...
| 權限 | 說明 |
|---|---|
| [!UICONTROL Manage Datasets] | 此許可權提供結構描述的唯讀存取權，並可讓您讀取、建立、編輯和刪除資料集，以搭配查詢服務使用。 |
| [!UICONTROL View Datasets] | 此許可權允許搭配查詢服務使用的資料集和結構描述的唯讀存取權。 |
...
若要針對加速資料集建立欄位式存取限制，您可以使用查詢服務CTAS查詢來建立加速資料集，並根據現有的XDM結構描述或臨時結構描述來建構這些資料集。 然後，系統管理員可以[新增並編輯結構描述](../../xdm/tutorials/labels.md#edit-the-labels-for-the-schema-or-field)或[臨時結構描述](./ad-hoc-schema-labels.md#edit-governance-labels)的資料使用標籤。 您可以從[!UICONTROL Labels] UI的[!UICONTROL Schemas]工作區套用、建立及編輯標籤至您的結構描述。

資料使用標籤也可以透過「資料集」UI直接套用或編輯至資料集[，或是從「存取控制」](../../data-governance/labels/user-guide.md#add-labels)工作區建立[!UICONTROL Labels]。 如需詳細資訊，請參閱如何[建立新標籤](../../access-control/abac/ui/labels.md)的指南。

接著，即可透過附加的資料使用標籤，以及套用至指派給使用者的角色之許可權集，來控制使用者對個別欄的存取權。

### 連線能力 {#connectivity}

查詢服務可透過Experience Platform UI或透過與外部相容使用者端建立連線來存取。 存取所有可用前端由一組憑證控制。
...
可透過[!UICONTROL Logs]工作區之[!UICONTROL Queries]索引標籤中的Experience Platform UI存取查詢記錄。
...
結構描述資料欄位可以透過Experience Platform UI設定為身分欄位，而且查詢服務也可讓您使用SQL命令&#39;ALTER TABLE&#39;[來](../sql/syntax.md#alter-table)標籤主要身分。 使用`ALTER TABLE`命令設定身分，在資料集是使用SQL建立的，而非透過Experience Platform UI直接從結構描述建立時，特別有用。 請參閱檔案，瞭解在使用標準結構描述時，如何[在UI](../../xdm/ui/fields/identity.md)中定義身分欄位。
