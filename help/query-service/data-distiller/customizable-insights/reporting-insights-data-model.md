...
![對象分析使用者模型的實體關聯圖表(ERD)。](../../images/data-distiller/customizable-insights/audience-insight-user-model.png)
...
>[!NOTE]
>
>`ACCOUNT=acp_query_batch`命令需要Data Distiller SKU。 若不使用這項功能，資料湖上就會建立一般資料模型。
...
成功執行兩個`ALTER TABLE`命令後，事實與維度資料表之間的關係就會形成。

執行陳述式後，請使用`SHOW datagroups;`命令從`audienceinsight.audiencemodel`傳回加速存放區上可用資料集的清單。 您的清單化結果應類似於以下提供的範例。

>[!IMPORTANT]
>
>只有加速存放區中的資料可從查詢服務無狀態API端點`POST /data/foundation/query/accelerated-queries`存取。
...
![連結Real-Time CDP分析資料模型和Query加速存放區模型的ERD圖表。](../../images/data-distiller/customizable-insights/updatingAudienceInsightUserModel.png)

## 建立維度表格以擴充您的報表見解模型

使用查詢服務將關鍵描述性屬性從擴充的Real-Time CDP維度資料集新增到`audienceinsight`資料模型，並建立事實表格與新維度表格之間的關係。 下列SQL示範如何將現有的維度表格整合至您的報表見解資料模型。
...
現在`audienceinsight`資料模型已擴增，可以開始查詢了。 以下SQL顯示對應的目的地和對象清單。
...
![依據目的地和受眾Widget的符合計數，是根據新的報表深入分析資料模型建立的。](../../images/data-distiller/customizable-insights/user-defined-dashboard-widget.png)
