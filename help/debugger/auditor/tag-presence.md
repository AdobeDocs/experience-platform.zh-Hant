...
| 測試 | 粗細 | 標準 | 建議 |
| --- | --- | --- | --- |
| Adobe Advertising — 程式碼是否存在 | 5 | DOM中不提供Adobe Advertising標籤。 | 使用[Adobe Advertising標籤擴充功能](../../destinations/catalog/advertising/adobe-advertising-cloud.md)實作Adobe Advertising標籤。 |
| Adobe Advertising — 實作區段畫素 | 5 | 將您的Adobe Advertising區段畫素升級為新的Adobe Advertising僅限影像標籤。 使用過時的 AMO 區段標記可能會導致資料遺失。 | 使用[Adobe Advertising標籤擴充功能](../../destinations/catalog/advertising/adobe-advertising-cloud.md)實作Adobe Advertising區段畫素。 |
...
| 啟動 — `pageBottom`回呼存在於`<body>`中 | 5 | 在頁面的`_satellite.pageBottom()`內找不到必要的`<body>`回呼。 如果在頁面上完全找不到`pageBottom`呼叫，或該呼叫位於`<head>`標籤中（或其他非預期的位置），則此測試不會通過。 只有在`pageBottom`位於`<body>`標籤內的某處時，測試才會通過。 | 在結尾的`</body>`標籤前面加上緊鄰的內嵌指令碼，以確保標籤可正常運作。<br><br>[其他資訊](../../tags/ui/client-side/asynchronous-deployment.md) |
