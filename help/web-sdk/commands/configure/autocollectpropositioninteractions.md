...
`autoCollectPropositionInteractions`屬性是選擇性設定，可決定Web SDK是否應自動收集主張互動。
...
在[!DNL Web SDK]中預設已啟用主張追蹤。 不過，您可以在執行[`configure`](../configure/overview.md)命令時使用`autoCollectPropositionInteractions`值進一步設定它。
