...
description: 本文會示範如何使用Edge Network Server API在Web屬性上部署伺服器端個人化。
...
伺服器端個人化涉及使用 [Edge Network伺服器API](../../server-api/overview.md) 以個人化您Web屬性的客戶體驗。
...
需要個人化請求來取得主張並傳送顯示通知。 使用伺服器端實作時，應用程式伺服器會向Edge Network Server API提出這些要求。

| 請求 | 製作者 |
|---|---|
| 擷取主張的互動請求 | 呼叫Edge Network Server API的應用程式伺服器。 |
| 傳送顯示通知的互動請求 | 呼叫Edge Network Server API的應用程式伺服器。 |
...
1. 原地複製 [此存放庫](https://github.com/adobe/alloy-samples) 到您的本機電腦。
...
2. 瀏覽器要求網頁。 瀏覽器先前儲存的任何Cookie，前置詞為 `kndctr_`，已包括在內。
3. 當從應用程式伺服器要求頁面時，會將事件傳送至 [互動式資料收集端點](../../../server-api/interactive-data-collection.md) 以擷取個人化內容。 範例應用程式使用Helper方法來簡化建立及傳送請求至API的程式(請參閱 [aepEdgeClient.js](https://github.com/adobe/alloy-samples/blob/main/common/aepEdgeClient.js))。 此 `POST` 請求包含 `event` 和 `query`. 上個步驟的Cookie （若有）會包含在 `meta>state>entries` 陣列。
...
6. [!DNL Visual Experience Composer (VEC)] 選件會遭到忽略，因為它們只能透過Web SDK呈現。
