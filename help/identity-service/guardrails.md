...
| 護欄 | 限制 | 附註 |
| --- | --- | --- |
| 圖表中的身分數量 | 150 | 此限制會套用至沙箱層級。 當身分數量達到150個或更多時，不會新增任何身分，且不會更新身分圖表。 圖形可能會因為連結一或多個具有少於150個身分的圖形而顯示大於150個身分。 **注意**：身分圖表中的身分數量上限 **適用於個別合併的設定檔** 為50。 根據具有超過50個身分的身分圖表合併的設定檔會從即時客戶設定檔中排除。 如需詳細資訊，請閱讀以下指南： [設定檔資料的護欄](../profile/guardrails.md). |
