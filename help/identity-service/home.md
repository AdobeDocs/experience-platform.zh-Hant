...
| 身分名稱空間 | 身分識別命名空間用於區分身分識別的內容或類型。例如，身分識別會將「name<span>@email.com」區分為電子郵件地址或將「443522」區分為數值的 CRM ID。身分識別名稱空間是用來查詢個別身分，並提供身分識別值的內容。 這可讓您判斷以下兩個 [!DNL Profile] 包含不同主要ID，但共用相同值的片段 `email` 身分名稱空間，實際上是同一個人。 請參閱 [身分名稱空間總覽](./namespaces.md) 以取得詳細資訊。 |
