...
身分欄位建立時，必須與現有的身分名稱空間相關聯。 任何新名稱空間都必須使用API[建立](#how-do-i-create-a-custom-namespace-for-my-organization)，才能與身分欄位建立關聯。
...
當請求路徑中為`graph-type`查詢引數指定無效值時，會顯示此錯誤訊息。 請參閱[總覽中有關](./home.md)身分圖表[!DNL Identity Service]的章節，以瞭解支援的圖表型別。
...
以下是使用[!DNL Identity Service]批次擷取API[時，您可能會遇到的與](https://developer.adobe.com/experience-platform-apis/references/batch-ingestion/)相關的錯誤訊息清單。
