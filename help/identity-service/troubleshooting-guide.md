...
當您的IMS組織尚未為其設定適當的權限時，將顯示此錯誤消息 [!DNL Identity Service]。 請與系統管理員聯繫以解決此問題。
...
以下是與 [!DNL Identity Service] 使用 [資料接收API](https://www.adobe.io/experience-platform-apis/references/data-ingestion/)。
...
當接收的記錄顯示其關聯的命名空間不存在或IMS組織無法訪問的標識時，將顯示此錯誤。
...
在接收批處理資料時，當您的IMS組織尚未為其設定適當的權限時，將顯示此錯誤消息 [!DNL Identity Service]。 請與系統管理員聯繫以解決此問題。
