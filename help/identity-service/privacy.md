...
>[!NOTE]
>
>本指南僅涵蓋如何在Experience Platform中向Identity資料存放區提出隱私權請求。 如果您也計畫提出Experience Platform資料湖或[!DNL Real-Time Customer Profile]的隱私權請求，請參閱資料湖[中](../catalog/privacy.md)隱私權請求處理指南，以及本教學課程以外的[設定檔隱私權請求處理指南](../profile/privacy.md)。
>
...
以下各節概述如何使用[!DNL Identity Service] API或UI為[!DNL Privacy Service]提出隱私權請求。 在閱讀這些章節之前，強烈建議您檢閱[Privacy Service API](../privacy-service/api/getting-started.md)或[Privacy Service UI](../privacy-service/ui/overview.md)檔案，以取得有關如何提交隱私權工作的完整步驟，包括如何在請求裝載中正確格式化使用者資料。

### 使用 API

在API中建立工作請求時，在userIDs內提供的任何ID都必須使用特定的名稱空間和型別。 必須為名稱空間值提供身分服務可辨識的有效身分名稱空間。 將`standard`用於標準名稱空間，並將`custom`用於自訂名稱空間。

此外，請求承載的`include`陣列必須包含請求所針對的不同資料存放區的產品值。 向[!DNL Identity]提出請求時，陣列必須包含值`Identity`。

以下請求會根據GDPR為[!DNL Identity]存放區中的單一客戶資料建立新的隱私權工作。 在`userIDs`陣列中為客戶提供了兩個身分值；一個使用標準`Email`身分名稱空間，另一個使用`ECID`名稱空間，它同時包含[!DNL Identity]陣列中`Identity` (`include`)的產品值：
...
在UI中建立工作請求時，請務必在&#x200B;**[!UICONTROL Identity]**&#x200B;下選取&#x200B;**[!UICONTROL Products]**，以處理儲存在[!DNL Identity Service]中之資料的工作。
...
視您是否也在身分服務的隱私權要求(`ProfileService`)中包含即時客戶設定檔(`aepDataLake`)和資料湖(`identity`)作為產品而定，與身分相關的不同資料集可能會在不同時間從系統中移除：
