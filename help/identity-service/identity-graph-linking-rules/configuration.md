...
如果您的資料需要這些資料，您必須先為組織建立適當的名稱空間。 如需如何建立自訂名稱空間的步驟，請閱讀以下指南： [在UI中建立自訂名稱空間](../features/namespaces.md#create-custom-namespaces).

## 使用圖表模擬工具 {#graph-simulation}

接下來，導覽至 [圖表模擬工具](./graph-simulation.md) 在Identity Service UI工作區中。 您可以使用圖表模擬工具來模擬使用各種不同唯一名稱空間和名稱空間優先順序設定所建立的身分圖表。

透過建立不同的設定，您可以使用圖表模擬工具來學習和更好地瞭解身分最佳化演演算法和特定設定如何影響您的圖表行為。

## 設定身分設定 {#identity-settings}

當您更清楚想要圖表的行為方式後，請導覽至 [身分設定工具](./identity-settings-ui.md) 在Identity Service UI工作區中。
...
如需如何建立結構的說明，請閱讀以下指南： [在UI中建立XDM結構描述](../../xdm/tutorials/create-schema-ui.md).
...
如需任何意見回饋，請使用 **[!UICONTROL Beta意見反應]** Identity Service UI工作區中的選項。