...
若要深入瞭解名稱空間優先順序及其完整的功能和用法，請參閱 [名稱空間優先順序指南](./namespace-priority.md).
...
![將身分最佳化演演算法視覺化的圖表。](../images/ido.png)
...
在此範例中，CRM ID和電子郵件都被指定為不重複的名稱空間。 在 `timestamp=0`，CRM記錄資料集會擷取並因為唯一的名稱空間設定而建立兩個不同的圖形。 每個圖表都包含CRM ID和電子郵件名稱空間。
