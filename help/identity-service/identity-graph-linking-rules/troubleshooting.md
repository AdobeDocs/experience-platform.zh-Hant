...
* 在此案例中，傳入的體驗事件包含2個CRMID和1個GAID。 在這些名稱空間之間，CRMID會設定為具有最高名稱空間優先順序的名稱空間。 不過，因為有2個CRMID，設定檔&#x200B;**不會**&#x200B;儲存此體驗事件。

**案例3： 1個CRMID、2個GAID**

* 在此案例中，傳入的體驗事件包含1個CRMID和2個GAID。 在這些名稱空間之間，CRMID會設定為具有最高名稱空間優先順序的名稱空間。 由於只有一個CRMID，設定檔將會擷取體驗事件，因為只有一個具有最高名稱空間優先順序的名稱空間身分。
