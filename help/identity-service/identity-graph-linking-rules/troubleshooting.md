...
在名稱空間優先順序的內容中，設定檔將拒絕任何在&#x200B;**給定的傳入事件**&#x200B;中包含兩個以上具有最高名稱空間優先順序之身分的事件。 例如，假設您的身分設定是依照以下方式設定：
...
* 在此案例中，傳入的體驗事件包含2個GAID和1個ECID。 在這些名稱空間之間，GAID會設定為具有最高名稱空間優先順序的名稱空間。 不過，因為有2個GAID，設定檔&#x200B;**不會**&#x200B;儲存此體驗事件。

**案例2： 2個CRMID，1個GAID**

* 在此案例中，傳入的體驗事件包含2個CRMID和1個GAID。 在這些名稱空間之間，CRMID會設定為具有最高名稱空間優先順序的名稱空間。 不過，因為有2個GAID，設定檔&#x200B;**不會**&#x200B;儲存此體驗事件。

**案例3： 1個CRMID、2個GAID**

* 在此案例中，傳入的體驗事件包含1個CRMID和2個GAID。 在這些名稱空間之間，CRMID會設定為具有最高名稱空間優先順序的名稱空間。 由於只有一個CRMID，設定檔將會擷取體驗事件，因為只有一個名稱空間執行個體具有最高的名稱空間優先順序。
...
您也可以在UI[中使用](./graph-simulation.md)圖形模擬工具來模擬事件，並設定您自己的唯一名稱空間和名稱空間優先順序設定。 這麼做有助於您基本瞭解「身分最佳化演演算法」應有的行為。
...
| 分段 | 建立四個區段定義（**注意**：每一對區段定義都應該使用批次評估一個區段定義，並使用另一個串流評估。） <ul><li>區段定義A：根據John的已驗證事件和/或屬性的區段資格。</li><li>區段定義B：根據Jane的已驗證事件和/或屬性的區段資格。</li></ul> | 無論共用裝置情況為何，John和Jane都應該符合各自的區段資格。 |
