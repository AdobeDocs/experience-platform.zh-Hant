...
在Experience Platform UI中，選取左側導覽中的&#x200B;**[!UICONTROL Identities]**，然後從標題的索引標籤清單中選取&#x200B;**[!UICONTROL Identity Graph]**。

![Experience Platform UI中的身分工作區，已選取身分圖表索引標籤。](../images/graph-viewer/identity-graph.png)

若要檢視身分圖表，請提供身分名稱空間及其對應的值，然後選取&#x200B;**[!UICONTROL View]**。
...
身分圖表會顯示連結至您輸入的身分名稱空間和值組合的所有身分。 每個節點都包含身分名稱空間及其對應的值。 您可以選取、保持和拖曳任何節點，與圖形互動。 或者，您可以將滑鼠停留在節點上，檢視其對應身分值的相關資訊。 選取&#x200B;**[!UICONTROL View graph]**&#x200B;以隱藏或顯示圖表。
...
[!UICONTROL Identities]表格提供您身分資料的不同檢視，以表格格式列出身分名稱空間和身分值組合。 在圖形中選取節點將會更新[!UICONTROL Identities]表格中醒目提示的條列專案。

![身分資料表，其身分清單連結在圖形內。](../images/graph-viewer/identities-table.png)

使用下拉式選單來排序圖表資料，並反白顯示特定身分名稱空間的資訊。 例如，從功能表選取&#x200B;**[!UICONTROL Email]**&#x200B;以檢視電子郵件身分名稱空間的特定資料。
...
您也可以使用&#x200B;*[!UICONTROL Data source]*&#x200B;來檢視有助於您身分識別的資料來源清單。 選取[!UICONTROL Data source]以取得資料集和批次ID的表格檢視。

![資料來源標籤已選取。](../images/graph-viewer/data-source-table.png)

使用滑桿來依首次建立身分的時間篩選圖表資料。 依預設，身分圖表檢視器會顯示圖表內連結的所有身分。 按住並拖曳滑桿，將時間調整至新身分連結至圖表的最後一個時間戳記。 在以下範例中，圖表顯示最近建立的身分連結(GAID)位於&#x200B;**[!UICONTROL 08/19/2020, 4:29:29 PM]**。

![已選取圖表檢視器時間戳記滑桿。](../images/graph-viewer/slider-one.png)

調整滑桿以檢視已在&#x200B;**[!UICONTROL 08/19/2020, 4:25:30 PM]**&#x200B;上建立另一個身分連結（電子郵件）。

![圖表檢視器時間戳記滑桿已調整為上次建立的新連結。](../images/graph-viewer/slider-two.png)

您也可以調整滑桿來檢視圖形的最早版序。 在下列範例中，身分圖表檢視器顯示圖表首次建立於&#x200B;**[!UICONTROL 08/19/2020, 4:11:49 PM]**，其第一個連結為ECID、電子郵件和電話。
...
您也可以使用資料集介面來存取身分圖表檢視器。 從資料集[!UICONTROL Browse]頁面，選取您要互動的資料集，然後選取&#x200B;**[!UICONTROL Preview dataset]**
