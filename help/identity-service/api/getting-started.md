...
的 [!DNL Identity Service] API採用區域特定的端點，需要包含 `{REGION}` 作為請求路徑的一部分。 在配置IMS組織期間，確定一個區域並將其儲存在IMS組織配置檔案中。 使用每個端點的正確區域可確保使用 [!DNL Identity Service] API被路由到相應區域。
...
如果您無法在IMS組織配置檔案中找到該區域，請與系統管理員聯繫以獲得支援。
