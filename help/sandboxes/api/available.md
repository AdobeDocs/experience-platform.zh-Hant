...
成功的回應會傳回目前使用者可用的沙箱清單，包括`name`、`title`、`state`和`type`等詳細資訊。
...
| `title` | 沙箱的顯示名稱。 |
| `state` | 沙箱的目前處理狀態。 沙箱的狀態可以是下列任一專案： <ul><li>`creating`：沙箱已建立，但系統仍在布建中。</li><li>`active`：沙箱已建立且作用中。</li><li>`failed`：由於發生錯誤，沙箱無法由系統布建，因此已停用。</li><li>`deleted`：沙箱已手動停用。</li></ul> |
| `type` | 沙箱型別：「開發」或「生產」。 |
