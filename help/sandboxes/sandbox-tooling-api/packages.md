...
此 `/packages` 沙箱工具API中的端點可讓您以程式設計方式管理組織中的套件，包括發佈套件和將套件匯入沙箱。

## 建立套件 {#create}

您可以透過向以下專案發出POST請求，建立多成品套件： `/packages` 端點，同時提供封裝名稱和封裝型別的值。
...
| `packageType` | 封裝型別為 **部分** 表示您正在封裝中包含特定成品。 | 字串 | 是 |
| `sourceSandbox` | 套件的來源沙箱。 | 字串 | 無 |
| `expiry` | 定義封裝到期日期的時間戳記。 預設值是從建立日期算起的90天。 回應到期欄位將會是epoch UTC時間。 | 字串（UTC時間戳記格式） | 無 |
| `artifacts` | 要匯出至封裝的成品清單。 此 `artifacts` 值應為 **null** 或 **空白**，當 `packageType` 是 `FULL`. | 陣列 | 無 |
...
您可以透過向發出PUT請求來更新套件 `/packages` 端點。

### 將成品新增至封裝 {#add-artifacts}

若要將成品新增至封裝，您必須提供 `id` 並包含 **新增** 針對 `action`.
...
| `artifacts` | 要新增到封裝中的成品清單。 如果清單為，封裝將不會有變更 **null** 或 **空白**. 成品在新增至封裝之前，會先去除重複專案。 | 陣列 | 無 |
...
若要從封裝中刪除成品，您必須提供 `id` 並包含 **DELETE** 針對 `action`.
...
| `artifacts` | 要從封裝中刪除的成品清單。 如果清單為，封裝將不會有變更 **null** 或 **空白**. | 陣列 | 無 |
...
>[!NOTE]
>
>此 **更新** 動作是用來更新封裝的封裝中繼資料欄位，以及 **無法** 用於將成品新增/刪除至封裝。

若要更新封裝中的中繼資料欄位，您必須提供 `id` 並包含 **更新** 針對 `action`.
...
| `sourceSandbox` | 來源沙箱應屬於請求標頭中指定的相同組織。 | 字串 | 是 |
...
若要刪除套件，請向以下發出DELETE請求： `/packages` 端點，並指定您要刪除之套件的ID。
...
為了啟用將套件匯入沙箱，您必須發佈它。 向發出GET要求 `/packages` 端點，同時指定您要發佈的套件ID。
...
您可以透過向以下網址發出GET請求來查詢個別套件： `/packages` 端點包含要求路徑中封裝的對應ID。
...
您可以透過向以下網站發出GET請求，列出組織中的所有套件 `/packages` 端點。
...
| 參數 | 說明 |
| --- | --- |
| {QUERY_PARAMS} | 篩選結果的選用查詢引數。 請參閱以下小節： [查詢引數](./appendix.md) 以取得詳細資訊。 |

**要求**

以下請求會根據 {QUERY_PARAMS}.
...
回應中會傳回衝突。 回應會顯示原始套件加上 `alternatives` 以陣列形式依排名排序。
...
一旦您檢閱了衝突並透過向以下網站發出POST請求提供了替代，即可提交套件的匯入 `/packages` 端點。 結果會以承載的形式提供，這會開始針對在承載中指定的目的地沙箱匯入作業。
...
藉由向發出POST請求，列出封裝中匯出物件的所有相依物件。 `/packages` 端點，同時指定封裝的ID。
...
您可以透過向以下發出GET請求，檢查您是否擁有匯入套件成品的許可權： `/packages` 端點，同時指定套件的ID和目標沙箱名稱。
...
您可以透過向以下網站發出GET請求，列出目前的匯出/匯入作業： `/packages` 端點。
