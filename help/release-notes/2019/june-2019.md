...
| 排程 | 與[!DNL Experience Platform]協調服務整合，以透過API使用使用者定義的排程來自動化模型訓練、評分和功能管道。 |
...
| 根據設定檔和非設定檔資料的決定規則 | 與[!DNL Real-Time Customer Profile]緊密整合可讓開發人員建立運用設定檔資料的決定規則。 您不僅可以透過設定檔屬性來做出決策，也可以根據設定檔的體驗事件歷史記錄，以及根據與使用者身分無關的商業實體（例如流量條件、產品詳細目錄）來做出決策。 任何結構描述存在於[!DNL Experience Data Model]中的[!DNL Schema Registry] (XDM)實體均可用於決定規則。 規則是第一類實體，可重複用於任何決定選項和活動。 |
