...
description: Adobe Experience Platform 2020 年 10 月版本注意事項。
...
| 功能 | 說明 |
| ------- | ----------- |
| SFTP的SSH驗證支援 | 您可以使用RSA/DSA Open SSH金鑰將您的SFTP帳戶連線至[!DNL Platform]。 如需詳細資訊，請參閱[SFTP總覽](../../sources/connectors/cloud-storage/sftp.md)。 |
...
升級可用性和工作流程，並提供現成的深入分析、標準化，以及結構描述組合中關鍵欄位的透明度。 針對以「聯合結構描述」代表的個別資料模型組合公開資料譜系，提供結構和成分對即時客戶個人檔案的深入分析。
...
確保資料流程的透明度，並提供來源聯結器進入系統資料流量健康狀況的深入分析，提供更自助的服務和更快的疑難排解情況可操作性。
