...
| 功能 | 說明 |
| -----------| ---------- |
| 自動執行 [!DNL Real-time Customer Data Platform] | 現在，當發生違反動作(包括將區段啟 [!DNL Real-time CDP] 用至目標)時，資料使用原則會自動強制執行。 觸發原則違規時，使用者即時檢視啟動工作流程中的使用限制，指出他們無法使用哪些資料及原因。<br><br>如需詳細資訊，請參 [閱中的概述中](../../rtcdp/privacy/data-governance-overview.md#enforce-data-usage-compliance) ，關於強制資料使 [!DNL Data Governance] 用規範的 [!DNL Real-time CDP] 章節。 |
...
| 自訂資料使用標籤 | 您現在可以使用原則服務API或在UI中建立自訂資料使用標籤。 See the [labels overview](../../data-governance/labels/overview.md) for more information. |
