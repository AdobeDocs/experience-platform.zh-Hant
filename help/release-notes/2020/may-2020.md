...
- [Experience PlatformWeb SDK和Experience PlatformEdge Network](#edge)
...
如需[!DNL Platform Web SDK]和[!DNL Edge Network]的詳細資訊，請參閱[檔案](../../web-sdk/home.md)。
...
| 大量GET端點 | 已將大量GET端點新增至[!DNL Real-Time Customer Profile] API，以便在單一API呼叫中擷取多個結果。 您現在可以為區段定義、區段作業和合併原則批次GET最多100個ID。 |
| 依身分瀏覽設定檔 | 您現在可以在[!DNL Platform] UI中選取身分名稱空間並提供身分值，以便瀏覽設定檔。 |
