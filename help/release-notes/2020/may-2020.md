...
| 配方 | 新的PySpark和[!DNL Spark]配方現在會遵循類似[!DNL Docker]和R配方的[!DNL Python]工作流程。 |
...
| Adobe Audience Manager到[!DNL Experience Platform]資料共用 | 已更新[!DNL Audience Manager]聯結器的布建程式。 [!DNL Audience Manager]的[!DNL Real-Time Customer Profile]資料集現在預設為停用。 您可以手動選擇要提升至[!DNL Profile]的資料集。 新的預設設定不具可回溯性，只會影響新[!DNL Audience Manager]聯結器的布建。 請參閱[資料集使用手冊](../../catalog/datasets/user-guide.md)中的詳細資訊。 |
