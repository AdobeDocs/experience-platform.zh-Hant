...
| 功能 | 說明 |
| --- | --- |
| 授權權益更正 | 已更新客戶的[!UICONTROL License Usage]儀表板和個別授權資料，以解決銷售訂單中記錄的授權權益與最初在Experience Platform [!UICONTROL License Usage]儀表板中報告的授權權益之間以前的不一致問題。 個別授權資料的更新會在 2023 年 6 月至 2023 年 6 月 11 日之間分階段進行。您的實際使用值仍然準確。<br><br>Experience Platform 會提供多種管理使用量的功能：<br><ul><li>[檢閱並應用最佳做法管理資料和授權使用情況](https://experienceleague.adobe.com/docs/experience-platform/landing/license/data-management-best-practices.html)。</li><li>套用篩選規則和條件以[選擇性地將擷取的資料包含至即時客戶輪廓或從其中排除](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics.html#filtering-for-profile)，以便在將其擷取至輪廓之前用於 Analytics 資料。</li><li>[若要為匿名輪廓套用期限，請和 Adobe 支援人員聯絡。](https://experienceleague.adobe.com/docs/experience-platform/profile/pseudonymous-profiles.html)</li><li>[若要在所需資料集上啟用體驗事件期限，請和 Adobe 支援人員聯絡。](https://experienceleague.adobe.com/docs/experience-platform/profile/event-expirations.html)</li><li>若要提高您的授權權益，請和您的 Adobe 代表討論有哪些選項。</li></ul> |
...
[!DNL Destinations] 是預先建立的和目標平台的整合，可讓來自 Adobe Experience Platform 的資料順暢啟動。您可以使用目標啟用已知和未知的資料，以進行跨通路行銷活動、電子郵件行銷活動、定向廣告和其他諸多使用案例。

**新目的地** {#new-destinations}

| 目標 | 說明 |
| ----------- | ----------- |
| **[[!UICONTROL Mailchimp Interest Categories]](../../destinations/catalog/email-marketing/mailchimp-interest-categories.md)** | **[!UICONTROL Mailchimp]**&#x200B;是熱門的行銷自動化平台和電子郵件行銷服務，企業使用郵寄清單和電子郵件行銷活動來管理聯絡人（客戶、客戶或其他感興趣的當事方）並與之交談。 可使用此連接器根據聯絡人的興趣和偏好將他們排序。 |
...
| 欄位群組 | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1721/files) | 中繼資料標籤的`partnerProspect`選項已新增至[!UICONTROL XDM Individual Prospect Profile]類別。 |
| 資料類型 | (多個) | 已為[[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1716/files)資料型別新增數個欄位。 |
| 資料類型 | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1716/files) | 已新增一個新欄位以表示是否發生重新導向。 |
| 欄位群組 | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1716/files) | 已新增和媒體報表相關的新欄位。 |
...
| 功能 | 說明 |
| --- | --- |
| 支援 Adobe Experience Cloud 應用程式 [!BADGE (Beta)]{type=Informative} 中的合作夥伴 ID | 身分識別服務中現在可使用合作夥伴 ID。合作夥伴 ID 指資料合作夥伴用於代表人員的識別碼。在 Real-Time Customer Data Platform 中，合作夥伴 ID 主要用於展開的客群啟動和資料擴充。合作夥伴 ID 不會儲存在身分識別圖譜中。如需詳細資訊，請閱讀有關[身分識別類型](../../identity-service/features/namespaces.md#identity-types)的文件。 |
...
| 草稿模式的 UI 支援 | 您現在可在來源工作流程期間透過使用者介面暫停和儲存您的進度。您可以在工作流程的資料流詳細資料、對應和排程步驟期間選取&#x200B;**[!UICONTROL Save as draft]**，以將您的資料流儲存為草稿以供稍後完成。 如需詳細資訊，請至「[在 UI 中將資料流另存為草稿](../../sources/tutorials/ui/draft.md)」詳閱指南。 |
