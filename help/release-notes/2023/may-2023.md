...
| 功能 | 說明 |
| --- | --- |
| 計算 ADLS 資料集的欄層級統計資料 | 此 `ANALYZE TABLE` 命令已擴充為 `COMPUTE STATISTICS` SQL命令。 您現在可以計算 ADLS 資料集的子集或該資料集中特定欄的統計資料。如需詳細資訊，請閱讀[資料集統計資料計算指南](../../query-service/essential-concepts/dataset-statistics.md)。 |
