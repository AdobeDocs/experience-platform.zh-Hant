...
Adobe Experience Platform 會提供一組豐富的功能，用於擷取任何類型和任何延遲的資料。您可以使用批次或串流 API (使用 Adobe 建置的來源、資料整合合作夥伴或 Adob&#x200B;&#x200B;e Experience Platform UI) 進行擷取。
...
| 功能 | 說明 |
| ----------- | ----------- |
| 透過 [Adobe Target](../../destinations/catalog/personalization/adobe-target-connection.md) 和[自訂個人化](../../destinations/catalog/personalization/custom-personalization.md)目的地的屬性型個人化正式推出。 | 透過 Adob&#x200B;&#x200B;e Target 或 Experience Platform 中的其他自訂個人化目的地，即時利用設定檔屬性實現一對一的 Web 和行動個人化。如需更多詳細資料，請閱讀[專屬文件](../../destinations/ui/activate-edge-personalization-destinations.md)和[常見問題集](/help/destinations/destinations-faq.md#same-next-page-personalization)。 |
...
| 功能 | 說明 |
| --- | --- |
| 支援 Adob&#x200B;&#x200B;e Experience Cloud 應用程式 [!BADGE (Beta)] 中的合作夥伴 ID{type=Informative} | 身分識別服務中現在可使用合作夥伴 ID。合作夥伴 ID 指資料合作夥伴用於代表人員的識別碼。在 Real-Time Customer Data Platform 中，合作夥伴 ID 主要用於展開的對象啟動和資料擴充。身分識別圖中不會儲存合作夥伴 ID。如需詳細資訊，請閱讀有關[身分類型](../../identity-service/namespaces.md#identity-types)的文件。 |
...
| 功能 | 說明 |
| --- | --- |
| 計算 ADLS 資料集的欄層級統計資料 | 此 `ANALYZE TABLE` 命令已透過 `COMPUTE STATISTICS` 和 `SHOW STATISTICS` SQL 命令擴展。您現在可以計算 ADLS 資料集的子集或該資料集中特定欄的統計資料。如需詳細資訊，請閱讀[資料集統計資料計算指南](../../query-service/essential-concepts/dataset-statistics.md)。 |
