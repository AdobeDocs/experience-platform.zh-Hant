...
| 功能 | 說明 |
| --- | --- |
| 支援Adobe Experience Cloud應用程式[!BADGE Beta]{type=Informative}中的合作夥伴ID | 身分識別服務中現在可使用合作夥伴 ID。合作夥伴 ID 指資料合作夥伴用於代表人員的識別碼。在 Real-Time Customer Data Platform 中，合作夥伴 ID 主要用於展開的對象啟動和資料擴充。合作夥伴 ID 不會儲存在身分識別圖譜中。如需詳細資訊，請閱讀有關[身分類型](../../identity-service/features/namespaces.md#identity-types)的文件。 |
