...
| 功能 | 說明 |
| --- | --- |
| 「警示歷史記錄」索引標籤 | 警報[!UICONTROL History]標籤現在將包含所有事件，包括延遲、開始、成功和失敗。 請閱讀[警示UI檔案](../../observability/alerts/ui.md)，以取得有關[歷程記錄]索引標籤的詳細資訊。 |
...
| 類型 | 功能 | 說明 |
| --- | --- | --- |
| 資料串流 | 裝置查詢支援 | 設定資料流時，您現在可以選取要收集的裝置查詢資訊層級。 裝置查詢資訊包括裝置、硬體、作業系統以及用來與頁面互動的瀏覽器的相關資料。 <br>裝置查詢資訊無法連同使用者代理程式和使用者端提示一起收集。 選擇收集裝置資訊將會停用收集使用者代理程式和使用者端提示，反之亦然。 所有裝置查閱資訊都儲存在`xdm:device`欄位群組中。 從有關[設定資料串流](../../datastreams/configure.md#geolocation-device-lookup)的檔案瞭解更多資訊。 |
| 擴充功能 | [!DNL TikTok] 網頁事件 API 擴充功能 | [[!DNL TikTok] 網頁事件API](https://exchange.adobe.com/apps/ec/109834/tiktok-web-events-api)擴充功能可讓您運用Adobe Experience Platform Edge Network中擷取的資料，並使用[!DNL TikTok]網頁事件API，以伺服器端事件的形式將其傳送至[!DNL TikTok]。 |
...
| 功能 | 說明 |
| --- | --- |
| 適用於協力廠商資料的新合作夥伴生態系統標籤 | 提供適用於協力廠商擴充和潛在客戶的新資料使用標籤。 如需詳細資訊，請參閱合作夥伴生態系統標籤[上的](../../data-governance/labels/reference.md#partner)檔案。 |
...
Experience Platform提供一套資料檢疫功能，可讓您透過程式化刪除消費者記錄和資料集，以管理儲存的資料。 在UI中使用[!UICONTROL Data Lifecycle]工作區或透過呼叫資料衛生API，您就能有效管理資料存放區。 利用這些功能可確保資訊如預期般使用、在不正確的資料需要修正時更新資訊，以及在組織原則認為必要時刪除資訊。
...
| 目標 | 全新或更新 | 說明 |
| ----------- |----------------|----------- |
| [[!DNL LiveRamp - Distribution]](../../destinations/catalog/advertising/liveramp-distribution.md) | 新增 | 啟用先前在行動裝置、網頁、顯示和連線電視媒體中上線至[!DNL LiveRamp]之進階發行者的對象。 <br>透過[!DNL LiveRamp]LiveRamp — 上線[連線將對象上線到您的](../../destinations/catalog/advertising/liveramp-onboarding.md)帳戶後，請使用新的[[!DNL LiveRamp - Distribution]](../../destinations/catalog/advertising/liveramp-distribution.md)連線將對象啟動到下游目的地。 |
...
| [[!DNL Microsoft Dynamics 365]](../../destinations/catalog/crm/microsoft-dynamics-365.md) | 已更新 | 新增對未在[!DNL Dynamics 365]中預設方案內建立之自訂欄位的[!DNL Dynamics 365]自訂欄位首碼的支援。 已在&#x200B;**[!UICONTROL Customization Prefix]**&#x200B;填寫目的地詳細資料[步驟中新增輸入欄位](#destination-details)。 |
...
| 功能 | 說明 |
| ----------- | ----------- |
| Real-Time CDP中的資料匯出 | [資料集匯出](../../destinations/ui/export-datasets.md)功能現在已可供一般使用。 檢視您可根據所購買的Experience Platform應用程式[匯出哪些資料集](../../destinations/ui/export-datasets.md#datasets-to-export)，並檢查匯出資料集的[護欄](/help/destinations/guardrails.md#dataset-exports)。 |
...
| 元件類型 | 名稱 | 說明 |
| --- | --- | --- |
| 資料類型 | [[!UICONTROL Return]](https://github.com/adobe/xdm/pull/1773/files) | RMA （退貨授權）簽發。 |
| 資料類型 | [[!UICONTROL Return Item]](https://github.com/adobe/xdm/pull/1773/files) | RMA （退貨授權）中退回的料號資訊。 |
...
| 元件類型 | 名稱 | 更新說明 |
| --- | --- | --- |
| 擴充功能 | [!UICONTROL AJO Entity Fields] | [[!UICONTROL flag for multi-variant]](https://github.com/adobe/xdm/pull/1774/files)已新增至[!UICONTROL AJO Entity Fields]以識別變體是否為多變體。 |
| 資料類型 | [!UICONTROL Product list item] | 已新增[[!UICONTROL Return Item]](https://github.com/adobe/xdm/pull/1773/files)以包含退貨授權資訊。 |
| 資料類型 | 訂單 | 已新增[[!UICONTROL Return Info]](https://github.com/adobe/xdm/pull/1773/files)，以包含發行的RMA （退貨授權）。 |
...
| 身分識別圖譜限制的變更 | 身分圖表限制已從150個身分變更為50個身分。 將新身分擷取到完整圖表時，會刪除根據擷取時間戳記和身分型別的最舊身分。 Cookie身分型別會優先刪除。 如果您的生產沙箱包含： <ul><li>個人識別碼 (例如 CRM ID) 設定為 cookie/裝置身分識別類型的自訂命名空間。</li><li>cookie/裝置識別碼被設定為跨裝置身分識別類型的自訂命名空間。</li></ul> Adobe 工程部門將手動處理這些要求。如需詳細資訊，請閱讀[身分識別服務資料的護欄](../../identity-service/guardrails.md)以及[資料管理授權權益最佳實務](../../landing/license-usage-and-guardrails/data-management-best-practices.md)的指南。 |
...
| 功能 | 說明 |
| --- | --- |
| 自助來源（批次SDK）中`offset`分頁的新引數 | 使用`endConditionName`分頁時，您現在可以為您的來源指定`endConditionValue`和`offset`。 這些引數可讓您指定將在下一個HTTP請求中結束分頁回圈的條件。 如需詳細資訊，請閱讀[自助式來源（批次SDK）分頁指南](../../sources/sources-sdk/config/sourcespec.md#pagination)。 |
