...
| 功能 | 說明 |
| --- | --- |
| 多資料集記錄刪除（僅限API） | 刪除單一API請求中一個、多個或所有資料集的身分，簡化資料衛生工作流程。 您也可以將刪除限製為僅限設定檔服務，保留Data Lake記錄不變。 如需詳細資訊，請參閱[記錄刪除工單指南](../../hygiene/api/workorder.md)。 |
...
容量提供組織[護欄](../../rtcdp/guardrails/overview.md)的完整檢視，並提供如何在沙箱層級配置容量以解決潛在容量違規的建議。

**新功能**

| 功能 | 說明 |
| ------- | ----------- |
| 邊緣細分輸送量 | 您現在可以檢視和管理與邊緣分段輸送量相關的護欄。 如需詳細資訊，請閱讀[容量概觀](/help/landing/license-usage-and-guardrails/capacity.md#edge-segmentation-throughput)。 |
| 支援監控邊緣資料串流 | 現在提供邊緣資料串流的即時監視，可提供輸送量和其他量度的透明度。 如需詳細資訊，請閱讀[監視邊緣指南](/help/dataflows/ui/monitor-edge.md)。 |
