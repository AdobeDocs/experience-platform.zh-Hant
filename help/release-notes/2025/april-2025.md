...
| 功能 | 說明 |
| --- | --- |
| [!BADGE 有限可用性]{type=Informative} [!DNL Identity Graph Linking Rules] | 開發沙箱中的所有客戶現在都能存取身分圖表連結規則。 <ul><li>**啟用需求**：在您設定並儲存[!DNL Identity Settings]之前，此功能將保持非使用中狀態。 若沒有此設定，系統將繼續正常運作，且行為不會有任何變更。</li><li>**重要附註**：在此「有限可用性」階段期間，Edge區段可能會產生非預期的區段會籍結果。 不過，串流和批次區段將如預期運作。</li><li>**後續步驟**：如需如何在生產沙箱中啟用此功能的詳細資訊，請聯絡您的Adobe客戶團隊。</li></ul> |
