...
| 功能 | 說明 |
| --- | --- |
| Sandbox tooling plugin support expansion | 在沙箱工具中複製Journey物件時，現在可以將自訂動作復製為相依物件。 此外，您可以選取要在目標沙箱中重複使用的現有動作。 它們也可以獨立新增到套件中。 如需有關受支援的Adobe Journey Optimizer物件的完整資訊，請閱讀[沙箱工具](../../sandboxes/ui/sandbox-tooling.md#adobe-journey-optimizer-objects)指南。 |
