...
目的地是預先建立的與目的地平台的整合，可無縫地從Adobe Experience Platform啟用資料。 您可以使用目標來啟用已知和未知的資料，以供跨通道行銷活動、電子郵件行銷活動、定向廣告及其他許多使用案例使用。
...
| [支援重複資料刪除索引鍵](../../destinations/ui/activate-batch-profile-destinations.md#deduplication-keys) | 選取重複資料刪除金鑰，以消除匯出檔案中同一設定檔的多筆記錄。您可以選取單一名稱空間或最多兩個XDM結構描述屬性作為重複資料刪除索引鍵。 |
...
| [!DNL Salesforce Marketing Cloud] (Beta) | 您現在可以使用[!DNL Salesforce Marketing Cloud] API或UI將[!DNL Flow Service]連線至Experience Platform。 如需詳細資訊，請參閱[[!DNL Salesforce Marketing Cloud] 聯結器總覽](../../sources/connectors/marketing-automation/salesforce-marketing-cloud.md)。 |
...
| 前一個詞語 | 更新詞語 |
| --- | --- |
| Experience Platform Launch （使用者端） | [標記](/help/tags/home.md) |
| Experience Platform Launch Server Side | [事件轉送](/help/tags/ui/event-forwarding/overview.md) |
