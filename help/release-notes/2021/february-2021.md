...
Adobe Experience Platform使您能夠為您的客戶提供協調、一致和相關的體驗，無論客戶在何處或何時與您的品牌進行交互。 通過即時客戶概要資訊，您可以看到每個客戶的整體視圖，該視圖將來自多個渠道的資料組合在一起，包括線上、離線、CRM和第三方資料。 [!DNL Profile] 允許您將客戶資料整合到一個統一視圖中，為每次客戶交互提供一個可操作且時間戳記的帳戶。

**新功能**

| 功能 | 說明 |
| ------- | ----------- |
| 計算屬性(Alpha) | ***注：此功能當前位於Alpha中，並且不適用於所有用戶。 文件和功能可能會有所變更。*** <br/><br/>計算屬性是用於將事件級資料聚合到配置檔案級屬性中的函式。 然後，可以在分段、激活和個性化中使用聚合。 這些函式的一些示例包括count 、 sum 、 average 、 min 、 max 、 true/false。 計算屬性當前僅通過API可用。 有關詳細資訊，請參見 [計算屬性概述](../../profile/computed-attributes/overview.md)。 |
