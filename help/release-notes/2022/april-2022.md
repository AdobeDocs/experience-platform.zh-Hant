...
| [!UICONTROL 所有目的地的啟用對象] | 目的地 | 此Widget提供在單一量度中跨所有目的地啟用的對象總數。 |
...
對於Amazon Kinesis、Azure事件中樞和HTTP API目的地，在發行程式（從4月27日開始）期間，您會在目的地目錄中看到原來的Beta目的地卡片和新的一般可用(GA)目的地卡。 客戶使用Beta版目的地設定的任何資料流將在未來幾天移轉至相同目的地的GA版本。 此移轉作業應於4月29日星期五當天結束前完成。 在這段短時間內，Beta目的地會繼續顯示，並標示為&#x200B;**已棄用**。
...
如需[!DNL Intelligent Services]的詳細資訊，請參閱[[!DNL Intelligent Services] 總覽](../../intelligent-services/home.md)。
...
若要深入瞭解Real-time Customer Data Platform B2B版本，請參閱[B2B概觀](../../rtcdp/b2b-overview.md)。
...
| 功能 | 說明 |
| --- | --- |
| 支援 [!DNL OneTrust Integration] | 您現在可以使用[!DNL OneTrust Integration]來源從您的[!DNL OneTrust]帳戶將同意和偏好設定資料擷取到Platform。 如需詳細資訊，請參閱有關[建立 [!DNL OneTrust Integration] 來源連線](../../sources/connectors/consent-and-preferences/onetrust.md)的檔案。 |
| 支援 [!DNL Square] | 您現在可以使用[!DNL Square]來源，將付款資料從您的[!DNL Square]帳戶擷取到Platform。 |
