...
| 欄位群組 | [[!UICONTROL 目前的天氣]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | 的欄位群組 [!UICONTROL XDM ExperienceEvent] 和 [!UICONTROL 天氣] 類別，用來擷取郵遞區號的目前天氣狀況。 |
| 欄位群組 | [[!UICONTROL 預報的天氣]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/forecasted-weather.schema.json) | 的欄位群組 [!UICONTROL XDM ExperienceEvent] 和 [!UICONTROL 天氣] 類別，用於擷取郵遞區號的預測天氣狀況。 |
...
| 欄位群組 | [[!UICONTROL 媒體互動詳情]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-media.schema.json) | 的欄位群組 [!UICONTROL XDM ExperienceEvent] 擷取媒體互動相關細節的類別。 |
...
| 預測性銷售機會和帳戶評分 | 預測性銷售線索和帳戶評分使用以樹狀結構（隨機森林/漸層提升）的機器學習方法，其中涉及從機會階段轉換事件中進行學習和預測，以及將個人活動彙總到帳戶層級，以產生帳戶分數。 彙總和單位層級也提供主要影響因素，以協助B2B行銷人員更瞭解哪些元素驅動分數。 <br><br>如需詳細資訊，請參閱以下檔案： [預測性銷售線索與帳戶評分](../../rtcdp/b2b-ai-ml-services/predictive-lead-and-account-scoring.md). 如需如何管理分數的相關資訊，請參閱以下指南： [在Real-time Customer Data Platform B2B版本中管理預測性銷售線索和帳戶評分。](../../rtcdp/b2b-ai-ml-services/manage-predictive-lead-and-account-scoring.md) |
