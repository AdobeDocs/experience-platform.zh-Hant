...
請參閱[Real-Time CDP，B2B版本總覽](../../rtcdp/b2b-overview.md)，深入瞭解可用的B2B功能，或[端對端教學課程](../../rtcdp/b2b-tutorial.md)，深入瞭解如何在B2B工作流程中建立帳戶設定檔。
...
| 功能 | 說明 |
| --- | --- |
| 透過Adobe Admin Console進行許可權管理 | 現在可透過Adobe Experience Platform資料收集資訊卡底下的Adobe Admin Console管理資料收集功能的存取權。 如需詳細資訊，請參閱[資料彙集許可權](../../collection/permissions.md)的指南。<br><br>資料串流的許可權現在也可透過Adobe Experience Platform資訊卡下的Admin Console來管理，比起先前為每個使用者手動設定這些許可權的方法，來提升安全性。 |
...
| 功能 | 說明 |
| --- | --- |
| [!DNL Data Prep] Recommendations的增強功能 | [!DNL Data Prep] Recommendations現在更聰明、更快。 新的驗證檢查可大幅減少最常見的對應錯誤，進一步縮短實現價值的時間。 |
...
| 功能 | 說明 |
| ----------- | ----------- |
| [立即匯出檔案(Beta)](../../destinations/ui/export-file-now.md) | 匯出完整的檔案，而不中斷先前排程區段的目前匯出排程。 除了先前排程的匯出外，此匯出作業不會變更區段的匯出頻率。 <br>檔案匯出會立即觸發，並從Experience Platform細分執行中擷取最新結果。<br> <br>請聯絡您的Adobe代表以取得此功能的存取權。 |
...
| 目的地 | 說明 |
| ----------- | ----------- |
| [Marketo V2](../../destinations/catalog/adobe/marketo-engage.md) | Marketo Engage目的地更新可讓您透過自動化簡化靜態清單建立流程，並讓使用者能夠在其潛在客戶上引入其他欄位。 請參閱下列Marketo V2中增強功能的詳細資訊： <br><ul><li>在啟動工作流程的&#x200B;**[!UICONTROL 排程區段]**&#x200B;步驟中，在Marketo V1中，您需要手動新增&#x200B;**對應ID**，才能成功將資料匯出至Marketo。 Marketo V2不再需要此手動步驟。</li><li>在啟動工作流程的&#x200B;**[!UICONTROL 對應]**&#x200B;步驟中，在Marketo V1中，您只能將XDM欄位對應到Marketo中的三個目標欄位： `firstName`、`lastName`和`companyName`。 透過Marketo V2版本，您現在可以將XDM欄位對應至Marketo中的更多欄位。 如需詳細資訊，請參閱Marketo V2](../../destinations/catalog/adobe/marketo-engage.md#supported-attributes)中的[支援屬性。  </li></ul> |
...
| [(Beta) [!DNL Snap Inc.]](../../destinations/catalog/advertising/snap-inc.md) | 此目的地可讓行銷人員將Experience Platform中建立的使用者區段匯入至Snapchat廣告，並使用這些區段來鎖定其廣告。 <br><br>此目的地目前在Beta中。 檔案和功能可能會有所變更。 |
...
| 預測性銷售線索與帳戶評分 | 預測性銷售線索和帳戶評分使用以樹狀結構（隨機森林/漸層提升）的機器學習方法，其中涉及從機會階段轉換事件中進行學習和預測，以及將個人活動彙總到帳戶層級，以產生帳戶分數。 彙總和單位層級也提供主要影響因素，以協助B2B行銷人員更瞭解哪些元素驅動分數。 <br><br>如需詳細資訊，請參閱有關[預測性銷售線索和帳戶評分](../../rtcdp/b2b-ai-ml-services/predictive-lead-and-account-scoring.md)的檔案。 Real-time Customer Data Platform如需有關如何管理分數的相關資訊，請參閱B2B Edition中[管理預測性銷售線索和帳戶評分的指南。](../../rtcdp/b2b-ai-ml-services/manage-predictive-lead-and-account-scoring.md) |

如需如何監視設定檔擴充的指南，請參閱[在UI](../../dataflows/ui/b2b/monitor-profile-enrichment.md)中監視設定檔擴充的檔案。 若要深入瞭解Real-Time CDP B2B版本，請參閱[Real-Time CDP B2B概觀](../../rtcdp/overview.md)。
...
| 功能 | 說明 |
| --- | --- |
| [!DNL Azure Data Explorer] 來源正式推出 | 使用AzureData Explorer來源將資料從您的[!DNL Azure]執行個體帶入Experience Platform。 如需詳細資訊，請參閱[[!DNL Azure Data Explorer] 來源概觀](../../sources/connectors/databases/data-explorer.md)。 |
| [!DNL Generic OData]來源的一般可用性 | 使用[!DNL Generic OData]來源從支援開放資料通訊協定的系統帶入Experience Platform資源。 如需詳細資訊，請參閱[[!DNL Generic OData] 來源概觀](../../sources/connectors/protocols/odata.md)。 |
| 支援在Experience PlatformUI中自動偵測[!DNL Data Landing Zone]的來源檔案內容 | 使用Experience PlatformUI時，[!DNL Data Landing Zone]來源現在支援自動偵測檔案屬性。 如需詳細資訊，請參閱有關[建立 [!DNL Data Landing Zone] 來源連線](../../sources/tutorials/ui/create/cloud-storage/data-landing-zone.md)的檔案。 |
