...
| 使用者定義的儀表板 | 使用者定義儀表板可讓您建置和管理自訂儀表板，有助於加速深入分析和自訂視覺效果。 透過使用者定義儀表板，您可以建立、新增和編輯自訂的Widget，以視覺化方式呈現與貴組織相關的關鍵量度。 閱讀 [功能指南](../../dashboards/user-defined-dashboards.md) 以深入瞭解。 |
...
| 資料類型 | (多個) | [已更新數個媒體屬性名稱](https://github.com/adobe/xdm/pull/1626/files) 資料型別之間的一致性。 |
