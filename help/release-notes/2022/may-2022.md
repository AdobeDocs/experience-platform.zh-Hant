...
| 功能 | 說明 |
| --- | --- |
| 資料集到期 | 為Platform資料集[排程到期時間](../../hygiene/ui/dataset-expiration.md)。 |
...
| 功能 | 名稱 | 說明 |
| --- | --- | --- |
| 已新增資源 | <ul><li> 存取控制原則 </li><li> 角色 </li><li> 稽核記錄 </li><li> 工單 </li><li> 身分命名空間 </li><li> 識別圖 </li><li> 查詢 </li><li> 資料集 </li><li> 來源資料流 </li></ul> | 活動發生時，會自動記錄稽核記錄資源。 如果已啟用此功能，您就不需要手動啟用記錄收集。 |
...
Experience Platform提供了一套技術，可讓您收集使用者端客戶體驗資料並將資料傳送至Adobe Experience PlatformEdge Network，在那裡可以擴充和轉換資料，並將其分發至Adobe或非Adobe目的地。
...
| 與資料流整合 | 使用Platform中的來源目錄來存取您的PlatformEdge Network資料，包括資料收集的資料準備以及改善對資料準備警告的支援。 如需詳細資訊，請參閱[Adobe資料收集來源概觀](../../sources/connectors/adobe-applications/data-collection.md)。 |
...
| 支援Adobe資料收集 | 使用Platform中的來源目錄來存取您的PlatformEdge Network資料，包括資料收集的資料準備以及改善對資料準備警告的支援。 如需詳細資訊，請參閱[Adobe資料收集來源概觀](../../sources/connectors/adobe-applications/data-collection.md)。 |
| 支援使用`ISO-8859-1`編碼擷取檔案 | 使用`encoding`引數，使用[!DNL Flow Service] API將雲端儲存空間來源的`ISO-8859-1`編碼檔案擷取到Platform。 如需詳細資訊，請參閱[建立雲端儲存空間來源連線](../../sources/tutorials/api/collect/cloud-storage.md)的指南。 |
