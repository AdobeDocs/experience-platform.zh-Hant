...
Experience Platform提供一套資料檢疫功能，可讓您透過程式化刪除消費者記錄和資料集，以管理儲存的資料。 在UI中使用[!UICONTROL Data Hygiene]工作區或透過呼叫資料衛生API，您可以管理資料存放區，以確保資訊如預期使用、在不正確的資料需要修正時更新，並在組織原則認為必要時刪除。
...
| 功能 | 說明 |
| ------- | ----------- |
| [!DNL Data Prep]中的屬性型存取控制 | 您現在只能對應您有權存取的屬性。 您無權存取的屬性無法用於傳遞對應和計算欄位。 如需詳細資訊，請參閱[中的 [!DNL Data Prep]](../../data-prep/home.md)屬性型存取控制。 **注意**：以屬性為基礎的存取控制目前在美國醫療保健客戶的限量版本中提供。 此功能在完整發行後，將可供所有Real-Time Customer Data Platform客戶使用。 |
...
| 元件類型 | 名稱 | 說明 |
| --- | --- | --- |
| 欄位群組 | [[!UICONTROL Change set]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/changeset.schema.json) | 擷取資料集的列層級變更。 此欄位群組可由任何類別使用。 |
| 欄位群組 | [[!UICONTROL Reference keys]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-reference-keys.schema.json) | 擷取ExperienceEvent綱要的參考索引鍵，讓您根據其他類別建立與綱要的關係。 |
...
| 元件類型 | 名稱 | 更新說明 |
| --- | --- | --- |
| 行為 | [[!UICONTROL Time-series Schema]](https://github.com/surbhi114/xdm/blob/master/components/behaviors/time-series.schema.json) | 更新`eventType`以包含數個與媒體相關的新事件型別，以及Adobe Journey Optimizer的Web頻道傳入使用案例。 |
| 全域結構描述 | [[!UICONTROL Destination]](https://github.com/tumulurik/xdm/blob/master/schemas/destinations/destination.schema.json) | 已從`xdm:destinationCategory`移除列舉值。 |
| 欄位群組 | [[!UICONTROL Record Status]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/record-status.schema.json) | 已將欄位群組狀態從`experimental`更新為`stable`。 |
| 欄位群組 | （多個） | 已更新多個B2B欄位群組，因此某些ID欄位已過時，而改用使用[[!UICONTROL B2B Source]](../../xdm/data-types/b2b-source.md)資料型別的索引鍵型別欄位。 先前的ID欄位將在未來的更新中被取代。 如需受影響欄位群組的完整變更清單，請參閱下列[提取要求](https://github.com/adobe/xdm/pull/1533/files#diff-720c0bb1d1cbaf622f5656c2a4b62d35830c75f6563794da72a280a6a520fbc1)。 |
| 資料類型 | [[!UICONTROL Browser details]](https://github.com/liljenback/xdm/blob/master/components/datatypes/browserdetails.schema.json) | 新增欄位`xdm:userAgentClientHints`，擷取使用者代理程式與瀏覽器互動的相關內容資訊。 |
| 資料類型 | [[!UICONTROL Media information]](https://github.com/lidiaist/xdm/blob/master/docs/reference/datatypes/media.schema.json) | 新增`xdm:playhead`欄位以擷取媒體內容的播放點時間。 修正`xdm:videoSegment`的模式驗證。 |
| 資料類型 | [[!UICONTROL Rating]](https://github.com/lidiaist/xdm/blob/master/components/datatypes/external/iptc/rating.schema.json) | `iptc4xmpExt:RatingSourceLink`不再是必要欄位。 |
...
| 支援使用`ISO-8859-1`編碼擷取檔案 | 使用`encoding`引數，透過`ISO-8859-1` API將雲端儲存空間來源的[!DNL Flow Service]個編碼檔案擷取至Experience Platform。 如需詳細資訊，請參閱[建立雲端儲存空間來源連線](../../sources/tutorials/api/collect/cloud-storage.md)的指南。 |
