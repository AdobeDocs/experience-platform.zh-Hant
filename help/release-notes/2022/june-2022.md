...
| 功能 | 說明 |
| ----------- | ----------- |
| (Beta)支援[[!DNL Google Cloud Storage]](../../destinations/destination-sdk/functionality/destination-server/server-specs.md#gcs-example)以檔案為基礎的目的地和[可設定的檔案名稱](../../destinations/destination-sdk/functionality/destination-configuration/batch-configuration.md#file-name-configuration)的Destination SDK。 | 您現在可以使用Destination SDK建立Google Cloud Storage目的地，並透過檔案名稱巨集定義匯出檔案的自訂檔案名稱。 Adobe Experience Platform Destination SDK中的<br><br>檔案型目的地支援目前位於Beta。 檔案和功能可能會有所變更。 |
...
| 擴充功能 | [[!UICONTROL AdobeCJM ExperienceEvent — 訊息互動詳細資料]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/customerJourneyManagement/message-interaction.schema.json) | 在登入頁面物件中新增訂閱、同意、自訂電子郵件和其他資料資訊。 |
...
| 功能 | 說明 |
| --- | --- |
| [!DNL Mixpanel]來源的Beta版本 | 您現在可以使用[!DNL Mixpanel]來源從[!DNL Mixpanel]帳戶擷取分析資料以進行Experience Platform。 如需詳細資訊，請參閱[[!DNL Mixpanel] 來原始檔](../../sources/connectors/analytics/mixpanel.md)。 |
