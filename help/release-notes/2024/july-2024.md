...
| 類型 | 功能 | 說明 |
| --- | --- | --- |
| Web SDK | 自動追蹤提議互動 | 您現在可以使用 Web SDK 中的 `autoCollectPropositionInteractions` 屬性來決定 Web SDK 是否應自動收集提議互動。如需詳細資訊，請參閱 [`autoCollectPropositionInteractions`](/help/collection/js/commands/configure/autocollectpropositioninteractions.md) 文件。 |
...
| [(Beta) Merkury Enterprise Identity](/help/destinations/catalog/data-partners/merkury-enterprise-identity.md) | 使用 [!DNL Merkury Enterprise Identity] 目標可建置更精準、全方位且洞察的消費者輪廓。透過改善的輪廓資料，行銷人員能夠提供更好的洞察、區段和模型，進而達到更精準的目標選擇及預測性模型建立。 |
