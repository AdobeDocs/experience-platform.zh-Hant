...
| (Beta) 匯出陣列支援測試階段中支援的其他功能 | 以往，將客群啟用至檔案型目標和選取使用計算欄位時，您只能使用資料準備提供的客群子集。現在已無此限制，客戶將客群匯出至檔案型目標時，可以存取資料準備提供的所有功能。[閱讀全文](/help/destinations/ui/export-arrays-calculated-fields.md#supported-functions)。 |
...
| 功能 | 說明 |
| --- | --- |
| [!BADGE Beta]{type=Informative} 識別圖連結規則 | 測試版方案的參與者可以使用識別圖來連結規則，藉由避免「共用裝置」和其他圖表摺疊情境來確保系統中的個人實體展現方案。為了實現此目標，測試版方案的參與者將能夠存取開發沙箱環境中的三個功能： <ul><li>圖表模擬工具，用來了解圖表演算法如何運作。</li><li>身分設定畫面，用來設定唯一命名空間和命名空間優先順序。</li><li>身分儀表板，用來取得已攝取之圖表的深入解析。</li></ul> 此外，測試版方案還會包括輪廓行為穩定性的提升。如需詳細資訊，請閱讀[識別圖連結規則](../../identity-service/identity-graph-linking-rules/overview.md)文件。 |
