...
| Data Mirror | 使用關聯式結構描述，將資料列層級變更從雲端資料倉儲(例如Snowflake、Databricks、BigQuery)擷取到Adobe Experience Platform。 Data Mirror會直接將現有資料庫結構映象至資料湖，藉此消除上游ETL並保留關係、版本設定和刪除專案。 時間系列和記錄事件結構描述行為，以及變更資料擷取功能全都受到支援。 此功能目前可供Campaign Orchestration授權持有人使用，並將在這個有限版本中進一步擴展，包括Customer Journey Analytics客戶。 如需詳細資訊，請參閱[Data Mirror檔案](../../xdm/data-mirror/overview.md)。 請聯絡您的Adobe代表以取得存取權。 |
