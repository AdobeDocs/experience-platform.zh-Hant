...
| 監視設定檔擷取工作狀態 | 您現在可以監視批次設定檔擷取資料流執行的工作層級進度百分比。 此功能可即時顯示批次擷取工作的目前進度，包括指出擷取是否已準備好進行客戶細分和Adobe Journey Optimizer查詢的重要查核點。 對於可能需要數小時處理的大型擷取，此進度透明度可協助您瞭解工作是否正常進行或遇到問題，減少資料處理期間的不確定性。 如需詳細資訊，請閱讀[監視器設定檔指南](/help/dataflows/ui/monitor-profiles.md)。 |
