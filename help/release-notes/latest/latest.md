...
>[!TIP]
>
>Experience Platform](https://platform.adobe.com)中的[AI助理現已可用。 使用AI助理加速您在Adobe應用程式中的工作流程。 [深入瞭解](#ai-assistant)新功能。
...
| 功能 | 說明 |
| --- | --- |
| Experience Platform中的AI助理 | 您現在可以在Experience Platform中使用AI助理。 AI助理支援Experience Platform、Real-time Customer Data Platform、Adobe Journey Optimizer和Customer Journey Analytics。<br> Experience Platform中的![AI助理。Experience Platform中的](../2024/assets/june/ai-assistant-full.png "AI助理。"){width="100" zoomable="yes"} <br>如需有關此功能的詳細資訊，請閱讀[AI助理使用者介面指南](../../ai-assistant/ui-guide.md)。 |
...
| 針對營運見解問題[!BADGE Beta]{type=Informative}支援 | [操作深入分析](../../ai-assistant/home.md#operational-insights)是回答AI助理產生的有關中繼資料物件的答案，包括計數、查閱和歷程影響。 操作深入分析不會檢視沙箱中的任何資料。 您可以詢問AI Assistant操作深入分析問題，例如： <ul><li>哪些目的地處於作用中狀態？</li><li>我有多少資料集？</li><li>列出即時歷程中使用的對象。</li></ul> 下列網域支援營運深入分析：屬性、受眾、資料流程、資料集、目的地、歷程、結構描述和來源。 |
| 存取AI助理 | 若要存取Experience Platform、Real-Time CDP和Journey Optimizer的AI小幫手，您必須新增至包含&#x200B;**啟用AI小幫手**&#x200B;和&#x200B;**檢視營運分析**&#x200B;許可權的角色。 如需詳細資訊，請閱讀[功能存取指南](../../ai-assistant/access.md)。 您必須使用Admin Console在Customer Journey Analytics](https://experienceleague.adobe.com/en/docs/analytics-platform/using/ai-assistant?lang=en#feature-access)中進行[存取。 |

如需有關AI助理的詳細資訊，請閱讀[AI助理概述](../../ai-assistant/home.md)。
...
| 功能 | 說明 |
| ----------- | ----------- |
| 增強隨選匯出API，可匯出外部對象 | 您現在可以使用臨機匯出API來匯出外部（自訂上傳）對象。 [瞭解詳情](/help/destinations/api/ad-hoc-activation-api.md) 。 |
...
| 僅顯示含有對應步驟中資料的欄位 | 將設定檔屬性對應至目的地時，您現在可以在所有設定檔屬性之間切換，或僅切換包含資料的屬性。 依預設，僅顯示包含資料的欄位。 如需詳細資訊，請參閱[批次](../../destinations/ui/activate-batch-profile-destinations.md#mapping)和[串流](../../destinations/ui/activate-segment-streaming-destinations.md#mapping)目的地的啟用指南。 |
...
幾項法律和組織法規授予使用者權利，讓使用者有權應要求從您的資料存放區存取或刪除其個人資料。 Adobe Experience Platform [!DNL Privacy Service]提供RESTful API和使用者介面，協助您管理客戶的這些資料請求。 透過[!DNL Privacy Service]，您可以提交存取和刪除Adobe Experience Cloud應用程式中私人或個人客戶資料的請求，協助自動遵守法律和組織隱私權法規。

**新功能**

| 功能 | 說明 |
|--- | ---|
| Adobe Journey Optimizer的Privacy Service支援 | Privacy Service功能現在與Adobe Journey Optimizer通訊協定相容，以處理刪除請求。 如需詳細資訊，請參閱[Adobe Journey Optimizer隱私權要求檔案](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/privacy/requests)，或參閱Experience Platform檔案，以取得與Privacy Service](../../privacy-service/experience-cloud-apps.md)整合的[Experience Cloud應用程式清單。 |

如需服務的詳細資訊，請參閱[Privacy Service概觀](../../privacy-service/home.md)。
...
所有Adobe Experience Platform客戶都可免費使用[!DNL Use Case Playbooks]。 若要在Experience PlatformUI中存取豐富的使用案例教戰手冊，您現在可以從左側導覽中選取&#x200B;**[!UICONTROL 教戰手冊]**。

[!DNL Use Case Playbooks]的設計目的，是為了協助您從Real-time Customer Data Platform或Adobe Journey Optimizer開始克服挑戰。 即使您不確定從何處開始或是如何針對您的預期使用案例產生正確的資產，這類資產仍可提供指引，並產生各種資產，讓您可以在準備就緒時測試並匯入生產環境。

若要開始使用，請閱讀[使用案例行動手冊概觀](/help/use-case-playbooks/playbooks/overview.md)，其中提供行動手冊的功能、用途概觀，以及端對端示範，包括如何建立執行個體和將產生的資產匯入其他沙箱環境。
...
- 取得所有[可用教戰手冊](/help/use-case-playbooks/playbooks/playbooks-list.md)的清單，依產品(Real-Time CDP或Journey Optimizer)分組。
- 瞭解您需要哪些[許可權](/help/use-case-playbooks/playbooks/get-started.md#grant-your-team-the-required-access-permissions)才能使用教戰手冊及其建立的資產。
