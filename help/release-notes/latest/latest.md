...
| 由於存取標籤，UI呈現灰色的資料流 | 為了解決部分使用者因無法存取的目的地資料流完全隱藏而看到空白頁面的問題，UI現在會以灰色狀態顯示這些受限制的資料流，而非完全省略。 如需詳細資訊，請閱讀有關[使用存取標籤來管理使用者對目的地資料流](../../access-control/abac/apply-access-labels-destinations.md#important-callouts-and-items-to-know)的存取的檔案。 |
