...
| 功能 | 說明 |
| --- | --- |
| API支援篩選MicrosoftDynamics、Salesforce CRM和SalesforceMarketing Cloud的行級資料 | 使用邏輯運算子和比較運算子來篩選MicrosoftDynamics、Salesforce CRM和SalesforceMarketing Cloud源的行級資料。 閱讀上的指南 [使用API篩選源的資料](../../sources/tutorials/api/filter.md) 的子菜單。 |
