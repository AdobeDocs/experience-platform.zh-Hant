...
| 功能 | 說明 |
| --- | --- |
| 授權使用附加表 | 取得授權使用情況的細微可見度，並使用核心產品和附加元件的專用來表管理您的平台資源。透過沙箱層級的鑽研式視圖，追蹤和分析每個核心產品的關鍵指標。附加指標與核心產品指標無縫整合，可以提供全面的使用情況視圖。增強可見度能協助您最佳化授權管理，並且可讓資源完全符合組織的需求。有關更多詳細資訊，請參閱「[[!UICONTROL 授權使用情況]」儀表板指南](../../dashboards/guides/license-usage.md#overview-tab)。 |
...
| 功能 | 說明 |
| --- | --- |
| 識別圖連結規則的有限適用性 | 識別圖連結規則是身分服務中的一套工具，您可以使用該規則確保您的使用者準確進行個人化。 <ul><li>您現在可以利用[身分最佳化演算法](../../identity-service/identity-graph-linking-rules/identity-optimization-algorithm.md)來確保識別圖代表單一個人，以此防止在即時客戶輪廓中進行不必要的身分合併。</li><li>設定[命名空間優先順序](../../identity-service/identity-graph-linking-rules/namespace-priority.md)，以此來界定各別命名空間的重要程度，並影響您的輪廓形成和細分的方式。</li><li>使用 UI 中的[圖表模擬工具](../../identity-service/identity-graph-linking-rules/graph-simulation.md)，以模擬不同設定的識別圖。</li><li>使用[身分設定介面](../../identity-service/identity-graph-linking-rules/identity-settings-ui.md)來指定您的唯一命名空間，並為組織中的所有命名空間建立優先順序。</li><li>請參閱[身分儀表板](../../identity-service/identity-graph-linking-rules/implementation-guide.md#validate-your-graphs)，了解有關圖表資料的指標和趨勢。</li></ul> 若要嘗試識別圖連結規則，請聯絡您的 Adob&#x200B;&#x200B;e 帳戶團隊以取得開發沙箱的存取權。 |
