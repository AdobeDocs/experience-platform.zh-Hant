...
如需詳細資訊，請閱讀[資料彙集檔案](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/collection/home)。
...
| 限制編輯含有資料集的結構描述 | 一旦結構描述的資料集存在，造成重大變更的編輯作業現在會受到限制。 當資料集相關聯時，您將無法再重新命名或刪除欄位、變更欄位資料型別或格式、修改身分描述元、管理相關欄位以移除現有欄位，或變更指派的類別；仍支援增加變更和欄位棄用。 |
