...
| 目標 | 說明 |
| --- | --- |
| [Snowflake串流](../destinations/catalog/warehouses/snowflake.md)多區域支援 | Snowflake串流聯結器現在可供美國VA7區域以外的客戶使用。 使用地區下拉式選取器來選取您的帳戶所在的Snowflake地區。 檔案已更新，其中包含Snowflake串流表格的預期資料結構。 |
| [Snowflake串流](../destinations/catalog/warehouses/snowflake.md)和[Snowflake批次](../destinations/catalog/warehouses/snowflake-batch.md)區域選擇器 | 現在，您可以使用新的可搜尋下拉式清單更輕鬆地找到您的地區，該下拉式清單將搜尋和下拉式清單整合到一個控制項中。 |
...
| 啟動工作流程中的對象篩選 | 您現在可以在&#x200B;**[!UICONTROL Select audiences]**&#x200B;步驟中找到並篩選與「對象」頁面具有相同體驗的對象；例如，您可以依對象來源進行篩選，以輕鬆找到您正在尋找的對象。 |
