...
建立新工作請求前，您必須先收集有關資料主體的識別資訊，這些資料主體的資料要存取、刪除或選擇退出銷售。 取得所需資料後，必須在POST要求的裝載中提供給 `/jobs` 端點。
...
| `regulation` **(必填)** | 隱私權工作的規範。 接受下列值： <ul><li>`gdpr` （歐盟）</li><li>`ccpa` （加州）</li><li>`lgpd_bra` (巴西)</li><li>`nzpa_nzl` (紐西蘭)</li><li>`pdpa_tha` (泰國)</li></ul> |
...
| 屬性 | 說明 |
| --- | --- |
| `jobId` | 作業的唯讀、唯一的系統產生ID。 此值將用於查找特定作業的下一步。 |
