...
* 臨機啟動工作無法與排程的[對象匯出工作](../../segmentation/api/export-jobs.md)並行執行。 在執行臨機啟動工作之前，請確定已排程的對象匯出工作已完成。 如需如何監視啟用流程狀態的資訊，請參閱[目的地資料流程監視](../../dataflows/ui/monitor-destinations.md)。 例如，如果您的啟用資料流顯示&#x200B;**[!UICONTROL Processing]**&#x200B;狀態，請等待它完成後再執行臨機操作啟用工作。
* 請勿對每個對象同時執行多個臨機操作啟用工作。

## 分段考量事項 {#segmentation-considerations}

[!DNL Adobe Experience Platform]每24小時執行一次排程的區段工作。 臨機啟動API會根據最新的分段結果執行。

## 步驟1：必要條件 {#prerequisites}

呼叫[!DNL Adobe Experience Platform] API之前，請確定您符合下列必要條件：

* 您擁有可存取[!DNL Adobe Experience Platform]的組織帳戶。
* 您的Experience Platform帳戶已針對`developer` API產品設定檔啟用`user`和[!DNL Adobe Experience Platform]角色。 請連絡您的[Admin Console](../../access-control/home.md)管理員，為您的帳戶啟用這些角色。
...
在執行臨機啟動工作之前，請確定您對象的排程對象匯出工作已完成。 如需如何監視啟用流程狀態的資訊，請參閱[目的地資料流程監視](../../dataflows/ui/monitor-destinations.md)。 例如，如果您的啟動資料流顯示&#x200B;**[!UICONTROL Processing]**&#x200B;狀態，請等待它完成後再執行臨機操作啟動工作以匯出完整檔案。
...
>[!IMPORTANT]
>
>必須將`Accept: application/vnd.adobe.adhoc.activation+json; version=2`標頭包含在您使用Ad Hoc Activation API v2的請求中。

對於非細分服務對象（例如[外部或自訂上傳對象](../../segmentation/ui/audience-portal.md#import-audience)），您必須在請求中指定Experience Platform產生的對象ID，而非外部對象ID。 在對象UI中開啟對象詳細資訊頁面時，您可以在[對象摘要面板](../../segmentation/ui/audience-portal.md#audience-summary)頂端找到系統產生的ID，顯示為&#x200B;**ID#**，後面接著UUID。

![顯示系統產生的ID欄位（在面板頂端反白顯示）的「對象」摘要面板。](../assets/api/ad-hoc-activation/audience-summary-id.png)
...
| 屬性 | 說明 |
| -------- | ----------- |
| <ul><li>`destinationId1`</li><li>`destinationId2`</li></ul> | 您想要啟用對象之目的地執行個體的ID。 您可以導覽至「**[!UICONTROL Destinations]** > **[!UICONTROL Browse]**」標籤，並按一下所需的目的地列，在右側邊欄中叫出目的地ID，從Experience Platform UI取得這些ID。 如需詳細資訊，請閱讀[目的地工作區檔案](/help/destinations/ui/destinations-workspace.md#browse)。 |
...
* [使用Experience Platform UI隨選將檔案匯出至批次目的地](/help/destinations/ui/export-file-now.md)