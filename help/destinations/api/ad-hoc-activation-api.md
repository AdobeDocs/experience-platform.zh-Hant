...
title: (Beta)通過即席激活API激活受眾段以批處理目標
...
# (Beta)通過即席激活API激活受眾段以批處理目標

>[!IMPORTANT]
>
>的 [!DNL ad-hoc activation API] 在平台中，當前處於測試版。 文件和功能可能會有所變更。
...
>[!NOTE]
>
>僅支援即席受眾激活 [基於批檔案的目標](../destination-types.md#file-based)。
...
* 目前，每個即席激活作業最多可激活20個段。 嘗試激活每個作業20個以上的段將導致該作業失敗。 此行為在未來版本中可能會發生更改。
...
在運行即席激活作業之前，請確保已完成段的計畫段導出作業。 請參閱 [目標資料流監視](../../dataflows/ui/monitor-destinations.md) 有關如何監視激活流狀態的資訊。 例如，如果激活資料流顯示 **[!UICONTROL 處理]** 狀態，等待其完成，然後運行即席激活作業。
