---
description: 本頁說明透過Adobe Experience Platform Destination SDK擷取目標伺服器設定所使用的API呼叫。
title: 檢索目標伺服器配置
...
此頁面是API要求和裝載的範例，您可借此使用來擷取關於現有目的地伺服器設定的資訊 `/authoring/destination-servers` API端點。

如需目的地伺服器所使用功能的詳細說明，請參閱下列文章：
...
您可以借由建立 `GET` 請求 `/authoring/destination-servers` 端點。
...
成功的回應會根據 [!DNL IMS Org ID] 和您使用的沙箱名稱。 一 `instanceId` 對應至一個目標伺服器。 以下範例回應包含兩個目標伺服器設定。
...
成功的回應會傳回HTTP狀態200，且目標伺服器的設定會對應至 `{INSTANCE_ID}` 你提供的。
...
閱讀本檔案後，您現在知道如何透過Destination SDK擷取目標伺服器組態 `/authoring/destination-servers` API端點。
