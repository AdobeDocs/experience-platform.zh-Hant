...
您可以更新 [現有](create-destination-server.md) 目標伺服器組態，透過設定 `PUT` 要求給 `/authoring/destination-servers` 具有已更新裝載的端點。
...
| 參數 | 說明 |
| -------- | ----------- |
| `{INSTANCE_ID}` | 您要更新的目的地伺服器組態ID。 若要取得現有的目的地伺服器組態及其對應組態 `{INSTANCE_ID}`，請參閱 [擷取目的地伺服器設定](retrieve-destination-server.md). |
...
| `httpTemplate.httpMethod` | 字串 | *必要.* Adobe將在對伺服器呼叫中使用的方法。 選項包括 `GET`， `PUT`， `PUT`， `DELETE`， `PATCH`. |
