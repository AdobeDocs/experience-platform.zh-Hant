---
description: 本頁說明了用於通過Adobe Experience Platform Destination SDK檢索目標配置的API調用。
title: 檢索目標配置
...
本頁說明了API請求和負載，您可以使用 `/authoring/destination` API終結點。
...
可以檢索 [現有](create-destination-configuration.md) 通過建立 `GET` 請求 `/authoring/destination` 端點。
...
以下兩個請求將檢索IMS組織或特定目標配置的所有目標配置，具體取決於您是否通過 `INSTANCE_ID` 中的設定。
...
成功的響應將返回HTTP狀態200，並根據 [!DNL IMS Org ID] 和沙盒名稱。 一 `instanceId` 對應於一個目標配置。
...
成功的響應返回HTTP狀態200，其目標配置的詳細資訊對應於 `{INSTANCE_ID}` 提供。
...
閱讀此文檔後，您現在知道如何通過Destination SDK檢索目標配置 `/authoring/destinations` API終結點。
