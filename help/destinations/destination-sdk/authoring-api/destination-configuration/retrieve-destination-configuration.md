---
description: 本頁說明透過Adobe Experience Platform Destination SDK擷取目的地設定所使用的API呼叫。
title: 檢索目標配置
...
此頁面是API要求和裝載的範例，您可使用來擷取關於現有目的地設定的資訊， `/authoring/destination` API端點。
...
您可以擷取 [現有](create-destination-configuration.md) 目標配置，方法是： `GET` 請求 `/authoring/destination` 端點。
...
使用下列API格式來擷取您帳戶的所有目的地設定。

```http
GET /authoring/destinations
```

使用下列API格式來擷取由 `{INSTANCE_ID}` 參數。
...
成功的回應會根據 [!DNL IMS Org ID] 和您使用的沙箱名稱。 一 `instanceId` 對應至一個目標配置。
...
成功的回應會傳回HTTP狀態200，且目的地組態的詳細資訊會對應至 `{INSTANCE_ID}` 在呼叫中提供。
...
閱讀本檔案後，您現在知道如何透過Destination SDK擷取目標設定 `/authoring/destinations` API端點。
