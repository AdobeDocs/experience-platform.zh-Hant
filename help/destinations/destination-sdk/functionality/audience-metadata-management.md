...
您可以使用 `/authoring/audience-templates` API端點。 閱讀 [建立中繼資料範本](../metadata-api/create-audience-template.md) 可在端點上執行的操作的完整清單。
...
透過Destination SDK中的受眾中繼資料支援，當您設定Experience Platform目的地時，當Platform使用者對應並啟用區段至您的目的地時，可提供其中一個選項。 您可以透過 [對象中繼資料設定](../functionality/destination-configuration/audience-metadata-configuration.md) 區段。
...
您可以使用一般範本來 [建立新受眾範本](../metadata-api/create-audience-template.md) 如果您的API支援：
...
在 [建立受眾範本](../metadata-api/create-audience-template.md) API參考。
