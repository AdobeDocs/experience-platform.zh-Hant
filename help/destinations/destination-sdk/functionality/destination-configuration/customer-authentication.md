...
使用本頁所述的任何客戶驗證設定時，請一律將`authenticationRule`目的地傳遞[中的](destination-delivery.md)引數設為`"CUSTOMER_AUTHENTICATION"`，如下所示。
...
若要設定目的地的基本驗證，請透過`customerAuthenticationConfigurations`端點設定`/destinations`區段，如下所示：
...
若要設定您目的地的持有者型別驗證，請透過`customerAuthenticationConfigurations`端點設定`/destinations`區段，如下所示：
...
使用者選取「**[!UICONTROL Connect to destination]**」以觸發前往您目的地的OAuth 2驗證流程，如以下適用於Twitter自訂對象目的地的範例所示。 如需針對目的地端點設定OAuth 2驗證的詳細資訊，請閱讀專用的[Destination SDK OAuth 2驗證頁面](oauth2-authorization.md)。

![使用OAuth 2驗證的UI轉譯](../../assets/functionality/destination-configuration/oauth2-authentication-ui.png)

若要設定目的地的[!DNL OAuth2]驗證，請透過`customerAuthenticationConfigurations`端點設定`/destinations`區段，如下所示：
...
若要設定目的地的[!DNL Amazon S3]驗證，請透過`customerAuthenticationConfigurations`端點設定`/destinations`區段，如下所示：
...
若要設定目的地的[!DNL Azure Blob]驗證，請設定`customerAuthenticationConfigurations`端點中的`/destinations`引數，如下所示：
...
當您設定[!DNL Azure Data Lake Storage]驗證型別時，使用者必須輸入Azure服務主要憑證及其租使用者資訊。

![具有[!DNL Azure Data Lake Storage]驗證的UI轉譯](../../assets/functionality/destination-configuration/adls-authentication-ui.png)

若要設定目的地的[!DNL Azure Data Lake Storage] (ADLS)驗證，請設定`customerAuthenticationConfigurations`端點中的`/destinations`引數，如下所示：
...
## 使用密碼驗證的SFTP {#sftp-password-auth}
...
若要設定您目的地之密碼的SFTP驗證，請在`customerAuthenticationConfigurations`端點中設定`/destinations`引數，如下所示：
...
## 使用SSH金鑰驗證的SFTP {#sftp-ssh-key-auth}

Experience Platform中的檔案型目的地支援使用[!DNL SFTP]金鑰的[!DNL SSH]驗證。
...
若要設定您目的地的SFTP驗證（使用SSH金鑰），請在`customerAuthenticationConfigurations`端點中設定`/destinations`引數，如下所示：
...
當您設定[!DNL Google Cloud Storage]驗證型別時，使用者必須輸入其[!DNL Google Cloud Storage] [!UICONTROL access key ID]和[!UICONTROL secret access key]。

![使用Google雲端儲存空間驗證的UI轉譯](../../assets/functionality/destination-configuration/google-cloud-storage-ui.png)

若要設定目的地的[!DNL Google Cloud Storage]驗證，請設定`customerAuthenticationConfigurations`端點中的`/destinations`引數，如下所示：
