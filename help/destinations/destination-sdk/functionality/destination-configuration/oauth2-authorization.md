...
首先，您必須在您的系統中為[!DNL Adobe Experience Platform]建立應用程式，或在您的系統中註冊Experience Platform。 目標是產生使用者端ID和使用者端密碼，這是驗證Experience Platform至您的目的地所需的密碼。

在您的系統中進行此設定時，您需要[!DNL Adobe Experience Platform] OAuth 2重新導向/回呼URL，您可從下列清單中取得。
...
>[!IMPORTANT]
>
>只有在授權碼為[!DNL Adobe Experience Platform]授權型別的[OAuth 2中，才需要執行在您的系統中註冊](#authorization-code)的重新導向/回呼URL的步驟。 對於其他兩種支援的授權型別（密碼和使用者端認證），您可以略過此步驟。
...
>* 在OAuth 2授權流程中，表格中以粗體強調顯示的輸入引數為必要引數。 其他引數為選用引數。 還有其他自訂輸入引數未顯示於此處，但會在[自訂您的OAuth 2設定](#customize-configuration)及[存取權杖重新整理](#access-token-refresh)等節中詳細說明。
...
如果您的目的地支援標準OAuth 2.0授權代碼流程（請閱讀[RFC標準規格](https://tools.ietf.org/html/rfc6749#section-4.1)）或其變數，請參閱下列必要和選用欄位：
...
| `authorizationUrl` | 字串 | 授權伺服器的URL，您可在此處將使用者重新導向至您的應用程式。 |
| `refreshTokenUrl` | 字串 | *選擇性。*&#x200B;您這邊的URL，這會產生重新整理Token。 通常`refreshTokenUrl`與`accessTokenUrl`相同。 |
| `clientId` | 字串 | 您的系統指派給[!DNL Adobe Experience Platform]的使用者端ID。 |
| `clientSecret` | 字串 | 您的系統指派給[!DNL Adobe Experience Platform]的使用者端密碼。 |
...
## 具有密碼授予的OAuth 2 {#oauth2-password-grant}
...
## 具有使用者端憑證授權的OAuth 2 {#oauth2-client-credentials-grant}

您可以設定OAuth 2使用者端認證（讀取[RFC標準規格](https://tools.ietf.org/html/rfc6749#section-4.4)）目的地，其支援下列標準輸入和輸出。 您可以自訂值。 如需詳細資訊，請參閱[自訂您的OAuth 2組態](#customize-configuration)。
...
在此範例中，目的地平台的重新整理權杖會在特定時間後到期。 在此情況下，合作夥伴會設定`refreshTokenExpiration`自訂欄位，以從API回應中的`refresh_token_expires_in`欄位取得重新整理權杖有效期。
...
在此範例中，客戶必須輸入使用者端ID、使用者端密碼和帳戶ID （客戶用來登入目的地的識別碼），而非建立全域使用者端ID和使用者端密碼，如系統[必要條件](#prerequisites)一節所示
...
您可以在`authenticationDataFields`中使用下列引數來自訂您的OAuth 2組態：
...
若要設定存取權杖重新整理，您可能需要設定範本化HTTP請求，以允許Adobe使用重新整理權杖取得新的存取權杖。 如果存取權杖已過期，Adobe會採用您提供的樣板化請求，並新增您提供的引數。 使用`accessTokenRequest`引數來設定存取權杖重新整理機制。
...
您可以在`accessTokenRequest`中使用下列引數來自訂Token重新整理程式：
...
| `accessTokenRequest.httpTemplate.requestBody.value` | 字串 | 使用範本化語言來自訂HTTP請求中的欄位以存取權杖端點。 如需有關如何使用範本化來自訂欄位的資訊，請參閱[範本化慣例](#templating-conventions)區段。 |
...
| `accessTokenRequest.responseFields.value` | 字串 | 使用範本化語言從存取權杖端點存取HTTP回應中的欄位。 如需有關如何使用範本化來自訂欄位的資訊，請參閱[範本化慣例](#templating-conventions)區段。 |
...
| `accessTokenRequest.validations.actualValue.value` | 字串 | 使用範本化語言來存取HTTP回應中的欄位。 如需有關如何使用範本化來自訂欄位的資訊，請參閱[範本化慣例](#templating-conventions)區段。 |
...
| `accessTokenRequest.validations.expectedValue.value` | 字串 | 使用範本化語言來存取HTTP回應中的欄位。 如需有關如何使用範本化來自訂欄位的資訊，請參閱[範本化慣例](#templating-conventions)區段。 |
...
您現在瞭解[!DNL Adobe Experience Platform]支援的OAuth 2授權模式，並知道如何使用OAuth 2授權支援來設定您的目的地。 接下來，您可以使用Destination SDK設定您的OAuth 2支援目的地。 閱讀[使用Destination SDK設定您的目的地](../../guides/configure-destination-instructions.md)以瞭解後續步驟。
