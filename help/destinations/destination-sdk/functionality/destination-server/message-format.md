...
* **Experience Data Model(XDM)**. [XDM概觀](../../../../xdm/home.md) 和  [如何在Adobe Experience Platform中建立XDM結構](../../../../xdm/tutorials/create-schema-ui.md).
* **類別**. [在UI中建立和編輯類](../../../../xdm/ui/resources/classes.md).
* **IdentityMap**. 身分對應代表Adobe Experience Platform中所有一般使用者身分的對應。 請參閱 `xdm:identityMap` 在 [XDM欄位字典](../../../../xdm/schema/field-dictionary.md).
* **區段成員資格**. 此 [segmentMembership](../../../../xdm/schema/field-dictionary.md) XDM屬性會通知設定檔是的成員區段。 對於 `status` 欄位，請閱讀 [區段成員資格詳細資料結構欄位群組](../../../../xdm/field-groups/profile/segmentation.md).
...
|---|---|
| 即時（串流）整合 | 是 |
| 檔案式（批次）整合 | 是（僅下圖中的步驟1和2） |

## 總覽 {#overview}

本頁面說明從Adobe Experience Platform匯出至目的地之資料中的訊息格式和設定檔轉換。
...
* 合作夥伴端(3)的預期訊息格式，以及
* XDM架構與預期訊息格式之間的轉換層，您可以透過建立 [消息轉換模板](#using-templating).

![結構轉換為JSON](../../assets/functionality/destination-server/transformations-3-steps.png)
...
**源XDM架構(1)**:此項目指客戶在Experience Platform中使用的結構。 在Experience Platform中，在 [對應步驟](../../../ui/activate-segment-streaming-destinations.md#mapping) 在啟用目標工作流程中，客戶將欄位從其XDM架構對應至目標的目標架構(2)。

**Target XDM結構(2)**:您可以根據目的地預期格式的JSON標準結構(3)，以及目的地可解譯的屬性，在目標XDM結構中定義設定檔屬性和身分。 您可以在目的地設定中，於 [schemaConfig](../../functionality/destination-configuration/schema-configuration.md) 和 [identityNamespaces](../../functionality/destination-configuration/identity-namespace-configuration.md) 對象。

**目的地設定檔屬性的JSON標準結構(3)**:此範例代表 [JSON結構](https://json-schema.org/learn/miscellaneous-examples.html) 平台支援的所有設定檔屬性及其類型(例如：物件、字串、陣列)。 目標可支援的欄位範例為 `firstName`, `lastName`, `gender`, `email`, `phone`, `productId`, `productName`等。 您需要 [消息轉換模板](#using-templating) 量身打造出Experience Platform的資料，使其符合您預期的格式。
...
>[!NOTE]
>
>客戶會將來源XDM結構的屬性對應至Adobe Experience Platform UI中的合作夥伴XDM結構(位於 **對應** 步驟 [啟動目標工作流程](../../../ui/activate-segment-streaming-destinations.md#mapping).
...
1. 簡單的轉換範例。 了解範本如何與 [設定檔屬性](#attributes), [區段成員資格](#segment-membership)，和 [身分](#identities) 欄位。
2. 結合上述欄位的範本，其複雜度增加範例： [建立可傳送區段和身分的範本](./message-format.md#segments-and-identities) 和 [建立可傳送區段、身分和設定檔屬性的範本](#segments-identities-attributes).
3. 包含聚合鍵的模板。 使用 [可配置聚合](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) 在目的地設定中，Experience Platform會根據區段ID、區段狀態或身分識別命名空間等條件，將匯出至目的地的設定檔分組。
...
>[!IMPORTANT]
>
>對於您使用的所有範本，您必須逸出非法字元，例如雙引號 `""` 在插入 [範本](../../functionality/destination-server/templating-specs.md) 在 [目標伺服器配置](../../authoring-api/destination-server/create-destination-server.md). 如需逸出雙引號的詳細資訊，請參閱 [JSON標準](https://www.ecma-international.org/publications-and-standards/standards/ecma-404/).
...
此 [segmentMembership](../../../../xdm/schema/field-dictionary.md) XDM屬性會通知設定檔是的成員區段。
對於 `status` 欄位，請閱讀 [區段成員資格詳細資料結構欄位群組](../../../../xdm/field-groups/profile/segmentation.md).
...
如需Experience Platform中身分的相關資訊，請參閱 [身分命名空間概觀](../../../../identity-service/namespaces.md).
...
使用 [可配置聚合](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) 在目的地設定中，您可以根據區段ID、區段別名、區段成員資格或身分識別命名空間等條件，將匯出至目的地的設定檔分組。
...
如果您使用 [可配置聚合](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) 設定 `includeSegmentId` 若設為true，則匯出至您目的地的HTTP訊息中的設定檔會依區段ID分組。 請參閱下方範本中如何存取區段ID。
...
如果您使用 [可配置聚合](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) 設定 `includeSegmentId` 若設為true，您也可以存取範本中的區段別名。
...
如果您使用 [可配置聚合](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) 設定 `includeSegmentId` 和 `includeSegmentStatus` 若設為true，您可以存取範本中的區段狀態。 如此一來，您就可以根據是否應新增或移除區段的設定檔，將匯出至目的地的HTTP訊息中的設定檔分組。
...
以下範例說明 [可配置聚合](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) 在目標設定中，設定為依身分識別命名空間匯總匯出的設定檔，格式為 `"namespaces": ["email", "phone"]` 和 `"namespaces": ["GAID", "IDFA"]`. 請參閱 `groups` 參數 [建立目標配置](../../authoring-api/destination-configuration/create-destination-configuration.md) 檔案，以取得分組的詳細資訊。
...
* [支援的Destination SDK轉換函式](../destination-server/supported-functions.md)
