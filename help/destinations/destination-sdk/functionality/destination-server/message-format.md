...
* **識別對應**。 身分對應代表Adobe Experience Platform中所有一般使用者身分的對應。 參考`xdm:identityMap`XDM欄位字典[中的](../../../../xdm/schema/field-dictionary.md)。
...
Adobe Experience Platform會以各種資料格式，將資料匯出至大量的目的地。 目的地型別的一些範例為廣告平台(Google)、社交網路(Facebook)和雲端儲存位置（Amazon S3、Azure事件中樞）。
...
**Source XDM結構描述(1)**：此專案是指客戶在Experience Platform中使用的結構描述。 在Experience Platform中，在激活目標工作流的[映射步驟](../../../ui/activate-segment-streaming-destinations.md#mapping)中，客戶將欄位從其XDM架構映射到目標架構(2)。

**目標XDM架構(2)**：基於目標預期格式的JSON標準架構(3)和目標可解釋的屬性，您可以在目標XDM架構中定義配置檔案屬性和標識。 您可以在目標配置中在[schemaConfig](../../functionality/destination-configuration/schema-configuration.md)和[identityNamespaces](../../functionality/destination-configuration/identity-namespace-configuration.md)對象中執行此操作。

**目標配置檔案屬性的JSON標準架構(3)**：此示例表示平台支援的所有配置檔案屬性及其類型（例如：對象、字串、陣列）的[JSON架構](https://json-schema.org/learn/miscellaneous-examples.html)。 目標可以支援的示例欄位可以是`firstName`、`lastName`、`gender`、`email`、`phone`、`productId`、`productName`等。 您需要[消息轉換模板](#using-templating)來將導出的Experience Platform外資料定制為預期格式。
...
請參閱下列Experience Platform中的兩個設定檔範例：
...
Adobe使用[Pebble範本](https://pebbletemplates.io/) （類似於[Jinja](https://jinja.palletsprojects.com/en/2.11.x/)的範本化語言）將欄位從Experience Platform XDM結構描述轉換為目的地支援的格式。

本節提供如何進行這些轉換的數個範例，從輸入XDM結構描述，透過範本，以及輸出為目的地接受的裝載格式。 以下範例是透過增加複雜性來呈現，如下所示：

1. 簡單轉換範例。 瞭解範本化如何與[設定檔屬性](#attributes)、[對象成員資格](#audience-membership)和[身分](#identities)欄位的簡單轉換搭配運作。
...
3. 包含彙總索引鍵的範本。 當您在目的地組態中使用[可設定的彙總](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)時，Experience Platform會根據對象ID、對象名稱空間、對象狀態或身分名稱空間等條件，將匯出至目的地的設定檔分組。
...
>[!IMPORTANT]
>
>針對您使用的所有範本，您必須先逸出不合法的字元，例如雙引號`""`，再於[目的地伺服器組態](../../functionality/destination-server/templating-specs.md)中插入[範本](../../authoring-api/destination-server/create-destination-server.md)。 如需有關逸出雙引號的詳細資訊，請參閱[JSON標準](https://www.ecma-international.org/publications-and-standards/standards/ecma-404/)中的第9章。
...
本節提供Adobe XDM結構描述和合作夥伴目的地結構描述之間常用轉換的範例。
...
>[!IMPORTANT]
>
>For all templates that you use, you must escape the illegal characters, such as double quotes `""` before inserting the [template](../../functionality/destination-server/templating-specs.md) in the [destination server configuration](../../authoring-api/destination-server/create-destination-server.md). 如需有關逸出雙引號的詳細資訊，請參閱[JSON標準](https://www.ecma-international.org/publications-and-standards/standards/ecma-404/)中的第9章。
...
Another common use case is exporting data that contains audience membership, identities (for example: email address, phone number, advertising ID), and profile attributes. 若要以此方式匯出資料，請參閱下列範例：
...
>[!IMPORTANT]
>
>對於您使用的所有模板，在`""`目標伺服器配置[中插入](../../functionality/destination-server/templating-specs.md)模板[之前，必須轉義非法字元，如雙引號](../../authoring-api/destination-server/create-destination-server.md)。 如需有關逸出雙引號的詳細資訊，請參閱[JSON標準](https://www.ecma-international.org/publications-and-standards/standards/ecma-404/)中的第9章。
...
當您在目的地組態中使用[可設定的彙總](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)時，您可以根據對象ID、對象名稱空間、對象別名、對象成員資格或身分名稱空間等條件，將匯出至目的地的設定檔分組。

在訊息轉換範本中，您可以存取上述彙總索引鍵，如下列章節的範例所示。 使用彙總金鑰來建構從Experience Platform匯出的HTTP訊息，以符合目的地預期的格式和速率限制。

#### 在範本中使用對象ID彙總索引鍵 {#aggregation-key-segment-id}

如果您使用[可設定的彙總](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)並將`includeSegmentId`設為true，則匯出至目的地的HTTP訊息中的設定檔會依對象ID分組。 請參閱下方說明如何在範本中存取對象ID和對象名稱空間。

**輸入**

請考量下列四個設定檔，其中：

* 前兩個對象屬於在`788d8874-8007-4253-92b7-ee6b6c20c6f3`名稱空間下具有對象識別碼`ups`的對象
* 第三個設定檔是`8f812592-3f06-416b-bd50-e7831848a31a`名稱空間下對象識別碼為`CustomerAudienceUpload`的對象的一部分
* 第四個設定檔是上述兩個對象的一部分，每個對象都在各自的名稱空間下。
...
請注意以下範本中如何使用`audienceId`和`audienceNamespace`來存取對象ID和名稱空間。 此範例假設您在目的地分類法中使用`audienceId`作為對象成員資格。 您可以改用任何其他欄位名稱，視您自己的分類法而定。
...
將設定檔匯出至目的地時，會根據其對象ID和名稱空間將設定檔分割為兩個群組。
...
以下是範例，其中目的地組態中的[可設定的彙總](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation)設定為依據身分名稱空間彙總匯出的設定檔，格式為`"namespaces": ["email", "phone"]`和`"namespaces": ["GAID", "IDFA"]`。 如需分組的詳細資訊，請參閱`groups`建立目的地組態[檔案中的](../../authoring-api/destination-configuration/create-destination-configuration.md)引數。
...
| `destination.namespaceSegmentAliases` | 從特定Adobe Experience Platform命名空間中的受眾ID映射到合作夥伴系統中的受眾別名。 | `destination.namespaceSegmentAliases["ups"]["seg-id-1"]` |
| `destination.namespaceSegmentNames` | 從特定Adobe Experience Platform命名空間中的受眾名映射到合作夥伴系統中的受眾名。 | `destination.namespaceSegmentNames["ups"]["seg-name-1"]` |
| `destination.namespaceSegmentTimestamps` | 返回以UNIX時間戳格式建立、更新或激活訪問群體的時間。 | <ul><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].createdAt`：返回從`seg-id-1`命名空間建立ID為`ups`的段以UNIX時間戳格式的時間。</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].updatedAt`：傳回從`seg-id-1`名稱空間更新識別碼為`ups`之對象的時間（UNIX時間戳記格式）。</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].mappingCreatedAt`：傳回識別碼為`seg-id-1`的對象，從`ups`名稱空間啟動至目的地的時間，格式為UNIX時間戳記。</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].mappingUpdatedAt`：傳回在目的地更新對象啟用的時間（UNIX時間戳記格式）。</li></ul> |
...
| `destination.segmentTimestamps` | **已棄用。 取代為`destination.namespaceSegmentTimestamps`** <br><br>傳回建立、更新或啟動對象的時間（以UNIX時間戳記格式）。 | <ul><li>`destination.segmentTimestamps["seg-id-1"].createdAt`：傳回識別碼為`seg-id-1`的對象建立時間（以UNIX時間戳記格式）。</li><li>`destination.segmentTimestamps["seg-id-1"].updatedAt`：傳回識別碼為`seg-id-1`的對象更新時間（以UNIX時間戳記格式）。</li><li>`destination.segmentTimestamps["seg-id-1"].mappingCreatedAt`：傳回識別碼為`seg-id-1`的對象啟動至目的地的時間（UNIX時間戳記格式）。</li><li>`destination.segmentTimestamps["seg-id-1"].mappingUpdatedAt`：以UNIX時間戳格式返回目標上更新訪問群激活的時間。</li></ul> |
...
* [建立並測試消息轉換模板](../../testing-api/streaming-destinations/create-template.md)
* [呈現模板API操作](../../testing-api/streaming-destinations/render-template-api.md)
* [支援的轉換函式在Destination SDK中](../destination-server/supported-functions.md)
