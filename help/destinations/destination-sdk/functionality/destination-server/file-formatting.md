---
description: 瞭解如何透過「/destination-servers」端點，為使用Adobe Experience Platform Destination SDK建立的檔案型目的地設定檔案格式選項。
title: 檔案格式設定
...
透過Destination SDK建立以檔案為基礎的目的地時，您可以定義轉存的CSV檔案的格式設定。 您可以自訂許多格式選項，例如但不限於：
...
您可以修改匯出檔案的幾個屬性，以符合目的地檔案接收系統的需求，以便以最佳方式讀取和解譯從Experience Platform接收的檔案。

>[!NOTE]
>
>只有在匯出CSV檔案時才支援CSV選項。 設定新的目的地伺服器時，`fileConfigurations`區段不是強制性的。 如果您沒有在API呼叫中為CSV選項傳遞任何值，則會使用位於[下方](#file-formatting-reference-and-example)參考表格中的預設值。
...
在以下的設定範例中，並未預先定義任何CSV選項。 每個`value`引數中的`csvOptions`是透過`/destinations`端點在對應的客戶資料欄位中設定的（例如[`customerData.quote`](../../functionality/destination-configuration/customer-data-fields.md#conditional-options)檔案格式選項的`quote`），使用者可以使用Experience Platform UI在您對應該客戶資料欄位中設定的各種選項之間選取。 您可以在檔案型目的地的[檔案格式選項](../../../ui/batch-destinations-file-formatting-options.md)檔案中看到這些選項的樣子。
