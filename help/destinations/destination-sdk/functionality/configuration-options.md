...
[!DNL Adobe Experience Platform]中的目的地服務使用數個元件的組態端點，這些元件會建立目的地功能。
...
此設定端點包含有關您目的地的基本和進階資訊。 例如，您可以在此處指定目的地可支援的身分型別、匯出檔案的所需格式（適用於檔案型目的地），以及[!DNL Adobe Experience Platform]使用者介面中目的地卡片的各種UI屬性。
