...
如需Destination SDK中所使用辭彙的定義，請參閱本辭彙表。 如需Adobe Experience Platform其他辭彙，請參閱[Experience Platform辭彙](/help/landing/glossary.md)。
...
**目的地執行個體**： Adobe Experience Platform中目的地組態的特定設定，透過Experience Platform UI建立及管理。 其中包含連線及傳送資料至目的地所需的所有引數和認證。 建立與目的地的連線後，您可以在[瀏覽與目的地的連線](/help/destinations/ui/destination-details-page.md)時取得目的地執行個體識別碼。
