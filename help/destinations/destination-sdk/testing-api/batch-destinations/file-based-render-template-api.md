...
此 `/authoring/testing/template/render` 端點可協助您將範本化情形視覺化 [客戶資料欄位](../../functionality/destination-configuration/customer-data-fields.md) 在目的地設定中定義的結果會類似。
...
| 參數 | 說明 |
| -------- | ----------- |
| `destinationId` | 的ID [目的地配置](../../authoring-api/destination-configuration/retrieve-destination-configuration.md) 你在測試。 |
...
閱讀本檔案後，您現在知道如何驗證中定義的客戶資料欄位設定 [目的地伺服器](../../authoring-api/destination-server/create-destination-server.md).
