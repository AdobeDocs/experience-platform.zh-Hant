---
description: 瞭解如何使用Destination SDK以自訂檔案格式選項和自訂檔案名稱設定來設定Azure Data Lake儲存體目的地。
title: 使用自訂檔案格式選項和自訂檔案名稱設定來設定Azure Data Lake儲存體目的地。
...
# 設定 [!DNL Azure Data Lake Storage] 具有自訂檔案格式選項和自訂檔案名稱設定的目的地

## 總覽 {#overview}

此頁面說明如何使用Destination SDK來設定 [!DNL Azure Data Lake Storage] 具有自訂功能的目的地 [檔案格式選項](configure-file-formatting-options.md) 和自訂 [檔案名稱設定](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).

此頁面顯示Azure Data Lake儲存體目的地可用的所有設定選項。 您可以視需要編輯下列步驟中顯示的配置，或刪除配置的某些部分。
...
以下裝載包含通用 [!DNL Azure Data Lake Storage] 設定，使用自訂 [CSV檔案格式設定](../../functionality/destination-server/file-formatting.md) 使用者可在Experience PlatformUI中定義的設定引數。
...
![熒幕錄製，顯示具有選定目的地卡片的目的地目錄頁面。](../../assets/guides/batch/adls-destination-card.gif)
...
閱讀本文章，您現在瞭解如何撰寫自訂 [!DNL Azure Data Lake Storage] 目的地使用Destination SDK。 接下來，您的團隊可以使用 [檔案型目的地的啟用工作流程](../../../ui/activate-batch-profile-destinations.md) 將資料匯出至目的地。
