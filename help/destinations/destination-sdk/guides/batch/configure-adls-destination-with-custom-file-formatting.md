---
description: 了解如何使用Destination SDK，使用自訂檔案格式設定選項和自訂檔案名稱設定來設定Azure Data Lake儲存目標。
title: 使用自訂檔案格式選項和自訂檔案名稱設定來設定Azure Data Lake Storage目的地。
...
# 設定 [!DNL Azure Data Lake Storage] 具有自定義檔案格式選項和自定義檔案名配置的目標
...
本頁說明如何使用Destination SDK來設定 [!DNL Azure Data Lake Storage] 自訂目的地 [檔案格式選項](../../server-and-file-configuration.md#file-configuration) 和自訂 [檔案名配置](../../file-based-destination-configuration.md#file-name-configuration).

此頁顯示Azure Data Lake儲存目的地的所有可用配置選項。 您可以編輯下列步驟中顯示的設定，或視需要刪除設定的某些部分。
...
從使用 `/destination-server` 端點來建立伺服器和檔案配置。 如需HTTP要求中參數的詳細說明，請參閱 [檔案型目的地的伺服器和檔案設定規格](../../server-and-file-configuration.md#adls-example) 和 [檔案格式設定](../../server-and-file-configuration.md#file-configuration).
...
以下裝載包含通用 [!DNL Azure Data Lake Storage] 配置，使用自定義 [CSV檔案格式](../../server-and-file-configuration.md#file-configuration) 設定參數，供使用者在Experience PlatformUI中定義。
...
* [驗證配置](../../authentication-configuration.md#adls)
...
![螢幕記錄顯示含有選定目的地卡片的目的地目錄頁面。](../../assets/adls-destination-card.gif)
...
閱讀本文，您現在知道如何製作自訂 [!DNL Azure Data Lake Storage] 目的地(使用Destination SDK)。 接下來，您的團隊可以使用 [檔案式目的地的啟用工作流程](../../../ui/activate-batch-profile-destinations.md) 將資料匯出至目的地。
