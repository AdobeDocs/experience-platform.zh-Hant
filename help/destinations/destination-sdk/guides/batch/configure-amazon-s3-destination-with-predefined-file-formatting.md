...
本頁說明如何使用Destination SDK來設定Amazon S3目的地，該目的地具有預先定義的預設[檔案格式選項](configure-file-formatting-options.md)和自訂[檔案名稱設定](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration)。
...
以下承載包含通用[!DNL Amazon S3]設定，具有預先定義的預設[CSV檔案格式](../../functionality/destination-server/file-formatting.md)設定引數，使用者可在Experience Platform UI中定義這些引數。
...
若要將[步驟1](#create-server-file-configuration)中的伺服器設定連線至此目的地設定，請以在`destinationServerId`步驟1`instanceId`中建立您的目的地伺服器時取得的[值，取代下列API要求中的](#create-server-file-configuration)值。
