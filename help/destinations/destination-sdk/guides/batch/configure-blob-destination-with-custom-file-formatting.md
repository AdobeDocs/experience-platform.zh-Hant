---
description: 瞭解如何使用Destination SDK來設定Azure Blob儲存目標，其中包含自訂檔案格式選項和自訂檔案名稱設定。
title: 使用自訂檔案格式選項和自訂檔案名稱設定來設定Azure Blob儲存目標。
...
# 使用自訂檔案格式設定選項和自訂檔案名稱設定來設定[!DNL Azure Blob Storage]目的地

## 概觀 {#overview}

本頁說明如何使用Destination SDK以自訂[!DNL Azure Blob Storage]檔案格式選項[和自訂](configure-file-formatting-options.md)檔案名稱組態[來設定](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration)目的地。

此頁面顯示Azure Blob儲存目的地可用的所有設定選項。 您可以編輯下列步驟中顯示的組態，或視需要刪除組態的特定部分。
...
以下承載包含通用的Azure Blob設定，其中包含使用者可在Experience Platform UI中定義的自訂[CSV檔案格式](../../functionality/destination-server/file-formatting.md)設定引數。
...
您現在知道如何使用Destination SDK編寫自訂[!DNL Azure Blob Storage]目的地。 接下來，您的團隊可以使用檔案型目的地的[啟動工作流程](../../../ui/activate-batch-profile-destinations.md)，將資料匯出至目的地。
