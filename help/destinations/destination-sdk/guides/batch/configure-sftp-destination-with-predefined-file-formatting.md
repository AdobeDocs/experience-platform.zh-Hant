---
description: 瞭解如何使用Destination SDK配置具有預定義檔案格式選項和自定義檔案名配置的SFTP目標。
title: (Beta)使用預定義的檔案格式選項和自定義檔案名配置配置SFTP目標。
...
# (Beta)使用預定義的檔案格式選項和自定義檔案名配置配置SFTP目標
...
本頁介紹如何使用Destination SDK配置具有預定義、預設值的SFTP目標 [檔案格式選項](../../server-and-file-configuration.md#file-configuration) 和習俗 [檔案名配置](../../file-based-destination-configuration.md#file-name-configuration)。

此頁顯示SFTP目標的所有可用配置選項。 您可以編輯下面步驟中顯示的配置，或根據需要刪除配置的某些部分。
...
開始使用 `/destination-server` 端點建立伺服器和檔案配置。 有關HTTP請求中參數的詳細說明，請閱讀 [用於基於檔案的目標的伺服器和檔案配置規範](../../server-and-file-configuration.md#sftp-example) 和 [檔案格式配置](../../server-and-file-configuration.md#file-configuration)。
...
下面的負載包括通用SFTP配置，具有預定義的預設 [CSV檔案格式](../../server-and-file-configuration.md#file-configuration) 用戶可以在Experience PlatformUI中定義的配置參數。
...
通過閱讀本文，您現在知道如何使用Destination SDK編寫自定義SFTP目標。 接下來，您的團隊可以 [用於基於檔案的目標的激活工作流](../../../ui/activate-batch-profile-destinations.md) 將資料導出到目標。
