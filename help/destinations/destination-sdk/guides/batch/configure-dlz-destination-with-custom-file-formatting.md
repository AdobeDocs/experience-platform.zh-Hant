...
本頁說明如何使用Destination SDK來設定 [!DNL Data Landing Zone] 自訂目的地 [檔案格式選項](configure-file-formatting-options.md) 和自訂 [檔案名配置](../../functionality/destination-configuration/batch-configuration.md#file-name-configuration).

此頁面顯示所有可用於 [!DNL Data Landing Zone] 目的地。 您可以編輯下列步驟中顯示的設定，或視需要刪除設定的某些部分。
...
以下裝載包含通用 [!DNL Data Landing Zone] 配置，使用自定義 [CSV檔案格式](../../functionality/destination-server/file-formatting.md) 設定參數，供使用者在Experience PlatformUI中定義。
...
![螢幕記錄顯示含有選定目的地卡片的目的地目錄頁面。](../../assets/guides/batch/dlz-destination-card.gif)
...
閱讀本文，您現在知道如何製作自訂 [!DNL Data Landing Zone] 目的地(使用Destination SDK)。 接下來，您的團隊可以使用 [檔案式目的地的啟用工作流程](../../../ui/activate-batch-profile-destinations.md) 將資料匯出至目的地。
