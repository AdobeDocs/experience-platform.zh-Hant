---
description: 瞭解如何使用Destination SDK配置具有自定義檔案格式設定選項和自定義檔案名配置的資料登錄區域(DLZ)目標。
title: (Beta)使用自定義檔案格式設定選項和自定義檔案名配置配置資料登錄區(DLZ)目標。
...
# （測試版）配置 [!DNL Data Landing Zone (DLZ)] 具有自定義檔案格式選項和自定義檔案名配置的目標
...
本頁介紹如何使用Destination SDK配置 [!DNL Data Landing Zone] 自定義目標 [檔案格式選項](../../server-and-file-configuration.md#file-configuration) 和習俗 [檔案名配置](../../file-based-destination-configuration.md#file-name-configuration)。

此頁顯示所有可用於 [!DNL Data Landing Zone] 目標。 您可以編輯下面步驟中顯示的配置，或根據需要刪除配置的某些部分。
...
開始使用 `/destination-server` 端點建立伺服器和檔案配置。 有關HTTP請求中參數的詳細說明，請閱讀 [用於基於檔案的目標的伺服器和檔案配置規範](../../server-and-file-configuration.md#adls-example) 和 [檔案格式配置](../../server-and-file-configuration.md#file-configuration)。
...
下面的負載包括通用 [!DNL Data Landing Zone] 配置，使用自定義 [CSV檔案格式](../../server-and-file-configuration.md#file-configuration) 用戶可以在Experience PlatformUI中定義的配置參數。
...
![螢幕錄制顯示具有選定目標卡的目標目錄頁。](../../assets/dlz-destination-card.gif)
...
通過閱讀這篇文章，您現在知道如何編寫自定義 [!DNL Data Landing Zone] 目標Destination SDK。 接下來，您的團隊可以 [用於基於檔案的目標的激活工作流](../../../ui/activate-batch-profile-destinations.md) 將資料導出到目標。
