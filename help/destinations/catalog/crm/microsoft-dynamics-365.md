...
作為行銷人員，您可以根據使用者[!DNL Adobe Experience Platform]設定檔中的屬性，將個人化體驗提供給使用者。 您可以從您的離線資料建立對象，並將這些對象傳送至[!DNL Dynamics 365]，以便在[!DNL Adobe Experience Platform]中更新對象和設定檔後立即顯示在使用者的摘要中。
...
請注意[!DNL Dynamics 365]中的下列先決條件，要將資料從Experience Platform匯出至您的[!DNL Dynamics 365]帳戶：
...
寫下您在&#x200B;**[!UICONTROL Customization prefix]**&#x200B;中建立的自訂欄位的[!DNL Dynamics 365]。 在[填寫目的地詳細資料](#destination-details)步驟期間，您將需要此首碼。 如需詳細資訊，請參閱[檔案的](https://learn.microsoft.com/en-us/dynamics365/customerengagement/on-premises/customize/create-edit-fields?view=op-9-1#create-and-edit-fields)建立及編輯欄位[!DNL Dynamics 365]區段。
...
您還需要在[中](https://docs.microsoft.com/en-us/power-platform/admin/manage-application-users#create-an-application-user)建立應用程式使用者[!DNL Azure Active Directory]，並將其與新建立的應用程式建立關聯。
...
| 認證 | 說明 | 範例 |
| --- | --- | --- |
| `Client ID` | [!DNL Dynamics 365]應用程式的[!DNL Azure Active Directory]使用者端識別碼。 請參閱[[!DNL Dynamics 365] 檔案](https://docs.microsoft.com/en-us/azure/active-directory/develop/howto-create-service-principal-portal#get-tenant-and-app-id-values-for-signing-in)以取得指引。 | `ababbaba-abab-baba-acac-acacacacacac` |
| `Client Secret` | 您的[!DNL Dynamics 365]應用程式的[!DNL Azure Active Directory]使用者端密碼。 您可能會在[[!DNL Dynamics 365] 檔案](https://docs.microsoft.com/en-us/azure/active-directory/develop/howto-create-service-principal-portal#authentication-two-options)中使用選項#2。 | `abcde~abcdefghijklmnopqrstuvwxyz12345678`作為指引。 |
| `Tenant ID` | 您[!DNL Dynamics 365]應用程式的[!DNL Azure Active Directory]租使用者識別碼。 請參閱[[!DNL Dynamics 365] 檔案](https://docs.microsoft.com/en-us/azure/active-directory/develop/howto-create-service-principal-portal#get-tenant-and-app-id-values-for-signing-in)以取得指引。 | `1234567-aaaa-12ab-ba21-1234567890` |
...
| 項目 | 類型 | 附註 |
|---------|----------|---------|
| 匯出類型 | **[!UICONTROL Profile-based]** | <ul><li>您正在匯出對象的所有成員，以及所需的結構描述欄位&#x200B;*（例如：電子郵件地址、電話號碼、姓氏）* （根據您的欄位對應）。</li><li> 根據[!DNL Dynamics 365]對象排程&#x200B;**[!UICONTROL Mapping ID]**&#x200B;步驟期間提供的[值，](#schedule-audience-export-example)中的每個對象狀態都會根據Experience Platform以對應的對象狀態更新。</li></ul> |
...
在&#x200B;**[!UICONTROL Destinations]** > **[!UICONTROL Catalog]**&#x200B;內搜尋[!DNL Dynamics 365]。 或者，您可以在&#x200B;**[!UICONTROL CRM]**&#x200B;類別下找到它。
...
* **[!UICONTROL Client ID]**：您[!DNL Dynamics 365]應用程式的[!DNL Azure Active Directory]使用者端識別碼。
* **[!UICONTROL Tenant ID]**：您[!DNL Dynamics 365]應用程式的[!DNL Azure Active Directory]租使用者識別碼。
* **[!UICONTROL Client Secret]**：您[!DNL Dynamics 365]應用程式的[!DNL Azure Active Directory]使用者端密碼。
* **[!UICONTROL Region]**：您的[[!DNL Dynamics 365]](https://learn.microsoft.com/en-us/power-platform/admin/new-datacenter-regions)地區。 例如：如果您的公司布建在北美(NAM)區域，則您的URL會是`crm.dynamics.com`，您需要選取`crm`。 如果您的公司已布建在加拿大(CAN)地區，則您的URL會是`crm3.dynamics.com`，您需要選取`crm3`。
* **[!UICONTROL Environment URL]**：您的[!DNL Dynamics 365]環境URL。
...
* **[!UICONTROL Customization Prefix]**：您在`Customization prefix`中建立的自訂欄位的[!DNL Dynamics 365]。 如需詳細資訊，請參閱[檔案的](https://learn.microsoft.com/en-us/dynamics365/customerengagement/on-premises/customize/create-edit-fields?view=op-9-1#create-and-edit-fields)建立及編輯欄位[!DNL Dynamics 365]區段。
...
若要將對象資料從[!DNL Adobe Experience Platform]正確傳送至[!DNL Dynamics 365]目的地，您必須完成欄位對應步驟。 對應包括在Experience Platform帳戶中的Experience Data Model (XDM)結構描述欄位與來自目標目的地的對應對應專案之間建立連結。 若要將您的XDM欄位正確對應到[!DNL Dynamics 365]目的地欄位，請遵循下列步驟：
...
1. 在&#x200B;**[!UICONTROL Select source field]**&#x200B;視窗中選擇&#x200B;**[!UICONTROL Select identity namespace]**&#x200B;類別並選取`contactid`。
...
1. 在&#x200B;**[!UICONTROL Select target field]**&#x200B;視窗中，選取您要將來源欄位對應到的目標欄位型別。
   ...
   * **[!UICONTROL Select custom attributes]**：選取此選項以將您的來源欄位對應到您在&#x200B;**[!UICONTROL Attribute name]**&#x200B;欄位中定義的自訂屬性。 如需支援屬性的完整清單，請參閱[[!DNL Dynamics 365] 檔案](https://docs.microsoft.com/en-us/dynamics365/customerengagement/on-premises/developer/entities/contact?view=op-9-1#entity-properties)。
...
在啟動工作流程的[[!UICONTROL Schedule audience export]](/help/destinations/ui/activate-segment-streaming-destinations.md#scheduling)步驟中，您必須手動將Experience Platform對象對應到[!DNL Dynamics 365]中的自訂欄位屬性。

若要這麼做，請選取每個對象，然後在[!DNL Dynamics 365]欄位中輸入來自&#x200B;**[!UICONTROL Mapping ID]**&#x200B;的對應自訂欄位屬性。

>[!IMPORTANT]
>
>用於&#x200B;**[!UICONTROL Mapping ID]**&#x200B;的值應該與[!DNL Dynamics 365]內建立的自訂欄位屬性名稱完全相符。 如果您需要尋找自訂欄位屬性的指引，請參閱[[!DNL Dynamics 365] 檔案](https://docs.microsoft.com/en-us/dynamics365/customerengagement/on-premises/customize/create-edit-fields?view=op-9-1)。
...
若要修正此錯誤，請確認您在&#x200B;**[!UICONTROL Mapping ID]**&#x200B;中為Experience Platform對象提供的[!DNL Dynamics 365]有效且存在於[!DNL Dynamics 365]內。
...
| 2023 年 8 月 | 功能和檔案更新 | 新增對未在[!DNL Dynamics 365]中預設方案內建立之自訂欄位的[!DNL Dynamics 365]自訂欄位首碼的支援。 已在&#x200B;**[!UICONTROL Customization Prefix]**&#x200B;填寫目的地詳細資料[步驟中新增輸入欄位](#destination-details)。 (PLATIR-31602)。 |
