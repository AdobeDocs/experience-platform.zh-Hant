---
title: Zendesk連線
description: 使用Zendesk目的地匯出您的帳戶資料，並在Zendesk中根據您的業務需求加以啟用。
...
多管道B2C平台的客戶服務部門想要確保其客戶獲得順暢的個人化體驗。 部門可以從自己的離線資料建立對象，以建立新的使用者設定檔，或更新不同互動（例如購買、退貨等）的現有設定檔資訊，並從[!DNL Adobe Experience Platform]將這些對象傳送至[!DNL Zendesk]。 在[!DNL Zendesk]中取得更新的資訊，可確保客戶服務代理程式能立即取得客戶的最新資訊，以更快的速度回應和解決問題。
...
若要將對象資料從[!DNL Adobe Experience Platform]正確傳送至[!DNL Zendesk]目的地，您必須完成欄位對應步驟。 對應包括在Experience Platform帳戶中的Experience Data Model (XDM)結構描述欄位與來自目標目的地的對應對應專案之間建立連結。
...
1. 前往[!DNL Zendesk]網站，然後導覽至&#x200B;**[!UICONTROL Contacts]**&#x200B;頁面，以檢查是否已新增對象中的設定檔。 此清單可設定為顯示使用audience**[!UICONTROL Mapping ID]**和對象狀態所建立之其他欄位的欄。
