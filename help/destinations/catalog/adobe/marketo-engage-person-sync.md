...
如果貴公司可存取多個組織，請務必在Marketo Engage和[!DNL Real-Time CDP]中使用相同的組織（您正在其中設定Marketo的目的地聯結器）。  如果您已設定目的地，您可以選取要搭配新設定使用的現有Marketo帳戶。  如果沒有，請按一下「聯結器至目的地」提示，此提示可讓您設定所要目的地的名稱、說明和Marketo Munchkin ID。  您可以在管理員 — >Munchkin功能表中找到您的Marketo執行個體的Munchkin ID。
