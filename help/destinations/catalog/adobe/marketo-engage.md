...
![兩張Marketo目的卡並排顯示的影像。](../..//assets/catalog/adobe/marketo-side-by-side-view.png)
...
>[!IMPORTANT]
>
>該視頻不完全反映當前功能。 有關最新資訊，請參閱上面連結的指南。 視頻的以下部分已過時：
> 
>* 在Experience PlatformUI中應使用的目標卡是 **[!UICONTROL MarketoV2]**。
>* 視頻不顯示新 **[!UICONTROL 人員建立]** 連接到目標工作流中的「選擇器」欄位。
>* 視頻中提到的兩個限制不再適用。 現在，除了錄制視頻時支援的段成員身份資訊之外，您還可以映射許多其他配置檔案屬性欄位。 您還可以將段成員導出到Marketo靜態清單中尚不存在的Marketo，這些成員將添加到清單中。
>* 在 **[!UICONTROL 計畫段步驟]** 在MarketoV1中，需要手動添加 **[!UICONTROL 映射ID]** 成功將資料導出到Marketo。 MarketoV2不再需要此手動步驟。
