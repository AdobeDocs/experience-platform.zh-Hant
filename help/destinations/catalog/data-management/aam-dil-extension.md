---
keywords: Audience ManagerDIL擴展；目標受眾管理器；dil擴展
title: Audience ManagerDIL擴展
description: Audience ManagerDIL擴展是Adobe Experience Platform的資料管理平台(DMP)目的地。 有關擴展功能的詳細資訊，請參閱AdobeExchange上的擴展頁。
...
# Audience ManagerDIL擴展 {#aam-dil-extension}

## 總覽 {#overview}

這是Adobe Audience ManagerData Integration Library擴展（客戶端實施）。 注：此擴展不應用於Adobe Analytics資料的伺服器端轉發(SSF)。 對於SSF，請使用Adobe Analytics分機。 重要提示：從8.0版開始，DIL對 [!DNL Experience Cloud] ID服務，版本3.3或更高版本。 請同時實施 [!DNL Experience Cloud] ID服務和DIL（完整） [!DNL Audience Manager] 資料整合功能。

[!DNL Audience Manager] DIL是Adobe Experience Platform的資料管理平台(DMP)擴展。 有關擴展功能的詳細資訊，請參見 [Audience Manager擴展頁](../../../tags/extensions/client/audience-manager/overview.md) 的下界。

此目標是標籤擴展。 有關擴展在平台中如何工作的詳細資訊，請參見 [標籤擴展概述](../launch-extensions/overview.md)。
...
有關擴展配置選項的資訊，請參見 [Audience Manager擴展頁](../../../tags/extensions/client/audience-manager/overview.md) 的下界。
