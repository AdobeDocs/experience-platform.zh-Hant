...
[!DNL LINE]使用持有人權杖作為驗證機制，以與[!DNL LINE]傳訊API通訊。 在[!DNL LINE]向目的地驗證[區段中，進一步說明如何向您的](#authenticate)執行個體進行驗證。
...
2. 選擇「**[!UICONTROL Create an account]**」。

#### 從[!DNL LINE channel access token (long-lived)]開發人員主控台收集[!DNL LINE] {#gather-credentials}

若要允許Experience Platform存取[!DNL LINE]資源，您將需要所需&#x200B;*[!DNL Channel access token (long-lived)]* [!DNL LINE]傳訊API *通道的*。
...
1. 最後，導覽至&#x200B;***[!DNL Channel access token]***&#x200B;區段，並複製&#x200B;***[!DNL Channel access token (long-lived)]***&#x200B;向目的地驗證[步驟中所需的](#authenticate)值。
...
| 項目 | 類型 | 附註 |
|---------|----------|---------|
| 匯出類型 | **[!UICONTROL Profile-based]** | 您正在匯出具有[!DNL LINE]目的地中所使用識別碼（名稱、電話號碼或其他）的對象的所有成員。 |
...
在&#x200B;**[!UICONTROL Destinations]** > **[!UICONTROL Catalog]**&#x200B;內搜尋[!DNL LINE]。 或者，您可以在&#x200B;**[!UICONTROL Mobile engagement]**&#x200B;類別下找到它。
...
* **[!UICONTROL Bearer token]**：您從[!DNL LINE Channel access token (long-lived)]開發人員主控台的[!DNL LINE]。 請參閱[收集認證](#gather-credentials)區段。
...
* **[!UICONTROL Audience Type]**：如果您要匯出的身分識別屬於廣告商(IFA)**[!UICONTROL ID for Advertisers(IFAs)]**&#x200B;的型別&#x200B;*ID，請選取*。 如果您要匯出的身分識別為&#x200B;**[!UICONTROL LINE user IDs]** LINE使用者ID *型別，請選取*。 如需身分型別的詳細資訊，請參閱[支援的身分](#supported-identities)區段。
...
若要將對象資料從[!DNL Adobe Experience Platform]正確傳送至[!DNL LINE]目的地，您必須完成欄位對應步驟。 對應包括在Experience Platform帳戶中的Experience Data Model (XDM)結構描述欄位與來自目標目的地的對應對應專案之間建立連結。 若要將您的XDM欄位正確對應到[!DNL LINE]目的地欄位，請遵循下列步驟：
...
1. 接下來，導覽至「**[!UICONTROL Data Controls]** > **[!UICONTROL Audiences]**」，並檢查&#x200B;**[!UICONTROL Audience name]**&#x200B;欄中符合所選對象的名稱。

1. 更新的磁碟區會符合區段內的計數。

1. 如果您匯出的身分屬於&#x200B;*UserID*&#x200B;型別，則&#x200B;**[!UICONTROL UserID]** Type *資料行將提及*。 同樣地，如果您匯出的身分屬於&#x200B;*IDFA*&#x200B;型別，則&#x200B;**[!UICONTROL Mobile ad Id]** Type *欄將提及*。
