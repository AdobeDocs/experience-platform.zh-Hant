...
| 項目 | 類型 | 附註 |
|---------|----------|---------|
| 匯出類型 | **[!UICONTROL Profile-based]** | 您正在匯出區段的所有成員，以及所需的結構欄位（例如：電子郵件地址、電話號碼、姓氏）和/或身分，視您的欄位對應而定。 |
...
[!DNL Adobe Experience Platform]個人資料屬性類似於[!DNL Airship]屬性，並可使用本頁面下方進一步示範的對應工具，在Experience Platform中輕鬆相互對應。
...
為協助您更清楚瞭解您應如何及何時使用[!DNL Airship Attributes]目的地，以下是[!DNL Adobe Experience Platform]客戶可以使用此目的地解決的範例使用案例。

### 使用案例#1 {#use-case-1}

利用[!DNL Adobe Experience Platform]內收集的設定檔資料，在[!DNL Airship]的任何管道內個人化訊息和豐富的內容。 例如，運用[!DNL Experience Platform]設定檔資料來設定[!DNL Airship]內的位置屬性。 這可讓飯店品牌為每位使用者顯示最近的飯店位置影像。

### 使用案例#2 {#use-case-2}

運用[!DNL Adobe Experience Platform]的屬性進一步擴充[!DNL Airship]設定檔，並將其與SDK或[!DNL Airship]預測資料結合。 例如，retailer可建立具有忠誠度狀態和位置資料（來自Experience Platform的屬性）以及[!DNL Airship]預測會流失資料的對象，以傳送高針對性訊息給住在內華達州拉斯維加斯且具有高流失率的金級忠誠度狀態使用者。
...
[!DNL Airship]屬性可以在代表裝置執行個體（例如iPhone）的頻道上設定，或是可將使用者的所有裝置對應到通用識別碼（例如客戶ID）的具名使用者上設定。 如果您的結構描述中有純文字（未雜湊）電子郵件地址作為主要身分，請在&#x200B;**[!UICONTROL Source Attributes]**&#x200B;中選取電子郵件欄位，並將對應到[!DNL Airship]下右側欄中的&#x200B;**[!UICONTROL Target Identities]**&#x200B;具名使用者，如下所示。
...
* 將Adobe `fullName`屬性新增至[!DNL Airship]「全名」屬性

>[!NOTE]
>
>選擇屬性對應的目標欄位時，請使用與`attribute_id`儀表板中屬性相對應的[!DNL Airship]。
