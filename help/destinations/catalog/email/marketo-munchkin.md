...
從銷售機會管理到以帳戶為基礎的行銷， [!DNL Marketo Engagement Platform] 簡化您在客戶和潛在客戶體驗的每個階段規劃、編排和衡量參與度的方式。

[!DNL Marketo’s Munchkin][!DNL Marketo] JavaScript 可追蹤一般使用者頁面瀏覽次數，以及您的 登陸頁面和外部網頁的點擊次數。

[!DNL Marketo Munchkin] 是Adobe Experience Platform中的電子郵件擴充功能。 如需Marketo Munchkin的詳細資訊，請參閱 [銷售機會追蹤](https://developers.marketo.com/javascript-api/lead-tracking/) 在Marketo檔案中。
