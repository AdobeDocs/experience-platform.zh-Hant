---
keywords: Marketo·蒙奇金；marketo munchkin;Marketo·蒙奇金擴展；marketo munchkin擴展；marketo munchkin擴展；marketo;Marketo
title: Marketo Munchkin 擴充功能
description: Marketo門奇金分機是Adobe Experience Platform的個性化目的地。 有關擴展功能的詳細資訊，請參閱AdobeExchange上的擴展頁。
...
從銷售線索管理到基於客戶的營銷， [!DNL Marketo Engagement Platform] 簡化您在客戶和潛在客戶體驗的每個階段的計畫、協調和衡量與他們的接觸的方式。

[!DNL Marketo’s Munchkin][!DNL Marketo] JavaScript 可追蹤一般使用者頁面瀏覽次數，以及您的 登陸頁面和外部網頁的點擊次數。

[!DNL Marketo Munchkin] 是Adobe Experience Platform的電子郵件分機。 有關Marketo·蒙奇金的詳細資訊，請閱讀 [線索跟蹤](https://developers.marketo.com/javascript-api/lead-tracking/) 在Marketo檔案里。
