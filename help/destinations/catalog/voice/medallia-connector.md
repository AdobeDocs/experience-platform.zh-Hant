...
| 項目 | 類型 | 附註 |
|---------|----------|---------|
| 匯出類型 | **[!UICONTROL Profile-based]** | 您正在匯出區段的所有新合格成員，以及所需的結構描述欄位（例如：電子郵件地址、電話號碼、姓氏），如[目的地啟用工作流程](/help/destinations/ui/activate-batch-profile-destinations.md#select-attributes)的選取設定檔屬性畫面中所選。 |
...
* **[!UICONTROL OAuth Token Endpoint URL]**：通常採用https://instance.medallia.tld/oauth/tenant/token的形式。
* **[!UICONTROL Client ID]**：從您的Medallia傳遞團隊取得。
* **[!UICONTROL Client Secret]**：從您的Medallia傳遞團隊取得。
...
* **[!UICONTROL API Gateway URL]**：從您的Medallia傳遞團隊取得。 通常採用https://instance-tenant.apis.medallia.com的形式。
* **[!UICONTROL Import API Name]**：從您的Medallia傳遞團隊取得。 用於此連線的Medallia Import API （也稱為Web摘要）名稱。 您可以對不同的匯入API啟用不同的對象，以觸發不同的調查計畫。
