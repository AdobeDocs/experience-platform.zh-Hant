...
HTTP API目的地是Experience Platform串流目的地，可協助您將設定檔資料傳送至第三方HTTP端點。

若要將設定檔資料傳送至HTTP端點，您必須先[連線至Experience Platform中的目的地](#connect-destination)。

## 使用案例 {#use-cases}

使用HTTP API目標將XDM設定檔資料和對象匯出至一般HTTP端點。 在那裡，您可以對從Experience Platform匯出的設定檔資料執行自己的分析或執行任何其他您可能需要的操作。
...
| 項目 | 類型 | 附註 |
| ---------|----------|---------|
| 匯出類型 | **[!UICONTROL Profile-based]** | 您正在匯出對象的所有成員，以及所需的結構描述欄位（例如：電子郵件地址、電話號碼、姓氏），如[目的地啟用工作流程](../../ui/activate-segment-streaming-destinations.md#mapping)的對映畫面中所選。 |
...
* 如果您打算使用mTLS：您的資料接收端點必須停用TLS，而且只能啟用mTLS。
...
您可以使用[!DNL Mutual Transport Layer Security] (mTLS)來確保與您的HTTP API目的地連線的輸出連線中的增強安全性。

mTLS是一種相互驗證通訊協定，可確保共用資訊的雙方在共用資料之前，都是聲稱的身分。 mTLS包括比標準TLS更多的步驟，其中伺服器也會要求並驗證使用者端的憑證，而使用者端會驗證伺服器的憑證。
...
若要搭配HTTP API目的地使用mTLS，您在&#x200B;**[!UICONTROL HTTP Endpoint]**&#x200B;目的地詳細資料[頁面中設定的](#destination-details) （資料接收端點）必須停用TLS通訊協定，並且只啟用mTLS。 如果端點上仍啟用TLS 1.2通訊協定，則不會傳送使用者端驗證的憑證。 這表示若要將mTLS與HTTP API目的地搭配使用，您的資料接收伺服器端點必須是啟用mTLS的連線端點。

### 擷取及檢查憑證詳細資料 {#certificate}

如果您想要檢查憑證詳細資訊，例如一般名稱(CN)和主體替代名稱(SAN)以進行其他第三方驗證，請使用API來擷取憑證，並從回應中擷取這些欄位。

如需詳細資訊，請參閱[公用憑證端點檔案](../../../data-governance/mtls-api/public-certificate-endpoint.md)。

## IP位址允許清單 {#ip-address-allowlist}

為了滿足客戶的安全性和合規性要求，Experience Platform提供您可以允許列出HTTP API目的地的靜態IP清單。 如需允許清單的IP完整清單，請參閱串流目的地的[IP位址允許清單](/help/destinations/catalog/streaming/ip-address-allow-list.md)。
...
如果您選取&#x200B;**[!UICONTROL Bearer token]**&#x200B;驗證型別以連線至您的HTTP端點，請輸入下列資訊並選取&#x200B;**[!UICONTROL Connect to destination]**：

![具有[!UICONTROL Bearer token]欄位的HTTP API驗證畫面。](../../assets/catalog/http/http-api-authentication-bearer.png)

* **[!UICONTROL Bearer token]**：輸入持有人權杖以驗證您的HTTP位置。

#### 無驗證 {#no-authentication}

如果您選取&#x200B;**[!UICONTROL None]**&#x200B;驗證型別以連線至您的HTTP端點：

![已選取[!UICONTROL None]驗證型別的HTTP API驗證畫面。](../../assets/catalog/http/http-api-authentication-none.png)

當您選取此驗證選項時，您只需要選取&#x200B;**[!UICONTROL Connect to destination]**，即可建立與端點的連線。

#### OAuth 2密碼驗證 {#oauth-2-password-authentication}

如果您選取&#x200B;**[!UICONTROL OAuth 2 Password]**&#x200B;驗證型別以連線至您的HTTP端點，請輸入下列資訊並選取&#x200B;**[!UICONTROL Connect to destination]**：

![具有[!UICONTROL OAuth 2 Password]欄位的HTTP API驗證畫面。](../../assets/catalog/http/http-api-authentication-oauth2-password.png)

* **[!UICONTROL Access Token URL]**：您發行存取權杖以及選擇性地重新整理權杖的URL。
* **[!UICONTROL Client ID]**：您的系統指派給Adobe Experience Platform的`client ID`。
* **[!UICONTROL Client Secret]**：您的系統指派給Adobe Experience Platform的`client secret`。
...
如果您選取&#x200B;**[!UICONTROL OAuth 2 Client Credentials]**&#x200B;驗證型別以連線至您的HTTP端點，請輸入下列資訊並選取&#x200B;**[!UICONTROL Connect to destination]**：

![具有[!UICONTROL OAuth 2 Client Credentials]欄位的HTTP API驗證畫面。](../../assets/catalog/http/http-api-authentication-oauth2-client-credentials.png)
...
* **[!UICONTROL Client ID]**：您的系統指派給Adobe Experience Platform的`client ID`。
* **[!UICONTROL Client Secret]**：您的系統指派給Adobe Experience Platform的`client secret`。
* **[!UICONTROL Client Credentials Type]**：選取端點支援的OAuth 2使用者端認證授權型別：
   * **[!UICONTROL Body Form Encoded]**：在此情況下，`client ID`和`client secret`會包含在傳送至您目的地的要求&#x200B;*內文中*。 如需範例，請參閱[支援的驗證型別](#supported-authentication-types)區段。
   * **[!UICONTROL Basic Authorization]**：在此情況下，`client ID`和`client secret`在經過base64編碼並傳送至您的目的地之後，會包含在&#x200B;*標頭`Authorization`中的*。 如需範例，請參閱[支援的驗證型別](#supported-authentication-types)區段。
...
>title="HTTP 端點"
>abstract="用於將輪廓資料傳送到的 HTTP 端點的 URL。 這是您的資料接收端點，並且支援 mTLS (若已設定)。"
...
![包含已完成欄位的HTTP API目的地詳細資訊畫面。](../../assets/catalog/http/http-api-destination-details.png)
...
* **[!UICONTROL Include Segment Names]**：如果您希望資料匯出包含您正在匯出的對象名稱，請切換按鈕。 **注意**：對象名稱僅包含在對映至目的地的對象。 匯出中出現的未對應對象將不包括`name`欄位。 如需選取此選項的資料匯出範例，請參閱下方的[匯出的資料](#exported-data)區段。
...
您可以啟用警報以接收有關傳送到您目的地的資料流狀態的通知。 從清單中選取警報以訂閱接收有關資料流狀態的通知。 如需警示的詳細資訊，請參閱[使用UI訂閱目的地警示指南](../../ui/alerts.md)。
...
* 設定檔更新是由[身分對應](/help/xdm/field-groups/profile/identitymap.md)中的變更所決定。 例如，已符合對應至目的地之其中一個對象資格的設定檔，已將新身分新增至身分對應屬性。
* 設定檔更新是由對應到目的地的至少一個屬性的變更所決定。 例如，會將對應步驟中對應至目的地的其中一個屬性新增至設定檔。

在上述所有情況下，只有已發生相關更新的設定檔才會匯出至您的目的地。 例如，如果對應至目的地流程的受眾有一百個成員，且有五個新設定檔符合受眾資格，則匯出至您的目的地的作業會以增量方式進行，且僅包含五個新設定檔。

>[!NOTE]
>
>無論變更位於何處，都會匯出設定檔的所有對應屬性。 因此，在上述範例中，將會匯出這五個新設定檔的所有對應屬性，即使屬性本身並未變更亦然。
...
|---------|----------|
| <ul><li>對應的屬性和受眾可作為目的地匯出的提示。 這表示如果設定檔的`segmentMembership`狀態變更為`realized`或`exiting`，或任何對應的屬性已更新，將會啟動目的地匯出。</li><li>由於身分目前無法對應至HTTP API目的地，因此特定設定檔上任何身分的變更也會決定目的地匯出專案。</li><li>屬性的變更定義為屬性上的任何更新，無論其是否為相同的值。 這表示即使值本身並未變更，屬性上的覆寫也會視為變更。</li></ul> | <ul><li>`segmentMembership`物件包含啟動資料流中對應的對象，在資格或對象退出事件後，設定檔的狀態已針對該對象變更。 請注意，如果其他未對應對象與啟動資料流中對應的對象屬於相同的[合併原則](/help/profile/merge-policies/overview.md)，則符合設定檔資格的其他未對應對象可以屬於目的地匯出的一部分。<br> **重要**：啟用&#x200B;**[!UICONTROL Include Segment Names]**&#x200B;選項時，區段名稱僅會包含在對應至目的地的對象中。 匯出中顯示的未對應對象不會包含`name`欄位，即使該選項已啟用亦然。 </li><li>`identityMap`物件中的所有身分也包括在內（Experience Platform目前不支援HTTP API目的地中的身分對應）。</li><li>目的地匯出僅包含對應的屬性。</li></ul> |
...
當設定檔符合或退出&#x200B;*三個對應對象*&#x200B;其中之一時，就會觸發設定檔匯出至目的地。 在資料匯出中，`segmentMembership`物件（請參閱下方的[匯出的資料](#exported-data)）也可以包含未對應的對象，如果該設定檔為其成員，且這些對象與觸發匯出的對象共用相同的合併原則。 例如，如果設定檔符合&#x200B;**擁有DeLorean Cars的客戶**&#x200B;對象的資格，但同時也是&#x200B;**觀看的「回到未來」**&#x200B;電影和&#x200B;**科幻迷**&#x200B;對象的成員，則這兩個對象也會出現在`segmentMembership`物件中 — 前提是它們與&#x200B;**擁有DeLorean Cars的客戶**&#x200B;對象共用相同的合併原則。
...
您匯出的Experience Platform資料會以JSON格式登陸至HTTP目的地。 例如，以下匯出包含一個符合特定對象資格的設定檔、是另一個兩個對象的成員，並退出另一個對象。 匯出也包含設定檔屬性的名字、姓氏、出生日期和個人電子郵件地址。 此設定檔的身分識別為ECID和電子郵件。
...
>[!NOTE]
>
>在此範例中，第一個對象(`5b998cb9-9488-4ec3-8d95-fa8338ced490`)已對應至目的地，並包含`name`欄位。 第二個對象(`354e086f-2e11-49a2-9e39-e5d9a76be683`)未對應到目的地，而且不包含`name`欄位，即使已啟用&#x200B;**[!UICONTROL Include Segment Names]**&#x200B;選項也是如此。
...
95%的情況下，Experience Platform會嘗試針對每個資料流向HTTP目的地的成功傳送訊息，以每秒少於10,000個要求的速率，提供少於10分鐘的輸送量延遲。

當對您的HTTP API目的地的請求失敗時，Experience Platform會儲存這些請求並重試兩次。

## 疑難排解 {#troubleshooting}

若要確保可靠的資料傳送並避免逾時問題，請確定您的HTTP端點在2秒內回應Experience Platform要求，如[先決條件](#prerequisites)區段中所指定。 需要更長時間的回應會導致逾時錯誤。
