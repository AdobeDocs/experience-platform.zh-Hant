...
| 項目 | 類型 | 附註 |
---------|----------|---------|
| 匯出類型 | **[!UICONTROL 以設定檔為基礎]** | 您正在匯出區段的所有成員，以及所需的結構欄位（例如：電子郵件地址、電話號碼、姓氏），如在對應畫面中所選。 [目的地啟用工作流程](../../ui/activate-segment-streaming-destinations.md#mapping). |
...
您可以使用 [!DNL Mutual Transport Layer Security] ([!DNL mTLS])，以確保與您的HTTP API目的地連線的輸出連線具有增強的安全性。

[!DNL mTLS] 是一種用於相互驗證的端對端安全性方法，可確保共用資訊的雙方在共用資料之前都是聲稱的身分。 [!DNL mTLS] 包含額外的步驟，以便 [!DNL TLS]，其中伺服器也會要求使用者端的憑證，並在其末端進行驗證。

如果您想使用 [!DNL mTLS] 替換為 [!DNL HTTP API] 目的地，也就是您放入 [目的地詳細資料](#destination-details) 頁面必須具有 [!DNL TLS] 通訊協定已停用，且僅限 [!DNL mTLS] 已啟用。 如果 [!DNL TLS] 端點上的1.2通訊協定仍處於啟用狀態，不會為使用者端驗證傳送任何憑證。 這表示要使用 [!DNL mTLS] 與您的 [!DNL HTTP API] 目的地，您的「接收」伺服器端點必須是 [!DNL mTLS]-only啟用的連線端點。

### 下載憑證 {#certificate}

如果您想檢查 [!DNL Common Name] (CN)和 [!DNL Subject Alternative Names] (SAN)若要進行其他協力廠商驗證，您可以下載以下憑證：
