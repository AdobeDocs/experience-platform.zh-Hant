...
為達到最佳比對準確度，除了上述[必要對應](#mandatory-mappings)之外，在啟動流程中加入下列選擇性對應。
...
* **主要比對**：如果存在`externalPersonId`，Demandbase會使用它作為人員比對的主要識別碼。
* **遞補比對**：如果無法使用`externalPersonId`，Demandbase會使用`email`欄位進行識別。
* **必要與建議**：雖然Demandbase只需要`email`，但Adobe建議從上方建議的對應表格中對應所有可用的欄位，以改善比對正確性與行銷活動效能。
