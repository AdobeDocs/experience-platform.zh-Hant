...
行銷人員可使用Adobe [!DNL Real-Time CDP]建立第一方聯絡人的人員清單，並在Demandbase中加以啟用，以便在其需求端平台(DSP)和其他管道（例如LinkedIn）間最佳化和協調參與情形。
...
### 建議的對應 {#recommended-mappings}

為獲得最佳比對準確度，除了上述[必要對應](#mandatory-mappings)之外，還應在啟動流程中加入下列選擇性對應。
...
將欄位對應到[!DNL Demandbase People]時，請考慮下列相符行為：

* **主要比對**： Demandbase使用`externalPersonId`作為人員比對的主要識別碼。
* **遞補比對**：如果無法使用`externalPersonId`，Demandbase會使用`email`欄位進行識別。
* **建議的欄位**：雖然只需要`email`和`externalPersonId`，但Adobe建議從上述建議的對應表格中對應所有可用的欄位，以改善比對正確性和行銷活動效能。
...
* **對象命名**：如果先前在Demandbase中啟用相同名稱的使用者對象，則無法透過不同資料流在Demandbase目的地再次啟用。
