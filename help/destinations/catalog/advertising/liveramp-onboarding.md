...
* **[!UICONTROL Port]**：用於您[!DNL LiveRamp - Onboarding]儲存位置的連線埠。  使用與您地理位置對應的連線埠，如下所述：
   * **[!UICONTROL NA]**：使用連線埠`22`
   * **[!UICONTROL AU]**：使用連線埠`2222`
* **[!UICONTROL Username]**： [!DNL LiveRamp - Onboarding]儲存位置的使用者名稱。
* **[!UICONTROL Password]**：您[!DNL LiveRamp - Onboarding]儲存位置的密碼。
* **[!UICONTROL PGP/GPG encryption key]**：您可以選擇附加您的RSA格式公開金鑰，以新增加密至匯出的檔案。 在下圖中檢視格式正確的加密金鑰範例。
  ![影像顯示UI中格式正確的PGP金鑰範例](../../assets/catalog/advertising/liveramp-onboarding/pgp-key.png)
* **[!UICONTROL Subkey ID]**:If您提供加密金鑰，您也必須提供加密&#x200B;**[!UICONTROL Subkey ID]**。 請參閱[!DNL LiveRamp] [加密檔案](https://docs.liveramp.com/connect/en/encrypting-files-for-uploading.html#downloading-the-current-encryption-key)以瞭解如何取得子金鑰ID。
...
* **[!UICONTROL Username]**： [!DNL LiveRamp - Onboarding]儲存位置的使用者名稱。
* **[!UICONTROL SSH Key]**：用來登入[!DNL SSH]儲存位置的私人[!DNL LiveRamp - Onboarding]金鑰。 私密金鑰必須格式化為[!DNL Base64]編碼的字串，且不得受密碼保護。
...
在&#x200B;**[!UICONTROL Mapping]**&#x200B;步驟中，您可以選取要為設定檔匯出哪些屬性和身分。
...
在&#x200B;**[!UICONTROL Mapping]**&#x200B;步驟中，**[!UICONTROL Target field]**&#x200B;對應會定義匯出的CSV檔案中欄標題的名稱。 您可以為&#x200B;**[!UICONTROL Target field]**&#x200B;提供自訂名稱，將匯出檔案中的CSV欄標題變更為任何您想要的易記名稱。
...
這表示&#x200B;**[!UICONTROL Identities activated]**&#x200B;資料流執行&#x200B;**[!UICONTROL Profiles received]**&#x200B;頁面中的[和](../../../dataflows/ui/monitor-destinations.md#dataflow-runs-for-batch-destinations)量度，會針對使用相同合併原則的每個對象群組進行彙總，而非針對每個對象顯示。
