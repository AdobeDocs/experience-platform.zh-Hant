---
title: 菱形批次目的地
description: 使用此目的地，以批次方式將Adobe CDP對象傳送至Magnite串流平台。
...
Adobe [!DNL Real-Time CDP]對象有兩種傳送至Magnite串流平台的方式 — 可以每天傳送一次，或是即時傳送：
...
回顧：如果您每天只想傳送Adobe [!DNL Real-Time CDP]對象一次，您只會使用Magnite：批次目的地，而且每天會傳送對象一次。 如果您想要即時傳送Adobe [!DNL Real-Time CDP]對象，您將會使用&#x200B;*both* Magnite：批次目的地和Magnite即時目的地。 如需詳細資訊，請造訪Magnite：串流。


繼續閱讀以下內容，瞭解有關Magnite：批次目的地、如何連線至該目的地，以及如何為其啟用Adobe [!DNL Real-Time CDP]對象。
...
為協助您更清楚瞭解您應如何及何時使用Magnite：批次目的地，以下是[!DNL Adobe Experience Platform]客戶可使用此目的地解決的範例使用案例。
...
若要在[!DNL Magnite]中使用[!DNL Adobe Experience Platform]目的地，您必須先擁有Magnite串流帳戶。 如果您有[!DNL Magnite Streaming]帳戶，請洽詢您的[!DNL Magnite]帳戶管理員，以取得存取[!DNL Magnite's]目的地的認證。 如果您沒有[!DNL Magnite Streaming]帳戶，請連絡adobe-tech@magnite.com

## 支援的身分 {#supported-identities}

Magnite：批次目的地可以從Adobe CDP接收&#x200B;*任何*&#x200B;身分來源。 目前，此目的地有三個目標身分欄位可供您對應。
...
在Adobe Experience目錄中找到Magnite：批次目的地。 按一下其他選項按鈕(\...)，然後設定目的地連線/執行個體。

如果您已經有現有的帳戶，您可以將「帳戶型別」選項變更為「現有帳戶」來尋找該帳戶。 否則，您將在下方建立帳戶：

若要建立新帳戶並首次驗證至目的地，請填寫必要的「S3存取金鑰」和「S3秘密金鑰」欄位（透過帳戶管理員提供給您），然後選取&#x200B;**[!UICONTROL Connect to destination]**
...
* **[!UICONTROL Name]**：您用來辨識此目的地連線/執行個體的名稱
未來。
* **[!UICONTROL Description]**：可協助您識別此專案的說明
未來的目的地連線/執行個體。
...
選取之後，或如果您要略過此選擇性畫面，請選取&#x200B;**[!UICONTROL Create]**
...
在&#x200B;**[!UICONTROL Source field]**中，您可以為您的裝置選取任何屬性或身分。 在此範例中，我們已選取名為「DeviceId」的自訂IdentityMap
...
在此範例中，我們已選取**[!UICONTROL Target field]**： magnite_deviceId_CUSTOM，因為我們的&#x200B;**[!UICONTROL Source field]**&#x200B;已定義為自訂IdentityMap： DeviceID。
...
* The Magnite：批次目的地以每日順序將S3檔案傳送至Magnite串流。 傳送和內嵌後，對象/區段預計會顯示在Magnite串流中，並可套用至交易。 您可以查詢[!DNL Adobe Experience Platform]中啟動步驟期間共用的區段ID或區段名稱來確認此專案。
