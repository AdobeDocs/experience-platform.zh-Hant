...
>[!IMPORTANT]
>
>此目標目前為Beta版，僅對有限數量的客戶可用。 請求訪問 [!DNL Google Ad Manager 360] 連接，請與您的Adobe代表聯繫，並 [!DNL organization ID]。
...
在設定第一個允許清單之前，必須列出 [!DNL Google Ad Manager 360] 目標。 在建立目標之前，請確保完成下面所述的允許清單過程。

>[!NOTE]
>
>此規則的例外是現有規則 [Audience Manager](https://experienceleague.adobe.com/docs/audience-manager/user-guide/aam-home.html) 客戶。 如果您已在Audience Manager中建立到此Google目標的連接，則無需再次執行允許清單過程，您可以繼續執行下一步。
...
* **[!UICONTROL 秘密訪問密鑰]**:一個40個字元的base64編碼字串，用於驗證 [!DNL Google Cloud Storage] 帳戶到平台。
...
* **[!UICONTROL 將段ID追加到段名稱]**:選擇此選項使Google廣告管理器360中的段名稱包括來自Experience Platform的段ID，如下所示： `Segment Name (Segment ID)`。
