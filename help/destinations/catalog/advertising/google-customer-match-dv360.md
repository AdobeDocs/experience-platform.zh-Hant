...
為了提升此版本，他們會使用電子郵件地址作為識別碼，從CRM資料庫將電子郵件地址上傳到Experience Platform。 建立受眾的基礎是擁有舊款手機型號的客戶。 然後會將對象傳送到[!DNL Google Customer Match]，以便公司可以鎖定目前客戶、擁有舊版手機型號的客戶，以及具有[!DNL Google Display & Video 360]屬性（例如[!DNL Search]、[!DNL YouTube]、[!DNL Gmail]和[!DNL Google Display Network]）的類似客戶。
