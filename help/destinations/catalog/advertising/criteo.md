...
| 目標身分 | 說明 | 考量事項 |
| --- | --- | --- |
| `email_sha256` | 使用SHA-256演演算法雜湊的電子郵件地址 | [!DNL Adobe Experience Platform]同時支援純文字和SHA-256雜湊電子郵件地址。 當您的來源欄位包含未雜湊的屬性時，請核取[!UICONTROL Apply transformation]選項，讓Experience Platform在啟用時自動雜湊資料。 |
...
為協助您更清楚瞭解如何使用[!DNL Criteo]目的地，以下是[!DNL Adobe Experience Platform]客戶可以透過[!DNL Criteo]達成的一些目標：

### 使用案例1 ：取得流量 {#use-case-1}

利用相關的產品選件和彈性的創意來展示您的企業。 有了智慧型產品推薦，您的廣告會自動加入最有可能觸發造訪和參與的產品。 彈性鎖定目標可讓您從Criteo的商務資料集或您自己的潛在客戶清單和Adobe CDP區隔建立對象。

### 使用案例2 ：提高網站轉換率 {#use-case-2}
...
1. 登入[!DNL Adobe Experience Platform]並連線到Criteo目的地。
