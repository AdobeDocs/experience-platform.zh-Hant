---
keywords: 按兩下泛光燈；按兩下泛光燈擴展；按兩下泛光燈；按兩下泛光燈
title: 按兩下泛光燈(Beta)擴展
description: 按兩下泛光燈(Beta)擴展是Adobe Experience Platform的廣告目的地。 有關擴展功能的詳細資訊，請參閱AdobeExchange上的擴展頁。
...
# [!DNL DoubleClick Floodlight] (Beta)擴展

## 總覽 {#overview}

此擴展允許快速、方便地部署 [!DNL DoubleClick Floodlight] 使用傳統泛光燈格式（而非全局站點標籤）的標籤。 注：此擴展為Beta。

[!DNL DoubleClick Floodlight] (Beta)是Adobe Experience Platform的廣告推廣。 有關擴展功能的詳細資訊，請參見 [!DNL Google] 支援文檔 [按兩下泛光燈](https://support.google.com/dcm/answer/2823388?hl=en)。

此目標是標籤擴展。 有關標籤擴展在平台中如何工作的詳細資訊，請參見 [標籤擴展概述](../launch-extensions/overview.md)。

![按兩下泛光燈擴展](../../assets/catalog/advertising/doubleclick-floodlight/catalog.png)
