...
* 將受眾回填啟用至 [!DNL Google Display & Video 360] 目的地會排程在對象首次對應至目的地連線的24到48小時後發生。 此更新是為了回應Google等候24小時直到擷取資料的原則，並旨在提高Real-time CDP與之間的匹配率 [!DNL Google Display & Video 360]. 請注意，這是僅適用於此目的地的後端設定，並且和 UI 中任何客戶可設定的排程選項不相關。
