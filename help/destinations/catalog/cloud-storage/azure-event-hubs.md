...
>[!IMPORTANT]
>
>即時 [!DNL Azure Event Hubs] CDP中的目標當前處於測試階段。 文件和功能可能會有所變更。
...
* 若要連線至使 [!DNL Azure Event Hubs] 用API呼叫，請參閱串 [流目標API教學課程](../../api/streaming-destinations.md)。
* 要連接到 [!DNL Azure Event Hubs] 使用即時CDP用戶介面，請參見以下各節。
...
如需如 [何連線至您的雲端儲 ](./workflow.md)存目的地的指示，請參閱雲端儲存目的地工作流程，包括 [!DNL Azure Event Hubs]:
