...
Adobe Experience Platform可以將區段以資料檔案的形式提供至您的雲端儲存位置。 這可讓您透過的CSV檔案，將對象及其設定檔屬性傳送至內部系統 [!DNL Amazon S3], [!DNL Azure Blob] 和SFTP。 針對 [!DNL Amazon Kinesis] 和 [!DNL Azure Event Hubs] 目的地，資料會串流退出Experience Platform, [!DNL JSON] 格式。
