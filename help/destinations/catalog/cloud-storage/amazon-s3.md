...
若要驗證到目的地，請填寫必填欄位並選取&#x200B;**[!UICONTROL Connect to destination]**。 Amazon S3目的地支援兩種驗證方法：

* 存取金鑰和機密金鑰驗證
* 假定角色驗證

#### 使用S3存取金鑰和秘密金鑰進行驗證 {#s3-access-key-secret-key-auth}
...
5. 輸入角色名稱（例如，`destinations-role-customer`）。 角色名稱應視為機密名稱，類似於密碼。 其長度最多可為64個字元，並且可以包含英數字元和下列特殊字元： `+=,.@-_`。 然後確認：
   * Adobe帳戶ID `670664943635`存在於&#x200B;**[!UICONTROL Select trusted entities]**&#x200B;區段中
   * 先前建立的原則存在於&#x200B;**[!UICONTROL Permissions policy summary]**&#x200B;中
