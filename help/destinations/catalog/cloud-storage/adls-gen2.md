---
title: (Beta)Azure Data Lake Storage Gen2連接
description: 瞭解如何連接到Azure Data Lake Storage Gen2以激活段和導出資料集。
...
# (Beta) [!DNL Azure Data Lake Storage Gen2] 連接

>[!IMPORTANT]
>
>此目標目前為Beta版，僅對有限數量的客戶可用。 請求訪問 [!DNL Azure Data Lake Storage Gen2] 連接，請與您的Adobe代表聯繫，並 [!DNL Organization ID]。

## 總覽 {#overview}

閱讀此頁可瞭解如何建立到您的 [[!DNL Azure Data Lake Storage Gen2]](https://learn.microsoft.com/en-us/azure/storage/blobs/data-lake-storage-introduction) ([!DNL ADLS Gen2])資料湖，定期從Experience Platform導出資料檔案。
...
* **[!UICONTROL URL]**:的終結點 [!DNL Azure Data Lake Storage Gen2]。 端點模式為： `abfss://<container>@<accountname>.dfs.core.windows.net`。
* **[!UICONTROL 租戶]**:包含應用程式的租戶資訊。
* **[!UICONTROL 服務主體ID]**:應用程式的客戶端ID。
* **[!UICONTROL 服務主密鑰]**:應用程式的密鑰。
...
在 **[!UICONTROL 計畫]** 步驟 [設定導出計畫](/help/destinations/ui/activate-batch-profile-destinations.md#scheduling) 為 [!DNL Azure Data Lake Storage Gen2] 目標，您也 [配置導出檔案的名稱](/help/destinations/ui/activate-batch-profile-destinations.md#file-names)。
...
要驗證資料是否已成功導出，請檢查 [!DNL Azure Data Lake Storage Gen2] 並確保導出的檔案包含預期的配置檔案總體。
