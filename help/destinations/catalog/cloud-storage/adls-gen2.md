...
* 若要使用Experience Platform使用者介面連線至您的[!DNL ADLS Gen2]儲存位置，請閱讀以下章節： [連線至目的地](#connect)及[啟用對象至此目的地](#activate)。
* 若要以程式設計方式連線至您的[!DNL ADLS Gen2]儲存位置，請使用「流程服務API」教學課程[，讀取](../../api/activate-segments-file-based-destinations.md)將對象啟用至檔案型目的地。
...
* **[!UICONTROL Tenant]**：包含您應用程式的租使用者資訊。
* **[!UICONTROL Service principal ID]**：應用程式的使用者端識別碼。
* **[!UICONTROL Service principal key]**：應用程式的金鑰。
...
在&#x200B;**[!UICONTROL Scheduling]**&#x200B;步驟中，您可以[設定](/help/destinations/ui/activate-batch-profile-destinations.md#scheduling)目的地的匯出排程[!DNL Azure Data Lake Storage Gen2]，也可以[設定匯出檔案的名稱](/help/destinations/ui/activate-batch-profile-destinations.md#configure-file-names)。
