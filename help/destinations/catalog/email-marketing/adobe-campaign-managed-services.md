...
![Adobe Campaign Managed Cloud Services目的地詳細資料表單，其中顯示[名稱]、[說明]、[執行個體選擇]、[目標對應]及[同步型別]等欄位。](../../assets/catalog/email-marketing/adobe-campaign-managed-services/destination-details.png)
...
1. 識別強制屬性和重複資料刪除索引鍵。 請注意，標示為「必要」或「重複資料刪除索引鍵」的屬性中的值不可以是Null。
   ...
   * [重複資料刪除索引鍵](../../ui/activate-batch-profile-destinations.md#mandatory-attributes)是決定使用者要為其設定檔進行重複資料刪除之身分的主索引鍵。
   ...
   ![屬性對應畫面會顯示對應至Adobe Campaign目標欄位的XDM來源欄位，以及必要和重複資料刪除索引鍵。](../../assets/catalog/email-marketing/adobe-campaign-managed-services/mapping.png)
...
![Adobe Campaign對象載入工作畫面顯示從Adobe Experience Platform啟用的匯出工作。](../../assets/catalog/email-marketing/adobe-campaign-managed-services/campaign-jobs.png)

### 存取匯出的資料 {#data}

若為&#x200B;**[!UICONTROL Audience sync]**，您可以導覽至&#x200B;**[!UICONTROL Profile and target]** > **[!UICONTROL List]** > **[!UICONTROL AEP audiences]**&#x200B;功能表以檢查匯出的對象。

![Adobe Campaign AEP對象清單檢視會顯示從Experience Platform匯出的對象，可在「設定檔」和「目標」下使用。](../../assets/catalog/email-marketing/adobe-campaign-managed-services/campaign-audiences.png)
