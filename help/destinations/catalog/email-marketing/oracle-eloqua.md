...
若要將對象資料傳送至[!DNL Oracle Eloqua]，您必須先在[中](#connect-destination)連線目的地[!DNL Adobe Experience Platform]，然後[設定從儲存位置匯入](#import-data-into-eloqua)到[!DNL Oracle Eloqua]的資料。
...
將[!DNL Experience Platform]連線到您的[!DNL SFTP]儲存體後，您必須設定從儲存位置匯入到[!DNL Oracle Eloqua]的資料匯入。 若要瞭解如何完成此作業，請參閱[中的](https://docs.oracle.com/cloud/latest/marketingcs_gs/OMCAA/Help/DataImportExport/Tasks/ImportingContactsOrAccounts.htm)匯入連絡人或帳戶[!DNL Oracle Eloqua Help Center]。
