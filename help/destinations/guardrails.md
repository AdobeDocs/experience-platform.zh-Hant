...
| 資料集類型 | 護欄 | 護欄型別 | 說明 |
---------|----------|---------|-------|
| 資料集根據 **XDM體驗事件結構描述** | 過去365天的資料 | 強烈 | 系統會匯出上一個日曆年的資料。 |
| 資料集根據 **XDM個別設定檔結構描述** | 在資料流中所有匯出的檔案有100億筆記錄 | 強烈 | 壓縮的JSON或Parquet檔案的資料集記錄計數必須少於十億，未壓縮的Parquet檔案的資料集記錄計數必須少於一百萬個，否則匯出會失敗。 如果資料集大於允許的臨界值，請縮小您嘗試匯出的資料集大小。 |
