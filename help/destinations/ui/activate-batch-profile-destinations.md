...
本文說明在[!DNL Adobe Experience Platform]中啟用對象以批次設定檔檔案型目的地（例如雲端儲存空間與電子郵件行銷目的地）所需的工作流程。
...
1. 使用&#x200B;**[!UICONTROL Frequency]**&#x200B;選擇器來選取匯出頻率：
   ...
   * **[!UICONTROL Monthly]**：選取開始日期，後續的匯出會在該月的該日期發生，直到選取的結束日期為止。 對於少於 30 或 31 天的月份，匯出將在該月的最後一天進行。
   ...
   >[!NOTE]
   >
   > 目前只有下列以檔案為基礎的雲端儲存空間目的地才支援每週和每月排程選項，而且只有在啟用[人員對象](../../segmentation/types/overview.md#people-audience)和[潛在對象](../../segmentation/types/overview.md#prospect-audience)時才支援。
   > 
   > * [Amazon S3](../catalog/cloud-storage/amazon-s3.md)
   > * [Azure Blob儲存體](../catalog/cloud-storage/azure-blob.md)
...
使用&#x200B;**[!UICONTROL Scheduled]**&#x200B;選項讓啟動工作以固定時間執行。 此選項可確保每天的相同時間匯出Experience Platform設定檔資料。 不過，您匯出的設定檔可能不是最新的，這取決於批次分段工作是否在啟動工作開始之前完成。
   ...
   設定匯出排程時，請在完成啟動流程後設定開始時間至少&#x200B;**1小時**。 對象啟用最多可能需要1小時的時間，才能在整個系統中傳播。 如果您排程在啟用後1小時內執行匯出，排程的匯出可能會錯過。
...
### 瞭解排定的匯出行為 {#export-behavior}

已排程的匯出專案包括對象快照集資料，以及在快照集建立和匯出之間發生的任何增量設定檔或身分變更。 這與[隨選匯出](export-file-now.md)不同，後者只使用快照集資料。

下表著重說明排程匯出與隨選匯出的差異，尤其是資料新鮮度和預期用途方面：
...
如果設定檔在對象評估後更新，則排程的匯出將包括更新的屬性值，即使對象成員資格是在評估時確定的。

**範例**： 「retailID為null的設定檔」對象可能會匯出填入retailID的設定檔，如果該欄位在&#x200B;*評估後*&#x200B;但在&#x200B;*排程匯出前*&#x200B;更新。

**推薦**

* 設定[重複資料刪除索引鍵](#deduplication-keys)以防止重複記錄
* 使用隨選匯出來取得精確的快照集資料
* 讓批次內嵌與評估排程保持一致，以將差異降至最低

如需隨選匯出，請參閱有關[隨選匯出檔案](/help/destinations/ui/export-file-now.md#scheduled-vs-ondemand)的檔案。
...
1. 您選取要匯出的欄位現在會出現在對應檢視中。 如果需要，您可以編輯匯出檔案中標題的名稱。 若要這麼做，請選取目標欄位上的圖示。
   ...
   >[!NOTE]
   >
   >匯出檔案中的欄位名稱不支援點(`.`)。 如果欄位名稱包含點（例如`person.name.firstName`），匯出的欄名稱中每個點都會以底線(`_`)取代。 例如，`person.name.firstName`將在匯出的檔案中變成`person_name_firstName`。
...
>[!IMPORTANT]
>
>一律設定排程匯出的重複資料刪除索引鍵。 若沒有重複資料刪除，您可能會看到相同設定檔出現重複列或衝突的區段成員資格，因為排定的匯出同時處理快照集和增量資料。

重複資料刪除索引鍵是使用者定義的主索引鍵，可決定如何對設定檔進行重複資料刪除。 當同一個人存在多個記錄時，重複資料刪除可確保僅匯出最新的記錄。
...
#### 無法透過對應工作流程選取對象成員資格屬性 {#audience-membership-attribute-mapping}
...
>
>此&#x200B;**[!UICONTROL Select attributes]**&#x200B;步驟仍會針對[!DNL Adobe Campaign]、Oracle Responsys、Oracle Eloqua和Salesforce Marketing Cloud電子郵件行銷目標顯示。
...
>[!NOTE]
>
> [!DNL Adobe Experience Platform]會使用結構描述中四個建議且常用的屬性預填您的選取範圍： `person.name.firstName`、`person.name.lastName`、`personalEmail.address`、`segmentMembership.seg_namespace.seg_id.status`。
...
將對象匯出至雲端儲存空間目的地時，[!DNL Adobe Experience Platform]會在您提供的儲存位置中建立`.csv`、`.json`或`.parquet`檔案。 預期會根據您在工作流程中設定的排程，在您的儲存位置中建立新檔案。 預設檔案格式如下所示，但您可以[編輯檔案名稱的元件](#configure-file-names)：
