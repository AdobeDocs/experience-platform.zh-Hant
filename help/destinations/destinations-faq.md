...
本檔案提供有關Adobe Experience Platform目的地的常見問題解答。 如需與其他[!DNL Experience Platform]服務相關的問題和疑難排解，包括所有[!DNL Experience Platform] API中遇到的問題和疑難排解，請參閱[Experience Platform疑難排解指南](../landing/troubleshooting.md)。
...
### Facebook需要多久才能處理來自Adobe Experience Platform的資訊？ {#facebook-processing-time}
...
### Adobe Experience Platform與[!DNL Facebook Custom Audiences]的整合是否支援在使用者不再符合受眾資格時，取消其受眾資格？ {#facebook-disqualify-users}
...
### 我是否需要使用Experience Platform Web SDK將對象和屬性傳送至Adobe Target？ {#target-web-sdk}
...
### 我可以從Real-time Customer Data Platform傳送到Adobe Target或自訂Personalization目的地的屬性數量是否有限制？ {#target-attributes-limit}
...
### 我可以在哪裡檢視Adobe Target中已啟用的屬性？ {#target-activated-attributes-location}
...
### 如果我刪除Adobe Target目的地，會發生什麼事？ {#delete-target-destination}
...
### 我只能有一個邊緣主動的合併原則。 我可以建立使用不同合併原則的受眾，並且仍將其當作串流受眾傳送給Adobe Target嗎？ {#edge-merge-policy}
