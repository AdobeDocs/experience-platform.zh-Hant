...
請閱讀本頁以瞭解[!DNL Adobe Experience Platform]個目的地的不同型別和類別。

## 目的地型別 {#destination-types}

在[!DNL Adobe Experience Platform]中，我們會區分不同的目的地型別 — 連線、資料集匯出和副檔名。 有數種型別的連線目的地，可讓您將資料匯出至API型目的地、社交目的地、CRM平台等等。

最後，也可以區分目的地目錄中所有組織可用的公用目的地，以及[!DNL Real-Time CDP]個Ultimate客戶為滿足其特定匯出使用案例而建立的私人目的地。
...
在&#x200B;**[!UICONTROL Profile Export]**&#x200B;擷取事件資料中的&#x200B;**[!UICONTROL Streaming Audience Export]**、**[!DNL Edge Personalization]**&#x200B;和[!DNL Adobe Experience Platform]目的地，將其與其他資料來源結合，以形成[即時客戶設定檔](../profile/home.md)、套用細分，並將對象和合格的設定檔匯出到目的地。
...
使用進階企業目的地資料聯結器，以近乎即時的方式將Adobe [!DNL Real-Time Customer Data Platform]設定檔傳送至內部系統或其他協力廠商系統，以進行資料同步、分析和進一步擴充設定檔使用案例。
...
當事件資料觸發動作或在外部環境中進行細分時，擴充功能會很有幫助。 例如，如果行為資料需要轉送至外部系統，而不需要加入指定使用者檔案上的其他資料來源。
